...
theorem KitaevProjector_is_ZeroDivisor_Pure (γ1 γ2 : MajoranaOperator Q v0) 
  ...
  constructor
  · -- Доказателство, че X ≠ 0
    dsimp [KitaevSplitProjectorPure]
    intro h_zero
    injection h_zero with h_fst _
    sorry
  · -- Доказателство, че hNorm = 0
    dsimp [IsZeroDivisor, SplitOctonion.hNorm, KitaevSplitProjectorPure]
    simp only [mul_zero, sub_zero]
    have h_adj : hestenesAdjoint Q v0 (KitaevProjectorPlus Q v0 γ1 γ2) = 1 - γ1.val * γ2.val := by
      sorry
    rw [h_adj]
    have h_prod : (1 + γ1.val * γ2.val) * (1 - γ1.val * γ2.val) = 1 - (γ1.val * γ2.val) * (1 * (γ1.val * γ2.val)) := by sorry
    have h_sq : (γ1.val * γ2.val) * (γ1.val * γ2.val) = 1 := by
      sorry
    sorry
