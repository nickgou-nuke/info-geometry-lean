...
/-- Occupation and vacancy are complementary by the native single-site CAR law. -/
theorem localOccupationProjector_add_vacancy :
    localOccupationProjector +
      Cl11WittVacancyTensorFactorization.localVacancyProjector = 1 := by
  simpa only [localOccupationProjector,
    Cl11WittVacancyTensorFactorization.localVacancyProjector,
    realEncodedWittCreationBase_eq, realEncodedWittAnnihilationBase_eq] using
    realEncodedWitt_anticomm
...
theorem localOccupationProjector_eq :
  ...
  rw [eq_sub_of_add_eq localOccupationProjector_add_vacancy,
    Cl11WittVacancyTensorFactorization.localVacancyProjector_eq]
  ext i j
...
theorem modeChiralityFactor_last (n : ℕ) :
  ...
  calc
    ((1 : MatStage n) ⊗ₖ (1 : Matrix (Fin 2) (Fin 2) ℝ)) -
