...
abbrev VacuumChoice :=
  {imag : DoubledSpace E →L[ℝ] DoubledSpace E //
    isComplexStructureOp (E := E) imag}

abbrev VacuumChoice.imag (V : VacuumChoice (E := E)) :
    DoubledSpace E →L[ℝ] DoubledSpace E := V.1

abbrev VacuumChoice.imag_sq_neg_id (V : VacuumChoice (E := E)) :
    isComplexStructureOp (E := E) V.imag := V.2

noncomputable def vacuumChoicePlus : VacuumChoice (E := E) :=
  ⟨complex_i (E := E), complex_i_isComplexStructureOp (E := E)⟩

noncomputable def vacuumChoiceMinus : VacuumChoice (E := E) :=
  ⟨-(complex_i (E := E)), neg_complex_i_isComplexStructureOp (E := E)⟩
...
theorem vacuumChoice_switch_swaps_splits_plus :
  ...
  simp [vacuumSplitPlus, vacuumSplitMinus,
    vacuumChoicePlus, vacuumChoiceMinus, VacuumChoice.imag, sub_eq_add_neg]
