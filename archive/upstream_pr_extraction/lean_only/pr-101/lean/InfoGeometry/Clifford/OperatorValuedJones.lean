...
/-- The identity sheet-polarization matrix. -/
def I_mat : Matrix (Fin 2) (Fin 2) B :=
  !![1, 0; 0, 1]

/-- The Γ matrix (sheet imbalance / chiral polarization). -/
def Gamma_mat : Matrix (Fin 2) (Fin 2) B :=
  !![1, 0; 0, -1]

/-- The J matrix (sheet exchange). -/
def J_mat : Matrix (Fin 2) (Fin 2) B :=
  !![0, 1; 1, 0]

/-- The K = ΓJ matrix (relative phase / internal complex axis). -/
