...
live_path = "lean/InfoGeometry/Detector/FieldCorrelatorProjection.lean"
sandbox_path = ".agents/sandbox_correlator/lean/InfoGeometry/Detector/FieldCorrelatorProjection.lean"

# 1. Token scan
forbidden_tokens = ["sorry", "admit", "native_decide", "unsafe", "axiom "]
...
with open(".agents/sandbox_correlator/audit/audit_token_scan.log", "w") as f:
    ...
    if violations:
        ...
        f.write("Violations found: 0 (PASSED: 0 sorry, 0 admit, 0 native_decide)\n")

# 2. Declaration Fidelity
def extract_decls(filepath):
    decls = []
    decl_regex = re.compile(r"^(structure|def|theorem|inductive|abbrev|axiom|lemma)\s+([A-Za-z0-9_]+)", re.MULTILINE)
...
with open(".agents/sandbox_correlator/audit/audit_declaration_fidelity.log", "w") as f:
