...
/-! ## Honest short and long root sectors

These are the two six-dimensional sectors selected by the explicit root
labels.  No `sl₃ ⊕ 3 ⊕ 3*` identification is asserted here; that requires
additional bracket/module data.
-/

def shortRootIndex : Fin 6 → Fin 14 := ![0, 3, 4, 8, 9, 10]

def longRootIndex : Fin 6 → Fin 14 := ![1, 2, 5, 7, 11, 12]

def shortRootSpan : Submodule ℝ Der :=
  Submodule.span ℝ (Set.range (fun i : Fin 6 => rootDerivation (shortRootIndex i)))

def longRootSpan : Submodule ℝ Der :=
  Submodule.span ℝ (Set.range (fun i : Fin 6 => rootDerivation (longRootIndex i)))

private theorem shortRootIndex_injective : Function.Injective shortRootIndex := by
  intro i j h
  fin_cases i <;> fin_cases j <;> simp [shortRootIndex] at h ⊢

private theorem longRootIndex_injective : Function.Injective longRootIndex := by
  intro i j h
  fin_cases i <;> fin_cases j <;> simp [longRootIndex] at h ⊢

theorem finrank_shortRootSpan : Module.finrank ℝ shortRootSpan = 6 := by
  letI : FiniteDimensional ℝ Der :=
    FiniteDimensional.of_fintype_basis rootDerivationBasis
