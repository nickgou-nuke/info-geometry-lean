...
theorem partitionFunction_zeroFreeInUnitDisk
  ...
  have hz_lt : ‖z‖ < 1 := by
    simpa [openUnitDisk] using hz_disk
  ...
  have hz_sq_lt : ‖z‖ ^ 2 < 1 := by
    nlinarith [norm_nonneg z, hz_lt]
  have hnot_lt : ¬ ‖z‖ ^ 2 < 1 := by
    rw [hcircle]
    linarith
  exact hnot_lt hz_sq_lt
...
theorem partitionFunction_reciprocalZeroSymmetric
  ...
  · intro hz
    ...
    exact (mul_eq_zero.mp hzero).resolve_left (pow_ne_zero 2 hz0)
  · intro hz
    rw [partitionFunction_formula_eval D lam z]
    rw [partitionFunction_formula_eval D lam z⁻¹] at hz
    have hscaled :
