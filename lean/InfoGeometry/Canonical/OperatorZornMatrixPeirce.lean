...
theorem coefficientProjector_mul_apply (k : J) (M : Matrix J J B) (i j : J) :
    ((HMul.hMul (coefficientProjector k) M : Matrix J J B) i j) =
      if i = k then M i j else 0 := by
  by_cases h : i = k <;>
...
theorem mul_coefficientProjector_apply (M : Matrix J J B) (k i j : J) :
    ((HMul.hMul M (coefficientProjector k) : Matrix J J B) i j) =
      if j = k then M i j else 0 := by
  by_cases h : j = k
  · simp [coefficientProjector, Matrix.mul_apply, Matrix.diagonal, h, eq_comm]
  · have h' : ¬ k = j := fun hkj => h hkj.symm
    simp [coefficientProjector, Matrix.mul_apply, Matrix.diagonal, h, h', eq_comm]
...
theorem coefficientProjector_complete :
  ...
  ext a b
  simp only [Matrix.sum_apply, coefficientProjector, Matrix.diagonal, Matrix.one_apply,
    Matrix.of_apply]
  by_cases h : a = b
  · subst b
    simp
