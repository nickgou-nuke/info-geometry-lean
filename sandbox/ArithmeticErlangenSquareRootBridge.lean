...
To ensure `FiniteAmplitudeSquareRootPacket` is not a vacuous shape, we provide
a trivial instantiation over `ℕ`.
-/

/-- A trivial instantiation of the amplitude packet. -/
def natTrivialPacket : FiniteAmplitudeSquareRootPacket ℕ ℕ where
  support := {1, 2}
  amplitude n := n
  probability n := n ^ 2
  local_square _ _ := rfl
