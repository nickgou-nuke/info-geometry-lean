...
/-- A trivial instantiation of the grading deformation to satisfy the mandate. -/
def trivialCuntzGradingDeformation (n : ℕ) : CuntzGradingDeformation n :=
  ⟨fun _ => false, 1⟩
...
/-- Trivial instantiation to satisfy instantiation rules. -/
def trivialCuntzPresentationOperator (n : ℕ) (D : CuntzGradingDeformation n) :
    CuntzPresentationOperator n D (Multiplicative Unit) where
  op _ := AlgHom.id ℂ (CuntzAlg n)
  op_one _ := rfl
  op_mul _ _ _ := rfl
  commutes_grading _ _ := rfl
...
/-- Trivial instantiation to satisfy instantiation rules. -/
def trivialCuntzLorentzPoincareOperatorPresentation {n : ℕ} (mode : Fin n) :
    CuntzLorentzPoincareOperatorPresentation n where
  deformation := trivialCuntzGradingDeformation n
  mode := mode
  LorentzGroup := Multiplicative Unit
  PoincareGroup := Multiplicative Unit
  lorentzGroup := inferInstance
  poincareGroup := inferInstance
  lorentz := trivialCuntzPresentationOperator n (trivialCuntzGradingDeformation n)
  poincare := trivialCuntzPresentationOperator n (trivialCuntzGradingDeformation n)
  lorentz_preserves_supercharge _ := rfl
  poincare_preserves_supercharge _ := rfl
...
theorem lorentz_operator_packet_momentum (Λ : P.LorentzGroup) :
    P.lorentz.op Λ P.momentum = P.momentum :=
  P.lorentz_preserves_momentum Λ

theorem lorentz_operator_packet_anticommutator (Λ : P.LorentzGroup) :
    P.lorentz.op Λ (algebraicAnticommutator P.supercharge P.supercharge) =
      algebraicAnticommutator P.supercharge P.supercharge :=
  P.lorentz_preserves_self_anticommutator Λ

theorem poincare_operator_packet_momentum (a : P.PoincareGroup) :
    P.poincare.op a P.momentum = P.momentum :=
  P.poincare_preserves_momentum a

theorem poincare_operator_packet_anticommutator (a : P.PoincareGroup) :
    P.poincare.op a (algebraicAnticommutator P.supercharge P.supercharge) =
      algebraicAnticommutator P.supercharge P.supercharge :=
  P.poincare_preserves_self_anticommutator a
...
/-- The canonical Cuntz packet has odd supercharge. -/
theorem canonical_cuntz_packet_supercharge_grade {n : ℕ} (i : Fin n) :
    (canonicalCuntzSuperPoincarePacket n i).superchargeGrade = 1 :=
  rfl

/-- The canonical Cuntz packet has even generated translation. -/
theorem canonical_cuntz_packet_momentum_grade {n : ℕ} (i : Fin n) :
    (canonicalCuntzSuperPoincarePacket n i).momentumGrade = 0 :=
  rfl

/-- The canonical Cuntz packet translation generator is exactly the superMomentum. -/
theorem canonical_cuntz_packet_translation_eq {n : ℕ} (i : Fin n) :
    (canonicalCuntzSuperPoincarePacket n i).poincareTranslation =
      superMomentum (canonicalCuntzSuperPoincarePacket n i).supercharge :=
  rfl
...
theorem finite_cuntz_super_poincare_lorentz_packet {n : ℕ} (i : Fin n) (t x y z : ℂ) :
    ...
    Matrix.det (exactBoostTransport (exactBoostTransport (hermitianSpacetimePoint t x y z))) =
      t * t - (x * x + y * y + z * z) := by
  simp only
  exact ⟨canonical_cuntz_packet_supercharge_grade i,
         canonical_cuntz_packet_momentum_grade i,
         canonical_cuntz_packet_anticommutator_translation i,
