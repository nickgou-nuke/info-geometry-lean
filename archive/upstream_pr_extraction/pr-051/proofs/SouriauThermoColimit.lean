...
The file records finite algebraic identities for the local factors and their
stagewise products.
-/
...
/-- The cubic relation selects one of the three named OP state branches. -/
theorem OPStateFromCubicValue_of_cubic {q : ℝ} (hq : souriauCubicOperator q) :
    ∃ s : OPState, OPStateFromCubicValue q = some s := by
  rcases souriau_cubic_operator_roots hq with hneg | hzero | hone
  · refine ⟨OPState.hyperbolic, ?_⟩
    norm_num [OPStateFromCubicValue, hneg]
  · refine ⟨OPState.parabolic, ?_⟩
    simp [OPStateFromCubicValue, hzero]
  · refine ⟨OPState.elliptic, ?_⟩
    simp [OPStateFromCubicValue, hone]
    ...
    ι := NatTrans.ofSequence
      ...
      (naturality := by
        ...
        have hmap : (SouriauFockDiagram modes).map (homOfLE (Nat.le_add_right n 1))
          ...
          simp [SouriauFockDiagram, Functor.ofSequence_map_homOfLE_succ]
        rw [hmap]
        by_cases hkn : k < n
