...
/-- Site-local coupling scale `Kᵢⱼ = β Jᵢⱼ`. -/
...
/-- High-temperature trivial partition function at `h = 0` is `2ⁿ`. -/
theorem partitionFunction_zero_kappa_zero_field (β : ℝ) :
    (PrimeFerromagneticChain.mk C.primes C.prime_ge_two 0 (by norm_num)).partitionFunction β 0 =
      2 ^ n := by
  unfold partitionFunction
  have hsum : (∑ σ : Fintype.elems (Configuration n),
      (PrimeFerromagneticChain.mk C.primes C.prime_ge_two 0 (by norm_num)).boltzmannWeight β 0 σ) =
        ∑ σ : Fintype.elems (Configuration n), (1 : ℝ) := by
    apply Finset.sum_congr rfl
    intro σ _
    exact C.boltzmannWeight_zero_kappa_zero_field β σ
  rw [hsum, Finset.sum_const, nsmul_eq_mul, mul_one]
  have hcard : (Fintype.elems (Configuration n)).card = 2 ^ n := by
    simp [Configuration, Fintype.card_fun, Fintype.card_fin]
...
theorem quadratic_leeyang_circle_theorem (a : ℝ) (z : ℂ)
  ...
  have h_prod : z * star z = 1 := by
    have h_star : star z = - 2 * (a : ℂ) - z := by
      linear_combination h_re
