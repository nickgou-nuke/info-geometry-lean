...
theorem isNilpotent_derivedSeries_of_traceForm_eq_zero_aux {K : Type*}
  ...
  let eigenDecomp := DirectSum.isInternal_submodule_of_iSupIndep_of_iSup_eq_top
    s.eigenspaces_iSupIndep
      (Module.End.IsSemisimple.iSup_eigenspace_eq_top hs_ss)
  ...
  suffices ∀ f : Dual ℚ E, f = 0 by
    suffices ∀ ν, s.HasEigenvalue ν → ν = 0 from
      (Module.End.IsSemisimple.eq_zero_iff_forall_eigenvalue hs_ss).mpr this
    intro ν hν
    ...
    simpa using (Subsingleton.elim (⟨ν, hν⟩ : E) (0 : E))
  intro f
  ...
  suffices ∑ i, (f ⟨μ i, hμ i⟩) • (⟨μ i, hμ i⟩ : E) = 0 by
    have hf := congrArg f this
    simpa only [map_sum, map_zero, map_smul, sq] using hf
    ...
    obtain ⟨q, hq⟩ : ∃ q : K[X], q.aeval (ad K _ (φ x)) = ad K _ y := by
      obtain ⟨r, hr⟩ : ∃ r : K[X], r.aeval (ad K _ s) = ad K _ y := by
        have h₁ (i j : I) : ⁅s, v.end (i, j)⁆ = (μ i - μ j) • v.end (i, j) := by
          exact Module.Basis.lie_end_of_apply_eq_smul v μ s hsv i j
        have h₂ (i j : I) : ⁅y, v.end (i, j)⁆ = (fμ i - fμ j) • v.end (i, j) := by
          refine v.ext fun k ↦ ?_
          simp only [Ring.lie_def, LinearMap.sub_apply, Module.End.mul_apply,
            LinearMap.smul_apply, Module.Basis.end_apply_apply, smul_ite,
            smul_zero, sub_smul]
          rcases eq_or_ne j k with rfl | hjk
          · simp [hyv, Module.Basis.end_apply_apply]
          · simp [hyv, Module.Basis.end_apply_apply, hjk]
        obtain ⟨r, hr⟩ := exists_polynomial_eval_sub_aux μ hμ f
        refine ⟨r, v.end.ext fun ⟨i, j⟩ ↦ ?_⟩
        rw [ad_apply, Module.End.aeval_apply_of_mem_apply_eq_smul (h₁ i j), hr, h₂]
        rfl
      obtain ⟨p, hp⟩ : ∃ p : K[X], p.aeval (ad K _ (φ x)) = ad K _ s :=
        adjoin_mem_exists_aeval K _ <| by
          have had := LieAlgebra.ad_mem_adjoin_of_isSemisimple hns hn_nil hs_ss
          simpa [hX_ns] using had
  ...
  have hny_comm : Commute n y := by
    suffices y ∈ Algebra.adjoin K {s} from commute_of_mem_adjoin_singleton_of_commute this hns
    ...
    rw [AlgHom.toRingHom_eq_coe, RingHom.coe_coe,
      Module.End.aeval_apply_of_mem_apply_eq_smul (hsv i), hq, hyv]
  /- By general results we need only prove `trace K M (φ x * y) = ∑ i, fμ i * μ i`. -/
  suffices trace K M (φ x * y) = ∑ i, fμ i * μ i from
    ⟨y, InfoGeometry.Lie.MathlibBackport.trace_toEnd_mul_eq_zero_of_traceForm_eq_zero
      h y hy_range x hx, this⟩
...
public theorem isNilpotent_derivedSeries_of_traceForm_eq_zero
  ...
  have nilp_ext : IsNilpotent (derivedSeries A (A ⊗[R] L) 1) (A ⊗[R] M) :=
    isNilpotent_derivedSeries_of_traceForm_eq_zero_aux <| by
      rw [InfoGeometry.Lie.MathlibBackport.traceForm_baseChange, h]
      exact InfoGeometry.Lie.MathlibBackport.bilinForm_baseChange_zero
  ...
  have hbc_inj : Injective (End.baseChangeHom R A M) :=
    LinearMap.baseChangeHom_injective_of_faithfulSMul (R := R) (S := A) (M := M) (N := M)
