...
sandbox_file = ".agents/sandbox_krein/lean/InfoGeometry/LLM/KreinAttentionEnergy.lean"

print("Acquiring build lock for profiling...")
with acquire_build_lock(None, "worker_krein_timing", block=True):
    t0 = time.time()
    res = subprocess.run(
...
with open(".agents/sandbox_krein/audit/kernel_timing.log", "w") as f:
...
with open(".agents/sandbox_krein/audit/audit_compilation.log", "w") as f:
