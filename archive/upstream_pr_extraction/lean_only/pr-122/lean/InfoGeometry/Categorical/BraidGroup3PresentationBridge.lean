...
theorem presentedGenerators_satisfy_braidProject_relations :
  ...
  · rcases hAdj with ⟨i, rfl⟩
    ...
    simp [braid_rel, braidProjectGeneratorToPresented,
...
def braidProjectToPresentedB3 : braid_group 3 →* BraidGroup3 :=
  braid_group.toGroup braidProjectGeneratorToPresented
...
theorem braidProjectToPresentedB3_sigmaZero :
  ...
  simp [braidProjectToPresentedB3]
...
def braidGroup3MulEquiv : BraidGroup3 ≃* braid_group 3 where
  ...
  left_inv := by
    intro g
    have h := MonoidHom.congr_fun
      braidProjectToPresented_comp_presentedToBraidProject g
    simpa using h
  right_inv := by
    intro g
    have h := MonoidHom.congr_fun
      presentedToBraidProject_comp_braidProjectToPresented g
    simpa using h
  map_mul' := by
    intro a b
