...
def matrixOp (M : Matrix κ ι ℂ) :
    FinKetSpace ι →L[ℂ] FinKetSpace κ :=
  LinearMap.toContinuousLinearMap (Matrix.toEuclideanLin M)

@[simp]
theorem matrixOp_apply (M : Matrix κ ι ℂ) (v : FinKetSpace ι) :
    (matrixOp M v : κ → ℂ) = M *ᵥ (v : ι → ℂ) := by
  simp [matrixOp, Matrix.toEuclideanLin_apply]
...
theorem matrixOp_apply_ket [DecidableEq ι]
  ...
  ext j
  simp [matrixOp, ketPi, Matrix.toEuclideanLin_apply, Matrix.mulVec_single_one]

/-- Matrix operator composition corresponds to matrix multiplication. -/
theorem matrixOp_comp (M : Matrix η κ ℂ) (N : Matrix κ ι ℂ) :
    (matrixOp M).comp (matrixOp N) = matrixOp (M * N) := by
  apply ContinuousLinearMap.ext
  intro v
  apply EuclideanSpace.ext
  simp [matrixOp, Matrix.toEuclideanLin_apply, Matrix.mulVec_mulVec]
...
theorem matrixOp_id [DecidableEq ι] :
  ...
  intro v
  apply EuclideanSpace.ext
  simp [matrixOp, Matrix.toEuclideanLin_apply, Matrix.one_mulVec]
...
theorem matrixOp_zero :
  ...
  intro v
  apply EuclideanSpace.ext
  simp [matrixOp, Matrix.toEuclideanLin_apply, Matrix.zero_mulVec]

/-- Matrix addition corresponds to operator addition. -/
theorem matrixOp_add (M N : Matrix κ ι ℂ) :
    matrixOp (M + N) = matrixOp M + matrixOp N := by
  apply ContinuousLinearMap.ext
  intro v
  apply EuclideanSpace.ext
  simp [matrixOp, Matrix.toEuclideanLin_apply, Matrix.add_mulVec]

/-! ## Finite matrix adjoint corridor from AFP `Extra_Jordan_Normal_Form` -/

theorem matrixOp_conjTranspose_apply_ket [DecidableEq κ]
...
theorem dotProduct_conjTranspose_mulVec
    ...
    Matrix.dotProduct (star v) (Mᴴ *ᵥ u) = Matrix.dotProduct (star (M *ᵥ v)) u := by
