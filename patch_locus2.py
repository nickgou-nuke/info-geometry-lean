...
zero_weight_proof = """theorem hyperbolicFlow_on_zeroWeight (t : ℝ) (x : Coord) (hx : x ∈ zeroWeightSubmodule) :
  ...
  · intro y hy
    rcases hy with rfl | rfl <;> ext i <;> fin_cases i <;>
      simp [hyperbolicFlowCoordinate_apply, InfoGeometry.Lie.SplitOctonionCircularAxialGrading.axialWeight, Pi.single_apply]
  · ext i; simp [hyperbolicFlowCoordinate_apply]
  · intro y z _ _ hy hz
    rw [map_add, hy, hz]
  · intro a y _ hy
    rw [map_smul, hy]"""
...
pos_weight_proof = """theorem hyperbolicFlow_on_positiveWeight (t : ℝ) (x : Coord) (hx : x ∈ positiveWeightSubmodule) :
  ...
  refine Submodule.span_induction (p := fun y _ => hyperbolicFlowCoordinate t y = Real.exp t • y)
    ?_ ?_ ?_ ?_ hx
  · intro y hy
    rcases hy with rfl | rfl | rfl <;> ext i <;> fin_cases i <;>
      simp [hyperbolicFlowCoordinate_apply, InfoGeometry.Lie.SplitOctonionCircularAxialGrading.axialWeight, Pi.single_apply]
  · ext i; simp [hyperbolicFlowCoordinate_apply]
  · intro y z _ _ hy hz
