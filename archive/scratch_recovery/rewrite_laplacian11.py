...
theorem laplacian_rank_eq_add_rank (A : Matrix m n ℚ) (B : Matrix p m ℚ) (hBA : B * A = 0) :
  ...
  have h_CD : C * D = 0 := by
    calc C * D = A * Aᵀ * Bᵀ * B := by simp [C, D, Matrix.mul_assoc]
      ...
      _ = 0 := by rw [hBA]; simp
  have h_DC : D * C = 0 := by
    calc D * C = Dᵀ * Cᵀ := by rw [← hD_sym, ← hC_sym]
      _ = (C * D)ᵀ := by rw [Matrix.transpose_mul]
      _ = (0 : Matrix m m ℚ)ᵀ := by rw [h_CD]
      _ = 0 := Matrix.transpose_zero
  have h_inter : LinearMap.range C.mulVecLin ⊓ LinearMap.range D.mulVecLin = ⊥ := by
    apply eq_bot_iff.mpr; intro v hv
    ...
    simp only [Matrix.mulVecLin_apply, LinearMap.mem_bot] at hv
    rw [← hy, hDy]
    exact hv
    ...
    have h_ker_sq : LinearMap.ker (C * C).mulVecLin = LinearMap.ker C.mulVecLin := by
      ...
      · intro hz'
        ...
        have h_nz : innerProduct (C.mulVec z) (C.mulVec z) = 0 := by
          calc
            innerProduct (C.mulVec z) (C.mulVec z) = innerProduct (Cᵀ.mulVec z) (C.mulVec z) := by rw [hC_sym]
            _ = innerProduct z (C.mulVec (C.mulVec z)) := by rw [← innerProduct_transpose_mulVec_eq C z (C.mulVec z)]
            _ = 0 := h_dz
        simp only [LinearMap.mem_ker, Matrix.mulVecLin_apply]
    ...
    have h_ker_sq : LinearMap.ker (D * D).mulVecLin = LinearMap.ker D.mulVecLin := by
      ...
      · intro hz'
        ...
        have h_nz : innerProduct (D.mulVec z) (D.mulVec z) = 0 := by
          calc
            innerProduct (D.mulVec z) (D.mulVec z) = innerProduct (Dᵀ.mulVec z) (D.mulVec z) := by rw [hD_sym]
            _ = innerProduct z (D.mulVec (D.mulVec z)) := by rw [← innerProduct_transpose_mulVec_eq D z (D.mulVec z)]
            _ = 0 := h_dz
        simp only [LinearMap.mem_ker, Matrix.mulVecLin_apply]
