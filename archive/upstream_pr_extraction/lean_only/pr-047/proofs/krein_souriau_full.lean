...
For a 1D model, we define an abstract linear form. -/
def souriauCocycle (g : ℝ) (J_M x : ℝ) : ℝ :=
  J_M * g - x * g
...
In 1D, we define this as the standard symplectic scalar product. -/
def poissonBracket (F G : ℝ → ℝ) (x : ℝ) : ℝ :=
  deriv F x * deriv G x - deriv G x * deriv F x

/-- Dissipative bracket (symmetric, produces entropy).
Defined using the standard Euclidean inner product on gradients. -/
def dissipativeBracket (F S : ℝ → ℝ) (x : ℝ) : ℝ :=
  deriv F x * deriv S x
...
theorem entropy_production (S : ℝ → ℝ) (x : ℝ) : dissipativeBracket S S x ≥ 0 := by
  unfold dissipativeBracket
  exact mul_self_nonneg (deriv S x)

/-- The Onsager operator L = -d²S/dX² (from T25 tri-projector) -/
noncomputable def onsagerOperator : ℝ :=
  fisherInfo⁻¹
