...
theorem hasDerivAt_eval_mul_generator
  ...
  have htranslatedAt :
      HasDerivAt (fun u : ℝ => F.eval (t + u))
        (F.eval t * L) (id t - t) := by
    simpa using htranslated
  have harg : HasDerivAt (fun u : ℝ => u - t) 1 t := by
    simpa using (hasDerivAt_id t).sub_const t
  have hcomp := htranslatedAt.scomp_of_eq t harg (by simp)
...
theorem continuousRopeLayer_relative_attentionScore
  ...
  change dotProduct ((rotor cs s) *ᵥ q) ((rotor cs t) *ᵥ k) =
    dotProduct q ((rotor cs (t - s)) *ᵥ k)
  rw [← Matrix.vecMul_transpose]
  rw [Matrix.dotProduct_mulVec]
  rw [Matrix.vecMul_vecMul]
  rw [← Matrix.dotProduct_mulVec]
...
theorem alibiRelativeFlow_mulVec_clockKey (t : ℝ) :
  ...
  fin_cases i <;>
    simp [parabolicMat, JordanBlock2.N2, alibiClockKey,
      Matrix.mulVec, Matrix.vecHead, Matrix.vecTail, Matrix.one_apply,
      Fin.sum_univ_two]
