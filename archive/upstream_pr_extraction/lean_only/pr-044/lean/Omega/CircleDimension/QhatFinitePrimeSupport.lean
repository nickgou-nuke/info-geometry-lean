...
noncomputable def qhatPrimeSupportSet (Q : Finset ℚ) : Finset ℕ :=
...
lemma denominatorSupportedBy_qhatPrimeSupportSet (Q : Finset ℚ) {q : ℚ} (hq : q ∈ Q) :
    denominatorSupportedBy (qhatPrimeSupportSet Q) q := by
  intro p hp hdiv
...
theorem cdim_qhat_finite_prime_support (Q : Finset ℚ) :
    ∃ S : Finset ℕ,
      (∀ q ∈ Q, denominatorSupportedBy S q) ∧
      ∀ coeff : ℚ → ℤ, (∀ q, coeff q ≠ 0 → q ∈ Q) → qhatCertificateFactorsThrough S coeff := by
  refine ⟨qhatPrimeSupportSet Q, ?_, ?_⟩
  · intro q hq
    exact denominatorSupportedBy_qhatPrimeSupportSet Q hq
  · intro coeff hcoeff q hqcoeff
    exact denominatorSupportedBy_qhatPrimeSupportSet Q (hcoeff q hqcoeff)
