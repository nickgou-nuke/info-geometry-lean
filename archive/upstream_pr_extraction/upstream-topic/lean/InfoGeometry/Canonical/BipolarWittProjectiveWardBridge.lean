...
/-- The analytic quadratic-vector-field carrier independently certifies that
the local third-derivative central term is zero. -/
theorem quadratic_vector_field_central_term_zero
    (c : ℂ) (v : ProjectiveVectorField) (s : ℂ) :
    (c / 12) * v.third s = 0 := by
  exact projectiveVectorField_centralTerm_zero c v s
...
theorem bipolar_projective_anomaly_packet
    ...
    (c / 12) * v.third s = 0 ∧
      ...
      ⁅lgen ℂ m, lgen ℂ n⁆ =
        (m - n : ℂ) • lgen ℂ (m + n) := by
  exact ⟨quadratic_vector_field_central_term_zero c v s,
