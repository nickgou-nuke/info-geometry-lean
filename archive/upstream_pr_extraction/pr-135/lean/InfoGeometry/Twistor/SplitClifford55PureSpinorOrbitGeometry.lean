...
variable {L A : Type*}
variable [LieRing L] [LieAlgebra ℝ L]
variable [Ring A] [Algebra ℝ A]

local instance : Module ℝ L := (inferInstance : LieAlgebra ℝ L).toModule
...
def kksResponse
    (D : OperatorPureSpinorReadout (L := L) (A := A))
    (μ : Module.Dual ℝ L)
    (p q : ProjectivePureSpinorPoint) : ℝ :=
  μ ⁅D.direction p, D.direction q⁆
...
theorem kksResponse_skew
    ...
    kksResponse D μ p q = - kksResponse D μ q p := by
  change μ ⁅D.direction p, D.direction q⁆ =
    -μ ⁅D.direction q, D.direction p⁆
  calc
    μ ⁅D.direction p, D.direction q⁆ =
        μ (-⁅D.direction q, D.direction p⁆) :=
      congrArg μ (lie_skew (D.direction p) (D.direction q)).symm
    _ = -μ ⁅D.direction q, D.direction p⁆ := by rw [map_neg]
...
theorem kksResponse_negOne_posOne_selection
    ...
    ∃ Z : L, Z ∈ D.grading.gZero ∧ kksResponse D μ p q = μ Z := by
...
theorem observable_transport_equivariant
    ...
    (φ : A ≃ₐ[ℝ] A)
    (p : ProjectivePureSpinorPoint) :
    (transportQuantumMomentMap (e := e) hLie φ D.momentMap).toLinearMap
        (e (D.direction p)) =
      φ (D.observable p) := by
  exact momentMap_equivariant (R := ℝ) (g := L) (A := A)
    e hLie φ D.momentMap (D.direction p)
...
theorem kksResponse_transport_invariant
    ...
    (InfoGeometry.Canonical.SouriauContragredientPairing.contragredient e μ)
      ...
      kksResponse D μ p q := by
  rw [← hLie]
  exact InfoGeometry.Canonical.SouriauContragredientPairing.contragredient_pairing_invariant
    e μ ⁅D.direction p, D.direction q⁆
...
def responsePair
  ...
  (OperatorPureSpinorReadout.kksResponse D.operator μ p q,
    D.paraKahler.qgtResponse p q)

@[simp] theorem responsePair_kks
    (μ : Module.Dual ℝ L)
    (p q : ProjectivePureSpinorPoint) :
    (D.responsePair μ p q).1 =
      OperatorPureSpinorReadout.kksResponse D.operator μ p q := by
  rfl
...
theorem responsePair_swap
  ...
  constructor
  · exact OperatorPureSpinorReadout.kksResponse_skew D.operator μ p q
