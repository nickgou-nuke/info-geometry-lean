...
theorem qplus_qminus_qplus (g : CuntzO2Generators R)
  ...
  dsimp [QPlus]
  calc
    g.S1 * g.S1star * (g.S1 * g.S2star) =
        g.S1 * (g.S1star * g.S1) * g.S2star := by noncomm_ring
    _ = g.S1 * g.S2star := by rw [hS1star_S1]; simp
...
theorem qminus_qplus_qminus (g : CuntzO2Generators R)
  ...
  dsimp [QMinus]
  calc
    g.S2 * g.S2star * (g.S2 * g.S1star) =
        g.S2 * (g.S2star * g.S2) * g.S1star := by noncomm_ring
    _ = g.S2 * g.S1star := by rw [hS2star_S2]; simp
...
/-! ## Cartan grading versus SUSY Hamiltonian -/

/-- The grading acts on the positive nilpotent channel with weight `+2`. -/
theorem grade_commutator_qPlus (g : CuntzO2Generators R)
    (hS1star_S1 : g.S1star * g.S1 = 1)
    (hS2star_S2 : g.S2star * g.S2 = 1)
    (hS2star_S1 : g.S2star * g.S1 = 0) :
    ParityGrading g * QPlus g - QPlus g * ParityGrading g = 2 * QPlus g := by
  dsimp [ParityGrading, QPlus]
  have h_left :
      (g.S1 * g.S1star - g.S2 * g.S2star) * (g.S1 * g.S2star) =
        g.S1 * g.S2star := by
    have h1 : g.S1 * g.S1star * (g.S1 * g.S2star) = g.S1 * g.S2star := by
      rw [mul_assoc, ← mul_assoc g.S1star, hS1star_S1, one_mul]
    have h2 : g.S2 * g.S2star * (g.S1 * g.S2star) = 0 := by
    ...
    calc
      (g.S1 * g.S1star - g.S2 * g.S2star) * (g.S1 * g.S2star) =
          g.S1 * g.S1star * (g.S1 * g.S2star) -
            g.S2 * g.S2star * (g.S1 * g.S2star) := by noncomm_ring
      _ = g.S1 * g.S2star := by rw [h1, h2, sub_zero]
  have h_right :
      (g.S1 * g.S2star) * (g.S1 * g.S1star - g.S2 * g.S2star) =
        - (g.S1 * g.S2star) := by
    have h1 : g.S1 * g.S2star * (g.S1 * g.S1star) = 0 := by
      rw [mul_assoc, ← mul_assoc g.S2star, hS2star_S1, zero_mul, mul_zero]
    have h2 : g.S1 * g.S2star * (g.S2 * g.S2star) = g.S1 * g.S2star := by
    ...
    calc
      (g.S1 * g.S2star) * (g.S1 * g.S1star - g.S2 * g.S2star) =
          g.S1 * g.S2star * (g.S1 * g.S1star) -
            g.S1 * g.S2star * (g.S2 * g.S2star) := by noncomm_ring
      _ = - (g.S1 * g.S2star) := by rw [h1, h2, zero_sub]
  rw [h_left, h_right]
  noncomm_ring

/-- The grading acts on the negative nilpotent channel with weight `-2`. -/
theorem grade_commutator_qMinus (g : CuntzO2Generators R)
    (hS1star_S1 : g.S1star * g.S1 = 1)
    (hS2star_S2 : g.S2star * g.S2 = 1)
    (hS1star_S2 : g.S1star * g.S2 = 0) :
    ParityGrading g * QMinus g - QMinus g * ParityGrading g = -2 * QMinus g := by
  dsimp [ParityGrading, QMinus]
  have h_left :
      (g.S1 * g.S1star - g.S2 * g.S2star) * (g.S2 * g.S1star) =
        - (g.S2 * g.S1star) := by
    have h1 : g.S1 * g.S1star * (g.S2 * g.S1star) = 0 := by
      rw [mul_assoc, ← mul_assoc g.S1star, hS1star_S2, zero_mul, mul_zero]
    have h2 : g.S2 * g.S2star * (g.S2 * g.S1star) = g.S2 * g.S1star := by
    ...
    calc
      (g.S1 * g.S1star - g.S2 * g.S2star) * (g.S2 * g.S1star) =
          g.S1 * g.S1star * (g.S2 * g.S1star) -
            g.S2 * g.S2star * (g.S2 * g.S1star) := by noncomm_ring
      _ = - (g.S2 * g.S1star) := by rw [h1, h2, zero_sub]
  have h_right :
      (g.S2 * g.S1star) * (g.S1 * g.S1star - g.S2 * g.S2star) =
        g.S2 * g.S1star := by
    have h1 : g.S2 * g.S1star * (g.S1 * g.S1star) = g.S2 * g.S1star := by
      rw [mul_assoc, ← mul_assoc g.S1star, hS1star_S1, one_mul]
    have h2 : g.S2 * g.S1star * (g.S2 * g.S2star) = 0 := by
    ...
    calc
      (g.S2 * g.S1star) * (g.S1 * g.S1star - g.S2 * g.S2star) =
          g.S2 * g.S1star * (g.S1 * g.S1star) -
            g.S2 * g.S1star * (g.S2 * g.S2star) := by noncomm_ring
      _ = g.S2 * g.S1star := by rw [h1, h2, sub_zero]
  rw [h_left, h_right]
  noncomm_ring

/-- The even SUSY Hamiltonian generated by the two chiral channels. -/
def susyHamiltonian (g : CuntzO2Generators R) : R :=
  QPlus g * QMinus g + QMinus g * QPlus g

/-- Nilpotency makes the SUSY Hamiltonian commute with `QPlus`. -/
theorem susyHamiltonian_comm_qPlus (g : CuntzO2Generators R)
    (hQPlus : QPlus g * QPlus g = 0) :
    susyHamiltonian g * QPlus g = QPlus g * susyHamiltonian g := by
  unfold susyHamiltonian
  calc
    (QPlus g * QMinus g + QMinus g * QPlus g) * QPlus g =
        QPlus g * QMinus g * QPlus g + QMinus g * (QPlus g * QPlus g) := by
          noncomm_ring
    _ = QPlus g * QMinus g * QPlus g := by rw [hQPlus, mul_zero, add_zero]
    _ = (QPlus g * QPlus g) * QMinus g + QPlus g * QMinus g * QPlus g := by
          rw [hQPlus, zero_mul, zero_add]
    _ = QPlus g * (QPlus g * QMinus g + QMinus g * QPlus g) := by
          noncomm_ring

/-- Nilpotency makes the SUSY Hamiltonian commute with `QMinus`. -/
theorem susyHamiltonian_comm_qMinus (g : CuntzO2Generators R)
    (hQMinus : QMinus g * QMinus g = 0) :
    susyHamiltonian g * QMinus g = QMinus g * susyHamiltonian g := by
  unfold susyHamiltonian
  calc
    (QPlus g * QMinus g + QMinus g * QPlus g) * QMinus g =
        (QPlus g * QMinus g) * QMinus g + QMinus g * QPlus g * QMinus g := by
          noncomm_ring
    _ = QPlus g * (QMinus g * QMinus g) + QMinus g * QPlus g * QMinus g := by
          noncomm_ring
    _ = QMinus g * QPlus g * QMinus g := by rw [hQMinus, mul_zero, zero_add]
    _ = QMinus g * (QPlus g * QMinus g + QMinus g * QPlus g) := by
          symm
          calc
            QMinus g * (QPlus g * QMinus g + QMinus g * QPlus g) =
                QMinus g * QPlus g * QMinus g +
                  (QMinus g * QMinus g) * QPlus g := by noncomm_ring
            _ = QMinus g * QPlus g * QMinus g := by
              rw [hQMinus, zero_mul, add_zero]
