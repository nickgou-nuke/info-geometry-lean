...
* Remaining conditional interface obligations, with exact names:
  - `ρ_preserves_B`: requires a repo-native preservation lemma
    for the standard basis form on `Fin 32 → ℝ` under `recursiveGamma` /
    `spinorRepresentation 5`.
  - `exists_pin55_krein_conformal_package`: requires a finite
...
The file keeps these obligations as explicit hypotheses; it does not promote
them to unconditional consequences of the existing finite definitions.
-/
