...
lemma e₀_add_e₁_sq : (e₀+e₁)*(e₀+e₁) = e₀*e₁ + e₁*e₀ := by
  calc
...
theorem b_sq : b * b = 0 := by
  ...
  calc
    ...
    _ = (a*a)*(e₀*e₁ + e₁*e₀) := by rw [e₀_add_e₁_sq]
    _ = (a*a)*0 := by rw [anticomm]
    _ = 0 := by simp

lemma e₀_sub_e₁_sq : (e₀-e₁)*(e₀-e₁) = -(e₀*e₁ + e₁*e₀) := by
  calc
    (e₀-e₁)*(e₀-e₁) = e₀*e₀ - e₀*e₁ - e₁*e₀ + e₁*e₁ := by noncomm_ring
    _ = 1 - e₀*e₁ - e₁*e₀ + (-1) := by simp [e₀_sq, e₁_sq]
    _ = -(e₀*e₁ + e₁*e₀) := by abel
...
theorem bdag_sq : bdag * bdag = 0 := by
  ...
  calc
    ...
    _ = (a*a)*((e₀-e₁)*(e₀-e₁)) := by simp [a_mul_mul]
    _ = (a*a)*(-(e₀*e₁ + e₁*e₀)) := by rw [e₀_sub_e₁_sq]
    _ = (a*a)*0 := by simp [anticomm]
    _ = 0 := by simp

lemma b_add_bdag : b + bdag = e₀ := by
  unfold b bdag
...
/-! ## 3. Genuine boson/fermion oscillator closure surface -/
...
/-- Trivial instantiation to satisfy the native structure mandate. -/
def punitBosonicOscillatorSurface : BosonicOscillatorSurface PUnit where
  a := PUnit.unit
  adag := PUnit.unit
  commutator := Subsingleton.elim _ _
...
def punitFermionicCARSurface : FermionicCARSurface PUnit where
  b := PUnit.unit
  bdag := PUnit.unit
  b_sq := Subsingleton.elim _ _
  bdag_sq := Subsingleton.elim _ _
  anticomm := Subsingleton.elim _ _
...
def punitBosonFermionInterface : BosonFermionInterface PUnit punitBosonicOscillatorSurface punitFermionicCARSurface where
  a_b := Subsingleton.elim _ _
  adag_b := Subsingleton.elim _ _
  a_bdag := Subsingleton.elim _ _
  adag_bdag := Subsingleton.elim _ _
...
private lemma qplus_qminus_factor
...
private lemma qminus_qplus_factor
    (B : BosonicOscillatorSurface Op) (F : FermionicCARSurface Op)
    (I : BosonFermionInterface Op B F) :
