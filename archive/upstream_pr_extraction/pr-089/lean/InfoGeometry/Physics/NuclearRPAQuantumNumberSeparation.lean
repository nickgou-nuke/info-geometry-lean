...
theorem numberOp_comm_phononNumberOp (i : ι_qp) (j : ι_ph) :
  ...
  have hQ := commute_of_comm_eq_zero (sys.numberOp_comm_Q i j)
  have hQdag := commute_of_comm_eq_zero (sys.numberOp_comm_Qdag i j)
...
theorem centeredCartan_comm_phononNumberOp (i : ι_qp) (j : ι_ph) :
  ...
  have hN := sys.numberOp_comm_phononNumberOp i j
...
theorem car_rpa_quantum_number_separation_packet (i : ι_qp) (j : ι_ph) :
    NuclearPhononRPA.comm (sys.car.numberOp i) (sys.rpa.numberOp j) = 0 ∧
      ...
      NuclearPhononRPA.comm (sys.car.fermionParity i)
        (sys.rpa.numberOp j) = 0 :=
  ⟨sys.numberOp_comm_phononNumberOp i j,
    sys.centeredCartan_comm_phononNumberOp i j,
    sys.vacancyProjector_comm_phononNumberOp i j,
    sys.fermionParity_comm_phononNumberOp i j⟩
