...
def test_master_synthesis_exposes_canonical_drazin_zpe_thermal_bott_witness_capstone_route() -> None:
    text = SOURCE.read_text(encoding="utf-8")

    anchor = (
        "private theorem "
        "bits_to_gravity_to_fluid_capstone_of_canonicalDrazinFluidHelicityWitness_"
        "and_zpeGravityWitness_and_thermalBottWitness"
    )
    assert anchor in text
    block = text.split(anchor, 1)[1].split(
        "/--\nCapstone-facing squeezing bound:", 1
    )[0]
    ...
    assert "bits_to_gravity_to_fluid_capstone_of_canonicalDrazinFluidHelicityWitness_and_zpeGravityWitness" in block
