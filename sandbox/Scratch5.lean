...
lemma tsum_complex_cast_inv_sq :
  ...
  have h_eq : (fun n => (1 / ((n + 1 : ℝ) ^ 2) : ℂ)) = fun n => ↑(1 / ((n + 1 : ℝ) ^ 2)) := by
    ext n; push_cast; rfl
  rw [h_eq]
  exact Complex.ofReal_tsum _

lemma sum_shift : (∑' (n : ℕ), 1 / ((n + 1 : ℝ) ^ 2)) = Real.pi ^ 2 / 6 := by
  have h := Mathlib.NumberTheory.ZetaValues.hasSum_zeta_two.tsum_eq
  have h2 : (∑' (n : ℕ), 1 / (n : ℝ) ^ 2) = (1 / (0 : ℝ) ^ 2) + ∑' (n : ℕ), 1 / ((n + 1 : ℝ) ^ 2) := by
    exact Mathlib.NumberTheory.ZetaValues.hasSum_zeta_two.summable.tsum_eq_zero_add
  rw [h] at h2
