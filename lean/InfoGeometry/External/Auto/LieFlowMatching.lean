...
The model uses only axioms on `exp`/`log`; no concrete Lie group machinery
is assumed.
-/
...
/-- Flow generator extracted from a sample transformation `g`. -/
...
/-- Step 5–7 boundary condition: at `t = 0`, we are at transformed noise `g • x1`. -/
...
/-- Step 7 boundary condition: at `t = 1`, we recover the data point `x1`. -/
...
/-- Step-8 single-sample loss along CFM trajectory. -/
def sampleLoss (L : LieFlowChart G V) (v : X → ℝ → V)
    (x1 : X) (g : G) (t : ℝ) : ℝ :=
  let A : V := algebraSpeed L g
  flowLoss v (geodesicPoint L x1 g t) t A

/-- If velocity model is perfect, sample loss vanishes. -/
theorem sampleLoss_eq_zero_if_perfect (L : LieFlowChart G V) (v : X → ℝ → V)
    (x1 : X) (g : G) (t : ℝ)
    (h : ∀ x t, v x t = algebraSpeed L g) :
    sampleLoss L v x1 g t = 0 := by
  simp [sampleLoss, flowLoss, h]
...
def batchLoss (L : LieFlowChart G V) (v : X → ℝ → V)
  ...
  Finset.sum ts (fun t => sampleLoss L v x1 g t)
...
theorem batchLoss_eq_zero_if_perfect (L : LieFlowChart G V) (v : X → ℝ → V)
  ...
  intro t ht
  simpa using (sampleLoss_eq_zero_if_perfect L v x1 g t h)
...
def scheduledSampleLoss (L : LieFlowChart G V) (v : X → ℝ → V)
    (x1 : X) (g : G) (γ : ℝ) (t : ℝ) : ℝ :=
  let A : V := algebraSpeed L g
...
theorem scheduledSampleLoss_eq_zero_if_perfect (L : LieFlowChart G V) (v : X → ℝ → V)
    (x1 : X) (g : G) (γ : ℝ) (t : ℝ)
    (h : ∀ x t, v x t = algebraSpeed L g) :
    scheduledSampleLoss L v x1 g γ t = 0 := by
  simp [scheduledSampleLoss, flowLoss, h]
