...
namespace InfoGeometry.Algebra.Zorn.G2ResidualPairCoordinateCellEleven
...
def residualPair (i : Fin 189) : ResidualPair :=
  (factorWordExponent (gapLeftWitness 11 i),
    factorWordExponent (gapRightWitness 11 i))

set_option maxRecDepth 100000 in
set_option maxHeartbeats 0 in
theorem residualPair_injective_on_cell_eleven :
    ∀ i j : Fin 189,
...
theorem residualPair_cell_eleven_coordinate_separation :
    ∀ i j : Fin 189,
      i ∈ orbitCells 11 → j ∈ orbitCells 11 → i ≠ j →
      residualPair i ≠ residualPair j := by
  intro i j hi hj hne heq
  exact hne (residualPair_injective_on_cell_eleven i j hi hj heq)

end InfoGeometry.Algebra.Zorn.G2ResidualPairCoordinateCellEleven
