...
The certificate subtype carries nonnegative couplings and strictly positive
...
theorem continuous_certificate_coupling :
    Continuous (fun T : Certificate Row Col => T.coupling) := by
  fun_prop

theorem continuous_certificate_row_mass (i : Row) :
  ...
  exact (transportRowMassContinuousMap i).continuous.comp
    continuous_certificate_coupling

theorem continuous_certificate_col_mass (j : Col) :
...
theorem continuous_certificate_row_penalty :
  ...
  let mass : C(Certificate Row Col, {x : ℝ // 0 < x}) :=
    ContinuousMap.mk
      (fun T => ⟨transportRowMass T.coupling i, T.row_mass_pos i⟩) (by
        ...
        exact continuous_certificate_row_mass i)
...
theorem continuous_certificate_col_penalty :
  ...
  let mass : C(Certificate Row Col, {x : ℝ // 0 < x}) :=
    ContinuousMap.mk
      (fun T => ⟨transportColMass T.coupling j, T.col_mass_pos j⟩) (by
        apply Continuous.subtype_mk
        exact continuous_certificate_col_mass j)
  let target : C(Certificate Row Col, {x : ℝ // 0 < x}) :=
    ContinuousMap.mk
...
theorem continuous_certificate_transport_cost
    (cost : Row → Col → ℝ) :
  ...
  have hcoupling : Continuous
    ...
    exact (continuous_apply j).comp
      ((continuous_apply i).comp continuous_certificate_coupling)
  exact hcoupling.mul continuous_const

theorem continuous_unbalancedTransportObjective_certificate
  ...
  have hcost := continuous_certificate_transport_cost cost
  have hrow : Continuous (fun T : Certificate Row Col =>
      rowPenalty * unbalancedRowPenalty T) :=
    continuous_const.mul continuous_certificate_row_penalty
  have hcol : Continuous (fun T : Certificate Row Col =>
      colPenalty * unbalancedColPenalty T) :=
    continuous_const.mul continuous_certificate_col_penalty
  exact (hcost.add hrow).add hcol

theorem unbalancedTransportObjective_certificate_sublevel_isClosed
  ...
  exact isClosed_Iic.preimage
    (continuous_unbalancedTransportObjective_certificate cost rowPenalty colPenalty)
