  ...
  have hsq :
    ...
    calc
      ...
      _ = (∑ x, μ x * (L x) ^ 2) - (2 * U) * (∑ x, μ x * L x) + (∑ x, μ x) * U ^ 2 := by
            rw [(Finset.mul_sum (Finset.univ : Finset α) (fun x => μ x * L x) (2 * U)).symm]
            rw [Finset.sum_mul]
        ...
        Real.exp c = ∑ x, P x := by
          calc
            ...
            _ = ∑ x, Real.exp c * Q.prob x := by
                  rw [(Finset.mul_sum (Finset.univ : Finset α) (fun x => Q.prob x) (Real.exp c)).symm]
            _ = ∑ x, P x := by
                  refine Finset.sum_congr rfl ?_
                  intro x hx
                  symm
...
theorem d2_grandMassieuPotential_beta_eq_varianceHamiltonian
  ...
  calc
    ...
    _ = M2 β / Z β - (M1 β / Z β) ^ 2 := by
          have hZβ : Z β ≠ 0 := grandCanonicalPartition_ne_zero E N β μ
          field_simp [hZβ]
  ...
  have hsq :
    ...
    calc
      ...
      _ = β ^ 2 * ∑ x, p x * (N x) ^ 2 := by
        rw [(Finset.mul_sum (Finset.univ : Finset α) (fun x => p x * (N x) ^ 2) (β ^ 2)).symm]
  have hlin :
      (∑ x, p x * (β * N x)) ^ 2 = β ^ 2 * (∑ x, p x * N x) ^ 2 := by
    have hsum :
