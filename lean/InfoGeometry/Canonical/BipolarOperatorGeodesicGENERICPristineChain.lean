...
theorem pristine_flat_coordinate_geodesic_core
    (p : InfoGeometry.Analysis.BipolarPlanarHodgePair.PlaneCovector) :
...
theorem pristine_flat_coordinate_derivative_core
    (p : InfoGeometry.Analysis.BipolarPlanarHodgePair.PlaneCovector) (t : ℝ) :
    HasDerivAt (negativeEtaLine p)
        (-InfoGeometry.Analysis.BipolarFlatCoordinateGeodesics.etaBasis) t ∧
      HasDerivAt (thetaLine p)
        InfoGeometry.Analysis.BipolarFlatCoordinateGeodesics.thetaBasis t ∧
...
theorem pristine_BKM_curvature_readout_core {n : ℕ}
    ...
    (Q Kη Kθ Lη Lθ : FiniteOperatorAlgebra n)
    (u v : Tangent2) :
...
theorem pristine_observable_metriplectic_core
  ...
  exact bipolar_observable_metriplectic_packet D ev H S F G K hS hH
