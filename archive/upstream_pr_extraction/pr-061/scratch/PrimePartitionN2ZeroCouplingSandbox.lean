...
namespace InfoGeometry.Canonical.PrimePartitionN2ZeroCouplingSandbox
...
theorem interactionEnergy_fin_two_formula
...
theorem occupiedCount_fin_two_formula (b₀ b₁ : Bool) :
    occupiedCount (configEquiv.symm (b₀, b₁)) =
...
theorem partitionPolynomial_fin_two_zero_eq (D : FinitePrimeChainData 2) :
    partitionPolynomial D 0 =
      Polynomial.C (1 : ℂ) + Polynomial.C (2 : ℂ) * Polynomial.X +
        Polynomial.X ^ 2 := by
  classical
  unfold partitionPolynomial
  let e := configEquiv
  let f : (Fin 2 → Bool) → Polynomial ℂ := fun σ =>
    Polynomial.C ((configurationWeight D 0 σ : ℝ) : ℂ) *
  ...
  · rw [Fintype.sum_prod_type, Fintype.sum_bool, Fintype.sum_bool]
    ...
    simp [g, f, configurationWeight, interactionEnergy, occupiedCount,
...
theorem partitionPolynomial_fin_two_eq
...
theorem palindromic_quadratic_root_on_unit_circle
  ...
  · have himfac : 2 * z.im * (a * z.re + b) = 0 := by
    ...
    have hrel : a * z.re + b = 0 := by
      ...
      · apply (hy (by nlinarith [hy0])).elim
      · exact hrel0
...
theorem partitionPolynomial_fin_two_root_on_unit_circle
    (D : FinitePrimeChainData 2) {lam : ℝ} (hLam : 0 ≤ lam)
    {z : ℂ} (hz : (partitionPolynomial D lam).IsRoot z) :
    OnUnitCircle z := by
  rw [partitionPolynomial_fin_two_eq D lam] at hz
...
theorem partitionPolynomial_fin_two_root_re_ne_neg_one
    (D : FinitePrimeChainData 2) {lam : ℝ} (hlam : 0 < lam)
    {z : ℂ} (hz : (partitionPolynomial D lam).IsRoot z) :
    z.re ≠ -1 := by
  rw [partitionPolynomial_fin_two_eq D lam] at hz
...
theorem partitionPolynomial_fin_two_root_maps_to_critical_line
    (D : FinitePrimeChainData 2) {lam : ℝ} (hlam : 0 < lam)
    {z : ℂ} (hz : (partitionPolynomial D lam).IsRoot z) :
  ...
  exact cayleyToTemperature_mem_criticalLine_of_unitCircle z
    (partitionPolynomial_fin_two_root_on_unit_circle D (le_of_lt hlam) hz)
    (partitionPolynomial_fin_two_root_re_ne_neg_one D hlam hz)

theorem partitionPolynomial_fin_two_zero_root_eq_neg_one
    (D : FinitePrimeChainData 2) {z : ℂ}
    (hz : (partitionPolynomial D 0).IsRoot z) :
    z = -1 := by
  rw [partitionPolynomial_fin_two_zero_eq D] at hz
  have hzpoly : 1 + 2 * z + z * z = 0 := by
    simpa [Polynomial.IsRoot, pow_two] using hz
  have hmul : (z + 1) * (z + 1) = 0 := by
    calc
      (z + 1) * (z + 1) = 1 + 2 * z + z * z := by ring
      _ = 0 := hzpoly
  have hzsum : z + 1 = 0 := by
...
theorem partitionPolynomial_fin_two_zero_root_on_unit_circle
    (D : FinitePrimeChainData 2) {z : ℂ}
    (hz : (partitionPolynomial D 0).IsRoot z) :
    OnUnitCircle z := by
  rw [partitionPolynomial_fin_two_zero_root_eq_neg_one D hz]
  norm_num [OnUnitCircle]

end InfoGeometry.Canonical.PrimePartitionN2ZeroCouplingSandbox
