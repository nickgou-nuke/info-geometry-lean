...
structure CompatibleOperatorFamily where
  ...
  intertwines :
    ∀ {i j : I} (hij : i ≤ j) (x : E i),
      sys.map hij (op i x) = op j (sys.map hij x)
  bound : ℝ
  norm_le :
    ∀ i (x : E i), ‖op i x‖ ≤ bound * ‖x‖

variable (T : CompatibleOperatorFamily E sys)
...
def algebraicOperatorReal :
  ...
  Module.DirectLimit.lift
    ...
    (stageOperatorToDirectLimitRealMap_compatible E sys T)

@[simp] theorem algebraicOperatorReal_of
    (i : I) (x : E i) :
    algebraicOperatorReal E sys T
        (Module.DirectLimit.of
          ℝ I E
          (fun _ _ hij => realTransition E sys hij)
          i x) =
...
def algebraicOperator :
    D∞ →ₗ[ℂ] D∞ where
  toFun := algebraicOperatorReal E sys T
  map_add' := (algebraicOperatorReal E sys T).map_add
  map_smul' := by
    intro c z
...
def algebraicOperatorCLM :
    D∞ →L[ℂ] D∞ :=
  (algebraicOperator E sys T).mkContinuous
    T.bound
    (algebraicOperator_norm_le E sys T)

@[simp] theorem algebraicOperatorCLM_apply
    (z : D∞) :
    algebraicOperatorCLM E sys T z =
      algebraicOperator E sys T z :=
  rfl
...
def completedOperator :
    H∞ →L[ℂ] H∞ :=
  (algebraicOperatorCLM E sys T).completion
...
theorem completedOperator_coe
    (z : D∞) :
    completedOperator E sys T
        (directLimitToCompletion E sys z) =
      directLimitToCompletion E sys
        (algebraicOperator E sys T z) := by
  apply UniformSpace.Completion.map_coe
  exact (algebraicOperatorCLM E sys T).uniformContinuous
