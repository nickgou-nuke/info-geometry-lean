...
def standardHadjiivanovBraidProjectHom (n : ℕ) :
  ...
  braid_group.toGroup (standardTensorPowerGenerator n)
    (standardTensorPower_braidRelations n)
...
theorem standardHadjiivanovBraidProject_sigma
  ...
  exact braid_group.toGroup_sigma (standardTensorPowerGenerator n)
    (standardTensorPower_braidRelations n) i
