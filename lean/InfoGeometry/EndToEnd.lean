...
theorem map_zero : D 0 = 0 := by
  have h : D 0 + D 0 = D 0 := by rw [← D.map_add, add_zero]
  have h2 : (D 0 + D 0) - D 0 = D 0 - D 0 := congrArg (fun x => x - D 0) h
  simpa using h2
...
theorem geometric_commutator_uncertainty_bound
    (ψ : H) (X Y : EndH)
    (h_cauchy : (fisherMetric ψ X X) * (fisherMetric ψ Y Y) ≥ Complex.normSq (QGT ψ X Y))
    (h_comm_curv : (berryCurvature ψ X Y : ℂ) * Complex.I = inner (𝕜 := ℂ) ψ (opCommutator X Y ψ)) :
    (fisherMetric ψ X X) * (fisherMetric ψ Y Y) ≥ (1 / 4 : ℝ) * Complex.normSq (inner (𝕜 := ℂ) ψ (opCommutator X Y ψ)) := by
