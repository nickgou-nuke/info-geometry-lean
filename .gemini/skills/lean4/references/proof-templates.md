...
theorem my_theorem (n : ℕ) : conclusion := by
  -- FIXME: Strategy - Describe proof approach here
  -- Step 1: [Describe what needs to be shown]
  have h1 : _ := by
    sorry
    -- FIXME: Prove first key property

  -- Step 2: [Describe next step]
  have h2 : _ := by
    sorry
    -- FIXME: Prove second key property

  -- Step 3: Combine results
  sorry
  -- FIXME: Apply h1 and h2 to conclude
```
...
theorem cases_example (h : a ∨ b) : c := by
  ...
  | inl h_left =>
    -- Case 1: Left branch
    sorry
    -- FIXME: Handle left case
    -- Available: h_left

  | inr h_right =>
    -- Case 2: Right branch
    sorry
    -- FIXME: Handle right case
    -- Available: h_right
```
...
theorem exists_example : ∃ x, P x ∧ Q x := by
  ...
  use witness_value
  -- FIXME: Provide the witness value

  constructor
  · -- Prove first property
    sorry
    -- FIXME: Show witness satisfies first condition

  · -- Prove second property
    sorry
    -- FIXME: Show witness satisfies second condition
```
...
theorem strong_induction (n : ℕ) : P n := by
  ...
  | _ n ih =>
    -- ih : ∀ m < n, P m
    sorry
    -- FIXME: Use ih for all smaller values
```
...
2. **Fill in FIXMEs** - Replace placeholders with actual proof steps
