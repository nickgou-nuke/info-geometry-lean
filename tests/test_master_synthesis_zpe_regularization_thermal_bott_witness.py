...
def test_master_synthesis_exposes_zpe_regularization_thermal_bott_witness_capstone_route() -> None:
    text = SOURCE.read_text(encoding="utf-8")

    anchor = (
        "private theorem "
        "bits_to_gravity_to_fluid_capstone_of_regularization_of_zpeGravityWitness_and_thermalBottWitness"
    )
    assert anchor in text
    block = text.split(anchor, 1)[1].split(
        "/--\nProduction-witness capstone with the ZPE/gravity owner packet.", 1
    )[0]
    assert "(hZPEGravity : ZPEGravityWitness (E := E) S c R Kgeo x)" in block
    assert "(hReg : RegularizationWitness (E := E) A B_mp B_dr)" in block
    assert "(hHelicity : MatchedHelicityWitness (E := E) A)" in block
