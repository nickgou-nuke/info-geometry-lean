...
This module records a Lean 4.28-compatible braid generator-index skeleton:

* finite braid stages have generators `Fin n`;
* the successor inclusion is `Fin.castSucc`;
* every finite generator embeds into the infinite generator boundary `ℕ`;
* adjacent and separated Artin index predicates are stable under the
  finite-to-infinite embedding.

It is intentionally independent of the heavier braid-group files.  It does not
construct a categorical colimit of braid groups; it only proves stability of
finite generator indices, relation predicates, and word readouts under the
finite-to-infinite boundary map.
-/

noncomputable section

namespace BraidProject.InductiveColimitComplement
...
end BraidProject.InductiveColimitComplement
