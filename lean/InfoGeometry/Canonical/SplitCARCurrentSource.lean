...
/-- Source-side closure theorem connecting raw CAR completion to the explicit charged-Fock current. -/
theorem rawCAR_to_exteriorFock_source_witness
...
Every raw CAR mode completion induces a `SplitCARCurrentWitness` on
`ChargedFockSpace` with truncation and the Heisenberg commutator law.
-/
theorem rawCAR_to_SplitCARCurrentWitness_exists
