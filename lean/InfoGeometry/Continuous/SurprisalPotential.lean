...
-- This is a function on the product manifold PositiveOrthant × PositiveOrthant
theorem contDiffOn_relativeModularPotential :
    ContDiffOn ℝ ⊤ (fun p : ((α → ℝ) × (α → ℝ)) => ∑ a : α, p.1 a * Real.log (p.1 a / p.2 a))
      ((PositiveOrthant : Set (α → ℝ)) ×ˢ (PositiveOrthant : Set (α → ℝ))) := by
  have h₁ : ∀ a : α, ContDiffOn ℝ ⊤
      (fun p : ((α → ℝ) × (α → ℝ)) => p.1 a * Real.log (p.1 a / p.2 a))
      ((PositiveOrthant : Set (α → ℝ)) ×ˢ (PositiveOrthant : Set (α → ℝ))) := by
    intro a
    have h₂ : ContDiffOn ℝ ⊤ (fun p : ((α → ℝ) × (α → ℝ)) => p.1 a) ((PositiveOrthant : Set (α → ℝ)) ×ˢ (PositiveOrthant : Set (α → ℝ))) := by
      apply ContDiffOn.comp (contDiffOn_fst) (contDiffOn_const (0 : ℕ))
      <;> simp [PositiveOrthant]
      <;> aesop
    have h₃ : ContDiffOn ℝ ⊤ (fun p : (α → ℝ) × (α → ℝ) => p.2 a) (PositiveOrthant : Set (α → ℝ) ×ˢ PositiveOrthant : Set (α → ℝ)) := by
      apply ContDiffOn.comp (contDiffOn_snd) (contDiffOn_const (0 : ℕ))
      <;> simp [PositiveOrthant]
      <;> aesop
    have h₄ : ContDiffOn ℝ ⊤ (fun p : (α → ℝ) × (α → ℝ) => p.1 a / p.2 a) (PositiveOrthant : Set (α → ℝ) ×ˢ PositiveOrthant : Set (α → ℝ)) := by
      have h₅ : ∀ p : (α → ℝ) × (α → ℝ), p ∈ (PositiveOrthant : Set (α → ℝ)) ×ˢ (PositiveOrthant : Set (α → ℝ)) → p.2 a ≠ 0 := by
        intro p hp
        ...
        have h₇ : 0 < p.2 a := h₆ a
        linarith
      have h₆ : ContDiffOn ℝ ⊤ (fun p : (α → ℝ) × (α → ℝ) => (p.1 a : ℝ) / p.2 a) (PositiveOrthant : Set (α → ℝ) ×ˢ PositiveOrthant : Set (α → ℝ)) := by
        apply ContDiffOn.div h₂ h₃
        intro p hp
        exact h₅ p hp
      have h₇ : ContDiffOn ℝ ⊤ (fun p : (α → ℝ) × (α → ℝ) => Real.log (p.1 a / p.2 a)) (PositiveOrthant : Set (α → ℝ) ×ˢ PositiveOrthant : Set (α → ℝ)) := by
        have h₈ : ∀ p : (α → ℝ) × (α → ℝ), p ∈ (PositiveOrthant : Set (α → ℝ)) ×ˢ (PositiveOrthant : Set (α → ℝ)) → (p.1 a / p.2 a : ℝ) > 0 := by
          intro p hp
        ...
        have h₉ : ContDiffOn ℝ ⊤ (fun p : (α → ℝ) × (α → ℝ) => Real.log (p.1 a / p.2 a)) (PositiveOrthant : Set (α → ℝ) ×ˢ PositiveOrthant : Set (α → ℝ)) := by
          ...
          intro p hp
          exact h₈ p hp
        have h₁₀ : ContDiffOn ℝ ⊤ (fun p : (α → ℝ) × (α → ℝ) => (p.1 a : ℝ) * Real.log (p.1 a / p.2 a)) (PositiveOrthant : Set (α → ℝ) ×ˢ PositiveOrthant : Set (α → ℝ)) := by
      ...
      exact h₁₀
  -- Sum over a is C^∞
  have h₂ : ContDiffOn ℝ ⊤ (fun p : (α → ℝ) × (α → ℝ) => ∑ a : α, p.1 a * Real.log (p.1 a / p.2 a)) ((PositiveOrthant : Set (α → ℝ)) ×ˢ (PositiveOrthant : Set (α → ℝ))) := by
    have h₃ : ∀ s : Finset α, ContDiffOn ℝ ⊤ (fun p : (α → ℝ) × (α → ℝ) => ∑ a in s, p.1 a * Real.log (p.1 a / p.2 a)) ((PositiveOrthant : Set (α → ℝ)) ×ˢ (PositiveOrthant : Set (α → ℝ))) := by
      intro s
      induction' s using Finset.induction_on with a s has
      · simp [contDiffOn_const]
      · rw [Finset.sum_insert has]
