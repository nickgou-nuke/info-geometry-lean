...
/-- Reversed product of the two scalar half-weights. -/
theorem minusWeight_mul_plusWeight (s : ℂ) :
    minusWeight s * plusWeight s = 1 := by
  rw [mul_comm, plusWeight_mul_minusWeight]

/-- The explicit inverse is a right inverse of the half-lift. -/
theorem halfLogLift_mul_inv (s : ℂ) :
  ...
  fin_cases i <;> fin_cases j <;>
    simp [halfLogLift, halfLogLiftInv, Matrix.mul_apply, Fin.sum_univ_two,
      plusWeight_mul_minusWeight, minusWeight_mul_plusWeight]

/-- The explicit inverse is also a left inverse. -/
theorem halfLogLift_inv_mul (s : ℂ) :
    halfLogLiftInv s * halfLogLift s = 1 := by
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [halfLogLift, halfLogLiftInv, Matrix.mul_apply, Fin.sum_univ_two,
      plusWeight_mul_minusWeight, minusWeight_mul_plusWeight]
