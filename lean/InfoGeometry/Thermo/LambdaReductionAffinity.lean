...
theorem cycleHolonomy_eq_exp_affinity
  ...
  exact (Real.exp_log
    (div_pos (rates.k_pos_pos (C.steps i))
      (rates.k_neg_pos (C.steps i)))).symm
...
theorem log_mean_inequality_nonneg {a b : ℝ}
  ...
  · have h_ratio_lt : a / b < 1 :=
    ...
    exact mul_nonneg_of_nonpos_of_nonpos (sub_nonpos.mpr h_lt.le) h_log_neg.le
  · subst b
    simp
...
abbrev Step : Term → Term → Prop :=
  InfoGeometry.Canonical.LambdaCausalNetNegativeGrammarBridge.LambdaTerm.BetaStepExists
...
theorem betaStep_to_step
    ...
    Step t u :=
  InfoGeometry.Canonical.LambdaCausalNetNegativeGrammarBridge.LambdaTerm.betaStep_exists h
