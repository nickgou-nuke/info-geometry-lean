...
Mathematically, `A = E_p[∇² E]`.
-/
def fisherStiffness
    (p : Data → ℝ) (Hi : Data → Matrix (Fin k) (Fin k) ℝ) :
    Matrix (Fin k) (Fin k) ℝ :=
  ∑ i : Data, p i • Hi i

/--
Fluctuation pressure `B`: the variance term of informational gradients.
Modeled as `(1/ε) * Cov_p(∇E)`.
-/
noncomputable def fluctuationPressure
    (p : Data → ℝ) (gi : Data → Fin k → ℝ) (ε : ℝ) :
    Matrix (Fin k) (Fin k) ℝ :=
  -- `B_ab = (1/ε) * Σ p_i * (g_ia - ḡ_a)(g_ib - ḡ_b)`.
  let g_avg : Fin k → ℝ := fun a => ∑ i : Data, p i * gi i a
  fun a b => (1 / ε) * ∑ i : Data, p i * (gi i a - g_avg a) * (gi i b - g_avg b)

/-- Exact Hessian decomposition `H = A - B`. -/
...
theorem phase_transition_catastrophe
  ...
  (h_unstable : ∃ x : Fin k →₀ ℝ, x ≠ 0 ∧
    (x.sum fun i xi => x.sum fun j xj => xi * B i j * xj) ≥
      (x.sum fun i xi => x.sum fun j xj => xi * A i j * xj)) :
...
by
  ...
  have h_sum_sub : (x.sum fun i xi => x.sum fun j xj => star xi * (A - B) i j * xj) =
    (x.sum fun i xi => x.sum fun j xj => xi * A i j * xj) -
      (x.sum fun i xi => x.sum fun j xj => xi * B i j * xj) := by
