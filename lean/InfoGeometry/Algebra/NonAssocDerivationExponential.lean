...
theorem isDerivation_of_representsNativeMul
    ...
    InfoGeometry.Lie.ContinuousDerivationExponential.IsDerivation mul D := by
  intro x y
...
theorem continuousDerivation_kills_one
  ...
  have h : D (1 : A) = D (1 : A) + D (1 : A) := by
    simpa [one_mul', mul_one'] using hD (1 : A) (1 : A)
  have hz : D (1 : A) + 0 = D (1 : A) + D (1 : A) := by
    simpa using h
...
theorem continuousDerivation_kills_smul_one
    ...
    D (c • (1 : A)) = 0 := by
  rw [map_smul]
  rw [continuousDerivation_kills_one D hD one_mul' mul_one']
  simp
...
def expNonAssocDerivation
  ...
  toLinearEquiv := flowLinearEquiv D t
  map_one' :=
    flowLinearEquiv_fixed_of_derivation_eq_zero D 1
      (continuousDerivation_kills_one D hD one_mul' mul_one') t
  map_mul' x y := by
    change flowLinearEquiv D t (x * y) =
      flowLinearEquiv D t x * flowLinearEquiv D t y
    rw [← hmul x y, ← hmul (flowLinearEquiv D t x)
      (flowLinearEquiv D t y)]
    exact exponential_derivation_is_automorphism mul D
...
theorem expNonAssocDerivation_neg_apply
  ...
  exact flowLinearEquiv_neg_apply D t x
...
theorem expNonAssocDerivation_fixed_of_mem_ker
  ...
  exact flowLinearEquiv_fixed_of_derivation_eq_zero D x hx t
...
theorem expNonAssocDerivation_fixed_smul_one
    ...
    expNonAssocDerivation mul hmul D hD one_mul' mul_one' t
        (c • (1 : A)) = c • (1 : A) := by
  exact flowLinearEquiv_fixed_of_derivation_eq_zero D _
...
theorem expNonAssocDerivation_orbit_hasDerivAt
  ...
  exact (hasStrictDerivAt_orbit D u₀ t).hasDerivAt
...
def expFiniteDimensionalNonAssocDerivation
    ...
    (mul_one' : ∀ x : A, x * (1 : A) = x)
    (t : ℝ) : NonAssocAut ℝ A :=
  expNonAssocDerivation mul hmul (continuousEndOfLinear D)
    (by intro x y; simpa using hD x y)
    one_mul' mul_one' t
...
theorem expFiniteDimensionalNonAssocDerivation_apply
    ...
    expFiniteDimensionalNonAssocDerivation mul hmul D hD
        one_mul' mul_one' t x =
      flow (continuousEndOfLinear D) t x :=
  rfl
