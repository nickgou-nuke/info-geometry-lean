...
theorem logCoordinates_injective [Nonempty ι] :
  ...
  change Real.exp (Real.log (w i / w j)) =
    Real.exp (Real.log (v i / v j)) at hij
  calc
    w i / w j = Real.exp (Real.log (w i / w j)) :=
      (Real.exp_log (div_pos (w.pos i) (w.pos j))).symm
    _ = Real.exp (Real.log (v i / v j)) := hij
    _ = v i / v j := Real.exp_log (div_pos (v.pos i) (v.pos j))
...
theorem rayMean_add (q : Ray ι) (f g : ι → ℝ) :
  ...
  refine Quotient.inductionOn q ?_
  intro w
  change (∑ i, w i * (f i + g i)) / PositiveMeasure.Z w =
    (∑ i, w i * f i) / PositiveMeasure.Z w +
      (∑ i, w i * g i) / PositiveMeasure.Z w
  rw [show (∑ i, w i * (f i + g i)) =
...
@[simp] theorem rayMean_const (q : Ray ι) (c : ℝ) :
  ...
  rw [div_eq_iff (PositiveMeasure.Z_ne_zero w)]
  ring
