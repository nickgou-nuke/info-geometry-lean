...
theorem sigma1_forward_backward
  ...
  change (evalGenerator q τ s hs hτ .sigma1).trans
      (evalGenerator q τ s hs hτ (SignedGenerator.sigma1).inv) = _
  rw [evalGenerator_inv]
  exact LinearEquiv.self_trans_symm _
...
theorem sigma2_forward_backward
    ...
    (evalGenerator q τ s hs hτ .sigma2).trans
      ...
      LinearEquiv.refl ℂ FusionTree := by
  change (evalGenerator q τ s hs hτ .sigma2).trans
      (evalGenerator q τ s hs hτ (SignedGenerator.sigma2).inv) = _
  rw [evalGenerator_inv]
  exact LinearEquiv.self_trans_symm _
