...
theorem mapCoefficients_mul (f : A →+* B) (X Y : OperatorZornMatrix A) :
  ...
  all_goals simp only [map_add, map_sub, map_mul]
...
theorem mapCoefficients_associator (f : A →+* B) (X Y Z : OperatorZornMatrix A) :
    mapCoefficients f (associator X Y Z) =
      associator (mapCoefficients f X) (mapCoefficients f Y) (mapCoefficients f Z) := by
  unfold associator
...
theorem transformed_generator_eq (g : Aˣ) (p : A) :
  ...
  rw [sub_mul, mul_assoc p (g : A), g.val_inv, mul_one]
  abel
