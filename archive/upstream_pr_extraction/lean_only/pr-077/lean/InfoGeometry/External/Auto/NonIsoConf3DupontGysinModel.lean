...
This file formalizes the finite stratum-poset and cooperad bookkeeping for these
three divisors: Boolean masks, Gysin covers, codimension shifts, generator data,
and the arity-three cluster-collapse map.
-/
...
def DupontToyGenerator.betaRank (g : DupontToyGenerator) : ℕ :=
  ((Finset.univ : Finset (Fin 2)).filter (fun i => g.beta i)).card
...
theorem toyDupontBoundary_codim_shift (g : DupontToyGenerator)
  ...
  have hdFalse : g.stratum.contains d = false := by
    have hd' : d ∈ divisorUniverse ∧ g.stratum.contains d = false := by
      simpa [StratumMask.inactiveDivisors] using hd
    exact hd'.2
  simpa using stratum_insert_codim g.stratum d hdFalse

-- Main finite scaffold combining masks and covers.
theorem dupont_gysin_scaffold :
    Fintype.card StratumMask = 8 ∧
    Fintype.card GysinCover = 12 ∧
    Fintype.card DupontToyGenerator = 32 := by
  exact ⟨stratumMask_card, gysinCover_card, dupontToyGenerator_card⟩
