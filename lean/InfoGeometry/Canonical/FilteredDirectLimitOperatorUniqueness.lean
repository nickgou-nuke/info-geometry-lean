...
theorem directLimitOperator_unique
    ...
    T = directLimitOperator Stage f F := by
  ...
  rw [hT n x]
  symm
  exact directLimitOperator_of Stage f F n x
    ...
    (hU :
      ∀ n x,
        U (AddCommGroup.DirectLimit.of Stage f n x) =
          AddCommGroup.DirectLimit.of Stage f n (F.op n x)) :
    T = U := by
  rw [F.directLimitOperator_unique Stage f T hT]
  symm
  exact F.directLimitOperator_unique Stage f U hU
...
theorem existsUnique_directLimitOperator :
  ...
  refine ⟨directLimitOperator Stage f F, ?_, ?_⟩
  · intro n x
    exact directLimitOperator_of Stage f F n x
  · intro T hT
    exact F.directLimitOperator_unique Stage f T hT
