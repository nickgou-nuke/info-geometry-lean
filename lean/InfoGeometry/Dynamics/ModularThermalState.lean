...
-/
noncomputable def unruhTrivialModularFlow : ModularAutomorphismFamily Obs where
...
-/
noncomputable def unruhTrivialSigmaC : ℂ → Obs → Obs :=
  fun z a => (unruhTrivialModularFlow (E := E)).sigma (z.re + z.im) a
...
noncomputable def unruhHestenesKreinAnalyticContinuation (beta : ℝ) :
  ...
  beta := beta
  modular := unruhTrivialModularFlow (E := E)
  sigmaC := unruhTrivialSigmaC (E := E)
  agrees_real_axis := by
    intro t a
    simp [unruhTrivialSigmaC]
  strip_top_boundary := by
    intro t a
    simp [unruhTrivialSigmaC, complexClockPoint, add_comm]
