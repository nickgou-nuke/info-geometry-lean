...
structure ContinuousStarRepresentationCocone
...
def topologicalRepresentationCocone
  ...
  ι :=
    { app := fun i =>
        TopCat.ofHom
          { toFun := R.ι i
            continuous_toFun := hcontinuous_ι i }
      naturality := by
        intro i j f
...
@[simp] theorem algebraicRepresentation_of_stage
    (hι_comm : ∀ {i j : I} (hij : i ≤ j),
      (R.ι j).comp (sys.map hij) = R.ι i)
    (i : I) (x : Stage i) :
    algebraicRepresentation Stage sys R hι_comm
...
@[simp] theorem topologicalRepresentation_of_stage
  ...
  have h := topologicalDirectDescend_stage
    ...
    (topologicalRepresentationCocone Stage sys R hι_comm hcontinuous_ι) i
...
theorem topologicalRepresentation_star_intertwines [ContinuousStar B]
  ...
  apply ContinuousMap.ext
  intro x
  change topologicalRepresentation Stage sys R hι_comm hcontinuous_ι
      (topologicalStarReadout Stage sys
        (topologicalInjection Stage sys i x)) =
    star (topologicalRepresentation Stage sys R hι_comm hcontinuous_ι
      (topologicalInjection Stage sys i x))
...
theorem topologicalRepresentation_comp_algebraicToTopological
    ...
    topologicalRepresentation Stage sys R hι_comm hcontinuous_ι
      ...
      algebraicRepresentation Stage sys R hι_comm x := by
...
theorem algebraicRepresentation_unique
    ...
    (hF : ∀ i,
      F.comp (algebraicStarDirectLimitOf Stage sys i) = R.ι i) :
        F = algebraicRepresentation Stage sys R hι_comm := by
  apply liftStarAlgHom_unique Stage sys R.ι
    (fun hij x =>
      congrArg (fun g : Stage _ →⋆ₐ[ℂ] B => g x) (hι_comm hij)) F hF
...
theorem topologicalRepresentation_unique
    ...
    (hf : ∀ (i : I) (x : Stage i),
      ...
      f = topologicalRepresentation Stage sys R hι_comm hcontinuous_ι := by
  apply topologicalDirectDescend_unique
    (topologicalDiagram Stage sys)
    (topologicalRepresentationCocone Stage sys R hι_comm hcontinuous_ι) f
  intro i
  apply TopCat.hom_ext
  apply ContinuousMap.ext
  intro x
