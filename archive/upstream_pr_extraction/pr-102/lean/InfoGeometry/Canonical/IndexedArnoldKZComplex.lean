...
def Conf (n : ℕ) (K : Type*) [Field K] : Type :=
...
def dlogForm
    {n : ℕ} {K : Type*} [Field K]
    (z : Conf n K) (dz : Fin n → Tangent n K)
    (i j : Fin n) : Tangent n K :=
...
theorem dlog_partial_fraction
  ...
  have hki' : z.1 k - z.1 i ≠ 0 := by
    exact sub_ne_zero.mpr (z.2 hik.symm)
...
def totalTripleCurvature
    ...
    (alg : ExteriorFormAlgebra (R := K) A)
    (z : Conf n K) (dz : Fin n → A)
    (R : InfinitesimalBraidResidues n A) : A :=
