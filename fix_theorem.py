...
proof = """theorem circularNullBoundaryFlow_fixed_iff_of_ne_zero (t : ℝ) (ht : t ≠ 0)
  ...
  set x := p.1.rep
  have hx : x ≠ 0 := Projectivization.rep_nonzero p.1
  ...
  have hproj_aux : hyperbolicFlowCoordinate t x ≠ 0 := by simp [hx]
  have hproj : circularNullBoundaryFlow t p = p ↔
    ...
    · intro h
      ...
      rw [hp_eq] at h1 ⊢
      have aux : (circularNullBoundaryFlow t ⟨Projectivization.mk ℝ x hx, (by rw [← hp_eq]; exact p.2)⟩).1 = Projectivization.mk ℝ (hyperbolicFlowCoordinate t x) hproj_aux := by
        rw [circularNullBoundaryFlow_mk_axialFlow]
      rw [aux] at h1
      exact h1
    · intro h
      apply Subtype.ext
      have h1 : (circularNullBoundaryFlow t p).1 = Projectivization.mk ℝ (hyperbolicFlowCoordinate t x) hproj_aux := by
        rw [hp_eq]
        have aux : (circularNullBoundaryFlow t ⟨Projectivization.mk ℝ x hx, (by rw [← hp_eq]; exact p.2)⟩).1 = Projectivization.mk ℝ (hyperbolicFlowCoordinate t x) hproj_aux := by
          rw [circularNullBoundaryFlow_mk_axialFlow]
        exact aux
  ...
  · intro hfixed
    obtain ⟨a, ha, hax⟩ :=
      (Projectivization.mk_eq_mk_iff ℝ (hyperbolicFlowCoordinate t x) x hproj_aux hx).mp hfixed
    have hc (i : Fin 8) : a * x i =
        Real.exp (t * InfoGeometry.Lie.SplitOctonionCircularAxialGrading.axialWeight i) * x i := by
      simpa [hyperbolicFlowCoordinate_apply, smul_eq_mul] using congrFun hax i
    ...
    have memZero : ∀ {y : Coord}, y 1 = 0 → y 2 = 0 → y 3 = 0 → y 5 = 0 → y 6 = 0 → y 7 = 0 → y ∈ zeroWeightSubmodule := by
      ...
      exact add_mem (Submodule.smul_mem _ _ (Submodule.subset_span (by simp)))
        (Submodule.smul_mem _ _ (Submodule.subset_span (by simp)))
    by_cases h1 : x 1 ≠ 0
    · have ha' : a = Real.exp t := by apply (mul_right_cancel₀ h1); simpa using hc 1
    ...
    by_cases h2 : x 2 ≠ 0
    · have ha' : a = Real.exp t := by apply (mul_right_cancel₀ h2); simpa using hc 2
      exact Or.inl (memPos (kill 0 1 (by simpa [ha']) (by simp)) (kill 4 1 (by simpa [ha']) (by simp))
        (kill 5 (Real.exp (-t)) (by simpa [ha'] using hsep) (by simp [Real.exp_neg]))
        (kill 6 (Real.exp (-t)) (by simpa [ha'] using hsep) (by simp [Real.exp_neg]))
        (kill 7 (Real.exp (-t)) (by simpa [ha'] using hsep) (by simp [Real.exp_neg])))
    by_cases h3 : x 3 ≠ 0
    · have ha' : a = Real.exp t := by apply (mul_right_cancel₀ h3); simpa using hc 3
      exact Or.inl (memPos (kill 0 1 (by simpa [ha']) (by simp)) (kill 4 1 (by simpa [ha']) (by simp))
        (kill 5 (Real.exp (-t)) (by simpa [ha'] using hsep) (by simp [Real.exp_neg]))
        (kill 6 (Real.exp (-t)) (by simpa [ha'] using hsep) (by simp [Real.exp_neg]))
        (kill 7 (Real.exp (-t)) (by simpa [ha'] using hsep) (by simp [Real.exp_neg])))
    by_cases h5 : x 5 ≠ 0
    · have ha' : a = Real.exp (-t) := by apply (mul_right_cancel₀ h5); simpa [Real.exp_neg] using hc 5
    ...
    by_cases h6 : x 6 ≠ 0
    · have ha' : a = Real.exp (-t) := by apply (mul_right_cancel₀ h6); simpa [Real.exp_neg] using hc 6
      exact Or.inr (Or.inl (memNeg (kill 0 1 (by simpa [ha']) (by simp))
        (kill 1 (Real.exp t) (by simpa [ha'] using hsep.symm) (by simp))
        (kill 2 (Real.exp t) (by simpa [ha'] using hsep.symm) (by simp))
        (kill 3 (Real.exp t) (by simpa [ha'] using hsep.symm) (by simp))
        (kill 4 1 (by simpa [ha']) (by simp))))
    by_cases h7 : x 7 ≠ 0
    · have ha' : a = Real.exp (-t) := by apply (mul_right_cancel₀ h7); simpa [Real.exp_neg] using hc 7
      exact Or.inr (Or.inl (memNeg (kill 0 1 (by simpa [ha']) (by simp))
        (kill 1 (Real.exp t) (by simpa [ha'] using hsep.symm) (by simp))
        (kill 2 (Real.exp t) (by simpa [ha'] using hsep.symm) (by simp))
        (kill 3 (Real.exp t) (by simpa [ha'] using hsep.symm) (by simp))
    ...
    · right; right
      ...
      simp_all
  · rintro (h | h | ⟨h, h04⟩)
    · have hp_new : p = twistorMk circularPeirceQuadratic x hx (positiveWeight_totallyNull x h) := by
        apply Subtype.ext
        exact hp_eq
      rw [hp_new]
      exact projective_positiveWeight_fixed t x hx h
    · have hp_new : p = twistorMk circularPeirceQuadratic x hx (negativeWeight_totallyNull x h) := by
        apply Subtype.ext
        exact hp_eq
      rw [hp_new]
      exact projective_negativeWeight_fixed t x hx h
    · have hp_new : p = twistorMk circularPeirceQuadratic x hx hQ := by
        apply Subtype.ext
        exact hp_eq
      rw [hp_new]
      rw [circularNullBoundaryFlow_mk_axialFlow]
