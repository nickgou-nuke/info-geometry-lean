...
* Diagnostic/lightcone/Hodge/Drazin packets are never promoted as theorem
  evidence.
* A declaration is promotable only when it is kernel-present, not blocked, not
  vacuous/surrogate, and mapped to an owner/root surface.

The script consumes repo-local artifacts when available and degrades honestly
when optional audit reports are absent.
"""
...
PROMOTABLE_CLASSES = {"kernel_proved", "certificate_backed", "owner_rooted"}
...
def policy_lint_flags(path: Path) -> dict[str, set[str]]:
    ...
    if not isinstance(data, dict):
        return flags
    for key in ("suspectTheorems", "suspects", "vacuous", "violations"):
        rows = data.get(key, [])
        if not isinstance(rows, list):
            continue
        for row in rows:
...
def classify(
    ...
    file_path = Path(str(decl.get("file", "")))
    ...
    if kind == "axiom":
        reasons.append("axiom_root:explicit_axiom_declaration")
        return "axiom_root", False, reasons
    ...
    if TRUE_TYPE_RE.search(block) and TRIVIAL_PROOF_RE.search(block) and kind in {"theorem", "lemma"}:
        reasons.append("vacuous_or_surrogate:true_only_trivial_certificate")
        return "vacuous_or_surrogate", False, reasons

    if TRIVIAL_PROOF_RE.search(block) and kind in {"theorem", "lemma"}:
        ...
        if reverse_theorem_users == 0 and role in {"isolated_theorem", "declaration", ""}:
            ...
            return "vacuous_or_surrogate", False, reasons
        reasons.append("kernel_proved:trivial_proof_but_graph_used")

    if paranoia is not None and not bool(paranoia.get("success")):
        reasons.append("kernel_proved:external_audit_not_clean")
        return "kernel_proved", False, reasons

    if CERTIFICATE_RE.search(name) and kind in {"theorem", "lemma", "def", "structure"}:
        reasons.append("certificate_backed:certificate_named_kernel_surface")
        return "certificate_backed", True, reasons

    if OWNER_ROOT_RE.search(name) and kind in {"theorem", "lemma", "def", "abbrev", "structure"}:
        reasons.append("owner_rooted:owner_or_bridge_named_kernel_surface")
        return "owner_rooted", True, reasons

    if kind in {"theorem", "lemma", "def", "abbrev", "structure", "inductive", "opaque"}:
        if sig is None:
            reasons.append("kernel_proved:no_graph_significance_row")
        else:
            reasons.append("kernel_proved:decl_index_and_graph_row_present")
        return "kernel_proved", True, reasons
...
def main() -> int:
    ...
    report = {
        ...
        "promotion_rule": "verdict in {kernel_proved, certificate_backed, owner_rooted} && trust_clean && !vacuous_or_surrogate && !graph_only && !diagnostic_only",
        "inputs": {
