...
@[simp]
theorem pullbackN_apply
    (n : ℕ) (ϕ : W →ₗ[R] V) (ω : OpForm R V A n) (v : Fin n → W) :
    pullbackN n ϕ ω v = ω (fun i => ϕ (v i)) :=
  rfl
...
/-- 
  ...
  (ψ ∘ ϕ)* ω = ψ* (ϕ* ω)
-/
theorem pullbackN_comp
    (n : ℕ) (ψ : U →ₗ[R] W) (ϕ : W →ₗ[R] V) (ω : OpForm R V A n) :
    pullbackN n (ϕ.comp ψ) ω = pullbackN n ψ (pullbackN n ϕ ω) := by
  dsimp [pullbackN]
  ext v
  simp only [AlternatingMap.compLinearMap_apply, LinearMap.coe_comp, Function.comp_apply]
...
def derivN (n : ℕ) (D : FormDerivation R A) (ω : OpForm R V A n) : OpForm R V A n where
  toMultilinearMap := D.toLinearMap.compMultilinearMap ω.toMultilinearMap
  map_eq_zero_of_eq' v i j hij h_eq := by
    dsimp
    rw [ω.map_eq_zero_of_eq v hij h_eq, D.map_zero]

@[simp]
theorem derivN_apply
    (n : ℕ) (D : FormDerivation R A) (ω : OpForm R V A n) (v : Fin n → V) :
    derivN n D ω v = D (ω v) :=
  rfl
...
/-- An R-bilinear transport tensor W(u, v) on V with values in A. -/
...
@[simp]
theorem altPart_skew
    (half : R) (u v : V) :
    W.altPart half u v = - W.altPart half v u := by
  dsimp [altPart]
  rw [← smul_neg]
  have h_sub : - (W v u - W u v) = W u v - W v u := by abel
  rw [h_sub]
...
theorem decomposition
  ...
  have h_add : (W u v + W v u) + (W u v - W v u) = (2 : R) • (W u v) := by
    calc
      (W u v + W v u) + (W u v - W v u) =
        (W u v + W u v) + (W v u - W v u) := by abel
