...
if [ ! -d .lake/packages ]; then
  echo "No .lake/packages directory; skipping v4.28.1 policy check." >&2
  exit 0
fi
...
check_file() {
  ...
  while IFS= read -r line; do
    if [[ "$line" == *"leanprover/lean4:"* ]]; then
      if [[ "$line" != *"leanprover/lean4:v4.28.1"* ]]; then
        echo "$file: $line"
        BAD=1
      fi
    fi
...
if [[ $BAD -ne 0 ]]; then
  echo "Nested package policy violation: non-v4.28.1 pin detected in .lake/packages." >&2
  exit 1
fi
