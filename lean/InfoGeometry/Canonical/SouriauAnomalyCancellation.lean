...
Proved theorem (zero axioms):
  `twisted_index_vanishing` — trace-cyclicity + J-invariance + J·K·J = -K ⇒ φ(K·P) = 0

All hypotheses are explicit: J²=1, J*=J, K*=-K, J·K·J=-K, φ cyclic, φ J-invariant.
-/
...
theorem twisted_index_vanishing
    ...
    (h_φ_J_invariant : ∀ A, φ (J.J * A * J.J) = φ A)
    (h_J_K : J.J * K.K * J.J = -K.K) :
    φ (K.K * P) = 0 := by
  have hJsq : J.J * J.J = 1 := J.J_sq
  -- J·(K·P)·J = J·K·(J·J)·P·J = (J·K·J)·(J·P·J)
  have h_conj : J.J * (K.K * P) * J.J = -(K.K * P) := by
    calc
      J.J * (K.K * P) * J.J
          = J.J * K.K * (J.J * J.J) * P * J.J := by
            rw [hJsq, mul_one, ← mul_assoc, mul_assoc (J.J * K.K)]
  ...
  have h_φ_additive : ∀ A, φ (-A) = -φ A := by
    ...
    have hzero_sum : φ (A + (-A)) = 0 := by simpa [add_neg_cancel] using hzero
