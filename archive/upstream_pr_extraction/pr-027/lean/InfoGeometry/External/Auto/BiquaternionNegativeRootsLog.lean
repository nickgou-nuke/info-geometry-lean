...
Lean formalization of the algebraic core of spinorial monodromy: traceless Pauli
vectors with quadratic norm `-1` square to `-I`, giving non-scalar square roots
of the negative identity.  This is the matrix-algebra shadow of braid/monodromy
representations where a full twist acts by `-I`.
-/
...
theorem T_sq (x y z : ℂ) :
  ...
  fin_cases i
  · fin_cases j
    · simp [T, σ₁, σ₂, σ₃, Matrix.smul_apply, Matrix.mul_apply, Fin.sum_univ_two]
      ring_nf
      rw [show (I : ℂ) ^ 2 = -1 by rw [pow_two, Complex.I_mul_I]]
      ring_nf
    · simp [T, σ₁, σ₂, σ₃, Matrix.smul_apply, Matrix.mul_apply, Fin.sum_univ_two]
      ring_nf
  · fin_cases j
...
theorem traceless_square_root_neg_one (x y z : ℂ) (h : x * x + y * y + z * z = -1) :
  ...
  all_goals simp [Matrix.smul_apply, Matrix.neg_apply]
...
theorem iσ₂_square_root_neg_one :
  ...
  all_goals fin_cases j
  all_goals
    simp [σ₂, Matrix.mul_apply, Matrix.neg_apply, Fin.sum_univ_two, Complex.I_mul_I]
