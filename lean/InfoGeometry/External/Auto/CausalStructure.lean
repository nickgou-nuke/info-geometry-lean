...
class CausalSpacetime (M : Type) where
  J_plus : M → (M → Prop)
  J_minus : M → (M → Prop)

open CausalSpacetime

def causal_diamond {M : Type} [CausalSpacetime M] (x y : M) : M → Prop :=
  fun z => (J_plus x z) ∧ (J_minus y z)

/-- A Cauchy surface meets each two-sided causal diamond in at most one point. -/
def is_Cauchy_surface {M : Type} [CausalSpacetime M] (S : M → Prop) : Prop :=
  ∀ ⦃x y z : M⦄, S z → causal_diamond x y z → J_plus x z ∧ J_minus y z

def GloballyHyperbolic (M : Type) [CausalSpacetime M] : Prop :=
...
theorem exists_cauchy
    [CausalSpacetime M] (h : GloballyHyperbolic M) :
    ∃ S : M → Prop, is_Cauchy_surface S :=
  h.1

/-- Compatibility projection for compact causal diamonds. -/
theorem diamond_compact
    [CausalSpacetime M] (h : GloballyHyperbolic M) (x y : M) :
    IsCompact (causal_diamond x y) :=
  h.2 x y

end GloballyHyperbolic

theorem diamond_is_compact {M : Type} [CausalSpacetime M]
