...
theorem closureDefect_eq_quadraticClosureTerm_add_remainder
  ...
  have hint : (∫ ω, φ (H ω) ∂μ) =
      (∫ ω, φ (mean μ H) + D (centeredField μ H ω) +
        (1 / 2 : ℝ) • B (centeredField μ H ω) (centeredField μ H ω) +
          secondOrderRemainder φ D B (mean μ H) (H ω) ∂μ) := by
    congr 1
    funext ω
    exact hdecomp ω
  have h1 : Integrable (fun ω => φ (mean μ H) + D (centeredField μ H ω) +
    (1 / 2 : ℝ) • B (centeredField μ H ω) (centeredField μ H ω)) μ :=
    (hconst.add hlinear).add hQ
  have h2 : Integrable (fun ω => φ (mean μ H) + D (centeredField μ H ω)) μ :=
    hconst.add hlinear
  unfold closureDefect quadraticClosureTerm
  rw [hint]
  rw [integral_add h1 hR]
  rw [integral_add h2 hQ]
