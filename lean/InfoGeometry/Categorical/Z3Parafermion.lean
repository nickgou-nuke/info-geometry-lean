...
namespace requested by the snippet.  No toy matrix model, no analytic claim.
-/
...
theorem proj_up_orthogonal_vacancy (O : A) (h : O ^ 3 = O) :
  ...
  simpa [proj_up, proj_vacancy, Algebra.smul_def] using
    (InfoGeometry.Canonical.FibonacciParafermionAtoms.Z3Parafermion.proj_up_orthogonal_vacancy
      (A := A) (O := O) h)
...
theorem proj_down_orthogonal_vacancy (O : A) (h : O ^ 3 = O) :
  ...
  simpa [proj_down, proj_vacancy, Algebra.smul_def] using
    (InfoGeometry.Canonical.FibonacciParafermionAtoms.Z3Parafermion.proj_down_orthogonal_vacancy
      (A := A) (O := O) h)

/-- The three projectors form a partition of unity. -/
theorem proj_completeness (O : A) :
    proj_up O + proj_down O + proj_vacancy O = 1 := by
  simpa [proj_up, proj_down, proj_vacancy, Algebra.smul_def] using
    (InfoGeometry.Canonical.FibonacciParafermionAtoms.Z3Parafermion.proj_completeness
      (A := A) (O := O))

/-- The Drazin projector is the chiral sum. -/
theorem drazin_eq_chiral_sum (O : A) :
    drazin_projector O = proj_up O + proj_down O := by
  simpa [proj_up, proj_down, drazin_projector, Algebra.smul_def] using
    (InfoGeometry.Canonical.FibonacciParafermionAtoms.Z3Parafermion.drazin_eq_chiral_sum
      (A := A) (O := O))

/-- The `+1` projector is idempotent under `O^3 = O`. -/
theorem proj_up_idempotent (O : A) (h : O ^ 3 = O) :
    proj_up O * proj_up O = proj_up O := by
  simpa [proj_up, Algebra.smul_def] using
    (InfoGeometry.Canonical.FibonacciParafermionAtoms.Z3Parafermion.proj_up_idempotent
      (A := A) (O := O) h)

/-- The `-1` projector is idempotent under `O^3 = O`. -/
theorem proj_down_idempotent (O : A) (h : O ^ 3 = O) :
    proj_down O * proj_down O = proj_down O := by
  simpa [proj_down, Algebra.smul_def] using
    (InfoGeometry.Canonical.FibonacciParafermionAtoms.Z3Parafermion.proj_down_idempotent
      (A := A) (O := O) h)
...
theorem proj_vacancy_idempotent (O : A) (h : O ^ 3 = O) :
  ...
  simpa [proj_vacancy] using
    (InfoGeometry.Canonical.FibonacciParafermionAtoms.Z3Parafermion.proj_vac_idempotent
      (A := A) (O := O) h)
...
theorem drazin_idempotent (O : A) (h : O ^ 3 = O) :
  ...
  simpa [drazin_projector] using
    (InfoGeometry.Canonical.FibonacciParafermionAtoms.Z3Parafermion.drazin_idempotent
      (A := A) (O := O) h)
...
theorem drazin_orthogonal_vacancy (O : A) (h : O ^ 3 = O) :
  ...
  simpa [drazin_projector, proj_vacancy] using
    (InfoGeometry.Canonical.FibonacciParafermionAtoms.Z3Parafermion.drazin_orthogonal_vacancy
      (A := A) (O := O) h)
