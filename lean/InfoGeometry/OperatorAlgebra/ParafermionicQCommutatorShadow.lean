...
theorem U_sq_zero : U * U = 0 := by
  ...
  fin_cases i <;> fin_cases j <;>
    simp [U, mUnit, Matrix.mul_apply, Fin.sum_univ_two]

/-- The lower Peirce-Witt slot is nilpotent. -/
theorem D_sq_zero : D * D = 0 := by
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [D, mUnit, Matrix.mul_apply, Fin.sum_univ_two]

/-- `U D` is the positive Peirce idempotent. -/
theorem U_mul_D : U * D = EPlus := by
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [U, D, EPlus, mUnit, Matrix.mul_apply, Fin.sum_univ_two]

/-- `D U` is the negative Peirce idempotent. -/
theorem D_mul_U : D * U = EMinus := by
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [U, D, EMinus, mUnit, Matrix.mul_apply, Fin.sum_univ_two]
...
theorem anticommutator_eq_one :
  ...
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [EPlus, EMinus, mUnit, Matrix.one_apply]
