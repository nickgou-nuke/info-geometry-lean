...
def main() -> None:
    ...
    eq('normalized reference probabilities scale',c*w[0]/sum(c*x for x in w),w[0]/Z)
    eq('relative partition comparison scale',s.log(c*part/(c*Z)),s.log(part/Z))
    eq('normalized reference entropy',-sum(prob[i]*(beta*scores[i]-s.log(part)+s.log(Z)) for i in range(3)),s.log(part)-s.log(Z)-beta*mean_score)
