...
-/
@[rep_depth operator]
def BoundarySubsystem (S : Set InfiniteBinaryWordSpace) : Prop :=
...
theorem boundarySubsystem_sUnion_of_chain
    (chain : Set (Set InfiniteBinaryWordSpace))
              ...
              seed ⊆ N →
                N ⊆ U →
                  BoundarySubsystem N →
                    M ⊆ N →
                      N = M := by
  let P : Set (Set InfiniteBinaryWordSpace) :=
