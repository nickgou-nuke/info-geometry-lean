...
private theorem zorn_add_mul (X Y Z : SplitCarrier) :
    (X + Y) * Z = X * Z + Y * Z := by
  apply ZornMatrix.ext <;>
    first | (funext i; fin_cases i) | skip
  all_goals
    simp [ZornMatrix.mul, ZornMatrix.add, Vec3.dot, Vec3.cross,
      Vec3.add, Vec3.sub, Vec3.smul] <;> ring

private theorem zorn_mul_add (X Y Z : SplitCarrier) :
    X * (Y + Z) = X * Y + X * Z := by
  apply ZornMatrix.ext <;>
    first | (funext i; fin_cases i) | skip
  all_goals
...
@[simp] theorem ell_sq : ell * ell = ZornMatrix.I := by
  change ((ZornMatrix.U 1 : SplitCarrier) + ZornMatrix.V 1) *
      (ZornMatrix.U 1 + ZornMatrix.V 1) = ZornMatrix.I
  rw [zorn_add_mul, zorn_mul_add, zorn_mul_add, ZornMatrix.U_mul_self_zero,
    ZornMatrix.U_mul_V_self, ZornMatrix.V_mul_U_self,
    ZornMatrix.V_mul_self_zero]
  simpa only [zero_add, add_zero] using
    (ZornMatrix.E11_add_E22 (R := ℝ))
...
theorem splitDouble_mul (A B C D : RealCore) :
  ...
  apply ZornMatrix.ext <;>
    first | (funext i; fin_cases i) | skip
  all_goals
    simp [coreConj, ZornMatrix.mul, Matrix.mul_apply, Matrix.vecMul,
      dotProduct, Fin.sum_univ_two, Vec3.dot, Vec3.cross, Vec3.add,
      Vec3.sub, Vec3.smul] <;> ring
...
theorem ell_not_supercommutative_odd : ¬ ell * ell = -(ell * ell) := by
  ...
  intro h
  have ha : (1 : ℝ) = -1 := by
    simpa [ZornMatrix.I, ZornMatrixRealModule.neg] using
      congrArg ZornMatrix.a h
  norm_num at ha
