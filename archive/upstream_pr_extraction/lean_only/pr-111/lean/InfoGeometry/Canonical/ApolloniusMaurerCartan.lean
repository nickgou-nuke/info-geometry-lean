...
theorem qInvDq_eq_rational (s : PuncturedPlane) (v : ℂ) :
    qInvDq s v = v / ((s : ℂ) * (1 - (s : ℂ))) := by
  unfold qInvDq cayleyDerivative crossRatio cayleyToFugacity
...
theorem qInvDq_smul (s : PuncturedPlane) (a v : ℂ) :
  ...
  change (a * v) / _ = a * (v / _)
  exact (mul_div_assoc a v _).symm
...
theorem globalConnection_eq_half_sigma3_qInvDq
    ...
    globalConnection s v = (qInvDq s v / 2) • sigma3 := by
  rfl
