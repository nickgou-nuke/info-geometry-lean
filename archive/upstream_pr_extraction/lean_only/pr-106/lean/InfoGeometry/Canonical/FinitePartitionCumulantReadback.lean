...
noncomputable abbrev partitionFunction (k : α → ℝ) (β : ℝ) : ℝ :=
  GrandCanonical.partition k β

noncomputable abbrev logPartition (k : α → ℝ) (β : ℝ) : ℝ :=
  GrandCanonical.potential k β

theorem partitionFunction_pos (k : α → ℝ) (β : ℝ) :
    0 < partitionFunction k β := by
  exact GrandCanonical.partition_pos k β

theorem logPartition_deriv_eq_neg_mean (k : α → ℝ) (β : ℝ) :
    deriv (logPartition k) β = -GrandCanonical.mean k β := by
  exact GrandCanonical.potential_deriv_eq_neg_mean k β

theorem logPartition_second_deriv_eq_variance (k : α → ℝ) (β : ℝ) :
    GrandCanonical.hessian k β = GrandCanonical.variance k β := by
  exact GrandCanonical.potential_second_derivative_eq_variance k β

theorem variance_nonneg (k : α → ℝ) (β : ℝ) :
    0 ≤ GrandCanonical.variance k β := by
...
lemma surprisal_partition_one
  ...
  simp only [GrandCanonicalParams.energy]
  calc
...
lemma mean_surprisal_at_one
    (p : α → ℝ) (hp : ∀ x, 0 < p x) (hsum : ∑ x, p x = 1) :
    GrandCanonical.mean (surprisal p) 1 =
  ...
  have hZ'' : GrandCanonical.partition (surprisal p) 1 = 1 := hZ'
  simp only [GrandCanonicalParams.energy, surprisal]
  rw [hZ'']
  apply Finset.sum_congr rfl
  intro x hx
  rw [hterm, div_one]
...
lemma logPartition_second_deriv_surprisal_at_one
    (p : α → ℝ) :
    GrandCanonical.hessian (surprisal p) 1 =
      GrandCanonical.variance (surprisal p) 1 := by
  exact logPartition_second_deriv_eq_variance (surprisal p) 1

lemma surprisal_variance_nonneg
    (p : α → ℝ) :
    0 ≤ GrandCanonical.variance (surprisal p) 1 := by
...
lemma surprisal_variance_eq_varentropy
    (p : α → ℝ) (hp : ∀ x, 0 < p x) (hsum : ∑ x, p x = 1) :
    GrandCanonical.variance (surprisal p) 1 =
      ∑ x, p x * ((-Real.log (p x)) -
        (∑ y, p y * (-Real.log (p y)))) ^ (2 : ℕ) := by
  unfold GrandCanonical.variance
  have hmean : GrandCanonical.mean (surprisal p) 1 =
      ∑ y, p y * (-Real.log (p y)) :=
  ...
  have hweight : GrandCanonical.gibbsWeight
      (surprisal p) 1 x = p x := by
    unfold GrandCanonical.gibbsWeight
    simp only [GrandCanonicalParams.energy, surprisal]
    rw [show -1 * (-Real.log (p x)) = Real.log (p x) by ring]
    rw [Real.exp_log (hp x)]
    have hZ : GrandCanonical.partition (surprisal p) 1 = 1 :=
      surprisal_partition_one p hp hsum
    rw [hZ]
    simp
  rw [hweight]
