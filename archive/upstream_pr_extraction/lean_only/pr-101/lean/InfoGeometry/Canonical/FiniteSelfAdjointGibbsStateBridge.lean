...
theorem exp_eq_star_exp_half_mul_exp_half
    ...
    NormedSpace.exp H =
      star (NormedSpace.exp ((1 / 2 : ℝ) • H)) *
        NormedSpace.exp ((1 / 2 : ℝ) • H) := by
  letI : NormedAlgebra ℚ (Operator n) :=
    NormedAlgebra.restrictScalars ℚ ℂ (Operator n)
  symm
  have hhalf :
      star ((1 / 2 : ℝ) • H) = (1 / 2 : ℝ) • H := by
    simpa [hH.star_eq]
  calc
    star (NormedSpace.exp ((1 / 2 : ℝ) • H)) *
          NormedSpace.exp ((1 / 2 : ℝ) • H) =
        NormedSpace.exp (star ((1 / 2 : ℝ) • H)) *
          NormedSpace.exp ((1 / 2 : ℝ) • H) := by
            rw [NormedSpace.star_exp]
    _ = NormedSpace.exp ((1 / 2 : ℝ) • H) *
          NormedSpace.exp ((1 / 2 : ℝ) • H) := by
            rw [hhalf]
    _ = NormedSpace.exp
          (((1 / 2 : ℝ) • H) + ((1 / 2 : ℝ) • H)) := by
...
theorem exp_nonneg_of_isSelfAdjoint
  ...
  rw [exp_eq_star_exp_half_mul_exp_half H hH]
  exact CStarAlgebra.nonneg_iff_eq_star_mul_self.mpr
    ⟨NormedSpace.exp ((1 / 2 : ℝ) • H), rfl⟩
...
theorem exp_isStrictlyPositive_of_isSelfAdjoint
  ...
  letI : NormedAlgebra ℚ (Operator n) :=
    NormedAlgebra.restrictScalars ℚ ℂ (Operator n)
  exact (NormedSpace.isUnit_exp H).isStrictlyPositive
    (exp_nonneg_of_isSelfAdjoint H hH)
...
theorem tracedExponential_re_pos_of_isSelfAdjoint
  ...
  exact finiteOperatorTrace_star_mul_self_re_pos _
    (NormedSpace.isUnit_exp ((1 / 2 : ℝ) • H)).ne_zero
...
theorem normalizedExponential_eq_faithfulGibbsDensity
  ...
  unfold normalizedExponential faithfulGibbsDensityOperator
  unfold faithfulNormalizedExponential
  rw [tracedExponential_eq_ofReal_re_of_isSelfAdjoint H hH]
  simp
