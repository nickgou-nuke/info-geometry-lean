...
/-- Pauli multiplication decomposes into its scalar dot channel and its
    oriented `Complex.I` times cross channel. -/
theorem pauli_vector_mul_decomposition
    (a₁ a₂ a₃ b₁ b₂ b₃ : ℂ) :
    (a₁ • σ1 + a₂ • σ2 + a₃ • σ3) *
        (b₁ • σ1 + b₂ • σ2 + b₃ • σ3) =
      (a₁ * b₁ + a₂ * b₂ + a₃ * b₃) • σ0 +
        (Complex.I * (a₂ * b₃ - a₃ * b₂)) • σ1 +
        (Complex.I * (a₃ * b₁ - a₁ * b₃)) • σ2 +
        (Complex.I * (a₁ * b₂ - a₂ * b₁)) • σ3 := by
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [σ0, σ1, σ2, σ3, Matrix.mul_apply, Fin.sum_univ_two] <;>
    ring_nf <;>
    simp [Complex.I_sq] <;>
    ring

@[simp] theorem σ1_sq : σ1 * σ1 = σ0 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [σ0, σ1, Matrix.mul_apply,
    Fin.sum_univ_two]

@[simp] theorem σ2_sq : σ2 * σ2 = σ0 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [σ0, σ2, Matrix.mul_apply,
    Fin.sum_univ_two, Complex.I_sq]

@[simp] theorem σ3_sq : σ3 * σ3 = σ0 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [σ0, σ3, Matrix.mul_apply,
    Fin.sum_univ_two]

theorem σ1_mul_σ2 : σ1 * σ2 = Complex.I • σ3 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [σ1, σ2, σ3, Matrix.mul_apply,
    Fin.sum_univ_two]

theorem σ2_mul_σ3 : σ2 * σ3 = Complex.I • σ1 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [σ1, σ2, σ3, Matrix.mul_apply,
    Fin.sum_univ_two]

theorem σ3_mul_σ1 : σ3 * σ1 = Complex.I • σ2 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [σ1, σ2, σ3, Matrix.mul_apply,
    Fin.sum_univ_two]

theorem σ2_mul_σ1 : σ2 * σ1 = -Complex.I • σ3 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [σ1, σ2, σ3, Matrix.mul_apply,
    Fin.sum_univ_two]

theorem σ3_mul_σ2 : σ3 * σ2 = -Complex.I • σ1 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [σ1, σ2, σ3, Matrix.mul_apply,
    Fin.sum_univ_two]

theorem σ1_mul_σ3 : σ1 * σ3 = -Complex.I • σ2 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [σ1, σ2, σ3, Matrix.mul_apply,
    Fin.sum_univ_two]

theorem σ1_σ2_anticommutator : σ1 * σ2 + σ2 * σ1 = 0 := by
  rw [σ1_mul_σ2, σ2_mul_σ1]
  module

theorem σ2_σ3_anticommutator : σ2 * σ3 + σ3 * σ2 = 0 := by
  rw [σ2_mul_σ3, σ3_mul_σ2]
  module

theorem σ3_σ1_anticommutator : σ3 * σ1 + σ1 * σ3 = 0 := by
  rw [σ3_mul_σ1, σ1_mul_σ3]
  module

/-- The anticommutator of arbitrary spatial Pauli vectors is twice their
    scalar dot product. -/
theorem pauli_vector_anticommutator
    (a₁ a₂ a₃ b₁ b₂ b₃ : ℂ) :
    ((a₁ • σ1 + a₂ • σ2 + a₃ • σ3) *
        (b₁ • σ1 + b₂ • σ2 + b₃ • σ3)) +
      ((b₁ • σ1 + b₂ • σ2 + b₃ • σ3) *
        (a₁ • σ1 + a₂ • σ2 + a₃ • σ3)) =
      (2 * (a₁ * b₁ + a₂ * b₂ + a₃ * b₃)) • σ0 := by
  rw [pauli_vector_mul_decomposition, pauli_vector_mul_decomposition]
  module

/-- The commutator of arbitrary spatial Pauli vectors is twice `Complex.I`
    times their oriented cross channel. -/
theorem pauli_vector_commutator
    (a₁ a₂ a₃ b₁ b₂ b₃ : ℂ) :
    ((a₁ • σ1 + a₂ • σ2 + a₃ • σ3) *
        (b₁ • σ1 + b₂ • σ2 + b₃ • σ3)) -
      ((b₁ • σ1 + b₂ • σ2 + b₃ • σ3) *
        (a₁ • σ1 + a₂ • σ2 + a₃ • σ3)) =
      (2 * Complex.I * (a₂ * b₃ - a₃ * b₂)) • σ1 +
        (2 * Complex.I * (a₃ * b₁ - a₁ * b₃)) • σ2 +
        (2 * Complex.I * (a₁ * b₂ - a₂ * b₁)) • σ3 := by
  rw [pauli_vector_mul_decomposition, pauli_vector_mul_decomposition]
  module
