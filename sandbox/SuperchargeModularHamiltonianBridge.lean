...
/-- Instantiation of the structure given a CertifiedInverseKernel and a positive calibration unit. -/
def instantiate (CIK : CertifiedInverseKernel E) (c : ℝ) (hc : 0 < c) :
    SuperchargeModularHamiltonianBridge (E := E) where
  CIK := CIK
  modularEnergyUnit := c
  modularEnergyUnit_pos := hc
  Ksur := c • DrazinSupercharge.CertifiedInverseKernel.regularRestrictedSuperHamiltonian CIK
  Ksur_True := rfl

/-- Concrete Native Instantiation via Inhabited -/
noncomputable instance (CIK : CertifiedInverseKernel E) :
    Inhabited (SuperchargeModularHamiltonianBridge (E := E)) where
  default := instantiate CIK 1 zero_lt_one
