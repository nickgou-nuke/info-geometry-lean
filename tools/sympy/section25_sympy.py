#!/usr/bin/env python3
"""Repaired Section 25: revised finite quaternionic extension checks.

This mirrors ``lean/InfoGeometry/Section25.lean``.  It keeps the corrected
finite quaternion corridor and adds the scalar reality check introduced by the
revised source: when ``B = conjugate(A)``, ``i/2*(A-B)`` is real.

Not claimed here: a full Clifford-isomorphism theorem, VEVs, smooth spin
connections, curvature/torsion decomposition, field equations, or the proposed
self-consistency loop.
"""

from __future__ import annotations

import sympy as sp


def assert_zero(expr, label: str) -> None:
    reduced = sp.expand(sp.simplify(expr))
    if reduced != 0:
        raise AssertionError(f"{label} failed: {reduced}")


def assert_matrix_zero(matrix: sp.Matrix, label: str) -> None:
    reduced = matrix.applyfunc(lambda x: sp.expand(sp.simplify(x)))
    if reduced != sp.zeros(*matrix.shape):
        raise AssertionError(f"{label} failed:\n{reduced}")


def qmul(p, q):
    pr, px, py, pz = p
    qr, qx, qy, qz = q
    return (
        pr * qr - px * qx - py * qy - pz * qz,
        pr * qx + px * qr + py * qz - pz * qy,
        pr * qy - px * qz + py * qr + pz * qx,
        pr * qz + px * qy - py * qx + pz * qr,
    )


def qconj(q):
    r, x, y, z = q
    return (r, -x, -y, -z)


def qnorm_sq(q):
    return sum(component**2 for component in q)


def main() -> None:
    print("=" * 72)
    print("REPAIRED SECTION 25: REVISED FINITE QUATERNIONIC CORE")
    print("=" * 72)
    print("Scope: revised finite algebra plus explicit scalar conjugacy reality.")
    print("Open debt: VEVs, spin connections, curvature, field equations, bootstrap.")

    ar, ai = sp.symbols("ar ai", real=True)
    A = ar + sp.I * ai
    B = sp.conjugate(A)
    readout = sp.I / 2 * (A - B)
    assert_zero(sp.im(readout), "i/2*(A-conj(A)) has zero imaginary part")
    assert_zero(sp.re(readout) + ai, "i/2*(A-conj(A)) real value")
    print("  scalar conjugacy reality readout verified")

    I = sp.I
    I2 = sp.eye(2)
    I4 = sp.eye(4)
    sigma1 = sp.Matrix([[0, 1], [1, 0]])
    sigma2 = sp.Matrix([[0, -I], [I, 0]])
    sigma3 = sp.Matrix([[1, 0], [0, -1]])
    tau3 = sp.Matrix([[1, 0], [0, -1]])
    epsilon = sp.Matrix([[0, 1], [-1, 0]])

    gamma0 = sp.kronecker_product(tau3, I2)
    gamma1 = sp.kronecker_product(epsilon, sigma1)
    gamma2 = sp.kronecker_product(epsilon, sigma2)
    gamma3 = sp.kronecker_product(epsilon, sigma3)
    _ = gamma0

    qi = gamma1 * gamma2
    qj = gamma2 * gamma3
    qk = gamma3 * gamma1
    assert_matrix_zero(qi * qi + I4, "qi^2 = -I")
    assert_matrix_zero(qj * qj + I4, "qj^2 = -I")
    assert_matrix_zero(qk * qk + I4, "qk^2 = -I")
    assert_matrix_zero(qi * qj - qk, "qi*qj = qk")
    assert_matrix_zero(qj * qk - qi, "qj*qk = qi")
    assert_matrix_zero(qk * qi - qj, "qk*qi = qj")
    assert_matrix_zero(qi * qj * qk + I4, "qi*qj*qk = -I")
    print("  revised gamma-bivector Hamilton relations verified")

    q = tuple(sp.symbols("q0 q1 q2 q3", real=True))
    norm_scalar = (qnorm_sq(q), 0, 0, 0)
    for lhs, rhs in zip(qmul(qconj(q), q), norm_scalar):
        assert_zero(lhs - rhs, "conj(Q)Q = normSq(Q)")
    print("  quaternion norm scalar identity verified")

    e = {
        (a, mu): sp.symbols(f"e_{a}_{mu}", real=True)
        for a in range(4)
        for mu in range(4)
    }

    def metric(mu, nu):
        return (
            e[(0, mu)] * e[(0, nu)]
            - e[(1, mu)] * e[(1, nu)]
            - e[(2, mu)] * e[(2, nu)]
            - e[(3, mu)] * e[(3, nu)]
        )

    for mu in range(4):
        for nu in range(4):
            assert_zero(metric(mu, nu) - metric(nu, mu), "metric symmetry")
    print("  finite metric symmetry verified")

    kappa = sp.symbols("kappa")
    coeff = {
        (lam, mu, nu): sp.symbols(f"B_{lam}_{mu}_{nu}")
        for lam in range(4)
        for mu in range(4)
        for nu in range(4)
    }

    def torsion(lam, mu, nu):
        return kappa * (coeff[(lam, mu, nu)] - coeff[(lam, nu, mu)])

    for lam in range(4):
        for mu in range(4):
            for nu in range(4):
                assert_zero(
                    torsion(lam, nu, mu) + torsion(lam, mu, nu),
                    "torsion lower-index antisymmetry",
                )
    print("  finite torsion antisymmetry verified")

    print("=" * 72)
    print("[SUCCESS] Section 25 theorem-safe finite checks verified.")
    print("=" * 72)


if __name__ == "__main__":
    main()
