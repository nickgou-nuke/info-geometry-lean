...
/-- Scalar multiplication is associative with respect to the scalars. -/
theorem scale_scale (r s : ℚ) (X : Herm2) : scale r (scale s X) = scale (r * s) X := by
  cases X with
  | mk xp xm a =>
    cases a with
    | mk ar ai =>
...
theorem scale_one (X : Herm2) : scale 1 X = X := by
  cases X with
  | mk xp xm a =>
    cases a with
    | mk ar ai =>
...
theorem cayleyInversion_involutive (X : Herm2) (hX : det X ≠ 0) :
  ...
  rw [hdet, trRev_scale, trRev_involutive, scale_scale]
  have hs : (1 / (1 / det X)) * (1 / det X) = 1 := by
    field_simp [hX]
  rw [hs, scale_one]
