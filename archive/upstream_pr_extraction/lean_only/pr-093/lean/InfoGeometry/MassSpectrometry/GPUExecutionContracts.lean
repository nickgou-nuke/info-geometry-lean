...
theorem mellinAngle_common_scale
    ...
    mellinAngle ω (λ * m) (λ * m₀) = mellinAngle ω m m₀ := by
  simp [mellinAngle, logMass_common_scale hλ hm₀]

/-- Consequently the corresponding 2D rotation is common-scale invariant. -/
theorem givensRotate_mellin_common_scale
    {ω λ m m₀ : ℝ} (hλ : λ ≠ 0) (hm₀ : m₀ ≠ 0)
    (x : ℝ × ℝ) :
    givensRotate (mellinAngle ω (λ * m) (λ * m₀)) x =
      givensRotate (mellinAngle ω m m₀) x := by
  rw [mellinAngle_common_scale hλ hm₀]
...
theorem certifiedBalancedAssignment_decomposes
  ...
  exact cert.balancedMatrix_birkhoff_decomposition
