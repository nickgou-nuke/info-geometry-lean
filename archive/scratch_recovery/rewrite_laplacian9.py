...
theorem laplacian_rank_eq_add_rank (A : Matrix m n ℚ) (B : Matrix p m ℚ) (hBA : B * A = 0) :
  ...
  have h_kerA : LinearMap.ker C.mulVecLin = LinearMap.ker Aᵀ.mulVecLin := by
    ext x; constructor
    · intro hx
      have hx_zero : C.mulVec x = 0 := by exact LinearMap.mem_ker.mp hx
    ...
    · intro hx
      have hx_zero : Aᵀ.mulVec x = 0 := LinearMap.mem_ker.mp hx
      rw [LinearMap.mem_ker]
      have h_mulA : C.mulVec x = A.mulVec (Aᵀ.mulVec x) := by dsimp [C]; rw [Matrix.mulVec_mulVec]
      rw [h_mulA, hx_zero, Matrix.mulVec_zero]
  ...
  have h_kerB : LinearMap.ker D.mulVecLin = LinearMap.ker B.mulVecLin := by
    ext x; constructor
    · intro hx
      have hx_zero : D.mulVec x = 0 := by exact LinearMap.mem_ker.mp hx
      have h_d : innerProduct x (D.mulVec x) = 0 := by rw [hx_zero]; simp [innerProduct]
    ...
    · intro hx
      have hx_zero : B.mulVec x = 0 := LinearMap.mem_ker.mp hx
      rw [LinearMap.mem_ker]
      have h_mulB : D.mulVec x = Bᵀ.mulVec (B.mulVec x) := by dsimp [D]; rw [Matrix.mulVec_mulVec]
      rw [h_mulB, hx_zero, Matrix.mulVec_zero]
  ...
  have h_im_Csq : LinearMap.range (C * C).mulVecLin = LinearMap.range C.mulVecLin := by
    ...
    have h_ker_sq : LinearMap.ker (C * C).mulVecLin = LinearMap.ker C.mulVecLin := by
      ext z; constructor
      · intro hz'
        have hz_zero : (C * C).mulVec z = 0 := by exact LinearMap.mem_ker.mp hz'
      ...
      · intro hz'
        have hz_zero : C.mulVec z = 0 := LinearMap.mem_ker.mp hz'
        rw [LinearMap.mem_ker]
        have h_mulCC : (C * C).mulVec z = C.mulVec (C.mulVec z) := by rw [Matrix.mulVec_mulVec]
        rw [h_mulCC, hz_zero, Matrix.mulVec_zero]
  ...
  have h_im_Dsq : LinearMap.range (D * D).mulVecLin = LinearMap.range D.mulVecLin := by
    ...
    have h_ker_sq : LinearMap.ker (D * D).mulVecLin = LinearMap.ker D.mulVecLin := by
      ext z; constructor
      · intro hz'
        have hz_zero : (D * D).mulVec z = 0 := by exact LinearMap.mem_ker.mp hz'
      ...
      · intro hz'
        have hz_zero : D.mulVec z = 0 := LinearMap.mem_ker.mp hz'
        rw [LinearMap.mem_ker]
        have h_mulDD : (D * D).mulVec z = D.mulVec (D.mulVec z) := by rw [Matrix.mulVec_mulVec]
        rw [h_mulDD, hz_zero, Matrix.mulVec_zero]
  ...
  have h_imC_sub : LinearMap.range C.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    calc
      (C + D).mulVec (C.mulVec y) = C.mulVec (C.mulVec y) + D.mulVec (C.mulVec y) := by rw [Matrix.add_mulVec]
  ...
  have h_imD_sub : LinearMap.range D.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    calc
      (C + D).mulVec (D.mulVec z) = C.mulVec (D.mulVec z) + D.mulVec (D.mulVec z) := by rw [Matrix.add_mulVec]
  ...
  have h_bot : Module.finrank ℚ (LinearMap.range C.mulVecLin ⊓ LinearMap.range D.mulVecLin) = 0 := by
    rw [h_inter]; exact finrank_bot ℚ (m → ℚ)
  rw [h_bot, add_zero] at h_dim
  have h_rank_sum : Matrix.rank (A * Aᵀ + Bᵀ * B) = Module.finrank ℚ (LinearMap.range (C + D).mulVecLin) := rfl
  rw [h_rank_sum, h_range, h_dim, ← hC_rank, ← hD_rank]
