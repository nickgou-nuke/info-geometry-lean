...
def plan(snapshot_path: Path, locks_path: Path | None = None) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    return vacuum, bridge, alignment, {
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate LeanTrail v1.2 surgery packet streams from a vacuity-enriched snapshot.")
...
def main() -> int:
    ...
    vacuum, bridge, alignment, report = plan(Path(args.snapshot).resolve(), locks if locks.exists() else None)
