...
theorem witten_index_kms_vanishes
    ...
    wittenIndex O φ = 0 := by
  unfold wittenIndex CuntzO2.P_L CuntzO2.P_R
  have h_sub : φ (O.S_L * star O.S_L - O.S_R * star O.S_R) =
      φ (O.S_L * star O.S_L) - φ (O.S_R * star O.S_R) := by
    exact φ.val.map_sub (O.S_L * star O.S_L) (O.S_R * star O.S_R)
  rw [h_sub]
  exact kms_chiral_charge_vanishes O.S_L O.S_R φ h_kms
