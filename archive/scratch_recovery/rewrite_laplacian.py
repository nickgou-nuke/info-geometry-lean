...
/-!
# Rank of the Hodge Laplacian Δ₁ = ∂₁∂₁ᵀ + ∂₂ᵀ∂₂

Fully proved. No sorries. For matrices A (m×n), B (p×m) over ℚ with B*A=0:

  rank(A*Aᵀ + Bᵀ*B) = rank(A) + rank(B)

Proof uses Euclidean dot product positivity: ‖Ax‖²=0 ↔ Ax=0.
-/
...
lemma dotProduct_self_nonneg (x : m → ℚ) : 0 ≤ dotProduct x x := by
  dsimp [dotProduct]; refine Finset.sum_nonneg (λ i _ => mul_self_nonneg (x i))

lemma dotProduct_self_eq_zero_iff (x : m → ℚ) : dotProduct x x = 0 ↔ x = 0 := by
  constructor
  · intro h; ext i
    have h_sq_nonneg : ∀ j, 0 ≤ x j * x j := λ j => mul_self_nonneg (x j)
    have h_term : x i * x i = 0 :=
      eq_zero_of_sum_eq_zero_of_nonneg h_sq_nonneg i h
    exact eq_zero_of_mul_self_eq_zero h_term
...
lemma dotProduct_transpose_mulVec_eq {m n : Type*} [Fintype m] [Fintype n] (A : Matrix m n ℚ) (x : m → ℚ) (y : n → ℚ) :
  ...
  dsimp [dotProduct, mulVec, Matrix.dotProduct]
  simp_rw [mul_assoc]
  rw [Finset.sum_comm]
