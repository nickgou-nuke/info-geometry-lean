...
noncomputable def realPin55ExteriorSpinorLinearEquiv (g : RealPin55) :
  ...
  LinearEquiv.ofLinear
    ...
    (realPin55ExteriorSpinorEnd g⁻¹)
    (by
      rw [← realPin55ExteriorSpinorEnd_mul, mul_inv_cancel,
        realPin55ExteriorSpinorEnd_one])
    (by
      rw [← realPin55ExteriorSpinorEnd_mul, inv_mul_cancel,
        realPin55ExteriorSpinorEnd_one])
...
theorem pin_mul_generator_action
    (g : RealPin55) (w : NeutralSpace) (ψ : Spinor) :
    neutralCliffordRep
        (neutralCliffordAlgEquiv.symm
