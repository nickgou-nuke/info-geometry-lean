...
theorem quantizedWindingFlux_eq_two_pi_mul_I
  ...
  rw [quantizedWindingFlux, circleIntegral_one_div R hR]
...
theorem quantizedWindingFlux_add
  ...
  exact deRhamClass_of_winding_add R hR m n
...
theorem winding_quantization_with_finite_ccr_obstruction
    (R : ℝ) (hR : 0 < R) (n : ℤ)
    (A B : ApolloniusScalarOperatorSurprisalNoGo.EndCZ)
    (c : ℝ) (hc : c ≠ 0) :
    quantizedWindingFlux R n = logarithmicPhase n ∧
      A * B - B * A ≠
        ApolloniusScalarOperatorSurprisalNoGo.scalarOperator c := by
  exact ⟨quantizedWindingFlux_eq_logarithmicPhase R hR n,
    ApolloniusScalarOperatorSurprisalNoGo.
