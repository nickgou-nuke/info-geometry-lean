...
theorem hasDerivAt_lapse (a x : ℝ) : HasDerivAt (lapse a) a x := by
  convert ((hasDerivAt_id x).const_mul a).const_add 1 using 1 <;>
    simp [lapse, mul_comm]
...
def toRindler (a x t : ℝ) (ha : 0 < a) (hx : 0 < lapse a x) :
  ...
  { radius := radius a x
    time := a * t
    radius_pos := radius_pos a x ha hx }

theorem lightcone_product (a x t : ℝ) (ha : 0 < a) (hx : 0 < lapse a x) :
    rindlerToMinkowski (toRindler a x t ha hx) 0 0 *
        rindlerToMinkowski (toRindler a x t ha hx) 1 0 = (radius a x) ^ 2 := by
  simpa [toRindler] using rindler_lightcone_product (toRindler a x t ha hx)
