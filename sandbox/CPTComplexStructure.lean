...
/-- Concrete instantiation of Cl11Atom using Cl(1,1) algebra over ℚ. -/
def cl11AtomRat : Cl11Atom (CliffordAlgebra q11) where
  r0 := e₀
  r5 := e₁
  r0_sq := e₀_sq
  r5_sq := e₁_sq
  anticommute := by
    have h : e₀ * e₁ + e₁ * e₀ = 0 := anticomm
    calc
      e₀ * e₁ = e₀ * e₁ + e₁ * e₀ - e₁ * e₀ := by abel
      _ = 0 - e₁ * e₀ := by rw [h]
      _ = -(e₁ * e₀) := by simp

variable {K : Type*} [Ring K] (atom : Cl11Atom K)
...
/-- Auxiliary lemma: r5 * r0 = - (r0 * r5) -/
lemma r5_r0_eq_neg_r0_r5 : atom.r5 * atom.r0 = -(atom.r0 * atom.r5) := by
...
theorem euler_operator_sq_one :
  ...
  calc
    ...
    _ = atom.r0 * (-(atom.r0 * atom.r5)) * atom.r5 := by rw [r5_r0_eq_neg_r0_r5 atom]
...
omit [Algebra ℝ K] in
/-- Auxiliary lemma for orthogonal chiral sheets. -/
lemma one_plus_r0_mul_one_minus_r0 : (1 + atom.r0) * (1 - atom.r0) = 0 := by
  calc
...
theorem chiral_sheets_orthogonal :
  ...
  calc
    ...
    _ = (1/4 : ℝ) • (0 : K) := by rw [one_plus_r0_mul_one_minus_r0 atom]
    _ = 0 := smul_zero _
...
omit [Algebra ℝ K] in
/-- Auxiliary lemma: r0 * r5 * r0 = -r5 -/
lemma r0_r5_r0_eq_neg_r5 : atom.r0 * atom.r5 * atom.r0 = - atom.r5 := by
