...
/- The inclusion of the positive orthant into the ambient space is a smooth immersion -/
theorem inclusion_immersion {s : PositiveOrthantManifold} :
    IsImmersionAt (modelWithCornersSelf ℝ (α → ℝ)) (modelWithCornersSelf ℝ (α → ℝ)) ⊤ (toAmbient : PositiveOrthantManifold → (α → ℝ)) s := by
  -- The inclusion of an open subset is an immersion
  have h : IsImmersionAt (modelWithCornersSelf ℝ (α → ℝ)) (modelWithCornersSelf ℝ (α → ℝ)) ⊤ (Subtype.val : PositiveOrthantManifold → (α → ℝ)) s := by
    apply IsImmersionAt.of_opens
    <;> infer_instance
  simpa [toAmbient] using h
