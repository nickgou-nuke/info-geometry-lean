  ...
  { toFun := fun v => ⟨{ a := 0, b := 0, x := ![v 0, v 1, v 2], y := ![v 3, v 4, v 5] }, by
    ...
    right_inv := fun X => by
      ...
      have h₃ : { a := (0 : ℝ), b := (0 : ℝ), x := ![X.1.x 0, X.1.x 1, X.1.x 2], y := ![X.1.y 0, X.1.y 1, X.1.y 2] } = X.1 := by
        ext <;> fin_cases <;> simp_all [InfoGeometry.Algebra.Zorn.KingdonCanonicalBridge.CanonicalZorn, imaginaryCoordLinearEquiv]
        <;> (try aesop) <;> (try linarith)
      exact h₃
    map_add' := fun v w => by
      ext i <;> fin_cases i <;> rfl
    map_smul' := fun r v => by
...
noncomputable def multispectral7Equiv : (Fin 7 → ℝ) ≃ₗ[ℝ] Imaginary :=
  { toFun := fun v => ⟨imaginaryCoordLinearEquiv.symm (v 0, ![v 1, v 2, v 3], ![v 4, v 5, v 6]), by
    ...
    left_inv := fun v => by
      ext i <;> fin_cases i <;>
      simp [imaginaryCoordLinearEquiv, ImaginaryCoords, Prod.mk.inj_iff]
      <;> aesop
    right_inv := fun X => by
