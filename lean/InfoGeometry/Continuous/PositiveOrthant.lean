...
This module formalizes the positive orthant as a smooth manifold (open subset of ℝ^α),
...
hence it inherits a canonical ChartedSpace structure. -/
instance : ChartedSpace ℝ PositiveOrthantManifold := by
  -- The positive orthant is an open subset of ℝ^α, so it inherits the charted space structure
