...
noncomputable instance : Module.Finite ℝ Zorn :=
  Module.Finite.of_basis InfoGeometry.Algebra.zornCoordinateBasis

noncomputable instance : Module.Finite ℝ V10 := inferInstance
...
theorem B10_sub_left (u v w : V10) :
    B10 (u - v) w = B10 u w - B10 v w := by
  rcases u with ⟨⟨u1, u2⟩, ux⟩
  rcases v with ⟨⟨v1, v2⟩, vx⟩
  rcases w with ⟨⟨w1, w2⟩, wx⟩
  simp [B10, zornPolar, ZornVectorMatrix.trace, ZornVectorMatrix.mul,
    ZornVectorMatrix.conj, ZornVectorMatrix.smul, ZornVectorMatrix.sub,
    ZornVectorMatrix.add, ZornVectorMatrix.neg, sub_eq_add_neg,
    ZornVec3.dot, Fin.sum_univ_three]
  ring

theorem B10_sub_right (u v w : V10) :
    B10 u (v - w) = B10 u v - B10 u w := by
  rw [B10_symm, B10_sub_left, B10_symm v u, B10_symm w u]
...
theorem rotGen_in_so66 (M : V10 → V10)
  ...
  simpa [B10_symm, add_comm] using hM x.2.1 y.2.1
...
theorem bracket_p_k (u v : V10) (x : V12) :
  ...
  ext <;>
    simp [bracket, pGen, kGen, dGen, rotGen, metricBivector,
      ...
      ZornVectorMatrix.smul, ZornVectorMatrix.sub,
    ZornVectorMatrix.add, ZornVectorMatrix.neg, sub_eq_add_neg] <;>
    ring
...
theorem metricBivector_skew (u v : V10) :
  ...
  intro x y
  simp only [metricBivector, B10_sub_left, B10_sub_right,
    B10_smul_left, B10_smul_right]
  rw [B10_symm v y, B10_symm u y, B10_symm v x, B10_symm u x]
  ring
...
theorem conformalTKKCarrier_finrank :
  ...
  norm_num [ConformalTKKCarrier, V10, PeirceZeroCoord, Module.finrank_prod,
    zorn_finrank_eq_eight, native_so55_finrank]
...
theorem conformalTKK_dimension_matches_so66 :
  ...
  calc
    Module.finrank ℝ ConformalTKKCarrier = 66 := conformalTKKCarrier_finrank
    _ = 12 * (12 - 1) / 2 := by norm_num
