...
noncomputable def probMeasureToNonzero (P : ProbabilityMeasure α) : NonzeroUState α :=
  ⟨probMeasureToUState P, probMeasureToUState_ne_zero P⟩

theorem normalize_probMeasureToNonzero (P : ProbabilityMeasure α) :
    (probMeasureToNonzero P).1.mass = 1 := by
  letI : IsProbabilityMeasure P.val := P.prop
  exact measure_univ
