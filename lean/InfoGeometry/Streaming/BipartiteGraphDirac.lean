...
theorem differential_adjoint (B : Matrix e v ℝ) (x y : Cochain v e) :
  ...
  simp only [cochainPair, differential, codifferential, Matrix.mulVec, dotProduct,
    Matrix.transpose_apply, Pi.zero_apply, zero_mul, mul_zero,
    Finset.sum_const_zero, zero_add, add_zero, Finset.sum_mul, Finset.mul_sum]
  rw [Finset.sum_comm]
...
theorem graphDirac_odd (B : Matrix e v ℝ) :
  ...
  apply LinearMap.ext
  intro x
  simp [graphDirac_apply, grading, Module.End.mul_apply]
