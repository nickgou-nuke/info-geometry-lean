...
def matrixGradeProjectionLinear (k : ℤ) : Mat10 →ₗ[ℂ] Mat10 where
  ...
  map_add' X Y := by
    ext p q
    by_cases h : entryDegree p q = k <;>
      simp [matrixGradeProjection, h] <;> ring
  map_smul' c X := by
    ext p q
    by_cases h : entryDegree p q = k <;>
      simp [matrixGradeProjection, h] <;> ring
...
theorem matrixGradeProjection_orthogonal
  ...
  · have hl : entryDegree p q ≠ l := by
    ...
    simp only [matrixGradeProjection, if_pos hk, if_neg hl]
    rfl
...
theorem matrixGradeProjection_mem_contactGradeSpace
  ...
  ext p q
  change (cartanMatrix contactCartanCoefficient * matrixGradeProjection k X -
      matrixGradeProjection k X * cartanMatrix contactCartanCoefficient) p q =
    (k : ℂ) * matrixGradeProjection k X p q
  rw [cartan_commutator_entry, indexWeight_contact, indexWeight_contact]
  by_cases hdeg : entryDegree p q = k
  · have hc : (indexContactWeight p : ℂ) - (indexContactWeight q : ℂ) =
        (k : ℂ) := by exact_mod_cast hdeg
    simp [matrixGradeProjection, hdeg, hc]
...
theorem sum_matrixGradeProjection (X : Mat10) :
  ...
  ext p q
  change (∑ k ∈ contactDegrees, (if entryDegree p q = k then X p q else 0)) = X p q
