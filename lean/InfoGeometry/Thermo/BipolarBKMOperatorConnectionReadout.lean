...
abbrev RealFiniteOperatorCarrier (n : ℕ) := FiniteOperatorAlgebra n
...
def finiteOperatorConnection {n : ℕ}
    ...
    Op1Form ℝ Tangent2 (RealFiniteOperatorCarrier n) where
  ...
  map_smul' c v := by
    apply ContinuousLinearMap.ext
    intro z
    simp [Pi.smul_apply, ContinuousLinearMap.add_apply,
      ContinuousLinearMap.smul_apply, smul_eq_mul, smul_add, smul_smul]

@[simp] theorem finiteOperatorConnection_eta {n : ℕ}
    (Kη Kθ : FiniteOperatorAlgebra n) :
    finiteOperatorConnection Kη Kθ etaTangent = Kη := by
  change (1 : ℝ) • (Kη : RealFiniteOperatorCarrier n) +
      (0 : ℝ) • (Kθ : RealFiniteOperatorCarrier n) = Kη
  have hz : (0 : ℝ) • (Kθ : RealFiniteOperatorCarrier n) = 0 := by
    apply ContinuousLinearMap.ext
    intro z
    simp
  rw [one_smul, hz, add_zero]

@[simp] theorem finiteOperatorConnection_theta {n : ℕ}
    (Kη Kθ : FiniteOperatorAlgebra n) :
    finiteOperatorConnection Kη Kθ thetaTangent = Kθ := by
  change (0 : ℝ) • (Kη : RealFiniteOperatorCarrier n) +
      (1 : ℝ) • (Kθ : RealFiniteOperatorCarrier n) = Kθ
  have hz : (0 : ℝ) • (Kη : RealFiniteOperatorCarrier n) = 0 := by
    apply ContinuousLinearMap.ext
    intro z
    simp
  rw [hz, one_smul, zero_add]
...
theorem bkm_connection_curvature_coordinate {n : ℕ}
  ...
  unfold bkmProbeReadout wedge
  change D.bkmRealBilinForm hD
      ((finiteOperatorConnection Kη Kθ etaTangent) *
        (finiteOperatorConnection Kη Kθ thetaTangent) -
        (finiteOperatorConnection Kη Kθ thetaTangent) *
        (finiteOperatorConnection Kη Kθ etaTangent)) Q = _
  rw [finiteOperatorConnection_eta, finiteOperatorConnection_theta]
