...
namespace InfoGeometry.MobiusCrossRatioEvalSandbox
...
lemma exists_eval_some_value
    (M : InfoGeometry.MobiusTransform) (z : ℂ) (hz : M.c * z + M.d ≠ 0) :
    ∃ w : ℂ, M.eval (some z) = some w := by
  exact ⟨(M.a * z + M.b) / (M.c * z + M.d), eval_some_eq_fractional M z hz⟩
...
lemma cross_ratio_eval_some_finite_preserving
  ...
  have e1 := eval_some_eq_fractional M z1 h1
  have e2 := eval_some_eq_fractional M z2 h2
  have e3 := eval_some_eq_fractional M z3 h3
  have e4 := eval_some_eq_fractional M z4 h4
...
end InfoGeometry.MobiusCrossRatioEvalSandbox
