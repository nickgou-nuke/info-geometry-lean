...
def diagonalAlgHom : A →ₐ[ℝ] Envelope A where
  ...
  commutes' r := by
    ext i j
    fin_cases i <;> fin_cases j <;>
      simp [diagonal, Matrix.algebraMap_matrix_apply]
...
def leftRegularRepresentation : A →ₐ[ℝ] Module.End ℝ A where
  ...
  commutes' r := by
    apply LinearMap.ext
    intro x
    simp [leftMultiply, Algebra.smul_def]
...
theorem leftRegularRepresentation_injective :
  ...
  simpa [leftRegularRepresentation, leftMultiply] using h1
...
theorem existingOperatorZornRepresentation_injective :
  ...
  InfoGeometry.Canonical.OperatorZornDiracRepresentation.zornDiracRepresentation_injective
