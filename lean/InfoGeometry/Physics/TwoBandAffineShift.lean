...
theorem gapSquare_nonneg (m : ℝ) (d : ℂ) : 0 ≤ gapSquare m d := by
  ...
  nlinarith [sq_nonneg m, sq_nonneg d.re, sq_nonneg d.im]
...
theorem same_point_particleHole_iff (b m : ℝ) (d : ℂ) :
  ...
  · intro h
    ...
    · apply Complex.ext
      · have hd0 : d = -d := by
          simpa [particleHoleMatrix, hamiltonian] using hd
        have hr := congrArg Complex.re hd0
        norm_num at hr ⊢
        linarith
      · have hd0 : d = -d := by
          simpa [particleHoleMatrix, hamiltonian] using hd
        have hi := congrArg Complex.im hd0
        norm_num at hi ⊢
        linarith
  · rintro ⟨rfl, rfl⟩
    ext i j
