...
noncomputable def sheetKreinAdjoint (A : Sheet) : Sheet :=
  fundamentalSymmetry * Matrix.conjTranspose A * fundamentalSymmetry

theorem sheetKreinAdjoint_involutive (A : Sheet) :
...
theorem diracConjugate_fPlus_left_to_fMinus_right {x : Sheet}
...
theorem diracConjugate_fMinus_left_to_fPlus_right {x : Sheet}
...
theorem fPlus_corner_eq_scalar_line {x : Sheet}
  ...
  · simpa [fPlus, uPlus, sheetIdentity, sheetParity, Matrix.mul_apply,
      ...
      (congrArg (fun M : Sheet => M 0 0) hx).symm
  · have h := congrArg (fun M : Sheet => M 0 1) hx
    simpa [fPlus, uPlus, sheetIdentity, sheetParity, Matrix.mul_apply,
      Matrix.vecMul, Fin.sum_univ_two] using h.symm
  · have h := congrArg (fun M : Sheet => M 1 0) hx
    simpa [fPlus, uPlus, sheetIdentity, sheetParity, Matrix.mul_apply,
      Matrix.vecMul, Fin.sum_univ_two] using h.symm
  · have h := congrArg (fun M : Sheet => M 1 1) hx
    simpa [fPlus, uPlus, sheetIdentity, sheetParity, Matrix.mul_apply,
      Matrix.vecMul, Fin.sum_univ_two] using h.symm

theorem fMinus_corner_eq_scalar_line {x : Sheet}
  ...
  ext i j
  fin_cases i <;> fin_cases j
  · have h := congrArg (fun M : Sheet => M 0 0) hx
...
theorem mem_fPlus_corner_iff_exists_scalar {x : Sheet} :
...
theorem mem_fMinus_corner_iff_exists_scalar {x : Sheet} :
...
theorem fPlus_scalar_coefficient_unique {c d : ℂ}
  ...
  have h00 := congrArg (fun M : Sheet => M 0 0) h
...
theorem fMinus_scalar_coefficient_unique {c d : ℂ}
  ...
  have h11 := congrArg (fun M : Sheet => M 1 1) h
  simpa [fMinus, uMinus, sheetIdentity, sheetParity] using h11

theorem fPlus_corner_existsUnique_scalar {x : Sheet}
    (hx : x ∈ peirceCorner fPlus fPlus) :
    ∃! c : ℂ, x = c • fPlus := by
...
theorem fMinus_corner_existsUnique_scalar {x : Sheet}
    (hx : x ∈ peirceCorner fMinus fMinus) :
    ∃! c : ℂ, x = c • fMinus := by
...
theorem fPlus_corner_set_eq_smul_range :
    {x : Sheet | x ∈ peirceCorner fPlus fPlus} =
...
theorem fMinus_corner_set_eq_smul_range :
    {x : Sheet | x ∈ peirceCorner fMinus fMinus} =
