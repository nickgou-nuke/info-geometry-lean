...
@[simp] theorem kawamuraZeta_star (C : CuntzO2Carrier Op) (x : Op) :
...
theorem kawamuraZeta_mul_zeta (C : CuntzO2Carrier Op) (x y : Op) :
  ...
  have h1 : C.S_left * x * star C.S_left * (C.S_left * y * star C.S_left) = C.S_left * (x * y) * star C.S_left := by
    calc
      C.S_left * x * star C.S_left * (C.S_left * y * star C.S_left) = C.S_left * x * (star C.S_left * C.S_left) * y * star C.S_left := by simp [mul_assoc]
  ...
  have h2 : C.S_right * x * star C.S_right * (C.S_right * y * star C.S_right) = C.S_right * (x * y) * star C.S_right := by
    calc
      C.S_right * x * star C.S_right * (C.S_right * y * star C.S_right) = C.S_right * x * (star C.S_right * C.S_right) * y * star C.S_right := by simp [mul_assoc]
  ...
  have h3 : C.S_left * x * star C.S_left * (C.S_right * y * star C.S_right) = 0 := by
    calc
      C.S_left * x * star C.S_left * (C.S_right * y * star C.S_right) = C.S_left * x * (star C.S_left * C.S_right) * y * star C.S_right := by simp [mul_assoc]
  ...
  have h4 : C.S_right * x * star C.S_right * (C.S_left * y * star C.S_left) = 0 := by
    calc
      C.S_right * x * star C.S_right * (C.S_left * y * star C.S_left) = C.S_right * x * (star C.S_right * C.S_left) * y * star C.S_left := by simp [mul_assoc]
...
theorem kawamuraCAR_base_anticomm (C : CuntzO2Carrier Op) :
  ...
  have h1 : C.S_left * star C.S_right * (C.S_right * star C.S_left) = C.S_left * (star C.S_right * C.S_right) * star C.S_left := by simp [mul_assoc]
  have h2 : C.S_right * star C.S_left * (C.S_left * star C.S_right) = C.S_right * (star C.S_left * C.S_left) * star C.S_right := by simp [mul_assoc]
  rw [h1, h2, C.right_isometry, C.left_isometry]
  simp only [mul_one]
  exact C.range_sum
...
theorem kawamuraCAR_base_nilpotent (C : CuntzO2Carrier Op) :
  ...
  have h : a * a = 0 := by
    ...
    have h1 : (C.S_left * star C.S_right) * (C.S_left * star C.S_right) = C.S_left * (star C.S_right * C.S_left) * star C.S_right := by simp [mul_assoc]
    rw [h1, C.orthogonal_ranges.2]
    simp
  rw [h, add_zero]
