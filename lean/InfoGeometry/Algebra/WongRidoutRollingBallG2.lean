...
1. **Cayley-Dickson General Doubling Construction**:
   3107597(a, b) \cdot (c, d) = (a c + arepsilon ar{d} b, d a + b ar{c})3107597
   Specializes to:
   - Compact Octonions $\mathbb{O}$ when $arepsilon = -1$
   - Split Octonions $\mathbb{O}'$ when $arepsilon = +1$
2. **Null Subalgebras & Incidence Geometry**:
   - A subspace  \subseteq \mathbb{O}'$ is a null subalgebra if  \cdot y = 0$ for all , y \in V$.
   - **Points** = 1-dimensional null subspaces.
   - **Lines** = 2-dimensional null subspaces.
   - **Incidence** = Submodule inclusion  \le L$.
...
4. **Root Counting &  \subset G_2$ Cartan Embedding**:
   - $\dim(\mathfrak{g}_2) = 14$, $\operatorname{rank}(\mathfrak{g}_2) = 2$, roots count $= 14 - 2 = 12$.
   - (3)$ stabilizer subgroup embedding inside $ with  + 6 = 14$ dimensional splitting.
...
/-- General Cayley-Dickson product with signature parameter $arepsilon \in R$.
    For compact octonions $\mathbb{O}$, $arepsilon = -1$.
    For split octonions $\mathbb{O}'$, $arepsilon = +1$. -/
...
/-- Split Cayley-Dickson product ($arepsilon = +1$). -/
def splitCayleyDicksonMul
    {H : Type*} [AddCommGroup H] [Mul H] [SMul R H]
    (conj : H → H) (p1 p2 : H × H) : H × H :=
  cayleyDicksonMul conj (1 : R) p1 p2

/-- Compact Cayley-Dickson product ($arepsilon = -1$). -/
...
theorem splitCayleyDicksonMul_def
    {H : Type*} [AddCommGroup H] [Mul H] [SMul R H]
    (conj : H → H) (a b c d : H) :
    splitCayleyDicksonMul conj (a, b) (c, d) = (a * c + (1 : R) • (conj d * b), d * a + b * conj c) :=
  rfl

theorem compactCayleyDicksonMul_def
    {H : Type*} [AddCommGroup H] [Mul H] [SMul R H]
    (conj : H → H) (a b c d : H) :
    compactCayleyDicksonMul conj (a, b) (c, d) = (a * c + (- (1 : R)) • (conj d * b), d * a + b * conj c) :=
  rfl
...
/-- A linear subspace  \le A$ is a **null subalgebra** (Wong-Ridout Section 5 & Baez-Huerta 2014)
    if the multiplication vanishes identically on $:  \cdot y = 0$ for all , y \in V$. -/
def IsNullSubspace (V : Submodule K A) : Prop :=
  ∀ x ∈ V, ∀ y ∈ V, x * y = 0

/-- The zero subspace is trivially a null subalgebra. -/
theorem isNullSubspace_bot : IsNullSubspace (⊥ : Submodule K A) := by
  intro x hx y hy
  have hx0 : x = 0 := Submodule.mem_bot.mp hx
  subst hx0
  simp
...
structure IncidencePoint (A : Type*) [AddCommGroup A] [Module K A] [Mul A] where
...
structure IncidenceLine (A : Type*) [AddCommGroup A] [Module K A] [Mul A] where
  subspace : Submodule K A
  null : IsNullSubspace subspace
  dim_two : Module.finrank K subspace = 2

/-- Incidence relation: A point $ lies on a line $ iff its subspace is contained in $. -/
def LiesOn (P : IncidencePoint A) (L : IncidenceLine A) : Prop :=
...
structure NonAssocAlgAut (K A : Type*) [CommRing K] [AddCommGroup A] [Module K A] [Mul A] where
...
theorem map_null_subspace (Φ : NonAssocAlgAut K A) (V : Submodule K A) (hV : IsNullSubspace V) :
  ...
  change Φ.toLinearEquiv.toLinearMap x * Φ.toLinearEquiv.toLinearMap y = 0
  have hxy : x * y = 0 := hV x hx y hy
  have h_mul := Φ.map_mul x y
  dsimp at h_mul
  rw [hxy, Φ.map_zero] at h_mul
  exact h_mul.symm

/-- Automorphism maps an incidence point to an incidence point. -/
def mapPoint (Φ : NonAssocAlgAut K A) (P : IncidencePoint A) : IncidencePoint A where
...
def mapLine (Φ : NonAssocAlgAut K A) (L : IncidenceLine A) : IncidenceLine A where
...
/-- 🏆 THEOREM (Geometric Collineation):
    Automorphisms of $\mathbb{O}'$ preserve point-line incidence:
     \in L \iff \Phi(P) \in \Phi(L)$. -/
theorem map_preserves_incidence (Φ : NonAssocAlgAut K A) (P : IncidencePoint A) (L : IncidenceLine A) :
...
/-- Total number of roots in the G₂ root system: 4 - 2 = 12$. -/
...
/-- 🏆 THEOREM: The Cartan-Cartan decomposition $\mathfrak{g}_2 = \mathfrak{su}(3) \oplus \mathbb{R}^6$
    satisfies exact dimensional equality  + 6 = 14$. -/
theorem g2_su3_decomposition_dim_eq_14 :
    su3SubgroupDimension + su3ComplementDimension = g2LieAlgebraDimension := rfl

/-- Critical rolling ball radius ratio  = 3$ (Baez-Huerta 2014 & Wong-Ridout 2024). -/
