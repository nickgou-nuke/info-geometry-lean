...
theorem faithfulGibbs_kuboMoriIntegrand_star_eq_reflectedDuhamel
        ...
        (c ^ s * c ^ (1 - s)) •
          (NormedSpace.exp (s • H) *
            A *
              NormedSpace.exp ((1 - s) • H) *
                B) := by
    simp only [real_smul_eq_complex_smul, smul_mul_assoc, mul_smul_comm, smul_smul]
    congr 1
    push_cast
    ring
  ...
  rw [hs, Real.rpow_one]
  have hc_eq : (c : ℂ) = (Z : ℂ)⁻¹ := by
    simp [c, Complex.ofReal_inv]
  rw [hc_eq]
...
theorem faithfulGibbs_kuboMoriPairing_star_eq_normalizedDuhamel
  ...
  rw [hreflect]
  congr 1
  simpa [f] using congrArg (fun g => ∫ s in (0 : ℝ)..1, g s) hreflect.symm
...
theorem normalizedFrechetTwoPoint_eq_kuboMoriPairing
  ...
  rw [trace_exponentialDerivative_mul_eq_duhamelTwoPoint]
