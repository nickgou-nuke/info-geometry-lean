...
lemma two_smul_eq (x : A) : (2 : K) • x = x + x := by
  have : (2 : K) = (1 + 1 : K) := by ring
...
theorem D_plus_add_D_minus (h2 : (2 : K) ≠ 0) (J : A) :
  ...
  have h_add : (1 + J) + (1 - J) = (2 : K) • (1 : A) := by
    have : (1 + J) + (1 - J) = (1 : A) + 1 := by abel
    rw [this, two_smul_eq (K := K)]
...
theorem D_plus_idempotent (h2 : (2 : K) ≠ 0) (J : A) (hJ : J * J = 1) :
  ...
  have h_sq : (1 + J) * (1 + J) = (2 : K) • (1 + J) := by
    have h_exp : (1 + J) * (1 + J) = 1 + J + J + J * J := by
      calc (1 + J) * (1 + J) = (1 + J) * 1 + (1 + J) * J := by rw [mul_add]
      _ = 1 + J + (J + J * J) := by simp only [add_mul, one_mul, mul_one]
      _ = 1 + J + J + J * J := by abel
    rw [h_exp, hJ]
    have h_comb : (1 : A) + J + J + 1 = (1 + 1 : A) + (J + J) := by abel
    rw [h_comb, ← two_smul_eq (K := K) (1 : A), ← two_smul_eq (K := K) J, ← smul_add]
  rw [h_sq, smul_smul]
...
theorem D_minus_idempotent (h2 : (2 : K) ≠ 0) (J : A) (hJ : J * J = 1) :
  ...
  have h_sq : (1 - J) * (1 - J) = (2 : K) • (1 - J) := by
    have h_exp : (1 - J) * (1 - J) = 1 - J - J + J * J := by
      calc (1 - J) * (1 - J) = (1 - J) * 1 - (1 - J) * J := by rw [mul_sub]
      _ = 1 - J - (J - J * J) := by simp only [sub_mul, one_mul, mul_one]
      _ = 1 - J - J + J * J := by abel
    rw [h_exp, hJ]
    have h_comb : (1 : A) - J - J + 1 = (1 + 1 : A) - (J + J) := by abel
    rw [h_comb, ← two_smul_eq (K := K) (1 : A), ← two_smul_eq (K := K) J, ← smul_sub]
  rw [h_sq, smul_smul]
  have h_inv : (2 : K)⁻¹ * (2 : K)⁻¹ * 2 = (2 : K)⁻¹ := by
    rw [mul_assoc, inv_mul_cancel₀ h2, mul_one]
  rw [h_inv]
...
theorem G_plus_square_zero (I j : A)
  ...
  have h_sq : (I + j) * (I + j) = 0 := by
    have h_exp : (I + j) * (I + j) = I * I + (I * j + j * I) + j * j := by
      calc (I + j) * (I + j) = (I + j) * I + (I + j) * j := by rw [mul_add]
      _ = I * I + j * I + (I * j + j * j) := by simp only [add_mul]
      _ = I * I + (I * j + j * I) + j * j := by abel
    rw [h_exp, hI, hj, h_anticomm]
    abel
  rw [h_sq, smul_zero]
...
theorem G_minus_square_zero (I j : A)
  ...
  have h_sq : (I - j) * (I - j) = 0 := by
    have h_exp : (I - j) * (I - j) = I * I - (I * j + j * I) + j * j := by
      calc (I - j) * (I - j) = (I - j) * I - (I - j) * j := by rw [mul_sub]
      _ = I * I - j * I - (I * j - j * j) := by simp only [sub_mul]
      _ = I * I - (I * j + j * I) + j * j := by abel
    rw [h_exp, hI, hj, h_anticomm]
    abel
  rw [h_sq, smul_zero]
...
theorem G_plus_mul_G_minus (h2 : (2 : K) ≠ 0) (I j J : A)
  ...
  have h_prod : (I + j) * (I - j) = (2 : K) • (1 - J) := by
    have h_exp : (I + j) * (I - j) = I * I - I * j + j * I - j * j := by
      ...
      _ = I * I + j * I - (I * j + j * j) := by simp only [add_mul]
      _ = I * I - I * j + j * I - j * j := by abel
    rw [h_exp, hI, hj, h_cross, hJ_def]
    have h_comb : (1 : A) - I * j + - (I * j) - -1 = (1 + 1 : A) - (I * j + I * j) := by abel
    rw [h_comb, ← two_smul_eq (K := K) (1 : A), ← two_smul_eq (K := K) (I * j), ← smul_sub]
  rw [h_prod, smul_smul]
  have h_inv : (2 : K)⁻¹ * (2 : K)⁻¹ * 2 = (2 : K)⁻¹ := by
    rw [mul_assoc, inv_mul_cancel₀ h2, mul_one]
  rw [h_inv]
...
theorem G_minus_mul_G_plus (h2 : (2 : K) ≠ 0) (I j J : A)
  ...
  have h_prod : (I - j) * (I + j) = (2 : K) • (1 + J) := by
    ...
    rw [h_exp, hI, hj, h_cross, hJ_def]
    have h_comb : (1 : A) + I * j - - (I * j) - -1 = (1 + 1 : A) + (I * j + I * j) := by abel
    rw [h_comb, ← two_smul_eq (K := K) (1 : A), ← two_smul_eq (K := K) (I * j), ← smul_add]
  rw [h_prod, smul_smul]
  have h_inv : (2 : K)⁻¹ * (2 : K)⁻¹ * 2 = (2 : K)⁻¹ := by
    rw [mul_assoc, inv_mul_cancel₀ h2, mul_one]
  rw [h_inv]
