...
@[simp] theorem majoranaMode_sq (i : ℕ) :
  ...
  calc
    majoranaMode i * majoranaMode i =
        limit_u i * limit_u i +
          (limit_u i * limit_v i + limit_v i * limit_u i) +
          limit_v i * limit_v i := by
            noncomm_ring
    _ = 1 := by
      rw [limit_u_sq_zero, limit_uv_anticomm, limit_v_sq_zero]
      simp
...
theorem majoranaMode_anticommute {i j : ℕ} (hij : i ≠ j) :
  ...
  calc
    majoranaMode i * majoranaMode j + majoranaMode j * majoranaMode i =
        (limit_u i * limit_u j + limit_u j * limit_u i) +
        (limit_u i * limit_v j + limit_v j * limit_u i) +
        (limit_v i * limit_u j + limit_u j * limit_v i) +
        (limit_v i * limit_v j + limit_v j * limit_v i) := by
          noncomm_ring
    _ = 0 := by
      rw [limit_u_cross_site_anticommute hij,
        limit_u_v_cross_site_anticommute hij,
        limit_v_u_cross_site_anticommute hij,
        limit_v_cross_site_anticommute hij]
      simp
...
def majoranaBraidUnit (i : ℕ) : Limitˣ where
  ...
  val_inv := by
    rw [mul_smul, majoranaBraidVal_mul_inverseNumerator]
    rw [smul_smul]
    norm_num
  inv_val := by
    rw [smul_mul, inverseNumerator_mul_majoranaBraidVal]
    rw [smul_smul]
    norm_num
...
private theorem adjacent_majorana_artin
  ...
  have hbcab : (b * c) * (a * b) = -(a * c) := by
    calc
      ...
      _ = -(a * c) := by rw [habbc]; noncomm_ring
  ...
  have hnegacbc : (-(a * c)) * (b * c) = a * b := by
    calc
      (-(a * c)) * (b * c) = -(a * (c * b) * c) := by noncomm_ring
      _ = -(a * (-(b * c)) * c) := by rw [hcb]
      _ = a * b * (c * c) := by noncomm_ring
      _ = a * b := by rw [hc]; simp
  calc
    (1 + a * b) * (1 + b * c) * (1 + a * b) =
        1 + a * b + b * c + (a * b) * (b * c) +
...
private theorem separated_bivectors_commute
  ...
  calc
    ...
    _ = c * a * b * d := by rw [hca]; noncomm_ring
    _ = c * a * (-(d * b)) := by rw [← hdb]; simp [mul_assoc]
    _ = -(c * a * d) * b := by noncomm_ring
    _ = c * d * a * b := by rw [hda]; noncomm_ring
...
theorem majoranaBraid_relations (n : ℕ) :
  ...
  · rcases hr with ⟨i, j, hij, rfl⟩
    apply Units.ext
    simp [adjacentWord, hij, majoranaBraidUnit_artin]
  · rcases hr with ⟨i, j, hij, rfl⟩
    apply Units.ext
    simp [distantWord]
    exact majoranaBraidVal_commute hij
