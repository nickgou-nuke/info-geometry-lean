...
theorem CuntzFibonacciOperator_fusion :
  CuntzFibonacciOperator S₁ S₂ *
  CuntzFibonacciOperator S₁ S₂ =
  1 + CuntzFibonacciOperator S₁ S₂ := by
  have h1212 :
    (S₁ * star S₂) * (S₁ * star S₂) = 0 := by
    calc
      (S₁ * star S₂) * (S₁ * star S₂) =
  ...
  calc
    ...
    _ =
      ...
      S₂ * star S₁ +
      S₂ * star S₂ := by
      rw [h1212, h1221, h1222, h2112, h2121, h2122, h2212, h2221, h2222]
      abel
    _ =
      1 +
      (S₁ * star S₂ + S₂ * star S₁ + S₂ * star S₂) := by
      rw [← hC.cuntz_sum]
      abel
