import CanonicalZornIntegralSpinTrialityClosure

/-!
# The integral-lattice-preserving real spin subgroup

The full real spin group acts on the real split quadratic carrier, but it need
not preserve the embedded integral Zorn lattice.  This file defines the honest
arithmetic subgroup by requiring preservation in both directions, constructs
its action on integral Zorn coordinates, and proves preservation of the
integral `II₄,₄` quadratic refinement.
-/
