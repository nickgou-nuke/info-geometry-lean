...
theorem modularAnomalyGenerator_eq_exp_commutator_shadow
  ...
  apply TopologicalMajoranaShadow.modularAnomalyGenerator_eq_commutator_shadow
    (M := topologicalShadowOfExp σGen hJConj) (U := U) (σGen := σGen)
  · simpa [expFlow] using
  ...
  · have hNeg :
        ...
        have hShift :
          ...
          HasDerivAt.comp_const_sub
            ...
            (a := (0 : ℝ)) (x := (0 : ℝ)) hBase'
        simpa [zero_sub, neg_smul] using hShift
    simpa [topologicalShadowOfExp, expFlow] using hNeg
...
noncomputable def canonicalCl11TopologicalShadow :
  ...
  topologicalShadowOfExp
    (σGen := canonicalCl11Generator (E := E))
    (hJConj := canonicalCl11Generator_J_conj_expFlow (E := E))
...
theorem modularAnomalyGenerator_eq_canonicalCl11_commutator_shadow
  ...
  simpa [canonicalCl11TopologicalShadow, canonicalCl11Generator] using
    modularAnomalyGenerator_eq_exp_commutator_shadow
      (E := E)
      (σGen := canonicalCl11Generator (E := E))
      (hJConj := canonicalCl11Generator_J_conj_expFlow (E := E))
      (U := U)
