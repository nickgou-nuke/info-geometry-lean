#!/usr/bin/env python3
"""Replace SocraticVerifier class in automath_pipeline.py with browser-harness version."""
    ...
    def critique(self, hypothesis: Hypothesis) -> str:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
                ...
                prompt_file = f.name
            try:
                # Use browser-harness with the socratic prompt
                env = os.environ.copy()
...
\"\"\"
            with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
                ...
                prompt_file = f.name
            try:
                # Use browser-harness with the repair prompt
                env = os.environ.copy()
                env["CHATGPT_PROMPT_FILE"] = prompt_file
                env["BU_CDP_WS"] = os.environ.get("BU_CDP_WS", "")
                env["CHATGPT_URL"] = "https://chatgpt.com/?temporary-chat=true"
