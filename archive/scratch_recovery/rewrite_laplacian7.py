...
lemma orthogonal_ranges (A : Matrix m n ℚ) (B : Matrix p m ℚ) (hBA : B * A = 0) (x : n → ℚ) (y : p → ℚ) :
  ...
  calc
    ...
    _ = innerProduct ((B * A).mulVec x) y := by rw [← Matrix.mulVec_mulVec]
...
theorem laplacian_rank_eq_add_rank (A : Matrix m n ℚ) (B : Matrix p m ℚ) (hBA : B * A = 0) :
  ...
  have h_rank_XX_tr {k : Type*} [Fintype k] [DecidableEq k] (X : Matrix m k ℚ) : Matrix.rank (X * Xᵀ) = Matrix.rank X := by
    have h_ker_eq : LinearMap.ker (X * Xᵀ).mulVecLin = LinearMap.ker Xᵀ.mulVecLin := by
      ext x; constructor
      · intro hx
        have hx_zero : (X * Xᵀ).mulVec x = 0 := LinearMap.mem_ker.mp hx
        have h_d : innerProduct x ((X * Xᵀ).mulVec x) = 0 := by rw [hx_zero]; simp [innerProduct]
        have h_mulX : (X * Xᵀ).mulVec x = X.mulVec (Xᵀ.mulVec x) := by rw [Matrix.mulVec_mulVec]
        rw [h_mulX] at h_d
        rw [innerProduct_transpose_mulVec_eq X x (Xᵀ.mulVec x)] at h_d
        rw [LinearMap.mem_ker]
        exact (innerProduct_self_eq_zero_iff (Xᵀ.mulVec x)).mp h_d
      · intro hx
        rw [LinearMap.mem_ker] at hx ⊢
        have h_mulX : (X * Xᵀ).mulVec x = X.mulVec (Xᵀ.mulVec x) := by rw [Matrix.mulVec_mulVec]
        rw [h_mulX, hx, Matrix.mulVec_zero]
    have h1 := LinearMap.finrank_range_add_finrank_ker (X * Xᵀ).mulVecLin
    have h2 := LinearMap.finrank_range_add_finrank_ker Xᵀ.mulVecLin
    have h_card_k : Module.finrank ℚ (m → ℚ) = Fintype.card m := by simp
    rw [h_card_k] at h1 h2
    rw [← Matrix.rank] at h1
    rw [← Matrix.rank] at h2
    rw [Matrix.rank_transpose] at h2
    rw [h_ker_eq] at h1
    omega
  have hC_rank : Matrix.rank C = Matrix.rank A := h_rank_XX_tr A
  have hD_rank : Matrix.rank D = Matrix.rank B := by simpa [D] using h_rank_XX_tr Bᵀ
  ...
  have h_inter : LinearMap.range C.mulVecLin ⊓ LinearMap.range D.mulVecLin = ⊥ := by
    ...
    have h_norm : innerProduct (D.mulVec y) (D.mulVec y) = 0 := by
      calc
        innerProduct y (D.mulVec (D.mulVec y)) = innerProduct (Dᵀ.mulVec y) (D.mulVec y) := by rw [innerProduct_transpose_mulVec_eq D y (D.mulVec y)]
  ...
  have h_im_Csq : LinearMap.range (C * C).mulVecLin = LinearMap.range C.mulVecLin := by
    ...
    have h_ker_sq : LinearMap.ker (C * C).mulVecLin = LinearMap.ker C.mulVecLin := by
      ext z; constructor
      · intro hz'
        have hz_zero : (C * C).mulVec z = 0 := LinearMap.mem_ker.mp hz'
        have h_dz : innerProduct z ((C * C).mulVec z) = 0 := by rw [hz_zero]; simp [innerProduct]
        have h_mulCC : (C * C).mulVec z = C.mulVec (C.mulVec z) := by rw [Matrix.mulVec_mulVec]
        rw [h_mulCC] at h_dz
        have h_nz : innerProduct (C.mulVec z) (C.mulVec z) = 0 := by
          calc
            innerProduct z (C.mulVec (C.mulVec z)) = innerProduct (Cᵀ.mulVec z) (C.mulVec z) := by rw [innerProduct_transpose_mulVec_eq C z (C.mulVec z)]
  ...
  have h_im_Dsq : LinearMap.range (D * D).mulVecLin = LinearMap.range D.mulVecLin := by
    ...
    have h_ker_sq : LinearMap.ker (D * D).mulVecLin = LinearMap.ker D.mulVecLin := by
      ext z; constructor
      · intro hz'
        have hz_zero : (D * D).mulVec z = 0 := LinearMap.mem_ker.mp hz'
        have h_dz : innerProduct z ((D * D).mulVec z) = 0 := by rw [hz_zero]; simp [innerProduct]
        have h_mulDD : (D * D).mulVec z = D.mulVec (D.mulVec z) := by rw [Matrix.mulVec_mulVec]
        rw [h_mulDD] at h_dz
        have h_nz : innerProduct (D.mulVec z) (D.mulVec z) = 0 := by
          calc
            innerProduct z (D.mulVec (D.mulVec z)) = innerProduct (Dᵀ.mulVec z) (D.mulVec z) := by rw [innerProduct_transpose_mulVec_eq D z (D.mulVec z)]
  ...
  have h_imC_sub : LinearMap.range C.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    calc
      (C + D).mulVec (C.mulVec y) = C.mulVec (C.mulVec y) + D.mulVec (C.mulVec y) := by simp [Matrix.add_mulVec]
  ...
  have h_imD_sub : LinearMap.range D.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    calc
      (C + D).mulVec (D.mulVec z) = C.mulVec (D.mulVec z) + D.mulVec (D.mulVec z) := by simp [Matrix.add_mulVec]
