...
/-- Example instantiation of a D4RootLabel. -/
def exampleD4RootLabel : D4RootLabel := {
  i := 0
  j := 1
  i_ne_j := by decide
  signI := .plus
...
structure D4SpinorWeight where
  signs : Fin 4 → D4Sign
  isPositiveChirality : d4PositiveChirality signs
...
/-- Example instantiation of a D4SpinorWeight. -/
def exampleD4SpinorWeight : D4SpinorWeight := {
  signs := fun _ => .plus
  isPositiveChirality := by
    simp [d4PositiveChirality, d4NegativeCount]
}

/--
Distribution of degeneracies for Weyl Character.
(Added to fix missing definition)
-/
...
/-- Example instantiation of a Cl44D4CartanCharacterSkeleton. -/
noncomputable def exampleCl44D4CartanCharacterSkeleton : Cl44D4CartanCharacterSkeleton := {
  cartanTemperature := fun _ => 0
  stressProjection := fun _ => 0
  centralChargeProjection := fun _ => 0
  stressCentralDistribution := {
    stressTensorDegeneracy := 0
    centralChargeDegeneracy := 0
    residualDegeneracy := 0
    totalDegeneracy := 0
    total_eq := by norm_num
  }
  totalProjection := fun _ => 0
  totalProjection_eq := fun _ => by norm_num
}
...
projection is compatible with the horizon/stress lane.
-/
...
Refactored to split into separate lemmas to avoid single large monolithic proofs.
-/
theorem bps_dominant_weight_capstone_bps
    (P : Cl44BPSDominantCharacterPacket) :
    P.isDominantBPSWeight P.dominantWeight :=
  P.dominant_isBPS

theorem bps_dominant_weight_capstone_central
    (P : Cl44BPSDominantCharacterPacket) :
    P.skeleton.centralChargeProjection P.dominantWeight = P.bpsCentralReadout :=
  P.central_saturates

theorem bps_dominant_weight_capstone_stress
    (P : Cl44BPSDominantCharacterPacket) :
    P.skeleton.stressProjection P.dominantWeight = P.stressReadout :=
  P.stress_matches
...
/-- Example instantiation of a Cl44BPSDominantCharacterPacket. -/
noncomputable def exampleCl44BPSDominantCharacterPacket : Cl44BPSDominantCharacterPacket := {
  skeleton := exampleCl44D4CartanCharacterSkeleton
  dominantWeight := exampleD4SpinorWeight
  bpsCentralReadout := 0
  stressReadout := 0
  isDominantBPSWeight := fun _ => 0 = 0
  dominantWeight_isBPS := rfl
  centralProjection_saturates := rfl
  stressProjection_matches := rfl
}
