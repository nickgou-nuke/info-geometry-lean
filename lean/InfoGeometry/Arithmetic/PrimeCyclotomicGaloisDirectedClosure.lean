...
private theorem edge01 : primeTowerGraph.edge 0 1 := by native_decide
private theorem edge12 : primeTowerGraph.edge 1 2 := by native_decide
private theorem edge23 : primeTowerGraph.edge 2 3 := by native_decide
private theorem edge34 : primeTowerGraph.edge 3 4 := by native_decide
private theorem edge45 : primeTowerGraph.edge 4 5 := by native_decide

/-- The fixed directed certificate from the first to the sixth prime stage. -/
def primeTowerPath : DirectedPath primeTowerGraph 0 5 :=
  .cons edge01 (.cons edge12 (.cons edge23 (.cons edge34 (.cons edge45 (.refl 5)))))

@[simp] theorem primeTowerPath_length : primeTowerPath.length = 5 := rfl
