...
def identityDoubled : BlockMat :=
  fromBlocks (1 : SubMat) (0 : SubMat) (0 : SubMat) (1 : SubMat)
...
def blockDiag (A B : SubMat) : BlockMat :=
  fromBlocks A (0 : SubMat) (0 : SubMat) B
...
theorem superTrace_identityDoubled :
  ...
  have h_mul : fromBlocks (1 : SubMat) (0 : SubMat) (0 : SubMat) (-1 : SubMat) *
      fromBlocks (1 : SubMat) (0 : SubMat) (0 : SubMat) (1 : SubMat) =
      fromBlocks (1 : SubMat) (0 : SubMat) (0 : SubMat) (-1 : SubMat) := by
...
theorem superTrace_blockDiag (A B : SubMat) :
  ...
  have h_mul : fromBlocks (1 : SubMat) (0 : SubMat) (0 : SubMat) (-1 : SubMat) *
      fromBlocks A (0 : SubMat) (0 : SubMat) B =
      fromBlocks A (0 : SubMat) (0 : SubMat) (-B) := by
    rw [fromBlocks_multiply]
    simp
...
lemma alpha_add_beta_mul_n (h_two_n : (2 * (Fintype.card ι : R)) * two_n_inv = 1) (A B : SubMat) :
...
lemma alpha_sub_beta_mul_n (h_two_n : (2 * (Fintype.card ι : R)) * two_n_inv = 1) (A B : SubMat) :
    (alphaCommon two_n_inv A B - betaChiral two_n_inv A B) * (Fintype.card ι : R) =
      Matrix.trace B := by
  dsimp [alphaCommon, betaChiral]
  have h_sub :
...
/-- 
  ...
  Tr(K₀) = 0
-/
theorem trace_K_zero (h_two_n : (2 * (Fintype.card ι : R)) * two_n_inv = 1) (A B : SubMat) :
...
/-- 
  ...
  STr(K₀) = 0
-/
theorem superTrace_K_zero (h_two_n : (2 * (Fintype.card ι : R)) * two_n_inv = 1) (A B : SubMat) :
...
theorem trifold_reconstruction (A B : SubMat) :
  ...
  · simp only [fromBlocks_apply₁₁, add_apply, smul_apply, sub_apply, one_apply, smul_eq_mul]
    ring
  · simp only [fromBlocks_apply₁₂, add_apply, smul_apply, zero_apply, smul_eq_mul]
    ring
  · simp only [fromBlocks_apply₂₁, add_apply, smul_apply, zero_apply, smul_eq_mul]
    ring
...
def adK (K : A) : A →ₗ[ℤ] A where
  ...
  map_smul' r X := by
    ...
    calc
      K * (r • X) - (r • X) * K = r • (K * X) - r • (X * K) := by
        rw [mul_smul_comm, smul_mul_assoc]
      _ = r • (K * X - X * K) := (smul_sub r (K * X) (X * K)).symm
...
theorem dlogRN_inv (D : R →ₗ[R] R) (hD : IsLinearDerivation D)
  ...
  have h_one : D 1 = 0 := by
    have hD1 : D 1 = D 1 + D 1 := by
      have h_step := hD 1 1
      rw [mul_one, mul_one, one_mul] at h_step
      exact h_step
    have h_canc : D 1 + 0 = D 1 + D 1 := by
      calc
        D 1 + 0 = D 1 := add_zero (D 1)
        _ = D 1 + D 1 := hD1
    exact (add_left_cancel h_canc).symm
  dsimp [dlogRN]
  have h_prod : D (Δ * inv_Δ) = 0 := by rw [h, h_one]
  have h_leib : D (Δ * inv_Δ) = D Δ * inv_Δ + Δ * D inv_Δ := hD Δ inv_Δ
  rw [h_leib] at h_prod
  have h_shift : Δ * D inv_Δ = - (D Δ * inv_Δ) := by
    linear_combination h_prod
  have h_final : inv_Δ * (Δ * D inv_Δ) = inv_Δ * (- (D Δ * inv_Δ)) := by
    rw [h_shift]
  have h_cancel : inv_Δ * (Δ * D inv_Δ) = (inv_Δ * Δ) * D inv_Δ := by ring
  rw [mul_comm inv_Δ Δ, h, one_mul] at h_cancel
  rw [h_cancel] at h_final
  have h_inv : D inv_Δ = - (inv_Δ * inv_Δ * D Δ) := by
    calc
      D inv_Δ = inv_Δ * (- (D Δ * inv_Δ)) := h_final
      _ = - (inv_Δ * inv_Δ * D Δ) := by ring
  calc
    Δ * D inv_Δ = Δ * (- (inv_Δ * inv_Δ * D Δ)) := by rw [h_inv]
    _ = - (Δ * inv_Δ) * (inv_Δ * D Δ) := by ring
    _ = - 1 * (inv_Δ * D Δ) := by rw [h]
    _ = - (inv_Δ * D Δ) := by ring
