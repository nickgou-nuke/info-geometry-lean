...
APPROVED_AXIOMS = ROOT / "scripts/quality/approved_axioms.txt"
...
def run_axiom_report(mode: str, included_paths: set[str], approved_axioms: set[str]) -> dict[str, Any]:
    cmd = ["lake", "env", "lean", "--run", "scripts/quality/audit_axioms_report.lean"]
    proc = subprocess.run(
        cmd,
        cwd=ROOT,
        text=True,
...
def build_report(mode: str, approved_axioms_path: Path) -> dict[str, Any]:
    ...
    axiom_report = run_axiom_report(mode, included_paths, approved_axioms)
