...
structure ClosureData
  ...
  driftWeightTwo : HasWeightTwoBKMDriftIntensity bkmFusion.metric
  gaugeWeightMinusTwo : HasInverseWeightTwoBKMGauge bkmFusion.metric
  bkm_state : State
  ...
  hestenesStage :
    ∀ n m : ℕ, ∀ x : DoubledSpace (D.compiled.hestenesCone.Base n),
      D.compiled.analyticFamily.colimitReadout (n + m)
          (D.compiled.hestenesCone.toFilteredPhaseCone.bondIterate n m x) =
...
def of (D : ClosureData Weight Tangent State) : CertifiedReadout D where
  ...
  dikinInterior := dikin_ellipsoid_cone_confinement D.tube D.v1 D.v2 D.radius
    D.radius_nonneg D.radius_lt_one D.dikin_bound
  barrierIdentity := universal_log_barrier_eq D.tube
  hessianDeterminant := cone_hessian_det D.tube
  logBarrierSelfConcordant := logBarrier_selfConcordant_on_Ioi
  surprisalNonnegative := modular_surprisal_deficit_nonneg D.surprisal_coordinate
  dissipationNonnegative := metriplectic_second_law D.metriplectic_state
  bkmDriftNonnegative :=
  ...
  bkmMassWeylInvariant := by
    ...
    exact D.bkmFusion.metric.physicalMass_weylInvariant
      D.gaugeFixed
      D.driftWeightTwo D.gaugeWeightMinusTwo c D.bkm_state hc
