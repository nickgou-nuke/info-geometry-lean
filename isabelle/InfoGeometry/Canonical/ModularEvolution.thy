...
locale modular_evolution = einstein_causality +
  ...
  assumes colimit_map_carrier: "\<And>A x. x \<in> carrier (F_obj A) \<Longrightarrow> colimit_map A x \<in> carrier colimit_ring"
...
theorem modular_evolution_preserves_commutativity:
  ...
  by (metis (full_types) assms colimit_map_carrier einstein_causality.commute_of_spacelike einstein_causality_axioms ring_hom_mult sigma_hom)
