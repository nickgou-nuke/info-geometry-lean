...
def wittPlus : Fin 5 → Vector55 :=
  ![coordinateVector 0, coordinateVector 1,
...
def wittMinus : Fin 5 → Vector55 :=
  ![coordinateVector 8, coordinateVector 9,
...
@[simp] theorem wittMinus_pairing (i j : Fin 5) :
  ...
  fin_cases i <;> fin_cases j <;>
    simp [wittMinus, splitPairing, coordinateVector] <;>
    norm_num <;> ring
...
def wittMinusCoefficient (x : Vector55) : Fin 5 → ℝ :=
  ![x 8, x 9, x 2 - x 5, x 3 - x 6, x 4 - x 7]
...
def FullMultiGrade.opposite (d : FullMultiGrade) : FullMultiGrade where
  ...
  parity_eq := by
    rw [d.parity_eq]
    simpa using congrArg Neg.neg d.parity_eq
  witt := d.witt
