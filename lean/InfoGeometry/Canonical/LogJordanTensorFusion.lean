...
with `N₁₂³ = 0`, and exposes the explicit generalized-eigenvector chain generated
from `e₁ ⊗ e₁`.

Thus the tensor product is a generalized eigenspace of weight `Δ₁ + Δ₂` with
logarithmic depth at most three.  This is the nearest formal DAG edge toward a
tensor-closed non-semisimple/logarithmic monoidal sector; no braided-category
packaging is asserted here.
-/
...
theorem logTensorNilpotent_cube_zero :
  ...
  change tensorJordanNilpotent (R := 𝕜) (jordanNilpotent 𝕜) (jordanNilpotent 𝕜) *
      tensorJordanNilpotent (R := 𝕜) (jordanNilpotent 𝕜) (jordanNilpotent 𝕜) *
        tensorJordanNilpotent (R := 𝕜) (jordanNilpotent 𝕜) (jordanNilpotent 𝕜) = 0
  exact tensorJordanNilpotent_cube_zero (R := 𝕜)
      (jordanNilpotent 𝕜) (jordanNilpotent 𝕜)
      (jordanNilpotent_sq_zero (𝕜 := 𝕜))
      (jordanNilpotent_sq_zero (𝕜 := 𝕜))
...
theorem jordanNilpotentLinear_sq_zero :
  ...
  apply LinearMap.ext
  intro v
  change Matrix.mulVec (jordanNilpotent 𝕜)
      (Matrix.mulVec (jordanNilpotent 𝕜) v) = 0
  rw [Matrix.mulVec_mulVec v]
...
/-- The primary basis state is killed by the Jordan nilpotent. -/
@[simp]
theorem jordanNilpotentLinear_e0 :
    jordanNilpotentLinear (𝕜 := 𝕜)
        (e0 : JordanCarrier 𝕜) = 0 := by
  ext i
  fin_cases i <;>
    simp [jordanNilpotentLinear, jordanNilpotent, e0, Matrix.mulVec,
      Pi.single, Matrix.vecHead, Matrix.vecTail]

/-- The logarithmic partner is sent to the primary basis state. -/
@[simp]
theorem jordanNilpotentLinear_e1 :
    jordanNilpotentLinear (𝕜 := 𝕜)
        (e1 : JordanCarrier 𝕜) = (e0 : JordanCarrier 𝕜) := by
  ext i
  fin_cases i <;>
    simp [jordanNilpotentLinear, jordanNilpotent, e0, e1, Matrix.mulVec,
      Pi.single, Matrix.vecHead, Matrix.vecTail]
...
/-- The first step of the explicit tensor Jordan chain generated by
`e₁ ⊗ e₁`. -/
theorem logTensorNilpotentEnd_e1_tmul_e1 :
    logTensorNilpotentEnd (𝕜 := 𝕜)
        ((e1 : JordanCarrier 𝕜) ⊗ₜ[𝕜] (e1 : JordanCarrier 𝕜)) =
      (e0 : JordanCarrier 𝕜) ⊗ₜ[𝕜] (e1 : JordanCarrier 𝕜) +
      (e1 : JordanCarrier 𝕜) ⊗ₜ[𝕜] (e0 : JordanCarrier 𝕜) := by
  simp
...
/-- The second step of the explicit tensor Jordan chain.  Starting from
`e₁ ⊗ e₁`, the square lands on `2 • (e₀ ⊗ e₀)`. -/
theorem logTensorNilpotentEnd_sq_e1_tmul_e1 :
    ((logTensorNilpotentEnd (𝕜 := 𝕜)).comp
        (logTensorNilpotentEnd (𝕜 := 𝕜)))
      ((e1 : JordanCarrier 𝕜) ⊗ₜ[𝕜] (e1 : JordanCarrier 𝕜)) =
        (2 : 𝕜) •
          ((e0 : JordanCarrier 𝕜) ⊗ₜ[𝕜] (e0 : JordanCarrier 𝕜)) := by
  simpa using
    (logTensorNilpotentEnd_sq_tmul (𝕜 := 𝕜)
      (e1 : JordanCarrier 𝕜) (e1 : JordanCarrier 𝕜))
...
theorem jordanCellLinear_apply (Δ : 𝕜) (v : JordanCarrier 𝕜) :
  ...
  change Matrix.mulVec (jordanCell Δ) v = _
  rw [show jordanCell Δ = Δ • (1 : Matrix (Fin 2) (Fin 2) 𝕜) + jordanNilpotent 𝕜 by
    ext i j
    fin_cases i <;> fin_cases j <;> simp [jordanCell, jordanNilpotent]]
  simp [Matrix.add_mulVec, Matrix.smul_mulVec, jordanNilpotentLinear]
...
/-- On the top logarithmic tensor state `e₁ ⊗ e₁`, the centered fused zero mode
starts the explicit length-three Jordan chain. -/
theorem centeredLogTensorZeroMode_e1_tmul_e1
    (Δ₁ Δ₂ : 𝕜) :
    centeredLogTensorZeroMode Δ₁ Δ₂
        ((e1 : JordanCarrier 𝕜) ⊗ₜ[𝕜] (e1 : JordanCarrier 𝕜)) =
      (e0 : JordanCarrier 𝕜) ⊗ₜ[𝕜] (e1 : JordanCarrier 𝕜) +
  ...
  exact logTensorNilpotentEnd_e1_tmul_e1 (𝕜 := 𝕜)
