...
@[rep_depth operator]
def wordDepth (w : TypeIIIModularCantorSystem.BinaryWord) : ℕ :=
  w.length

/-- A single binary refinement step increases depth by one. -/
@[rep_depth operator]
theorem binaryWord_child_length (w : TypeIIIModularCantorSystem.BinaryWord) (b : Bool) :
    wordDepth (TypeIIIModularCantorSystem.BinaryWord.child w b) = wordDepth w + 1 := by
  simp [wordDepth, TypeIIIModularCantorSystem.BinaryWord.child]

/-- Every word lies in its own closed Cantor cylinder. -/
@[rep_depth operator]
theorem binaryWord_mem_closedCylinder_self (w : TypeIIIModularCantorSystem.BinaryWord) :
    w ∈ TypeIIIModularCantorSystem.BinaryWord.closedCylinder w := by
  simpa using (TypeIIIModularCantorSystem.BinaryWord.mem_closedCylinder_self w)
...
theorem binaryWord_closedCylinder_split (w : BinaryWord) :
    TypeIIIModularCantorSystem.BinaryWord.closedCylinder w =
      ({w} : Set TypeIIIModularCantorSystem.BinaryWord)
        ∪ TypeIIIModularCantorSystem.BinaryWord.closedCylinder
            (TypeIIIModularCantorSystem.BinaryWord.child w false)
        ∪ TypeIIIModularCantorSystem.BinaryWord.closedCylinder
            (TypeIIIModularCantorSystem.BinaryWord.child w true) := by
  simpa using (TypeIIIModularCantorSystem.BinaryWord.closedCylinder_split w)
...
structure BinaryWordTiltReadout (Op : Type*) [Ring Op] where
  ...
  word : TypeIIIModularCantorSystem.BinaryWord
...
def IsLocalCl11Relation (positive negative : Op) : Prop :=
  positive * positive = 1 ∧
    negative * negative = -1 ∧
    positive * negative + negative * positive = 0

def localCl11Positive (P : BinaryWordTiltReadout Op) : Op :=
  P.bitOperator false

def localCl11Negative (P : BinaryWordTiltReadout Op) : Op :=
  P.bitOperator false * P.bitOperator true

theorem localCl11Positive_sq (P : BinaryWordTiltReadout Op) :
    localCl11Positive P * localCl11Positive P = 1 :=
  P.bitOperator_false_sq

theorem localCl11Negative_sq (P : BinaryWordTiltReadout Op) :
    localCl11Negative P * localCl11Negative P = -1 := by
  unfold localCl11Negative
  exact InfoGeometry.Clifford.LorentzianBivectorSignatureBridge.positive_bivector_sq_neg_one
    P.bitOperator_false_sq P.bitOperator_true_sq P.bitOperator_anticomm

theorem localCl11Positive_mul_negative (P : BinaryWordTiltReadout Op) :
    localCl11Positive P * localCl11Negative P * localCl11Positive P =
      -localCl11Negative P := by
  unfold localCl11Positive localCl11Negative
  calc
    (P.bitOperator false * (P.bitOperator false * P.bitOperator true)) *
        P.bitOperator false =
        (P.bitOperator false * P.bitOperator false) * P.bitOperator true *
          P.bitOperator false := by noncomm_ring
    _ = P.bitOperator true * P.bitOperator false := by
...
def binaryWordLocalCl11Relation (P : BinaryWordTiltReadout Op) :
    IsLocalCl11Relation (localCl11Positive P) (localCl11Negative P) := by
  refine ⟨localCl11Positive_sq P, localCl11Negative_sq P, ?_⟩
  unfold localCl11Positive localCl11Negative
  calc
    P.bitOperator false * (P.bitOperator false * P.bitOperator true) +
        P.bitOperator false * P.bitOperator true * P.bitOperator false =
        P.bitOperator false *
          (P.bitOperator false * P.bitOperator true +
            P.bitOperator true * P.bitOperator false) := by noncomm_ring
    _ = 0 := by rw [P.bitOperator_anticomm]; simp
...
theorem cantorBinaryTiltCARCCROwnerTarget :
    (∀ w : TypeIIIModularCantorSystem.BinaryWord,
        wordDepth (TypeIIIModularCantorSystem.BinaryWord.child w false) = wordDepth w + 1)
      ∧
