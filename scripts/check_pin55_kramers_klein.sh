...
lake build InfoGeometry.Clifford.Pin55KramersKleinFiveGradeClosure

log=$(mktemp)
trap 'rm -f "$log"' EXIT
lake env lean lean/InfoGeometry/Clifford/Pin55KramersKleinAudit.lean | tee "$log"
...
audit = Path('lean/InfoGeometry/Clifford/Pin55KramersKleinAudit.lean').read_text()
