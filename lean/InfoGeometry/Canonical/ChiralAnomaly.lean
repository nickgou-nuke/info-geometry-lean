...
χ = [a⁺, aᴰ].
-/
def inverseCommutator (a_mp a_d : R) : R :=
  a_mp * a_d - a_d * a_mp
...
ε = || [a⁺, aᴰ] ||.
-/
noncomputable def epsilon (a_mp a_d : R) : ℝ :=
  nnnorm (inverseCommutator a_mp a_d)
...
-/
theorem normal_inverse_anomaly_vanishes (a_mp a_d : R)
    (h_comm : a_mp * a_d = a_d * a_mp) :
    inverseCommutator a_mp a_d = 0 := by
