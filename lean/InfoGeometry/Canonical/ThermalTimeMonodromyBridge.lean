...
parameter.
-/
abbrev ThermalTimeWindingCalibration := {period : ℝ // period ≠ 0}

namespace ThermalTimeWindingCalibration

abbrev period (C : ThermalTimeWindingCalibration) : ℝ := C.1

theorem period_ne_zero (C : ThermalTimeWindingCalibration) : C.period ≠ 0 := C.2

end ThermalTimeWindingCalibration
...
-/
def cl11CanonicalCalibration : ThermalTimeWindingCalibration :=
  ⟨1, by norm_num⟩
...
theorem cl11_calibrated_flow_agrees (E : Type) [NormedAddCommGroup E] [InnerProductSpace ℝ E]
  ...
  change modularAutomorphismGroup modularData ((n : ℝ) * 1) A =
    modularAutomorphismGroup modularData (n : ℝ) A
  rw [mul_one]
