...
@[simp] theorem tracedExponentialSeriesTermDerivative_succ
  ...
  calc
    (((k + 1).factorial : ℂ)⁻¹) *
        (((k + 1 : ℕ) : ℂ) * finiteOperatorTrace (T * (H + z • T) ^ k)) =
      ((((k + 1).factorial : ℂ)⁻¹) * ((k + 1 : ℕ) : ℂ)) *
        finiteOperatorTrace (T * (H + z • T) ^ k) := by rw [mul_assoc]
    _ = ((k.factorial : ℂ)⁻¹) *
        finiteOperatorTrace (T * (H + z • T) ^ k) := by
      congr 1
      convert inverse_factorial_succ_mul_cast_succ k using 1 <;> norm_num
...
theorem summable_tracedExponentialDerivativeMajorant
  ...
  simpa [tracedExponentialDerivativeMajorant, div_eq_mul_inv, mul_assoc] using
...
private theorem norm_affine_operator_pow_le
  ...
  | zero =>
      ...
      exact le_max_right (1 : ℝ) ‖(1 : Operator n)‖
  | succ k =>
      calc
        ‖(H + z • T) ^ (k + 1)‖ ≤
            ‖H + z • T‖ ^ (k + 1) :=
...
private theorem norm_trace_mul_affine_pow_le
  ...
  calc
    ...
    _ = tracedExponentialDerivativeConstant n T *
        (tracedExponentialLineRadius H T) ^ k := by
      rw [tracedExponentialDerivativeConstant]
      ring
...
theorem summable_tracedExponentialSeriesTerm_zero
  ...
  · funext k
    change ((k.factorial : ℂ)⁻¹) *
      finiteOperatorTrace ((H + (0 : ℂ) • T) ^ k) = _
    rw [zero_smul ℂ T, add_zero]
    simpa only [finiteOperatorTraceCLM_apply, finiteOperatorTrace_complex_smul,
...
theorem tsum_tracedExponentialSeriesTermDerivative_zero
  ...
  calc
    (∑' k : ℕ,
      ...
      tsum_eq_zero_add' ((summable_nat_add_iff 1).mpr hDerivativeSummable)
    _ = ∑' k : ℕ,
        ((k.factorial : ℂ)⁻¹) *
      ...
      rw [tracedExponentialSeriesTermDerivative_succ]
      simp only [zero_smul ℂ T, add_zero]
...
theorem tracedExponentialDerivative_apply
  ...
  have hLine :
    ...
    simpa only [zero_smul ℂ, add_zero, zero_add, one_smul] using
      (hasDerivAt_const (x := (0 : ℂ)) H).add
        ((hasDerivAt_id (x := (0 : ℂ))).smul_const T)
  have hFromFrechet :
      HasDerivAt (partitionLine H T)
        (tracedExponentialDerivative H T) 0 := by
    convert
      (hasFDerivAt_tracedExponential H).comp_hasDerivAt_of_eq
        (0 : ℂ) hLine (by rw [zero_smul ℂ T, add_zero]) using 1 <;>
      try exact zero_smul ℂ T <;>
      try exact zero_smul ℝ T
...
theorem hasDerivAt_logPartitionLine_eq_expectation
  ...
  have hzero : (0 : ℂ) • T = 0 := zero_smul ℂ T
  have harg : H + (0 : ℂ) • T = H := by
    rw [hzero, add_zero]
  have hLog := hPartition.clog (by
    change tracedExponential (H + (0 : ℂ) • T) ∈ Complex.slitPlane
    rw [harg]
    exact hSlit)
  rw [operatorExpectation_eq_trace_div]
  convert hLog using 1 <;> simp [logPartitionLine, partitionLine, harg]

/- The same first-variation result for the translated line through an
 arbitrary base point.  This is the chain-rule form used when differentiating
 the first variation once more; it does not assume that `H` and `T` commute. -/
theorem hasDerivAt_logPartitionLine_at
    {n : ℕ} (H T : Operator n) (z : ℂ)
    (hSlit : tracedExponential (H + z • T) ∈ Complex.slitPlane) :
    HasDerivAt
      (fun s : ℂ => logTracedExponential (H + s • T))
      (logTracedExponentialDerivative (H + z • T) T) z := by
  have hline : HasDerivAt (fun s : ℂ => H + s • T) T z := by
    convert
      (hasDerivAt_const (x := z) H).add
        ((hasDerivAt_id (x := z)).smul_const T) using 1 <;>
      simp
  have hcomp := (hasFDerivAt_logTracedExponential
    (H := H + z • T) hSlit).comp_hasDerivAt z hline
