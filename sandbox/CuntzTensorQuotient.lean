...
namespace VirasoroProject.Extensions.CuntzTensorQuotient
...
theorem dagger_CuntzToeplitzRel {n : ℕ} : ∀ {x y : CuntzTensor n},
  ...
  intro x y h
  rcases h with ⟨i, j⟩
  change CuntzToeplitzRel n (dagger n (Sdag n i * S n j)) (dagger n (if i = j then 1 else 0))
  by_cases hij : i = j
  · subst j
...
/-- The Cuntz--Toeplitz algebra carries the star structure descended from formal dagger. -/
...
end VirasoroProject.Extensions.CuntzTensorQuotient
