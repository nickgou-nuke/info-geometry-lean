...
/--
The exact norm-one multiplicative associator corresponding to
`((S₁S₂)U₃)(S₁(S₂U₃))⁻¹`.
-/
theorem zorn_multiplicative_associator_exact :
...
theorem zorn_multiplicative_associator_ne_one :
    zornMultiplicativeAssociator zornS1 zornS2 zornU3 ≠ zornOne := by
  rw [zorn_multiplicative_associator_exact]
  intro h
  have ha := congrArg zornA h
  norm_num [zornA, zornMk, zornOne] at ha

end InfoGeometry.Canonical.MobiusKleinStatisticalZornPacket
