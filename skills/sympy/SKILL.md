...
metadata:
    ...
    adapted-by: Hermes Agent for info-geometry-lean-fusion
---
...
## Local Adaptation (info-geometry-lean-fusion)

This skill is borrowed from `K-Dense-AI/scientific-agent-skills` and adapted for local repo workflows.

- Execution lane: local shell/Python only (no external paid API dependency for core symbolic steps).
- Authority ladder for theorem-facing work:
  1. Lean kernel / successful Lean build is truth authority.
  2. DAG/Arango topology is structural authority.
  3. SymPy and Python heuristics are advisory unless re-checked in Lean.
- Do not present a SymPy-derived identity as theorem truth until a Lean artifact validates it.
- Preserve dimension-agnostic statements unless a finite-dimensional assumption is explicitly required.

### Lean handoff checklist for SymPy-derived artifacts

1. Keep symbolic derivations exact (rational/symbolic constants; avoid float contamination).
2. Export simplified expressions in a Lean-friendly form (clear naming and explicit assumptions).
3. Record assumptions used by SymPy (`real=True`, `positive=True`, etc.) and carry them into theorem statements.
4. Verify the resulting theorem/lemma in Lean (`lake build` target or strict-check surface) before promotion.
...
6. **`lean-handoff-notes.md`**: Assumption tracking, exactness discipline, theorem-facing promotion gate
   - Load when: Preparing SymPy output for Lean artifacts or theorem-facing docs
