...
def InScope (binderStack : List LambdaPath) (index : Nat) : Prop :=
  ∃ binder, binderStack[index]? = some binder
...
structure BindingWitness where
  ...
  binder_lookup : binderStack[index]? = some binder
...
structure UnboundWitness where
  ...
  index : Nat
  binderStack : List LambdaPath
  unbound_lookup : binderStack[index]? = none
...
theorem binding_lookup (witness : BindingWitness) :
    witness.binderStack[witness.index]? = some witness.binder :=
...
theorem unbound_lookup (witness : UnboundWitness) :
    witness.binderStack[witness.index]? = none :=
...
def variableEdges
  ...
  match h : binderStack[index]? with
  | some binder =>
...
abbrev ScopedPHOAS (B F : Type) :=
  DAG.PHOASExpressionLayer.PHOASExpr (Sum B F)

/-- PHOAS bound and free variables remain constructor-distinct. -/
theorem scopedPHOAS_bvar_ne_fvar
    {B F : Type} (bound : B) (free : F) :
    DAG.PHOASExpressionLayer.PHOASExpr.var (Sum.inl bound) ≠
      DAG.PHOASExpressionLayer.PHOASExpr.var (Sum.inr free) := by
  intro h
  cases h
