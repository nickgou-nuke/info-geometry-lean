...
theorem finite_boson_cancels_graded
  ...
  | cons x xs ih =>
      ...
      simp only [bosonicFockDeterminant, gradedFockSupertrace, List.map_cons, List.prod_cons]
      calc
        (1 - x)⁻¹ * (List.map bosonLocal xs).prod * ((1 - x) * (List.map gradedFermionLocal xs).prod)
            = ((1 - x)⁻¹ * (1 - x)) *
                ((List.map bosonLocal xs).prod * (List.map gradedFermionLocal xs).prod) := by
                  ring
        _ = 1 * 1 := by
                  rw [inv_mul_cancel₀ (sub_ne_zero.mpr (Ne.symm hx))]
                  have htailprod : (List.map bosonLocal xs).prod * (List.map gradedFermionLocal xs).prod = 1 := by
                    simpa [bosonicFockDeterminant, gradedFockSupertrace] using ih htail
                  rw [htailprod]
        _ = 1 := by ring
...
/-- Finite-stage compatibility under list concatenation for the bosonic cutoff. -/
theorem finitePrimeBosonicPartition_append
    (modes₁ modes₂ : List PrimeMode) :
    finitePrimeBosonicPartition (modes₁ ++ modes₂) =
      finitePrimeBosonicPartition modes₁ * finitePrimeBosonicPartition modes₂ := by
  unfold finitePrimeBosonicPartition weightsOfModes bosonicFockDeterminant
  simpa [List.map_append] using
    (List.prod_append (modes₁.map bosonLocal) (modes₂.map bosonLocal)).symm

/-- Finite-stage compatibility under list concatenation for the graded cutoff. -/
theorem finitePrimeGradedIndex_append
    (modes₁ modes₂ : List PrimeMode) :
    finitePrimeGradedIndex (modes₁ ++ modes₂) =
      finitePrimeGradedIndex modes₁ * finitePrimeGradedIndex modes₂ := by
  unfold finitePrimeGradedIndex weightsOfModes gradedFockSupertrace
  simpa [List.map_append] using
    (List.prod_append (modes₁.map gradedFermionLocal) (modes₂.map gradedFermionLocal)).symm

/-- Finite-stage compatibility under list concatenation for the ordinary trace. -/
theorem ordinaryFermionTrace_append
    (xs ys : List ℂ) :
    ordinaryFermionTrace (xs ++ ys) =
      ordinaryFermionTrace xs * ordinaryFermionTrace ys := by
  unfold ordinaryFermionTrace
  simpa [List.map_append] using
    (List.prod_append (xs.map ordinaryFermionLocal) (ys.map ordinaryFermionLocal)).symm
