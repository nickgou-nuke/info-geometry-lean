...
theorem biQuaternion_symplectic_poisson_packet :
  ...
  by
    refine ⟨?_, ?_⟩
    · exact symplecticI_skew
    · exact finitePoissonBracket_skew
...
theorem biQuaternion_legendre_packet :
  ...
  by
    refine ⟨?_, ?_⟩
    · exact legendreReadout_eq_hamiltonian
    · exact hamiltonian_nonneg_of_potential_nonneg
...
theorem biQuaternion_noether_packet :
  ...
  by
    refine ⟨?_, ?_⟩
    · exact finiteNoetherReadback_radial_zero
    · exact radialQuadraticHamiltonian_I4c_invariant
...
theorem hodge4D_packet :
  ...
  by
    refine ⟨?_, ?_, ?_, ?_⟩
    · exact InfoGeometry.Canonical.HodgeStar4DFinite.hodgeStar_sq
    · exact InfoGeometry.Canonical.HodgeStar4DFinite.self_plus_anti
    · exact InfoGeometry.Canonical.HodgeStar4DFinite.hodgeStar_selfDualPart
    · exact InfoGeometry.Canonical.HodgeStar4DFinite.hodgeStar_antiSelfDualPart
...
theorem massieu_legendre_packet (M : InfoGeometry.LogPotential.LegendreModel) :
  ...
  by
    refine ⟨?_, ?_, ?_, ?_⟩
    · exact M.fenchelGap_nonneg
    · exact M.fenchelGap_eq_zero_at_contact
    · exact M.entropy_eq_dual_at_contact
    · exact M.temperatureRegularizedHamiltonian_nonneg
...
theorem section27_capstone (M : InfoGeometry.LogPotential.LegendreModel) :
  ...
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  · exact biQuaternion_complexStructure_packet
  · exact symplecticI_skew
  · exact legendreReadout_eq_hamiltonian
  · exact finiteNoetherReadback_radial_zero
  · exact InfoGeometry.Canonical.HodgeStar4DFinite.hodgeStar_sq
  · exact (massieu_legendre_packet M).2.1
