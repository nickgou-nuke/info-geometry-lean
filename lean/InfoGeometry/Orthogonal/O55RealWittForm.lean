...
def realSplitO55Lie : LieSubalgebra ℝ RealMat10 where
  ...
  zero_mem' := by simp [IsRealSplitOrthogonal, realWittAdjoint]
  add_mem' := by
    intro X Y hX hY
  ...
  smul_mem' := by
    intro c X hX
    unfold IsRealSplitOrthogonal at hX ⊢
...
@[simp] theorem realCartan_mul_matrixUnit
  ...
  classical
  ext a b
  simp [Matrix.mul_apply, realCartanMatrix, realMatrixUnit]
...
theorem realCartan_commutator_rootMatrix
  ...
  all_goals
    ...
    rw [mul_sub, sub_mul,
      ...
      realCartan_mul_matrixUnit, realMatrixUnit_mul_cartan]
    simp [realRootCharacter, realIndexWeight, hL, hR]
    module
...
@[simp] theorem complexify_realMatrixUnit (p q : Index) :
  ...
  ext a b
  simp [complexifyMatrix, realMatrixUnit, matrixUnit]
...
@[simp] theorem complexify_realContactGradingMatrix :
  ...
  congr 1
  funext i
  simp [realContactCartanCoefficient, contactCartanCoefficient]
