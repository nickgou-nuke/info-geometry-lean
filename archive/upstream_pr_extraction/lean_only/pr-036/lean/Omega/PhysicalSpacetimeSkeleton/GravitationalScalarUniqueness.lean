...
theorem paper_physical_spacetime_gravitational_scalar_uniqueness
    ...
    (normalizeAffinePart :
      admissible →
        ∀ {a b divergence : ℝ},
          C.gravitationalScalar = a * C.ricciScalar + b + divergence →
            a = 1 ∧ b = -2 * C.cosmologicalConstant) :
    ∃ a b divergence : ℝ,
      C.gravitationalScalar = a * C.ricciScalar + b + divergence ∧
      a = 1 ∧
  ...
  obtain ⟨a, b, divergence, hAffine⟩ :=
    gravitational_scalar_action_equivalent_to_affine_ricci C admissible hAdm
      affineActionEquivalence
  obtain ⟨ha, hb⟩ := normalizeAffinePart hAdm hAffine
