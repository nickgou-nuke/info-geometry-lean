...
/-!
# Iwasawa (KAN) decomposition representative for `M₂(ℂ)`
...
Tiny bridge note:
The determinant signatures here `(det K = 1, det A = 1, det N = 1)` and zero locus `det (N - I) = 0`
mirror the `DeterminantSupergrading` package's parity/sign table: unit determinants for the
compact–dilation KAN core and grade transition at zero determinant for the nilpotent channel.
-/
...
/-- Shear displacement from identity, used for the square-zero unipotent locus. -/
...
theorem det_KPart (θ : ℝ) : (KPart θ).det = 1 := by
  ...
  calc
    ...
    _ = Complex.exp 0 := by rw [add_neg_cancel]
...
/-- KAN product is an explicit triangular representative matrix. -/
...
/--
The thermal-ray Cayley compactification tends to the limiting point `1` at
zero temperature (`β → +∞`).
