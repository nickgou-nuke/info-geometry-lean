...
/-- Concrete instantiation of `FiniteExpFamily` to verify it is not vacuous.
We model a simple coin flip over `Fin 2`. -/
def coinExpFamily : FiniteExpFamily (α := Fin 2) 1 where
  stat x _ := if x = 0 then 1 else 0
  base _ := 1
...
lemma fisher_quadratic_expand
  ...
  calc
    v i * (∑ x, density F θ x * (centeredStat F θ i x * centeredStat F θ j x)) * v j
        = (v i * v j) * ∑ x, density F θ x * (centeredStat F θ i x * centeredStat F θ j x) := by ring
    _ = ∑ x, (v i * v j) * (density F θ x * (centeredStat F θ i x * centeredStat F θ j x)) := by rw [Finset.mul_sum]
    _ = ∑ x, density F θ x * (v i * centeredStat F θ i x * (v j * centeredStat F θ j x)) := by
        refine Finset.sum_congr rfl ?_
        intro x _
        ring

lemma fisher_quadratic_swap_sums
    {d : ℕ}
    (F : FiniteExpFamily (α := α) d)
    (θ v : Fin d → ℝ) :
    (∑ i, ∑ j, ∑ x,
        density F θ x *
          (v i * centeredStat F θ i x * (v j * centeredStat F θ j x))) =
    ∑ x,
      density F θ x *
        (∑ i, v i * centeredStat F θ i x) *
        (∑ j, v j * centeredStat F θ j x) := by
  calc
    (∑ i, ∑ j, ∑ x, density F θ x * (v i * centeredStat F θ i x * (v j * centeredStat F θ j x)))
      = ∑ i, ∑ x, ∑ j, density F θ x * (v i * centeredStat F θ i x * (v j * centeredStat F θ j x)) := by
        refine Finset.sum_congr rfl ?_
        intro i _
        rw [Finset.sum_comm]
...
lemma fisher_as_variance
  ...
  rw [fisher_quadratic_expand, fisher_quadratic_swap_sums]
  unfold expectation
  refine Finset.sum_congr rfl ?_
  intro x _
  dsimp only
