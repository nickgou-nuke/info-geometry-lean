...
/-- 🏆 GRAND CAPSTONE: Projective Cross-Ratio PGL(2, ℂ) Invariance & Quantum Yang-Baxter Synthesis -/
theorem grand_canonical_cross_ratio_pgl2_synthesis
    ...
    (F * B * F = R) :=
  ⟨(grand_cross_ratio_pgl2_synthesis s hs M hM).1,
   (grand_cross_ratio_pgl2_synthesis s hs M hM).2,
   F_sq,
   F_B_F_eq_R⟩
