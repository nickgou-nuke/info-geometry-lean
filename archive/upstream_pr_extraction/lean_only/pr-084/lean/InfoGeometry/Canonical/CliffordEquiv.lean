...
/-- The quadratic map r ↦ -r^2 on ℝ. -/
def Q : QuadraticMap ℝ ℝ ℝ := -QuadraticMap.sq
...
noncomputable def complexLift {A : Type*} [Ring A] [Algebra ℝ A] (S : A) (hS : S * S = -1) :
    ℂ →ₐ[ℝ] A where
  toFun z := algebraMap ℝ A z.re + z.im • S
  map_one' := by simp
  map_mul' x y := by
    dsimp
    have hS' : (x.im • S) * (y.im • S) = - algebraMap ℝ A (x.im * y.im) := by
      have h_smul : (x.im • S) * (y.im • S) = (x.im * y.im) • (S * S) := by
        rw [smul_mul_smul]
      rw [h_smul, hS]
      simp [Algebra.algebraMap_eq_smul_one]
    have h1 : (algebraMap ℝ A x.re * (y.im • S)) = (x.re * y.im) • S := by
      rw [Algebra.algebraMap_eq_smul_one, smul_mul_assoc, one_mul, mul_smul]
    have h2 : ((x.im • S) * algebraMap ℝ A y.re) = (x.im * y.re) • S := by
      rw [Algebra.algebraMap_eq_smul_one, mul_smul_comm, mul_one, smul_smul, mul_comm y.re x.im]
    have h3 : algebraMap ℝ A x.re * algebraMap ℝ A y.re = algebraMap ℝ A (x.re * y.re) := by
      rw [← map_mul]
    have h_expand : (algebraMap ℝ A x.re + x.im • S) * (algebraMap ℝ A y.re + y.im • S) =
        algebraMap ℝ A (x.re * y.re - x.im * y.im) + (x.re * y.im + x.im * y.re) • S := by
      have h_ring : (algebraMap ℝ A x.re + x.im • S) * (algebraMap ℝ A y.re + y.im • S) =
          algebraMap ℝ A x.re * algebraMap ℝ A y.re + algebraMap ℝ A x.re * (y.im • S)
            + (x.im • S) * algebraMap ℝ A y.re + (x.im • S) * (y.im • S) := by
        rw [add_mul, mul_add, mul_add]
        abel
      rw [h_ring, h3, h1, h2, hS']
      simp only [map_sub, add_smul]
      abel
    change algebraMap ℝ A (x * y).re + (x * y).im • S = _
    rw [Complex.mul_re, Complex.mul_im, h_expand.symm]
  map_zero' := by simp
  map_add' x y := by
    dsimp
    rw [map_add, add_smul]
