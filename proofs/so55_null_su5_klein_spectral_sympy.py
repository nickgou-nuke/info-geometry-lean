...
spin_even=sp.binomial(5,0)+sp.binomial(5,2)+sp.binomial(5,4)
spin_odd=sp.binomial(5,1)+sp.binomial(5,3)+sp.binomial(5,5)

def involution(diag,off): return (diag,-off)
def mobius_inv(e,o): return ((e+o)/2,(e-o)/2)
...
eta=sp.zeros(10); eta[:5,5:]=sp.eye(5); eta[5:,:5]=sp.eye(5)
assert eta.T==eta and eta*eta==sp.eye(10)
A=sp.Matrix([[sp.symbols(f'a{i}{j}') for j in range(5)] for i in range(5)])
B=sp.zeros(5); C=sp.zeros(5)
for i in range(5):
 for j in range(i+1,5):
  b=sp.symbols(f'b{i}{j}'); c=sp.symbols(f'c{i}{j}')
  B[i,j]=b; B[j,i]=-b; C[i,j]=c; C[j,i]=-c
X=A.row_join(B).col_join(C.row_join(-A.T))
assert sp.simplify(X.T*eta + eta*X)==sp.zeros(10)
...
m=sp.symbols('m')
assert sp.expand(kspring(C1(m))-m*m)==0
assert klein_mode(Q(11,7))==0
edges=['decomposes_to','trace_splits_to','skew_splits_to_B','skew_splits_to_C','quotients_modes']
print({'null_dim':null_dim,'so55_dim':so_dim,'sl5_adj':sl5,'dilaton':1,'skew10':skew,'partition':partition,'eta_involutive':eta*eta==sp.eye(10),'block_constraint_zero':True,'spin_even16':spin_even,'spin_odd16':spin_odd,'mobius_roundtrip':True,'klein_average':klein_avg(1,2,3,4),'tripotent_roots':[-1,0,1],'brillouin_cancel':klein_mode(Q(11,7)),'edges':len(edges)})
