...
/-- The vacuum is trivially a Maya diagram. -/
def vacuum_diagram : MayaDiagram :=
  ⟨vacuum, by
    unfold IsMaya
    rw [symmDiff_self]
    exact Set.finite_empty⟩

/-- The dual vacuum is trivially a Dual Maya diagram. -/
def dual_vacuum_diagram : DualMayaDiagram :=
  ⟨dual_vacuum, by
    unfold IsDualMaya
    rw [symmDiff_self]
    exact Set.finite_empty⟩
...
theorem g2_symmDiff_subset {S : Set MayaIndex} :
    symmDiff (g2 S) dual_vacuum ⊆ 
    negate_index (shift_plus (S_plus S)) ∪ negate_index (Set.Iic 0 \ S) := by
  unfold g2 dual_vacuum
  intro j hj
  simp only [Set.symmDiff_def, Set.mem_union, Set.mem_diff, Set.mem_Ici] at hj ⊢
...
theorem g2_maps_maya_to_dual_maya {S : Set MayaIndex} (hS : IsMaya S) :
  ...
  exact g2_symmDiff_subset

theorem g1_symmDiff_subset {S : Set MayaIndex} :
    symmDiff (g1 S) dual_vacuum ⊆ 
    negate_index (shift_plus (S_plus S)) ∪ {0} ∪ negate_index (Set.Iic 0 \ S) := by
  unfold g1 dual_vacuum
  intro j hj
  simp only [Set.symmDiff_def, Set.mem_union, Set.mem_diff, Set.mem_singleton_iff, Set.mem_Ici] at hj ⊢
