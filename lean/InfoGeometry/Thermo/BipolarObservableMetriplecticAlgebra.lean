...
/-- Positivity after evaluating an algebra-valued diagonal metric response. -/
theorem metricBracket_nonneg (D : DerivationFrame A) (ev : A →+* ℝ) (F : A) :
    0 ≤ ev (metricBracket D F F) := by
  rw [metricBracket_sq, map_pow]
  exact sq_nonneg _

/-- Vanishing response is a condition on the evaluated derivative, not on the
value of the observable itself. -/
theorem metricBracket_eq_zero_iff (D : DerivationFrame A)
    (ev : A →+* ℝ) (F : A) :
    ev (metricBracket D F F) = 0 ↔ ev (D.Deta F) = 0 := by
  rw [metricBracket_sq, map_pow, sq_eq_zero_iff]
...
theorem metriplecticBracket_add_left
  ...
  unfold metriplecticBracket
  rw [poissonBracket_add_left, metricBracket_add_left]
  abel
...
theorem metriplecticBracket_add_right
    ...
    metriplecticBracket D F (G₁ + G₂) =
      metriplecticBracket D F G₁ + metriplecticBracket D F G₂ := by
  unfold metriplecticBracket
  rw [poissonBracket_add_right, metricBracket_add_right]
  abel
...
theorem metriplecticBracket_leibniz_right
  ...
  unfold metriplecticBracket
  rw [poissonBracket_leibniz_right, metricBracket_leibniz_right]
  ring
...
/-- Evaluation of algebra-valued entropy production is nonnegative. The
observable algebra need not be ordered, and its derivations need not vanish. -/
...
theorem bipolar_observable_metriplectic_packet
    (D : DerivationFrame A) (ev : A →+* ℝ) (H S F G K : A)
    (hS : IsPoissonCasimir D S)
    (hH : IsMetricKernel D H) :
  ...
  exact ⟨poissonBracket_jacobi D F G K,
    ...
    metriplectic_second_law_nonneg D ev H S hS hH⟩
