...
theorem even_comm_even {x y : A} (hx : P.IsEven x) (hy : P.IsEven y) :
  ...
  unfold comm IsEven
  rw [show P.act (x * y - y * x) = P.act (x * y) - P.act (y * x) by
    unfold InternalParity.act
    noncomm_ring]
  rw [P.act_mul, P.act_mul, hx, hy] <;> simp [sub_eq_add_neg]
...
theorem even_comm_odd {x y : A} (hx : P.IsEven x) (hy : P.IsOdd y) :
  ...
  rw [show P.act (x * y - y * x) = P.act (x * y) - P.act (y * x) by
    unfold InternalParity.act
    noncomm_ring]
  rw [P.act_mul, P.act_mul, hx, hy] <;>
    simp [sub_eq_add_neg, smul_eq_mul] <;> abel
...
theorem invariant_of_even_diagonal_odd_offDiagonal
  ...
  have hV' := hV
  have hW' := hW
  unfold InternalParity.IsEven InternalParity.act at hE0' hE1'
  unfold InternalParity.IsOdd InternalParity.act at hV' hW'
  rw [totalParity_conjugation_formula]
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [blockHamiltonian, InternalParity.act, InternalParity.IsEven,
      InternalParity.IsOdd, hE0', hE1', hV', hW']
...
theorem InternalParity.star_even
  ...
  simp only [InternalParity.act, StarMul.star_mul] at h
  rw [hP] at h
