...
lemma ker_mul_transpose_eq_ker_transpose {m n : ℕ} (A : Matrix (Fin m) (Fin n) ℚ) (x : Fin m → ℚ) :
  ...
  · intro h
    ...
    have h_mul_A : (A * Aᵀ).mulVec x = A.mulVec (Aᵀ.mulVec x) := Matrix.mulVec_mulVec.symm
    rw [h_mul_A] at h_dot
...
lemma ker_sq_eq_ker_psd {m : ℕ} (C : Matrix (Fin m) (Fin m) ℚ) (h_sym : Cᵀ = C)
  ...
  · intro h
    ...
    have h_mul_CC : (C * C).mulVec x = C.mulVec (C.mulVec x) := Matrix.mulVec_mulVec.symm
...
lemma im_sq_eq_im_AAtranspose {m n : ℕ} (A : Matrix (Fin m) (Fin n) ℚ) :
  ...
  have h_sub : LinearMap.range (C * C).mulVecLin ≤ LinearMap.range C.mulVecLin := by
    ...
    have h_mul_CC : C.mulVec (C.mulVec x) = (C * C).mulVec x := Matrix.mulVec_mulVec
...
lemma laplacian_rank_eq_sum {n₀ n₁ n₂ : ℕ}
  ...
  have h_inter : LinearMap.range C.mulVecLin ⊓ LinearMap.range D.mulVecLin = ⊥ := by
    ...
    have h_Dv : D.mulVec v = 0 := by
      ...
      have h_mul_DC : D.mulVec (C.mulVec x) = (D * C).mulVec x := Matrix.mulVec_mulVec
      rw [h_mul, h_mul_DC, h_DC, Matrix.zero_mulVec]
    have h_ker : (D * D).mulVec y = 0 := by
      simp only [Matrix.mulVecLin_apply] at hy
      have h_mul_DD : (D * D).mulVec y = D.mulVec (D.mulVec y) := Matrix.mulVec_mulVec.symm
      rw [h_mul_DD, hy, h_Dv]
  ...
  have h_imC_sub : LinearMap.range C.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    have step1 : (C + D).mulVec (C.mulVec y) = C.mulVec (C.mulVec y) + D.mulVec (C.mulVec y) := Matrix.add_mulVec
    have step2 : C.mulVec (C.mulVec y) = (C * C).mulVec y := Matrix.mulVec_mulVec
    have step3 : D.mulVec (C.mulVec y) = (D * C).mulVec y := Matrix.mulVec_mulVec
    simp only [Matrix.mulVecLin_apply] at hy ⊢
  ...
  have h_imD_sub : LinearMap.range D.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    have step1 : (C + D).mulVec (D.mulVec z) = C.mulVec (D.mulVec z) + D.mulVec (D.mulVec z) := Matrix.add_mulVec
    have step2 : C.mulVec (D.mulVec z) = (C * D).mulVec z := Matrix.mulVec_mulVec
    have step3 : D.mulVec (D.mulVec z) = (D * D).mulVec z := Matrix.mulVec_mulVec
  ...
  have h_range : LinearMap.range (C + D).mulVecLin = LinearMap.range C.mulVecLin ⊔ LinearMap.range D.mulVecLin := by
    ...
    · intro v hv
      ...
      have h_add : (C + D).mulVec x = C.mulVec x + D.mulVec x := Matrix.add_mulVec
      simp only [Matrix.mulVecLin_apply] at hx
      rw [h_add] at hx
