...
namespace InfoGeometry.Lie.CanonicalZornG2CartanCovariance
...
def fisherSouriauCovariance (beta : Fin 2 → ℝ) (i j : Fin 2) : ℝ :=
  ∑ x : State, realGibbsWeight D beta x *
    (D.momentMap x i - meanMomentMap D beta i) *
    (D.momentMap x j - meanMomentMap D beta j)
...
theorem fisherSouriauCovariance_diagonal_nonneg
  ...
  intro x _
  rw [mul_assoc]
  rw [show (D.momentMap x i - meanMomentMap D beta i) *
      (D.momentMap x i - meanMomentMap D beta i) =
      (D.momentMap x i - meanMomentMap D beta i) ^ (2 : ℕ) by ring]
  exact mul_nonneg (hw x) (sq_nonneg _)

end InfoGeometry.Lie.CanonicalZornG2CartanCovariance
