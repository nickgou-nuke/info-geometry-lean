...
/-- The constructed CAR witness carries the source CAR packet and the correct current law. -/
theorem rawCAR_to_SplitCARCurrentWitness_spec
    {A : Type*} [Ring A] (C : RawCARModeCompletion A)
    (𝕜 : Type*) [Field 𝕜] [CharZero 𝕜] (α : 𝕜) :
    (rawCAR_to_SplitCARCurrentWitness C 𝕜 α).source = C ∧
    (∀ v, ∀ᶠ l : Int in atTop, (rawCAR_to_SplitCARCurrentWitness C 𝕜 α).J l v = 0) ∧
    (∀ m n, ((rawCAR_to_SplitCARCurrentWitness C 𝕜 α).J m).commutator ((rawCAR_to_SplitCARCurrentWitness C 𝕜 α).J n) =
...
theorem chargedFockSpaceSplitCARCurrentWitness_readout
    ...
    (∀ m n : Int,
      ...
      if m + n = 0 then
        ...
        0) := by
  exact ⟨(chargedFockSpaceSplitCARCurrentWitness α).trunc,
    (chargedFockSpaceSplitCARCurrentWitness α).comm⟩
...
theorem chargedFockSpace_current_constructive_J_trunc_comm
    ...
    (∀ m n : Int,
      ...
      if m + n = 0 then
        ...
        0) := by
  exact ⟨(chargedFockSpaceCurrentHeisenbergRep 𝕜 α).trunc,
    (chargedFockSpaceCurrentHeisenbergRep α).comm⟩
