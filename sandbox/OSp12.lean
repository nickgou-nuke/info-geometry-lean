...
theorem projUp_add_projDown_eq_drazin (O : Op V) :
    projUp O + projDown O = InfoGeometry.Canonical.FibonacciParafermionAtoms.drazin_projector O := by
  calc
    projUp O + projDown O
        = InfoGeometry.Canonical.FibonacciParafermionAtoms.proj_up O +
...
theorem projVac_add_projUp_add_projDown (O : Op V) :
  ...
  calc
    ...
    _ = projVac O + InfoGeometry.Canonical.FibonacciParafermionAtoms.drazin_projector O := by
            rw [projUp_add_projDown_eq_drazin]
    _ = 1 := by
            simpa [projVac] using
