...
open InfoGeometry.Quantum.BerryKeatingCCR
...
3. **Entropy Foliation Leaves**:
   ...
   - $\lambda < 1$: right-half-plane sign leaves $\sigma > 1/2$ ($\Phi > 0$).
   - $\lambda > 1$: left-half-plane sign leaves $\sigma < 1/2$ ($\Phi < 0$).
...
/-!
### 3. Thermodynamic phase and sign foliation
-/

/-- 🏆 THEOREM 3 (Subharmonic Foliation):
    In the right half-plane Re(s) > 1/2 away from the zero z₀ = 3/2, the ratio satisfies R(s) < 1,
    rendering the Souriau entropy strictly positive: Φ(s) > 0.  These are
    sign/level-set results; no Laplace--Beltrami harmonicity is asserted. -/
...
/-- 🏆 THEOREM 4 (Superharmonic Foliation):
    ...
    rendering the Souriau entropy strictly negative: Φ(s) < 0.  These are
    sign/level-set results; no Laplace--Beltrami harmonicity is asserted. -/
...
/-!
### 4. Conformal logarithmic decomposition
-/
...
/-- 🏆 THEOREM 5 (Orthogonal Foliation Decoupling):
    The modulus potential Φ(s) and phase H_HP(s) satisfy the following
    complex logarithmic decomposition:
...
theorem grand_souriau_apollonius_foliation_synthesis
    ...
    (((sigma - 3 / 2) ^ 2 + t ^ 2) - ((sigma + 1 / 2) ^ 2 + t ^ 2) =
      -4 * sigma + 2) ∧
