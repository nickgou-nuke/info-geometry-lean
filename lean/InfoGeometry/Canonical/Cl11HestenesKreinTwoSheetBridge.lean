...
@[simp] theorem etaBase_fMinusBase_etaBase :
...
@[simp] theorem fMinusElem_fPlusElem_zero (n : ℕ) :
  ...
  have hη : etaElem n * etaElem n = (1 : Stage n) := etaElem_sq n
  have hswap : etaElem n * fPlusElem n * etaElem n = fMinusElem n :=
    etaElem_fPlusElem_etaElem n
  have hpm : fPlusElem n * fMinusElem n = 0 := fPlusElem_fMinusElem_zero n
  have hep : etaElem n * fPlusElem n = fMinusElem n * etaElem n := by
    calc
      etaElem n * fPlusElem n = (etaElem n * fPlusElem n) * 1 := by rw [mul_one]
      _ = (etaElem n * fPlusElem n) * (etaElem n * etaElem n) := by rw [hη]
      _ = (etaElem n * fPlusElem n * etaElem n) * etaElem n := by simp only [mul_assoc]
      _ = fMinusElem n * etaElem n := by rw [hswap]
  calc
    fMinusElem n * fPlusElem n =
        (etaElem n * fPlusElem n * etaElem n) * fPlusElem n := by rw [hswap]
    _ = etaElem n * fPlusElem n * (etaElem n * fPlusElem n) := by simp only [mul_assoc]
    _ = etaElem n * fPlusElem n * (fMinusElem n * etaElem n) := by rw [hep]
    _ = etaElem n * (fPlusElem n * fMinusElem n) * etaElem n := by simp only [mul_assoc]
...
@[simp] theorem etaElem_fMinusElem_etaElem (n : ℕ) :
    etaElem n * fMinusElem n * etaElem n = fPlusElem n := by
  rw [← map_mul, ← map_mul, etaBase_fMinusBase_etaBase]
...
private theorem rightColimit_on_stage (n : ℕ) (x : Stage n) :
    (rightColimit bilingualDatum) ((colimit.ι moduleDiagram n).hom x) =
      (colimit.ι moduleDiagram n).hom (fMinusElem n * x) := by
  exact congrArg Prod.snd (colimit_sheet_stage_readback n x)

/-- The descended Krein involution exchanges the two sheet projectors. -/
theorem colimitEta_fPlus_colimitEta :
  ...
  | _ n x =>
      rw [Module.End.mul_apply, Module.End.mul_apply,
        colimitEta_on_stage, leftColimit_on_stage,
        colimitEta_on_stage, rightColimit_on_stage]
      simp [etaElem_fPlusElem_etaElem, mul_assoc]

/-- The descended Krein involution exchanges the negative sheet back to the
positive sheet. -/
theorem colimitEta_fMinus_colimitEta :
  ...
  | _ n x =>
      rw [Module.End.mul_apply, Module.End.mul_apply,
        colimitEta_on_stage, rightColimit_on_stage,
        colimitEta_on_stage, leftColimit_on_stage]
      simp [etaElem_fMinusElem_etaElem, mul_assoc]
