...
lemma bracket_self [CharZero 𝕜] [IsDomain 𝕜] [Module.IsTorsionFree 𝕜 (WittAlgebra 𝕜)] (X : WittAlgebra 𝕜) :
    bracket 𝕜 X X = 0 := by
  ...
  simpa only [OfNat.ofNat_ne_zero, false_or] using smul_eq_zero.mp aux
...
variable [CharZero 𝕜] [IsDomain 𝕜] [Module.IsTorsionFree 𝕜 (WittAlgebra 𝕜)]
