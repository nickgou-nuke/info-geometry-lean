...
theorem continuousAlgHom_map_exp
  ...
  letI : Algebra ℚ EndW := RestrictScalars.algebra ℚ ℝ EndW
  exact NormedSpace.map_exp (𝕂 := ℝ) ρ ρ.continuous D
...
theorem map_flow
    ...
    (D : EndV)
    (t : ℝ) :
    ρ (CDE.flow D t) = representedFlow ρ D t := by
  simpa [CDE.flow, representedFlow] using
...
theorem map_exp_generator
    (ρ : EndV →A[ℝ] EndW)
    (D : EndV) :
    ρ (CDE.flow D 1) = CDE.flow (ρ D) 1 := by
...
theorem representedFlow_neg_generator
  ...
  simp [representedFlow, CDE.flow_neg_generator]
...
theorem representedFlow_mul_neg
  ...
  simpa [representedFlow] using CDE.flow_mul_flow_neg (ρ D) t
...
theorem representedFlow_neg_mul
    ...
    representedFlow ρ (-D) t * representedFlow ρ D t = 1 := by
  rw [representedFlow_neg_generator]
  simpa [representedFlow] using CDE.flow_neg_mul_flow (ρ D) t
...
theorem deriv_representedFlow_at_zero
  ...
  simpa [representedFlow] using CDE.deriv_flow_at_zero (ρ D)
...
theorem map_commutator_flow
    ...
    (D E : EndV)
    (t : ℝ) :
    ρ (CDE.flow (D * E - E * D) t) =
      CDE.flow (ρ D * ρ E - ρ E * ρ D) t := by
  rw [map_flow]
...
theorem map_canonicalZorn_coordFlow
    ...
    (D : CZD.EndCZ)
    (t : ℝ) :
    ρ (CZDE.coordFlow D t) =
      CDE.flow (ρ (CZDE.coordEnd D)) t := by
  simpa [CZDE.coordFlow, representedFlow] using
    map_flow ρ (CZDE.coordEnd D) t

/-- Time-one specialization of `map_canonicalZorn_coordFlow`. -/
theorem map_canonicalZorn_coordFlow_one
    (ρ : EndV8 →A[ℝ] EndW)
    (D : CZD.EndCZ) :
    ρ (CZDE.coordFlow D 1) =
      CDE.flow (ρ (CZDE.coordEnd D)) 1 := by
...
theorem canonicalZorn_derivation_representation_packet
    ...
    (t : ℝ) :
    ρ (CZDE.coordFlow D.1 t) =
      CDE.flow (ρ (CZDE.coordEnd D.1)) t := by
