...
theorem fullPeel_eq_one_of_seven_and_six_y0
    ...
    (h6y0 : ((fullPeel g).1 (basis8 6)).y0 = false) :
    fullPeel g = 1 := by
  exact fullPeel_unipotent_eq_one
    (fullPeel_mem_unipotent_of_seven_and_six_y0 hg h7 h6y0)
...
noncomputable def quotientIntrinsicFlagEquiv_of_final_readbacks
  ...
  apply quotientIntrinsicFlagEquiv_of_fullPeel_mem
  intro g hg
  exact fullPeel_mem_unipotent_of_seven_and_six_y0 hg (h7 g hg) (h6y0 g hg)
...
theorem quotient_card_eq_189_of_final_readbacks
  ...
  let e := quotientIntrinsicFlagEquiv_of_final_readbacks h7 h6y0
  rw [Nat.card_congr e]
