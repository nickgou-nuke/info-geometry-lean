...
DEFAULT_OUT = "UNIFICATION_INDEX.md"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the unification index markdown report."
    )
    parser.add_argument(
        "--out",
        default=DEFAULT_OUT,
        help="Output markdown path. Defaults to UNIFICATION_INDEX.md at repo root.",
    )
...
def main() -> int:
    args = parse_args()
    root = repo_root()
    out = normalize_user_path(args.out, root / DEFAULT_OUT)
    write_text(out, render_md())
