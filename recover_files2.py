            ...
            for call in step["tool_calls"]:
                if call.get("name") == "view_file":
                    ...
                    if "/lean/InfoGeometry/" in abs_path and abs_path not in recovered_files:
                        # Look forward for the VIEW_FILE response
                        for j in range(i + 1, min(i + 10, len(lines))):
                            ...
                            if next_step.get("type") == "VIEW_FILE":
                                ...
                                # Check if it's the right file by matching File Path in output
                                ...
                                for line in lines_with_nums:
                                    ...
                                    if match:
                                        ...
                                        # Keep empty lines or lines without numbers
                                        if not line.startswith("The following code") and line.strip() != "":
                                            recovered_content.append(line)
                                            
                                # Some lines might be truncated? No, transcript_full is untruncated.
