...
/-!
# Personal sandbox copy of the Fano-plane paravector octonion data.

This is a WORKING COPY of `InfoGeometry.Clifford.FanoOctonionParavector`
kept in `sandbox/pi_fano/` so the main library files are never touched.
The module name is `PiFanoOctonionParavector` to avoid any clash with the
original.

The original file already proves `dot7_symm`, `fanoCross_self`,
`fanoCross_anticomm`, the basis products `e₀×e₁=e₂`, and the paravector
multiplication facts. The open problems (alternativity, Lagrange identity,
norm multiplicativity, G₂) are solved in `sandbox/quarantine/FanoContractionSandbox.lean`.
-/

noncomputable section

namespace PiFanoOctonionParavector
