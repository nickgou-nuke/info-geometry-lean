...
def rank_admissibility_prechecks(
    ...
    for plan in declaration_plans:
        ...
        region_ok = (
            ...
            or replacement_region == "unknown"
            or candidate_region == replacement_region
        )
        checks.append(
            {
        ...
        checks.append(
            {
                ...
                "passed": overlap_ok,
                "evidence": (
                    f"shapeOverlap={overlap:.4f}" if isinstance(overlap, float) else "shapeOverlap unavailable"
                ),
            }
        )
        if overlap is None:
        ...
        payload: JsonObj = {
            ...
            "confidenceProvenance": [
                ...
                },
                {
                    ...
                    "reliability": 0.78,
                    "evidence": (
                        f"shape overlap={overlap:.4f}" if isinstance(overlap, float) else "shape overlap unavailable"
                    ),
                },
                {
        ...
        ranked.append(RankedEntry(key=f"{candidate}->{replacement_decl or 'none'}", score=score, confidence=confidence, payload=payload))
