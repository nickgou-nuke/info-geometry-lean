...
lake build InfoGeometry.OperatorAlgebra.OperatorZornTwinFormalism

log=$(mktemp)
trap 'rm -f "$log"' EXIT
lake env lean lean/InfoGeometry/OperatorAlgebra/OperatorZornTwinAudit.lean | tee "$log"
...
audit = Path(
    'lean/InfoGeometry/OperatorAlgebra/OperatorZornTwinAudit.lean'
).read_text()
