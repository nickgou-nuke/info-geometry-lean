...
theorem cartanInvolution_of_isKreinSkew (T : EndH) (hT : IsKreinSkew J T) :
...
theorem compactPart_cartan_eigenvalue (T : EndH) :
...
theorem noncompactPart_cartan_eigenvalue (T : EndH) :
...
/-- 
  ...
  its compact Cartan component is strictly skew-adjoint on the positive Hilbert space:
    (T_𝔨)† = - T_𝔨
-/
theorem compactPart_is_hilbert_skew (T : EndH) (hT : IsKreinSkew J T) :
...
/-- 
  ...
  self-adjoint on the positive Hilbert space:
    (T_𝔭)† = T_𝔭
-/
theorem noncompactPart_is_hilbert_self_adjoint (T : EndH) (hT : IsKreinSkew J T) :
