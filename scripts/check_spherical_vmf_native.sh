...
owners=(
  lean/InfoGeometry/LLM/SphericalVMFExponentialFamily.lean
  lean/InfoGeometry/LLM/SphericalVMFFieldDerivatives.lean
  lean/InfoGeometry/LLM/ContinuousMeanFieldClosure.lean
)

if grep -nE '\b(sorry|admit)\b' "${owners[@]}"; then
  echo 'ERROR: forbidden proof placeholder in spherical vMF theorem owners.' >&2
  exit 1
fi
...
for name in sys.argv[1:]:
    ...
    for line_no, line in enumerate(text.splitlines(), 1):
        if re.match(r"^\s*(axiom|constant)\b", line):
            bad.append(f"{name}:{line_no}:{line.strip()}")
if bad:
    raise SystemExit("Custom primitive declarations found:\n" + "\n".join(bad))
PY
...
lake env lean lean/InfoGeometry/LLM/SphericalVMFNativeAudit.lean | tee "$log"
...
audit = Path('lean/InfoGeometry/LLM/SphericalVMFNativeAudit.lean').read_text()
