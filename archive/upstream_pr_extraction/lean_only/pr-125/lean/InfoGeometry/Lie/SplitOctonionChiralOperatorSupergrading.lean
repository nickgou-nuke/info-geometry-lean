...
theorem eigenstate_leftRegular_eigenoperator
    ...
    (x : CZ) (c : ℝ) (hx : D x = c • x) :
    ⁅D, leftRegular x⁆ = c • leftRegular x := by
  rw [derivation_lie_leftRegular D hD x, hx]
  apply LinearMap.ext
  intro y
  change (c • x) * y = c • (x * y)
  exact zMul_smul_left c x y
...
theorem axialCartan_circular_rootPlus_eigenoperator
  ...
  exact axialCartan_upper_eigenoperator k hk i
...
theorem axialCartan_circular_rootMinus_eigenoperator
  ...
  exact axialCartan_lower_eigenoperator k hk i
...
theorem axialCartan_zornPlus_operator_grade_zero
  ...
  intro y
  change (0 : CZ) * y = 0
  apply ZornMatrix.ext <;> simp [ZornMatrix.mul, dot, cross]
