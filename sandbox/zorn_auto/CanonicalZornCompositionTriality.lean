import CanonicalZornFiveGradedClosure
...
/-! ## Interface with the concrete five-grading and projective null cone -/
...
theorem vectorGradePlus_mem (V : Vector8) :
    vectorGradePlus V ∈ conformalGrade TKKJordanPairData.TKKGrade.p1 := by
...
theorem spinorMinusGradeMinus_mem (C : SpinorMinus8) :
    spinorMinusGradeMinus C ∈
      conformalGrade TKKJordanPairData.TKKGrade.m1 := by
...
def realVector8 (X : ZornCore.Zorn) : Vector8 :=
  ⟨CanonicalZornProjectiveTKKBridge.coreToCanonical X⟩

/-- Capstone connecting the genuine three-carrier composition triality to the
existing five-graded matrix closure and the `(5,5)` projective null lift.
...
theorem composition_triality_five_grade_projective_bridge
    ...
    (cliffordMinus (realVector8 X)
      (cliffordPlus (realVector8 X) S)).val =
        zornSmul (zornNorm
          (CanonicalZornProjectiveTKKBridge.coreToCanonical X)) S.val := by
