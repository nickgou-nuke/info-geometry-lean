...
- For a non-associative algebra $(A, \cdot)$ and derivation Lie algebra $\mathfrak{g} = \operatorname{Der}(A)$:
  ...
  - Preservation of subalgebras: derivations in $\operatorname{Stab}(u)$ annihilate powers $u^2$.
...
theorem jointPeirce0_annihilated_by_sum (e1 e2 : J) (x : J) (hx : x ∈ Jdata.jointPeirce0 e1 e2) :
  ...
  have h1' : Jdata.mulMap e1 x = 0 := by simpa [mul] using h1
  have h2' : Jdata.mulMap e2 x = 0 := by simpa [mul] using h2
  dsimp [mul]
  calc
    Jdata.mulMap (e1 + e2) x = Jdata.mulMap e1 x + Jdata.mulMap e2 x := by
      simp only [map_add, LinearMap.add_apply]
    _ = 0 + 0 := by rw [h1', h2']
    _ = 0 := add_zero 0
...
/-! =========================================================================
    4. Dynkin Node Peeling via Derivation Lie Subalgebras
    ========================================================================= -/
...
theorem derivation_bracket_is_derivation
  ...
  rw [h2 x y, h1 x y]
  rw [map_add D1, map_add D2]
  rw [h1 (D2 x) y, h1 x (D2 y), h2 (D1 x) y, h2 x (D1 y)]
  abel
...
theorem stabilizer_bracket_closed (u : A) (D1 D2 : A →ₗ[R] A)
    (h1 : D1 ∈ StabilizerSubmodule u) (h2 : D2 ∈ StabilizerSubmodule u) :
    derivationBracket D1 D2 ∈ StabilizerSubmodule u := by
  change D1 u = 0 at h1
  change D2 u = 0 at h2
  change D1 (D2 u) - D2 (D1 u) = 0
...
theorem stabilizer_preserves_square
    ...
    (hder : IsNonAssocDerivation mul D) (hstab : D ∈ StabilizerSubmodule u) :
    D (mul u u) = 0 := by
  have hd := hder u u
