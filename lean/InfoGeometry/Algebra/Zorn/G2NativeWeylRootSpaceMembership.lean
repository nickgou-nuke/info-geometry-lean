...
theorem conjugateCanonical_rootDerivation_eq_rootCoefficient_smul_of_weight
    (i j : nonzeroIndex)
    (e : InfoGeometry.Lie.CanonicalZornCartanAdjointRootDecomposition.Der ≃ₗ[ℝ]
      InfoGeometry.Lie.CanonicalZornCartanAdjointRootDecomposition.Der)
    (c : axialCartanLieSubalgebra ≃ₗ[ℝ] axialCartanLieSubalgebra)
    ...
    (hweight : ∀ H : axialCartanLieSubalgebra,
      nativeRootWeight i (c.symm H) = nativeRootWeight j H) :
    e (rootDerivation i.1) =
      rootCoefficient j (e (rootDerivation i.1)) • rootDerivation j.1 := by
  exact eq_rootCoefficient_smul_of_mem_rootSpace j
    (e (rootDerivation i.1))
    (conjugateCanonical_rootDerivation_mem_of_weight i j e c hbracket hcartan hweight)

theorem conjugateCanonical_rootDerivation_rootCoefficient_ne_zero_of_weight
    (i j : nonzeroIndex)
    (e : InfoGeometry.Lie.CanonicalZornCartanAdjointRootDecomposition.Der ≃ₗ[ℝ]
      InfoGeometry.Lie.CanonicalZornCartanAdjointRootDecomposition.Der)
    (c : axialCartanLieSubalgebra ≃ₗ[ℝ] axialCartanLieSubalgebra)
    ...
    (hweight : ∀ H : axialCartanLieSubalgebra,
      nativeRootWeight i (c.symm H) = nativeRootWeight j H) :
    rootCoefficient j (e (rootDerivation i.1)) ≠ 0 := by
  apply rootCoefficient_ne_zero_of_ne_zero_of_mem_rootSpace j
    (e (rootDerivation i.1))
    (conjugateCanonical_rootDerivation_mem_of_weight i j e c hbracket hcartan hweight)
    (by
      intro hzero
      apply rootDerivation_ne_zero i.1
