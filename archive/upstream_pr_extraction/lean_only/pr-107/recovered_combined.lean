...
-/
def fiveGradedHomologicalPipeline (G : Type*) (bracket : G → G → G) :via the square-root embedding `pᵢ = xᵢ²` (see §1 `squareRootEmbedding`).
-/
...
theorem cycleSpectrumPacket_spectralVal_eq
    (pkt : CycleVolumeSpectrumPacket) (α : pkt.HomologyClass) :chain explicitly in the bridge architecture.
-/
