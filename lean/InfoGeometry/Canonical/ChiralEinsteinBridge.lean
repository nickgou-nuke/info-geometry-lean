...
noncomputable def inverseEpsilonSource (a_mp a_d : R) : ℝ → ℝ :=
  fun _ => epsilon a_mp a_d
...
theorem anomalyDriven_fixedpoint_eq_inverseEpsilon
    ...
    (hFlow : SatisfiesAnomalyDrivenKaehlerRicciFlow flow
      (inverseEpsilonSource a_mp a_d))
    (hFixed : ∀ s, scalarRicciBetaFunction (E := E) flow s = 0) :
    ∀ s, flow s = epsilon a_mp a_d := by
  intro s
  simpa [inverseEpsilonSource] using
    (anomalyDriven_fixedpoint_tracks_source
      (E := E) (flow := flow) (A := inverseEpsilonSource a_mp a_d) hFlow hFixed s)
