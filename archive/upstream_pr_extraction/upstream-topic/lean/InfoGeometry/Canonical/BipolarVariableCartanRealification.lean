...
/-- Restriction of scalars does not change the value of the connection. -/
theorem realVariableCartanConnection_eq_complex
    (s v : ℂ) :
    realVariableCartanConnection s v =
...
theorem bipolar_variable_cartan_realification_packet (s : ℂ) :
    (∀ v, realVariableCartanConnection s v =
      ...
      realVariableCartanCurvatureForm s = 0 := by
  exact ⟨realVariableCartanConnection_eq_complex s,
