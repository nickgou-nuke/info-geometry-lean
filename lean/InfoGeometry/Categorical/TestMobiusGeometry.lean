...
theorem pgl_equivalence (M : MobiusTransform) (lam : ℂ) (hlam : lam ≠ 0) :
  ...
  | some z' =>
    ...
    · have hdenom_scaled : lam * M.c * z' + lam * M.d = 0 := by
        ...
        _ = 0 := mul_zero lam
      change (if M.c * z' + M.d = 0 then none
        else some ((M.a * z' + M.b) / (M.c * z' + M.d))) =
        (if M_scaled.c * z' + M_scaled.d = 0 then none
        else some ((M_scaled.a * z' + M_scaled.b) /
          (M_scaled.c * z' + M_scaled.d)))
    ...
    · have hdenom_scaled : lam * M.c * z' + lam * M.d ≠ 0 := by
      ...
      calc
        (M.a * z' + M.b) / (M.c * z' + M.d) =
            (lam * (M.a * z' + M.b)) / (lam * (M.c * z' + M.d)) := by
              rw [mul_div_mul_left _ _ hlam]
        _ = (lam * M.a * z' + lam * M.b) /
            (lam * M.c * z' + lam * M.d) := by ring
