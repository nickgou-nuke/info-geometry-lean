import InfoGeometry.Canonical.ConformalProjectiveClosure
import InfoGeometry.Algebra.FiniteSpinAlgebra
import InfoGeometry.Canonical.NormalOrderedCurrent
import InfoGeometry.Canonical.BosonizationConstructiveCurrent
import InfoGeometry.Canonical.BosonizationTheorem

/-!
# InfoGeometry.Canonical.ConformalEngine

Import-only canonical conformal engine surface.

This file is declaration-free. It exists to gather the direct canonical owner
surfaces for:

* inversion;
* Möbius Jacobian;
* five-grade sector swap;
* boundary current readout;
* Majorana PHS zero-mode surface;
* bracket compatibility;
* projective closure.
-/
