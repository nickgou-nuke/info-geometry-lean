...
instance np_jones_basic_construction_blocks_pair_groupoid_fintype
...
def np_jones_basic_construction_blocks_multiplicity
    (fold : Ω → X) (x : X) : ℕ :=
  Fintype.card (foldFiber fold x)

end

theorem np_jones_basic_construction_blocks_certified :
        ...
        ∀ x : X,
          Nonempty (pairGroupoidArrow fold x ≃ foldFiber fold x × foldFiber fold x) ∧
            Fintype.card (pairGroupoidArrow fold x) =
              np_jones_basic_construction_blocks_multiplicity fold x ^ 2 := by
  intro Ω X _ _ _ _ fold
  refine ⟨paper_op_algebra_fold_basic_construction_pair_groupoid fold, ?_⟩
  intro x
  refine ⟨⟨Equiv.refl _⟩, ?_⟩
  simp [pairGroupoidArrow, np_jones_basic_construction_blocks_multiplicity,
    pow_two]
