...
namespace InfoGeometry.Algebra.Zorn.G2ResidualPairCoordinateCellFour
...
def residualPair (i : Fin 189) : ResidualPair :=
  (factorWordExponent (gapLeftWitness 4 i),
    factorWordExponent (gapRightWitness 4 i))

set_option maxRecDepth 100000 in
set_option maxHeartbeats 0 in
theorem residualPair_injective_on_cell_four :
    ∀ i j : Fin 189,
...
theorem residualPair_cell_four_coordinate_separation :
    ∀ i j : Fin 189,
      i ∈ orbitCells 4 → j ∈ orbitCells 4 → i ≠ j →
      residualPair i ≠ residualPair j := by
  intro i j hi hj hne heq
  exact hne (residualPair_injective_on_cell_four i j hi hj heq)

end InfoGeometry.Algebra.Zorn.G2ResidualPairCoordinateCellFour
