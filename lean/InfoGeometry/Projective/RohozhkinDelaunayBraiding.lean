...
theorem appendixPentagonWord_matrix_eq_one
    ...
    (h_jm : zj - zm ≠ 0)
    (h_jl : zj - zl ≠ 0) :
    Delaunay.rohozhkinMatrix (appendixPentagonWord zi zj zk zl zm) =
...
theorem one_cycle_scramble_identity
    ...
    trajectoryMatrix (moving := 1)
      (appendixPentagonWord zi zj zk zl zm) =
      (1 : Matrix (Fin (rohozhkinDim 1)) (Fin (rohozhkinDim 1)) ℚ) := by
  exact appendixPentagonWord_matrix_eq_one zi zj zk zl zm h_il h_ik h_km h_jm h_jl
...
theorem appendix_pentagon_matrix_identity_readout
  ...
  appendixPentagonWord_matrix_eq_one zi zj zk zl zm h_il h_ik h_km h_jm h_jl
