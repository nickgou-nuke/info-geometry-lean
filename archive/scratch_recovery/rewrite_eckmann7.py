...
lemma ker_mul_transpose_eq_ker_transpose {m n : ℕ} (A : Matrix (Fin m) (Fin n) ℚ) (x : Fin m → ℚ) :
  ...
  · intro h
    ...
    have h_mul_A : (A * Aᵀ).mulVec x = A.mulVec (Aᵀ.mulVec x) := (Matrix.mulVec_mulVec x A Aᵀ).symm
    rw [h_mul_A] at h_dot
...
lemma ker_sq_eq_ker_psd {m : ℕ} (C : Matrix (Fin m) (Fin m) ℚ) (h_sym : Cᵀ = C)
  ...
  · intro h
    ...
    have h_mul_CC : (C * C).mulVec x = C.mulVec (C.mulVec x) := (Matrix.mulVec_mulVec x C C).symm
...
lemma im_sq_eq_im_AAtranspose {m n : ℕ} (A : Matrix (Fin m) (Fin n) ℚ) :
  ...
  have h_sub : LinearMap.range (C * C).mulVecLin ≤ LinearMap.range C.mulVecLin := by
    ...
    have h_mul_CC : C.mulVec (C.mulVec x) = (C * C).mulVec x := Matrix.mulVec_mulVec x C C
...
lemma laplacian_rank_eq_sum {n₀ n₁ n₂ : ℕ}
  ...
  have h_inter : LinearMap.range C.mulVecLin ⊓ LinearMap.range D.mulVecLin = ⊥ := by
    ...
    have h_Dv : D.mulVec v = 0 := by
      ...
      have h_mul_DC : D.mulVec (C.mulVec x) = (D * C).mulVec x := Matrix.mulVec_mulVec x D C
      rw [h_mul, h_mul_DC, h_DC, Matrix.zero_mulVec]
    have h_ker : (D * D).mulVec y = 0 := by
      simp only [Matrix.mulVecLin_apply] at hy
      have h_mul_DD : (D * D).mulVec y = D.mulVec (D.mulVec y) := (Matrix.mulVec_mulVec y D D).symm
      rw [h_mul_DD, hy, h_Dv]
  ...
  have h_imC_sub : LinearMap.range C.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    have step2 : C.mulVec (C.mulVec y) = (C * C).mulVec y := Matrix.mulVec_mulVec y C C
    have step3 : D.mulVec (C.mulVec y) = (D * C).mulVec y := Matrix.mulVec_mulVec y D C
    simp only [Matrix.mulVecLin_apply] at hy ⊢
  ...
  have h_imD_sub : LinearMap.range D.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    have step2 : C.mulVec (D.mulVec z) = (C * D).mulVec z := Matrix.mulVec_mulVec z C D
    have step3 : D.mulVec (D.mulVec z) = (D * D).mulVec z := Matrix.mulVec_mulVec z D D
  ...
  have h_rank_add : (C + D).rank = C.rank + D.rank := by
    ...
    have hbot : Module.finrank ℚ (⊥ : Submodule ℚ (Fin n₁ → ℚ)) = 0 := finrank_bot ℚ (Fin n₁ → ℚ)
...
theorem eckmann_hodge_nullity {n₀ n₁ n₂ : ℕ}
  ...
  have h_rk := LinearMap.finrank_range_add_finrank_ker ((del1 * del1ᵀ + del2ᵀ * del2 : Matrix (Fin n₁) (Fin n₁) ℚ)).mulVecLin
  change (del1 * del1ᵀ + del2ᵀ * del2).rank + Module.finrank ℚ (LinearMap.ker ((del1 * del1ᵀ + del2ᵀ * del2)).mulVecLin) = Module.finrank ℚ (Fin n₁ → ℚ) at h_rk
  have h_dom : Module.finrank ℚ (Fin n₁ → ℚ) = Fintype.card (Fin n₁) := by simp
  have hn : Fintype.card (Fin n₁) = n₁ := Fintype.card_fin n₁
  rw [h_dom, hn] at h_rk
  have h_rank_Δ₁ : (del1 * del1ᵀ + del2ᵀ * del2).rank = del1.rank + del2.rank := laplacian_rank_eq_sum del1 del2 h_boundary
  rw [h_rank_Δ₁] at h_rk
  omega
