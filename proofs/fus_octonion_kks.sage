...
def mobius_inv(even,odd): return ((even+odd)/Q(2),(even-odd)/Q(2))
...
assert assoc_alt==[0]*8 and assoc_nonzero!=[0]*8 and comm_skew==[0]*8 and inner_inv==0
assert kks(3,5)==-15
...
assert stiff(C1(m))-m*m==0
assert 24+1+10+10==45
assert binomial(5,0)+binomial(5,2)+binomial(5,4)==16
assert binomial(5,1)+binomial(5,3)+binomial(5,5)==16
