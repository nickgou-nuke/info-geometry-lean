...
def mixedPairing : LinearMap.BilinForm ℝ Boundary55 :=
  LinearMap.mk₂ ℝ
    ...
    (by intro c m n; dsimp; simp_rw [mul_assoc, ← Finset.mul_sum]; ring)
    (by intros; simp [mul_add, Finset.sum_add_distrib] <;> ring)
    (by intro c m n; dsimp; simp_rw [mul_left_comm, ← Finset.mul_sum]; ring)
...
theorem boundaryMatrixAction_anticommutator (z w : Boundary55) :
  ...
  unfold boundaryMatrixAction
  rw [← map_mul boundaryCliffordMatrixEquiv,
      ← map_mul boundaryCliffordMatrixEquiv,
      ← map_add boundaryCliffordMatrixEquiv,
...
theorem boundary_scalar_nilpotents :
  ...
  constructor
  · rw [boundaryMatrixAction_sq, alphaRay_null, map_zero]
  · rw [boundaryMatrixAction_sq, betaRay_null, map_zero]
