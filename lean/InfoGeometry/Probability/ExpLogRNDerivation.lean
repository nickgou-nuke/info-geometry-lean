...
theorem derivation_one (D : A → A) (hD : IsDerivation D) : D 1 = 0 := by
  ...
  have hsub : D 1 - D 1 = (D 1 + D 1) - D 1 := congrArg (fun x => x - D 1) hmul
  rw [sub_self, add_sub_cancel_right] at hsub
  exact hsub.symm
...
theorem derivation_inv (D : A → A) (hD : IsDerivation D) (u : Aˣ) :
  ...
  rw [hD.2] at h_prod
  have h_shift : (u : A) * D (↑u⁻¹ : A) = - D (u : A) * (↑u⁻¹ : A) := by
...
theorem dlog_mul (D : A → A) (hD : IsDerivation D) (u v : Aˣ) :
  ...
  rw [h_inv, hD.2 (u : A) (v : A), mul_add]
  have h_left : (↑u⁻¹ : A) * (↑v⁻¹ : A) * (D (u : A) * (v : A)) = (↑u⁻¹ : A) * D (u : A) := by
    calc
      (↑u⁻¹ : A) * (↑v⁻¹ : A) * (D (u : A) * (v : A))
        = ((↑u⁻¹ : A) * D (u : A)) * ((↑v⁻¹ : A) * (v : A)) := by ring
      _ = ((↑u⁻¹ : A) * D (u : A)) * 1 := by rw [Units.inv_mul v]
      _ = (↑u⁻¹ : A) * D (u : A) := by rw [mul_one]
  have h_right : (↑u⁻¹ : A) * (↑v⁻¹ : A) * ((u : A) * D (v : A)) = (↑v⁻¹ : A) * D (v : A) := by
    calc
      (↑u⁻¹ : A) * (↑v⁻¹ : A) * ((u : A) * D (v : A))
        = ((↑v⁻¹ : A) * D (v : A)) * ((↑u⁻¹ : A) * (u : A)) := by ring
      _ = ((↑v⁻¹ : A) * D (v : A)) * 1 := by rw [Units.inv_mul u]
