...
-/
structure CliffordCPTBasis (M : ModularSignCPTDatum H) where

namespace CliffordCPTBasis

def scalarUnit (_ : CliffordCPTBasis M) : EndR H := 1

def modularParity (_ : CliffordCPTBasis M) : EndR H := M.eps

def modularReflection (_ : CliffordCPTBasis M) : EndR H := M.J

def hestenesPhase (_ : CliffordCPTBasis M) : EndR H := M.Kmod

end CliffordCPTBasis
