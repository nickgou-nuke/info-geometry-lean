...
lemma Z_add_constant (E : Ω → ℝ) (c ε : ℝ) :
    Z (fun ω => E ω + c) ε = Real.exp (-c / ε) * Z E ε := by
  unfold Z weight
  calc
    ∑ ω, Real.exp (-(E ω + c) / ε) =
        ∑ ω, Real.exp (-c / ε) * Real.exp (-E ω / ε) := by
          apply Finset.sum_congr rfl
          intro ω hω
          rw [← Real.exp_add]
          congr 1
          ring
    _ = Real.exp (-c / ε) * ∑ ω, Real.exp (-E ω / ε) := by
      rw [Finset.mul_sum]

lemma freeEnergy_add_constant (E : Ω → ℝ) (c ε : ℝ) (hε : ε ≠ 0) :
    freeEnergy (fun ω => E ω + c) ε = freeEnergy E ε + c := by
  unfold freeEnergy logZ
  rw [Z_add_constant]
  rw [Real.log_mul (Real.exp_ne_zero _) (Z_ne_zero E ε)]
  rw [Real.log_exp]
  field_simp [hε]
  ring

lemma gibbsProb_eq_exp_sub_logZ (E : Ω → ℝ) (ε : ℝ) (ω : Ω) :
    gibbsProb E ε ω = Real.exp (-(E ω) / ε - logZ E ε) := by
  unfold gibbsProb logZ weight
  rw [Real.exp_sub, Real.exp_log (Z_pos E ε)]

lemma gibbsProb_add_constant_invariant (E : Ω → ℝ) (c ε : ℝ)
    (hε : ε ≠ 0) (ω : Ω) :
    gibbsProb (fun ω => E ω + c) ε ω = gibbsProb E ε ω := by
  unfold gibbsProb weight
  have h_exp : Real.exp (-(E ω + c) / ε) =
      Real.exp (-c / ε) * Real.exp (-E ω / ε) := by
    rw [← Real.exp_add]
    congr 1
    field_simp [hε]
    ring
...
lemma internalEnergy_add_constant (E : Ω → ℝ) (c ε : ℝ) (hε : ε ≠ 0) :
  ...
  simp_rw [mul_add]
  rw [Finset.sum_add_distrib, ← Finset.sum_mul, gibbsProb_sum_one]
  ring

lemma shannonEntropy_add_constant (E : Ω → ℝ) (c ε : ℝ) (hε : ε ≠ 0) :
