...
structure LiouvilleGrading (C : CuntzMultiplicativeIndexing Op) where
  ...
  action_on_generators : ∀ n : ℕ+,
    Γ (C.generator n) = liouville n • C.generator n
...
theorem liouville_commutes_with_modular_flow
  ...
  simp [F.scaling, L.action_on_generators]
...
theorem liouville_modular_comm_explicit
  ...
  simp [F.scaling, L.action_on_generators, modularPhase]
  rw [smul_smul]
...
theorem witten_index_invariant_under_flow
    ...
    (t : ℝ) (N : ℕ+) (β : ℝ) :
    ∑ n ∈ Finset.Icc 1 (N : ℕ),
      LiouvilleFunc n * Complex.re (modularPhase t (if h : n = 0 then 1 else ⟨n, Nat.pos_of_ne_zero h⟩)) * (n : ℝ)^(-β) =
    wittenIndexPartial N β * Complex.re (modularPhase t 1) := by
  -- Since modularPhase t 1 = 1, this is trivial for now.
  sorry
