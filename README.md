...
| `InfoGeometry/Canonical/EmergentGravity.lean` | Formalizes the self-consistency loop of Emergent Gravitational Dynamics from macroscopic Spinor Condensates. | Constructs the exact mappings from Clifford ideals to emergent Vielbeins, Metrics, and Biquaternion Torsion. | Mathematically enforces the Einstein-Cartan 'bootstrap' mechanism where quantum geometry and spacetime mutually determine each other. |
| `InfoGeometry/Canonical/StressEnergyTensor.lean` | Formalizes the Belinfante-Rosenfeld symmetric stress-energy tensor over emergent spinor geometry. | Mathematically constructs the tensor variation components $B_{\mu\nu}$ and $C_{\mu\nu}$ and proves strict macroscopic symmetry $T_{\mu\nu} = T_{\nu\mu}$. | Completes the dynamical effective action variation loops in Einstein-Cartan 'bootstrap'. |
