...
@[simp] theorem canonicalCl11Generator_eq_spectral_epsilon :
    canonicalCl11Generator (E := E) = spectral_epsilon (E := E) := by
  rfl
...
theorem expFlow_modular_j_eq_sigmaMap
    (t : ℝ) :
    ((expFlow (modular_j (E := E)) t : Xc ≃L[ℝ] Xc) : Xc →L[ℝ] Xc) =
      sigmaMap (modular_j (E := E)) t := by
  simpa [InfoGeometry.Canonical.TomitaTakesaki.modularConjugationJ_eq_modular_j] using
    expFlow_modularConjugationJ_eq_sigmaMap (E := E) t
...
theorem berezinian_eq_diagonal_ratio_of_anomaly_free
  ...
  exact (latticeAnomalyCommutator_eq_zero_iff_blocks_zero U).mp hAnomaly
...
/-- The block Berezinian of the explicit hyperbolic shadow is the thermal
scalar readout used above. -/
theorem berezinian_sigmaMatrix_eq_thermalBerezinianEval (t : ℝ) :
  ...
  rw [blockA_sigmaMatrix, blockB_sigmaMatrix,
    blockC_sigmaMatrix, blockD_sigmaMatrix]
  simp [scalarBlock]
...
theorem log_berezinian_sigmaMatrix (t : ℝ) :
  ...
  rw [Real.log_rpow (Real.cosh_pos t).le]
  ring
...
theorem hasDerivAt_log_berezinian_sigmaMatrix (t : ℝ) :
  ...
  convert hcomp.const_mul (-2 * (N : ℝ)) using 1
  · rfl
  · rw [Real.tanh_eq_sinh_div_cosh]
    field_simp [ne_of_gt (Real.cosh_pos t)]
    ring
...
theorem latticeAvatar_modular_j :
    latticeAvatar (N := N) (modular_j (E := FinModel (N := N))) =
      Matrix.toEuclideanLin (jMatrix (N := N)) := by
  simpa [InfoGeometry.Canonical.TomitaTakesaki.modularConjugationJ_eq_modular_j] using
...
theorem latticeAvatar_sigmaMap_modular_j
    (t : ℝ) :
    latticeAvatar (N := N)
      (Cl11Shadow.sigmaMap (modular_j (E := FinModel (N := N))) t) =
      Matrix.toEuclideanLin (Lattice.sigmaMatrix (N := N) t) := by
  simpa [InfoGeometry.Canonical.TomitaTakesaki.modularConjugationJ_eq_modular_j] using
    latticeAvatar_sigmaMap_modularConjugationJ (N := N) t
...
theorem latticeAvatar_exp_modular_j
    (t : ℝ) :
    latticeAvatar (N := N)
      (NormedSpace.exp (t • modular_j (E := FinModel (N := N)))) =
      Matrix.toEuclideanLin (Lattice.sigmaMatrix (N := N) t) := by
  simpa [InfoGeometry.Canonical.TomitaTakesaki.modularConjugationJ_eq_modular_j] using
    latticeAvatar_exp_modularConjugationJ (N := N) t
