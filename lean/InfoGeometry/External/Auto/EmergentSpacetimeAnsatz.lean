...
def determinant_is_minkowski_norm (X : Matrix (Fin 2) (Fin 2) ℂ) (hx : is_hermitian X) : Prop :=
  X.det = 0

/-- Rigorous null-cone predicate. -/
def null_cone_locus (X : Matrix (Fin 2) (Fin 2) ℂ) (hx : is_hermitian X) : Prop :=
  X.det = 0

/-- Spacetime emergence ansatz kept explicit as a proposition for later specialization. -/
def emergent_spacetime_ansatz : Prop := True
