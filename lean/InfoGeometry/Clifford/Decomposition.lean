...
noncomputable def θ (X : Matn (n := n)) : Matn (n := n) := - (J (J1 := J1) (n := n) * Xᵀ * J (J1 := J1) (n := n))

noncomputable def θₗ : Matn (n := n) →ₗ[ℝ] Matn (n := n) :=
{ toFun := θ (J1 := J1) (n := n),
  map_add' := by intro X Y; dsimp [θ]; simp only [Matrix.transpose_add, Matrix.mul_add, Matrix.add_mul, neg_add],
  map_smul' := by intro r X; dsimp [θ]; simp only [Matrix.transpose_smul, Matrix.mul_smul, Matrix.smul_mul, smul_neg] }

noncomputable def kSub : Submodule ℝ (Matn (n := n)) := LinearMap.ker (θₗ (J1 := J1) (n := n) - LinearMap.id)
noncomputable def pSub : Submodule ℝ (Matn (n := n)) := LinearMap.ker (θₗ (J1 := J1) (n := n) + LinearMap.id)

lemma mem_k_iff (X : Matn (n := n)) : X ∈ kSub (J1 := J1) (n := n) ↔ θ (J1 := J1) (n := n) X = X := by
  simp [kSub, θₗ, sub_eq_zero, LinearMap.sub_apply]
lemma mem_p_iff (X : Matn (n := n)) : X ∈ pSub (J1 := J1) (n := n) ↔ θ (J1 := J1) (n := n) X = -X := by
...
lemma φ_mul_rev (hJ1_sq : J1 * J1 = 1) (X Y : Matn (n := n)) :
  ...
  have hJJ : J (J1 := J1) (n := n) * J (J1 := J1) (n := n) = (1 : Matn (n := n)) := TowerMatrix.Jn_sq J1 hJ1_sq n
  calc
    φ (J1 := J1) (n := n) (X * Y)
        = J (J1 := J1) (n := n) * (X * Y)ᵀ * J (J1 := J1) (n := n) := by rfl
    _   = J (J1 := J1) (n := n) * (Yᵀ * Xᵀ) * J (J1 := J1) (n := n) := by simp only [Matrix.transpose_mul, Matrix.mul_assoc]
    _   = J (J1 := J1) (n := n) * Yᵀ * 1 * Xᵀ * J (J1 := J1) (n := n) := by simp only [Matrix.mul_one, Matrix.mul_assoc]
    _   = (J (J1 := J1) (n := n) * Yᵀ * J (J1 := J1) (n := n)) * (J (J1 := J1) (n := n) * Xᵀ * J (J1 := J1) (n := n)) := by
