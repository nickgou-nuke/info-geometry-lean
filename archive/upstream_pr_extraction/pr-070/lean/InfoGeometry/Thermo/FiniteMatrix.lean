...
/-! ## The infinitesimal modular generator -/

/-- The inner commutator derivation generated by a finite matrix `H`. -/
def adH (H : Op n) : Op n →ₗ[ℝ] Op n :=
  LinearMap.mulLeft ℝ H - LinearMap.mulRight ℝ H

@[simp] theorem adH_apply (H X : Op n) :
    adH H X = H * X - X * H := by
  rfl

/-- The commutator generator satisfies the Leibniz rule. -/
theorem adH_leibniz (H X Y : Op n) :
    adH H (X * Y) = adH H X * Y + X * adH H Y := by
  simp only [adH_apply]
