...
theorem skewFromPairs_add (b c : AxisPair → ℂ) :
    skewFromPairs (b + c) = skewFromPairs b + skewFromPairs c := by
  ext i j
  rcases lt_trichotomy i j with hij | rfl | hji
  · simp [skewFromPairs, hij, not_lt_of_ge (le_of_lt hij)]
  · simp [skewFromPairs]
  · simp [skewFromPairs, hji, not_lt_of_ge (le_of_lt hji)]
    abel
...
theorem encodeMatrix_isSplitOrthogonal (c : Coordinates) :
  ...
  · simpa [wittAdjoint, encodeMatrix, flipIndex] using
      (skewFromPairs_swap c.2.1 i j)
  · simpa [wittAdjoint, encodeMatrix, flipIndex] using
      (skewFromPairs_swap c.2.2 i j)
  · simp [wittAdjoint, encodeMatrix, flipIndex]
...
def encode : Coordinates →ₗ[ℂ] splitO55Lie where
  ...
  map_add' c d := by
    ...
    fin_cases sp <;> fin_cases sq
    · rfl
    · exact congr_fun (congr_fun (skewFromPairs_add c.2.1 d.2.1) i) j
    · exact congr_fun (congr_fun (skewFromPairs_add c.2.2 d.2.2) i) j
    · simp [encodeMatrix, add_comm]
  map_smul' z c := by
    apply Subtype.ext
    ext p q
    rcases p with ⟨sp, i⟩
...
@[simp] theorem upperRight_diag_zero (X : splitO55Lie) (i : Axis) :
  ...
  have h2 : (2 : ℂ) * (X : Mat10) (plus i) (minus i) = 0 := by
...
@[simp] theorem lowerLeft_diag_zero (X : splitO55Lie) (i : Axis) :
  ...
  have h2 : (2 : ℂ) * (X : Mat10) (minus i) (plus i) = 0 := by
    linear_combination h
  exact (mul_eq_zero.mp h2).resolve_left (by norm_num)
...
theorem skewFromPairs_extractB (X : splitO55Lie) (i j : Axis) :
    skewFromPairs (extractB X) i j =
      (X : Mat10) (plus i) (minus j) := by
  rcases lt_trichotomy i j with hij | hij | hij
  · simp [skewFromPairs, hij, extractB, plus, minus]
  · subst j
    simp
  · have hskew := upperRight_skew X j i
    simp [skewFromPairs, hij, not_lt_of_ge (le_of_lt hij), extractB, plus, minus]
    exact hskew.symm
...
theorem skewFromPairs_extractC (X : splitO55Lie) (i j : Axis) :
    skewFromPairs (extractC X) i j =
      (X : Mat10) (minus i) (plus j) := by
  rcases lt_trichotomy i j with hij | hij | hij
  · simp [skewFromPairs, hij, extractC, plus, minus]
  · subst j
    simp
  · have hskew := lowerLeft_skew X j i
    simp [skewFromPairs, hij, not_lt_of_ge (le_of_lt hij), extractC, plus, minus]
    exact hskew.symm
...
theorem extract_encode (c : Coordinates) :
  ...
  ext i j <;> simp [extract, extractA, extractB, extractC,
    encode, encodeMatrix, plus, minus, skewFromPairs_extractB,
    skewFromPairs_extractC]
...
theorem encode_extract (X : splitO55Lie) :
  ...
  · change -(X : Mat10) (plus j) (plus i) =
      (X : Mat10) (minus i) (minus j)
    simpa [extractA, plus, minus] using (lowerRight_eq_neg_transpose X i j).symm
