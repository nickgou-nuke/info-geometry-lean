...
lean_lines = [
    ...
    if not (line.startswith('warning: manifest out of date:') or
            (line.startswith('warning: ') and 'repository ' in line and 'has local changes' in line))
]
filtered_text = '\n'.join(lean_lines)
if not expected or missing or bad or re.search(r'\b(?:error|warning):', filtered_text):
