...
algebra on a block decomposition `ℂ ⊕ ℂ⁸ ⊕ ℂ`, with block weights `-1, 0, 1`.
A matrix entry has grade equal to its row weight minus its column weight, so
commutators occupy grades `-2,-1,0,1,2` and brackets outside this window vanish.

The canonical complex Zorn carrier embeds concretely into grades `+1` and `-1`.
No identification with a particular simple real Lie group is asserted here.
-/
...
theorem conformalTriality_bracket (A B : ConformalMatrix) :
    conformalTriality ⁅A, B⁆ = ⁅conformalTriality A, conformalTriality B⁆ := by
