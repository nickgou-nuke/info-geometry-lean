...
theorem peirce_coordinate_finranks :
  ...
  · simp [PeirceOneCoord]
  constructor
  · norm_num [Module.finrank_prod, zorn_finrank_eq_eight]
  · norm_num [Module.finrank_prod, zorn_finrank_eq_eight]
...
@[simp] theorem explicit_zero_zorn_norm :
    ZornVectorMatrix.norm
      ({ a := 0, v := fun _ : Fin 3 => 0, w := fun _ : Fin 3 => 0, b := 0 } : Zorn) = 0 := by
  simp [ZornVectorMatrix.norm, ZornVec3.dot]
...
theorem normCubic_halfCurve (lambda t : ℝ) (a c : Zorn) :
  ...
  simp [halfCurve, H3Zorn.normCubic, ZornVectorMatrix.norm_smul,
    ...
    ZornVectorMatrix.mul,
    ZornVectorMatrix.norm_zero, explicit_zero_zorn_norm,
    ZornVec3.dot, Fin.sum_univ_three,
