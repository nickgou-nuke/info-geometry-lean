...
def wordS (c : InfoGeometry.Algebra.Cuntz.CuntzNAlgebra (N := n) A) : List (Fin n) → A
  | [] => 1
  | i :: w => c.S i * wordS c w

/-- Adjoint of monomial isometry corresponding to a word w : List (Fin n) -/
def wordSStar (c : InfoGeometry.Algebra.Cuntz.CuntzNAlgebra (N := n) A) : List (Fin n) → A
  | [] => 1
  | i :: w => wordSStar c w * star (c.S i)

/-- Multiplicative homomorphism for concatenated words: S_(w ++ v) = S_w * S_v -/
theorem wordS_append (c : InfoGeometry.Algebra.Cuntz.CuntzNAlgebra (N := n) A) (w v : List (Fin n)) :
...
theorem word_reduction_head_match (c : InfoGeometry.Algebra.Cuntz.CuntzNAlgebra (N := n) A) (i : Fin n) (w1 w2 : List (Fin n)) :
...
theorem word_reduction_head_mismatch (c : InfoGeometry.Algebra.Cuntz.CuntzNAlgebra (N := n) A) {i j : Fin n} (h : i ≠ j)
    (w1 w2 : List (Fin n)) :
...
theorem word_reduction_prefix_right (c : InfoGeometry.Algebra.Cuntz.CuntzNAlgebra (N := n) A) (w v : List (Fin n)) :
...
theorem word_reduction_prefix_left (c : InfoGeometry.Algebra.Cuntz.CuntzNAlgebra (N := n) A) (w v : List (Fin n)) :
...
theorem word_reduction_exact (c : InfoGeometry.Algebra.Cuntz.CuntzNAlgebra (N := n) A) (w : List (Fin n)) :
