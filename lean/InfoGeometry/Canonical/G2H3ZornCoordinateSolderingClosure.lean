...
noncomputable def splitOctEntryEnd (D : G2Derivation) :
  ...
  map_add' X Y := by
    change fromZorn ((Dv D).toFun (toZorn (X + Y))) = _
    rw [toZorn_add_apply]
    change fromZorn ((Dv D).toFun ((toZorn X).add (toZorn Y))) = _
    rw [(Dv D).map_add, fromZorn_add_apply]
  map_smul' r X := by
    change fromZorn ((Dv D).toFun (toZorn (r • X))) = _
    rw [toZorn_smul_apply]
    change fromZorn ((Dv D).toFun (r • toZorn X)) = _
    rw [(Dv D).map_smul, fromZorn_smul_apply]
...
theorem splitOctEntryEnd_mul
  ...
  rw [toZorn_mul, (Dv D).map_mul']
  change fromZorn (_ + _) = _
  rw [fromZorn_add, fromZorn_mul, fromZorn_mul]
