...
/-- The order-two symmetry generated by the five-graded inversion. -/
noncomputable def thetaAction :
    ZMod 2 →+ Additive (Equiv.Perm L) where
  toFun a :=
    Additive.ofMul (if a = 0 then Equiv.refl L else G.thetaEquiv)
  map_zero' := by
    apply Additive.ext
    apply Equiv.ext
    intro x
  ...
  map_add' := by
    ...
    · apply Additive.ext
      ...
      simp [h, thetaEquiv, G.theta_theta]

/-- The nonzero element of the finite symmetry action is the inversion. -/
theorem thetaAction_one_apply (x : L) :
    Additive.toMul (G.thetaAction (1 : ZMod 2)) x = G.theta x := by
  change (if (1 : ZMod 2) = 0 then x else G.thetaEquiv x) = G.theta x
  simp [thetaEquiv]

/-- The same nontrivial symmetry transports the five-grade label. -/
theorem thetaAction_one_grade (x : L) :
    G.grade (Additive.toMul (G.thetaAction (1 : ZMod 2)) x) =
      ConformalGrade.swap (G.grade x) := by
  rw [thetaAction_one_apply]
  exact G.grade_swap x
