...
precise: zero cohomological determinant defect at the boundary.
-/
structure InfiniteIwasawaAnalyticityLock
  ...
  h_zero_defect : determinantDefect = 0
...
theorem det_one
    ...
    L.modularFlowDet = 1 :=
  L.h_zero_defect_det L.h_zero_defect
...
theorem infinite_iwasawa_analyticity_lock
    {H} [NormedAddCommGroup H] [InnerProductSpace ℂ H] [CompleteSpace H]
    (L : InfiniteIwasawaAnalyticityLock H) :
    L.modularFlowDet = 1 ∧ L.modularTrace = 0 ∧ L.determinantDefect = 0 := by
  exact ⟨det_one L, trace_zero L, L.h_zero_defect⟩
...
/--
If the bridge identifies every determinant zero with a real spectral parameter
on `1/2 + iℝ`, every such zero lies on the critical line.
-/
theorem determinant_zero_on_critical_line
    {H : Type*} [NormedAddCommGroup H] [InnerProductSpace ℂ H] [CompleteSpace H]
    {D : H →L[ℂ] H}
    (B : BridgeCertificate H D)
    (s : ℂ)
    (hzero : B.spectralDeterminant s = 0) :
    s.re = 1 / 2 := by
  rcases B.zero_to_real_spectral_parameter s hzero with ⟨t, hs, _⟩
  rw [hs]
