...
R = PolynomialRing(QQ, ["Z", "N", "mp", "mn", "M", "c2", "a", "b", "c"])
Z, N, mp, mn, M, c2, a, b, c = R.gens()

def binding_energy(Z0, N0, mp0, mn0, M0, c20):
    return (Z0 * mp0 + N0 * mn0 - M0) * c20

B = binding_energy(Z, N, mp, mn, M, c2)
assert Z * mp + N * mn - B / c2 - M == 0

def delta_vpn_oe(Bfun, Z0, N0):
    return ((Bfun(Z0, N0) - Bfun(Z0, N0 - 2)) -
            (Bfun(Z0 - 1, N0) - Bfun(Z0 - 1, N0 - 2))) / 2

def delta_vpn_eo(Bfun, Z0, N0):
    return ((Bfun(Z0, N0) - Bfun(Z0, N0 - 1)) -
...
def mirror_delta(delta_tz_neg_half, delta_tz_pos_half):
    return delta_tz_neg_half - delta_tz_pos_half
