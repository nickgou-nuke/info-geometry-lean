...
Because the boundary is just another mapping space `c ⟶ CategoryTheory.Limits.colimit F`, the doubled-Krein
pattern directly enforces the vanishing of the cocycle here.
-/
theorem boundary_anomaly_cancellation
    (C : KreinCentralCocycle (c ⟶ CategoryTheory.Limits.colimit F))
    (x : c ⟶ CategoryTheory.Limits.colimit F) :
    C.charge x = 0 :=
...
∞-categorical boundary mapping space `c ⟶ CategoryTheory.Limits.colimit F`.
-/
