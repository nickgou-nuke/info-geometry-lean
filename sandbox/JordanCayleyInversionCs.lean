...
lemma mulTraceReversal_e11_eq_det (X : Herm2x2Cs) :
    X.mulTraceReversal.e11 = X.det := rfl

lemma mulTraceReversal_e22_eq_neg_det (X : Herm2x2Cs) :
    X.mulTraceReversal.e22 = -X.det := rfl
...
theorem fundamental_identity (X : Herm2x2Cs) :
  ...
  ext
  · exact X.mulTraceReversal_e11_eq_det
  · exact X.mulTraceReversal_e22_eq_neg_det
...
theorem on_klein_quadric (X : Herm2x2Cs) (h : X.det = 0) :
  ...
  rw [mulTraceReversal_e11_eq_det, mulTraceReversal_e22_eq_neg_det, h]
  exact ⟨rfl, neg_zero⟩
