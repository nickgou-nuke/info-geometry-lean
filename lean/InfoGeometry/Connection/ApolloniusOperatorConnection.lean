...
@[simp] theorem operatorExteriorDerivativeAt_eq_zero
  ...
  simpa [operatorExteriorDerivativeAt] using
    (variableExteriorDerivativeAt_eq_zero (s : ℂ) u v)
...
@[simp] theorem planarAngularCurvatureAt_eq_zero
  ...
  simpa [planarAngularCurvatureAt] using
    (coupledAngularCurvatureAt_eq_zero α (s : ℂ) u v)
...
/-- The elementary origin contour changes the sign of a spinor. -/
theorem spinContourHolonomy_origin_spinor
    {r : ℝ} (hr0 : 0 < r) (hr1 : r < 1) (ψ : Spinor2) :
    spinorAction (spinContourHolonomy 0 r) ψ = -ψ := by
  simpa [spinContourHolonomy] using
    (contourSpinHolonomy_origin_spinor hr0 hr1 ψ)
