...
-- We assume q ≠ 0 for these rewrite lemmas
variable (hq : q ≠ 0)
...
-- Define the fully rewritten terms
lemma term_p01_p23 : p01 * p23 =
...
lemma term_p02_p13 (hq : q ≠ 0) : q • (p02 * p13) =
...
lemma term_p03_p12 (hq : q ≠ 0) : q^2 • (p03 * p12) =
  ...
  calc q^2 • (p03 * p12)
    ...
    _ = q^2 • (a0 * a1 * b3 * b2) - q^2 • (q - q⁻¹) • (a0 * a3 * b1 * b2) -
        ...
        q^3 • (a3 * a1 * b0 * b2) + q^4 • (a3 * a2 * b0 * b1) := by
        simp only [mul_sub, sub_mul, mul_assoc, smul_mul_assoc, Algebra.mul_smul_comm]
        simp only [smul_sub, smul_add, sub_add]
        -- abel-ish step manually:
        sorry -- will fix calc block manually

-- For the calc block, we will break it up into smaller parts.
"""

with open("generate_lean.py", "w") as f:
    f.write(code)
