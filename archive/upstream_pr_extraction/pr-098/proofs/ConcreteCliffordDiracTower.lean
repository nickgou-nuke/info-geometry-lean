...
/-!
## Native Clifford modules and gamma operators

Spinor spaces are represented here in the canonical algebraic way: a spinor
module is an ordinary module carrying a representation of the Clifford algebra
of a quadratic form.  The gamma operators are the images of the universal
Clifford generators under this representation.
-/

section CliffordModule
...
/-- A spinor module for a quadratic form `Q` is a module carrying a Clifford
algebra representation. -/
structure CliffordModule (Q : QuadraticForm R V) where
  act : CliffordAlgebra Q →ₐ[R] Module.End R S

namespace CliffordModule

variable {Q : QuadraticForm R V}

/-- Gamma operator associated to a vector through the Clifford representation. -/
def gamma (M : CliffordModule (S := S) Q) (v : V) : Module.End R S :=
  M.act (CliffordAlgebra.ι Q v)

/-- Representation-level Clifford square law: `γ(v)^2 = Q(v)`. -/
theorem gamma_sq_scalar (M : CliffordModule (S := S) Q) (v : V) :
    gamma M v * gamma M v =
      algebraMap R (Module.End R S) (Q v) := by
  dsimp [gamma]
  exact CliffordAlgebra.comp_ι_sq_scalar M.act v

/-- Representation-level Clifford anticommutator/polar law. -/
theorem gamma_mul_add_swap (M : CliffordModule (S := S) Q) (v w : V) :
    gamma M v * gamma M w + gamma M w * gamma M v =
      algebraMap R (Module.End R S) (QuadraticMap.polar Q v w) := by
  calc
    gamma M v * gamma M w + gamma M w * gamma M v =
        M.act (CliffordAlgebra.ι Q v * CliffordAlgebra.ι Q w +
          CliffordAlgebra.ι Q w * CliffordAlgebra.ι Q v) := by
          simp [gamma]
    _ = M.act (algebraMap R (CliffordAlgebra Q) (QuadraticMap.polar Q v w)) := by
          rw [CliffordAlgebra.ι_mul_ι_add_swap]
    _ = algebraMap R (Module.End R S) (QuadraticMap.polar Q v w) :=
          M.act.commutes (QuadraticMap.polar Q v w)

/-- Orthogonal Clifford generators anticommute in any spinor representation. -/
theorem gamma_anticomm_of_isOrtho (M : CliffordModule (S := S) Q) {v w : V}
    (hvw : QuadraticMap.IsOrtho Q v w) :
    gamma M v * gamma M w + gamma M w * gamma M v = 0 := by
  rw [gamma_mul_add_swap M v w]
  simp [QuadraticMap.IsOrtho.polar_eq_zero hvw]

/-- Null vectors act by square-zero gamma operators in any Clifford module. -/
theorem gamma_sq_zero_of_apply_eq_zero (M : CliffordModule (S := S) Q) {v : V}
    (hv : Q v = 0) :
    gamma M v * gamma M v = 0 := by
  rw [gamma_sq_scalar M v, hv]
  simp

/-- Orthogonal Clifford generators anticommute as an equality `γ(v)γ(w) = -γ(w)γ(v)`. -/
theorem gamma_mul_eq_neg_mul_of_isOrtho (M : CliffordModule (S := S) Q) {v w : V}
    (hvw : QuadraticMap.IsOrtho Q v w) :
    gamma M v * gamma M w = -(gamma M w * gamma M v) := by
  rw [← add_eq_zero_iff_eq_neg]
  exact gamma_anticomm_of_isOrtho M hvw

/-- A one-vector Clifford-Dirac atom is a literal square root of its quadratic energy. -/
theorem oneVectorDirac_sq (M : CliffordModule (S := S) Q) (v : V) :
    gamma M v * gamma M v =
      algebraMap R (Module.End R S) (Q v) :=
  gamma_sq_scalar M v

/-- A null one-vector Clifford-Dirac atom is nilpotent. -/
theorem oneVectorDirac_sq_zero_of_null (M : CliffordModule (S := S) Q) {v : V}
    (hv : Q v = 0) :
    gamma M v * gamma M v = 0 :=
  gamma_sq_zero_of_apply_eq_zero M hv
