...
/-!
# First-quantized left multiplication on the Zorn carrier

This module treats split-octonion multiplication as a real-linear operator on
the additive Zorn carrier and records the associator defect of composing such
operators. Composition in the endomorphism algebra is associative, but the map
from Zorn elements to their left-multiplication operators is not thereby shown
to preserve products.

This owner does not construct the associative algebra generated by these
operators, identify it with a real Clifford algebra or a full matrix algebra,
or define a projector-generated minimal ideal. Those require explicit
representation and generation theorems on the relevant carriers.
-/
