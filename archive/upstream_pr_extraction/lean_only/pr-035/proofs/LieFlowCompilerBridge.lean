...
/-- Step 5: x0 = g • x1. -/
def endpointToBulk (C : KMSCompiler G X V) : X :=
  C.g • C.x1
...
/-- Step 7 endpoints (discrete endpoint = t=1, bulk seed = t=0). -/
lemma trajectory_start (C : KMSCompiler G X V) :
    trajectoryPoint C 0 = endpointToBulk C := by
  simpa [trajectoryPoint, endpointToBulk] using
...
def localLoss (C : KMSCompiler G X V) (v : X → ℝ → V) : ℝ :=
  trajectoryLoss C.chart v C.x1 C.g C.t

/-- Step 8 scheduled loss: t -> t^γ. -/
def localScheduledLoss (C : KMSCompiler G X V) (v : X → ℝ → V) : ℝ :=
  scheduledTrajectoryLoss C.chart v C.x1 C.g C.gamma C.t
...
theorem localLoss_eq_zero_if_exact (C : KMSCompiler G X V)
    ...
    localLoss C v = 0 := by
  simpa [localLoss, logGenerator, trajectoryLoss_eq_zero_if_perfect] using
    (trajectoryLoss_eq_zero_if_perfect (L := C.chart) v C.x1 C.g C.t h)
...
theorem scheduledLoss_eq_zero_if_exact (C : KMSCompiler G X V)
  ...
  simpa [localScheduledLoss, logGenerator] using
    (scheduledTrajectoryLoss_eq_zero_if_perfect (L := C.chart) v C.x1 C.g C.gamma C.t h)
...
/-- Surjectivity construction: exp has a right inverse by `hExpLog`; this is the
...
theorem exp_surjective (C : KMSCompiler G X V) : Function.Surjective C.chart.exp := by
  intro g
  refine Exists.intro (C.chart.log g) ?_
  calc
    C.chart.exp (C.chart.log g) = g := C.chart.hExpLog g
...
theorem compiler_diagnostic_packet
  ...
  refine And.intro ?roots ?projectors
  · exact TrifactorGeometry.cubic_real_roots hq
  · refine And.intro ?plus ?minus
