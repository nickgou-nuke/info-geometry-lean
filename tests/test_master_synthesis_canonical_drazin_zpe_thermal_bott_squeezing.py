...
def test_master_synthesis_exposes_canonical_drazin_zpe_thermal_bott_witness_squeezing_route() -> None:
    text = SOURCE.read_text(encoding="utf-8")

    anchor = (
        "private theorem "
        "squeezingLogShear_bound_of_bits_to_gravity_to_fluid_capstone_"
        "of_canonicalDrazinFluidHelicityWitness_and_zpeGravityWitness_and_thermalBottWitness"
    )
    assert anchor in text
    block = text.split(anchor, 1)[1].split(
        "/--\nCocycle-sourced capstone variant:", 1
    )[0]
    assert "(hZPEGravity : ZPEGravityWitness (E := E) S c R Kgeo x)" in block
    assert "(hFluidHelicity : CanonicalDrazinFluidHelicityWitness (E := E) A B_mp)" in block
    ...
    assert (
        "bits_to_gravity_to_fluid_capstone_of_canonicalDrazinFluidHelicityWitness_"
        "and_zpeGravityWitness_and_thermalBottWitness" in block
    )
