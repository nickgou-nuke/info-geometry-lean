...
noncomputable def zornFlowLinearEquiv
    ...
    (t : ℝ) :
    CZ ≃ₗ[ℝ] CZ :=
  (coordLELin.trans (CDE.flowLinearEquiv (coordEnd D) t)).trans coordLELin.symm

@[simp]
theorem zornFlowLinearEquiv_apply (D : EndCZ) (t : ℝ) (X : CZ) :
    zornFlowLinearEquiv D t X =
      coordLELin.symm (CDE.flow (coordEnd D) t (coordLE X)) := by
  simp [zornFlowLinearEquiv, CDE.flowLinearEquiv_apply, LinearEquiv.trans_apply]
  <;>
  simp_all [coordLELin]
  <;>
  rfl
...
theorem zornFlowLinearEquiv_neg_generator
  ...
  rw [coordLE_zornFlowLinearEquiv]
  rw [coordEnd_neg]
  have h := CDE.flow_neg_generator (coordEnd D) t
  exact congrArg (fun T : V8 →L[ℝ] V8 => T (coordLE X)) h
...
theorem zornFlow_map_one
  ...
  have h₄ : (1 : CZ) * ((zornFlowLinearEquiv D t).symm 1) = (zornFlowLinearEquiv D t).symm 1 := by simp
  rw [h₄] at h₃
  have h₅ : zornFlowLinearEquiv D t ((zornFlowLinearEquiv D t).symm 1) = zornFlowLinearEquiv D t 1 * 1 := by
    calc
      zornFlowLinearEquiv D t ((zornFlowLinearEquiv D t).symm 1) = zornFlowLinearEquiv D t (1 * ((zornFlowLinearEquiv D t).symm 1)) := by simp
