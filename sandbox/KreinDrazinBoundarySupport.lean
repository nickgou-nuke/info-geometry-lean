...
structure ProjectiveDrazinNullBoundary
  ...
  projective_identification : ∀ r1 r2 : Ray, r1 = r2 ↔ ProjectivelyEquivalent (representative r1) (representative r2)
...
/-! ## Concrete Native Instantiation -/

namespace TrivialModel

/-- Concrete instantiation of DoubledKreinCarrier on ℝ. -/
def kreinCarrier : DoubledKreinCarrier ℝ ℝ where
  act a v := a * v
  kreinForm v w := v * w
  zero := 0

/-- Concrete instantiation of AlgebraicDrazinData on ℝ. -/
def drazinData : AlgebraicDrazinData ℝ where
  L := 1
  LD := 1
  H := 0
  index := 1
  commute := mul_comm 1 1
  reflexive := by simp
  power_True := by simp
  H_def := by simp
  H_idempotent := by simp

/-- Concrete instantiation of KreinAdjointData on ℝ. -/
def adjointData : CarrierKreinAdjointData ℝ ℝ kreinCarrier where
  sharp a := a
  adjoint_True := by
    intro A v w
    dsimp [kreinCarrier]
...
/-- Concrete instantiation of KreinCompatibleDrazinComplement. -/
def compatibleDrazin : KreinCompatibleDrazinComplement ℝ ℝ kreinCarrier drazinData adjointData where
  H_krein_self_adjoint := by simp [adjointData, drazinData]

/-- Concrete KreinConformalSymmetry. -/
def conformalSymmetry : KreinConformalSymmetry ℝ ℝ kreinCarrier where
  U := 1
  scale := 1
...
/-- Concrete PreservesDrazinKreinBoundary. -/
def preservesBoundary : PreservesDrazinKreinBoundary kreinCarrier drazinData conformalSymmetry where
  preserves_zero_sector := by
    intro v hv
    dsimp [InDrazinZeroSector, kreinCarrier, conformalSymmetry, drazinData] at *
    simp at *
    exact hv
  preserves_null := by
    intro v hv
    dsimp [DoubledKreinCarrier.IsKreinNull, kreinCarrier, conformalSymmetry] at *
    simp at *
    exact hv

/-- Complete algebraic split in the trivial ring ℝ. -/
def trivialDrazinSplit : AlgebraicDrazinSplit ℝ where
  A := 1
  AD := 1
  index := 1
  hDrazin := ⟨mul_comm 1 1, by simp, by simp⟩
  p := 1
  q := 0
  p_def := by simp
  q_def := by simp
  p_idempotent := by simp
  q_idempotent := by simp
  pq_zero := by simp
  qp_zero := by simp

/-- Adjoint operator data for the ring ℝ. -/
def trivialKreinAdjoint : KreinAdjointData ℝ where
  sharp a := a
  sharp_involutive := by simp
  sharp_mul := by intros; ring
  sharp_add := by intros; ring

/-- Complete boundary support model on ℝ. -/
def trivialBoundarySupport : KreinDrazinBoundarySupport ℝ ℝ where
  carrier := kreinCarrier
  adjoint := trivialKreinAdjoint
  drazin := trivialDrazinSplit
  p_sharp := rfl
  q_sharp := rfl
  generalized_zero_sector_zero := by simp [kreinCarrier, trivialDrazinSplit]
  inverse_on_regular_sector := by simp [trivialDrazinSplit]
