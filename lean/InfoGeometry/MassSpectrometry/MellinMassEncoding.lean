...
/-- Evaluation of the scale character at a positive mass. -/
def MellinScaleCharacter.eval
    (χ : MellinScaleCharacter) (m : ℝ) : ℂ :=
  Complex.exp (Complex.I * (χ.omega : ℂ) *
    (logMass m χ.referenceMass : ℂ))

/-- Scale-character evaluation is invariant under common rescaling of the
observed mass and reference mass. -/
theorem mellinScaleCharacter_common_scale
    (ω λ m m₀ : ℝ) (hλ : λ ≠ 0) (hm₀ : 0 < m₀) :
    (MellinScaleCharacter.eval
      ⟨ω, λ * m₀, mul_pos (lt_of_le_of_ne (le_of_lt hm₀) hλ.symm) hm₀⟩
      (λ * m)) =
    Complex.exp (Complex.I * (ω : ℂ) * (logMass m m₀ : ℂ)) := by
  rw [MellinScaleCharacter.eval, logMass_common_scale hλ (ne_of_gt hm₀)]
...
/-- Relative pairwise log mass is invariant under common positive scaling. -/
theorem relative_log_mass_scale_invariant
    {λ m₁ m₂ : ℝ} (hλ : λ ≠ 0) (hm₂ : m₂ ≠ 0) :
    Real.log ((λ * m₁) / (λ * m₂)) = Real.log (m₁ / m₂) := by
  congr 1
  field_simp [hλ, hm₂]
