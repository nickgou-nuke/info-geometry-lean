...
theorem P_even_idempotent (n : ℕ) (x : CuntzAlg n) :
  ...
  change ((1 : ℂ) / 2) • (P_even n x + parity n (P_even n x)) = P_even n x
  rw [parity_P_even]
  calc
    ((1 : ℂ) / 2) • (P_even n x + P_even n x)
        = ((1 : ℂ) / 2) • ((2 : ℂ) • P_even n x) := by rw [two_smul]
    _ = (((1 : ℂ) / 2) * (2 : ℂ)) • P_even n x := by rw [smul_smul]
    _ = (1 : ℂ) • P_even n x := by norm_num
    _ = P_even n x := by simp

/-- P_odd is idempotent: P_odd(P_odd(x)) = P_odd(x). -/
theorem P_odd_idempotent (n : ℕ) (x : CuntzAlg n) :
    P_odd n (P_odd n x) = P_odd n x := by
  change ((1 : ℂ) / 2) • (P_odd n x - parity n (P_odd n x)) = P_odd n x
  rw [parity_P_odd]
  let a := P_odd n x
...
theorem P_even_P_odd (n : ℕ) (x : CuntzAlg n) :
  ...
  change ((1 : ℂ) / 2) • (P_odd n x + parity n (P_odd n x)) = 0
  rw [parity_P_odd]
  -- (1/2)·(a + (-a)) = (1/2)·0 = 0
  simp
...
theorem P_odd_P_even (n : ℕ) (x : CuntzAlg n) :
  ...
  change ((1 : ℂ) / 2) • (P_even n x - parity n (P_even n x)) = 0
  rw [parity_P_even]
  -- (1/2)·(a - a) = 0
  simp
