...
private lemma sup_even_odd_proof : evenPart ⊔ oddPart = ⊤ := by
  ...
  rw [evenPart, oddPart]
  have h1' : {fun x => match x with | B.H => 1 | x => 0, fun x => match x with | B.Ep => 1 | x => 0, fun x => match x with | B.Em => 1 | x => 0} = {Pi.single B.H (1 : ℝ), Pi.single B.Ep (1 : ℝ), Pi.single B.Em (1 : ℝ)} := by
    ext f; constructor
  ...
  have h2' : {fun x => match x with | B.G1 => 1 | x => 0, fun x => match x with | B.G2 => 1 | x => 0} = {Pi.single B.G1 (1 : ℝ), Pi.single B.G2 (1 : ℝ)} := by
    ext f; constructor
...
private lemma even_even_skew_proof (x y : OSp12) : x ∈ evenPart → y ∈ evenPart → bracket x y = - bracket y x := by
  have heven : ({fun x => match x with | B.H => 1 | x => 0, fun x => match x with | B.Ep => 1 | x => 0, fun x => match x with | B.Em => 1 | x => 0} : Set OSp12) =
  ...
  | mem x hxgen =>
      ...
      | zero =>
          ext k; simp [bracket, SuperBracket.zero_lie, SuperBracket.lie_zero_right]
