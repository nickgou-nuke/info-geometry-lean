...
async function main() {
  ...
  try {
    ...
    // Create or use the aiclaw_rag database
    try {
      await db.createDatabase("aiclaw_rag");
      console.log("✅ Created database 'aiclaw_rag'.");
    } catch {
      console.log("⚡ Database 'aiclaw_rag' already exists.");
    }

    db.useBasicAuth("root", "password");
    const aiclawDb = db.database("aiclaw_rag");
    ...
    console.log("   Database: aiclaw_rag");
