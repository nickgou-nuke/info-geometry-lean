...
This file gives the integral carrier which is needed before making lattice or
charge-lattice claims about the canonical Zorn algebra.  It uses four
hyperbolic pairs.  The quadratic refinement is
...
coordinate swap is the inverse Gram operator in the standard basis.  We do
not identify this carrier with the positive-definite `D₄` root lattice.
-/
