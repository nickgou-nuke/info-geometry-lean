...
theorem angle_common_scale (p : MellinRotaryPosition) {λ : ℝ}
    (hλ : λ ≠ 0) (hm₀ : p.referenceMass ≠ 0) :
    (MellinRotaryPosition.mk p.omega (λ * p.mass) (λ * p.referenceMass)).angle =
      p.angle := by
  exact mellinAngle_common_scale hλ hm₀
