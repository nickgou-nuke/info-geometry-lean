...
theorem neutralAction_pair_even (w z : NeutralSpace) :
  ...
  calc
    _ = (gradeLinear * neutralAction w) * neutralAction z := by rw [mul_assoc]
    _ = -(neutralAction w * (gradeLinear * neutralAction z)) := by
      rw [neutralAction_odd w, neg_mul, mul_assoc]
    _ = (neutralAction w * neutralAction z) * gradeLinear := by
      rw [neutralAction_odd z, mul_neg, neg_neg, mul_assoc]
...
theorem clifford_paravector_interval (Q : QuadraticForm ℝ V) (t : ℝ) (v : V) :
  ...
  have hc : r*x = x*r := (Algebra.commutes t x).eq
  change (r+x)*(r-x) = _
  calc
