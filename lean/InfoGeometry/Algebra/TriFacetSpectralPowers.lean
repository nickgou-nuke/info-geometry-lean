...
lemma O2_P_hyp (h2 : (2 : A) ≠ 0) {O : A} (hO : O ^ 3 = O) :
    O ^ 2 * P_hyp O = P_hyp O := by
  have h1 : O ^ 2 * P_hyp O = O * (O * P_hyp O) := by ring
  rw [h1]
  rw [tri_facet_eigenspace_hyp h2 hO]
  exact tri_facet_eigenspace_hyp h2 hO

lemma O2_P_ell (h2 : (2 : A) ≠ 0) {O : A} (hO : O ^ 3 = O) :
    O ^ 2 * P_ell O = P_ell O := by
  have h1 : O ^ 2 * P_ell O = O * (O * P_ell O) := by ring
  rw [h1]
  rw [tri_facet_eigenspace_ell h2 hO]
