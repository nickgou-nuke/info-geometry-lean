...
theorem paper_typed_address_biaxial_completion_visible_read_partial
    (D : VisibleReadPartialData)
    (guarded_read :
      ∀ ω, D.read ω =
        if D.inDomain ω && D.inVisiblePhase ω && D.checkPasses ω then some (D.obs ω) else none) :
    ∀ ω,
      D.read ω =
