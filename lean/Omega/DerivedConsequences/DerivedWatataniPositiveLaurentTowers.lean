...
theorem paper_derived_watatani_positive_laurent_towers :
    Omega.Conclusion.conclusion_qfold_channel_choi_rank_equals_qcollision_statement
        (fold := fun b : Bool => b) 2 ∧
      (let D : Omega.Conclusion.conclusion_tqft_genus_hausdorff_moment_sequence_data :=
        { n := 1
