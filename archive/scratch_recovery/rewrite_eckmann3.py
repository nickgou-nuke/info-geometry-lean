...
lemma ker_mul_transpose_eq_ker_transpose {m n : ℕ} (A : Matrix (Fin m) (Fin n) ℚ) (x : Fin m → ℚ) :
  ...
  · intro h
    ...
    have h_mul_A : (A * Aᵀ).mulVec x = A.mulVec (Aᵀ.mulVec x) := (Matrix.mulVec_mulVec A Aᵀ x).symm
    rw [h_mul_A] at h_dot
...
lemma rank_mul_transpose_eq_rank {m n : ℕ} (A : Matrix (Fin m) (Fin n) ℚ) :
  ...
  have hr1 : (A * Aᵀ).rank = Module.finrank ℚ (LinearMap.range (A * Aᵀ).mulVecLin) := rfl
  have hr2 : (Aᵀ : Matrix (Fin n) (Fin m) ℚ).rank = Module.finrank ℚ (LinearMap.range (Aᵀ : Matrix (Fin n) (Fin m) ℚ).mulVecLin) := rfl
  rw [← hr1] at h1
  rw [← hr2] at h2
  rw [h_ker_eq] at h1
...
lemma full_rank_trivial_kernel {n : ℕ} (A : Matrix (Fin n) (Fin n) ℚ)
  ...
  have h_ker_dim : Module.finrank ℚ (LinearMap.ker (Matrix.toLin' A)) = 0 := by
    ...
    rw [h_domain, hn, ← h_range, h_rank] at h_total; omega
...
lemma ker_sq_eq_ker_psd {m : ℕ} (C : Matrix (Fin m) (Fin m) ℚ) (h_sym : Cᵀ = C)
  ...
  · intro h
    ...
    have h_mul_CC : (C * C).mulVec x = C.mulVec (C.mulVec x) := (Matrix.mulVec_mulVec C C x).symm
...
lemma rank_sq_eq_rank_AAtranspose {m n : ℕ} (A : Matrix (Fin m) (Fin n) ℚ) :
  ...
  have h_RK (M : Matrix (Fin m) (Fin m) ℚ) : M.rank + Module.finrank ℚ (LinearMap.ker M.mulVecLin) = m := by
    ...
    have hn : Fintype.card (Fin m) = m := Fintype.card_fin m
    rw [h_dom, hn] at h
    have hR : M.rank = Module.finrank ℚ (LinearMap.range M.mulVecLin) := rfl
    rw [← hR] at h; exact h
...
lemma im_sq_eq_im_AAtranspose {m n : ℕ} (A : Matrix (Fin m) (Fin n) ℚ) :
  ...
  have h_sub : LinearMap.range (C * C).mulVecLin ≤ LinearMap.range C.mulVecLin := by
    ...
    have h_mul_CC : C.mulVec (C.mulVec x) = (C * C).mulVec x := Matrix.mulVec_mulVec C C x
  ...
  have h_finrank_eq : Module.finrank ℚ (LinearMap.range (C * C).mulVecLin) =
    ...
    have hr1 : (C * C).rank = Module.finrank ℚ (LinearMap.range (C * C).mulVecLin) := rfl
    have hr2 : C.rank = Module.finrank ℚ (LinearMap.range C.mulVecLin) := rfl
    rw [← hr1, ← hr2, rank_sq_eq_rank_AAtranspose A]
...
lemma laplacian_rank_eq_sum {n₀ n₁ n₂ : ℕ}
  ...
  have h_imC_sub : LinearMap.range C.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    have step1 : (C + D).mulVec (C.mulVec y) = C.mulVec (C.mulVec y) + D.mulVec (C.mulVec y) := Matrix.add_mulVec (C) (D) (C.mulVec y)
    have step2 : C.mulVec (C.mulVec y) = (C * C).mulVec y := Matrix.mulVec_mulVec C C y
    have step3 : D.mulVec (C.mulVec y) = (D * C).mulVec y := Matrix.mulVec_mulVec D C y
    simp only [Matrix.mulVecLin_apply] at hy ⊢
  ...
  have h_imD_sub : LinearMap.range D.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    have step1 : (C + D).mulVec (D.mulVec z) = C.mulVec (D.mulVec z) + D.mulVec (D.mulVec z) := Matrix.add_mulVec (C) (D) (D.mulVec z)
    have step2 : C.mulVec (D.mulVec z) = (C * D).mulVec z := Matrix.mulVec_mulVec C D z
    have step3 : D.mulVec (D.mulVec z) = (D * D).mulVec z := Matrix.mulVec_mulVec D D z
  ...
  have h_range : LinearMap.range (C + D).mulVecLin = LinearMap.range C.mulVecLin ⊔ LinearMap.range D.mulVecLin := by
    ...
    · intro v hv
      ...
      have h_add : (C + D).mulVec x = C.mulVec x + D.mulVec x := Matrix.add_mulVec (C) (D) (x)
  ...
  have h_rank_add : (C + D).rank = C.rank + D.rank := by
    ...
    have hbot : Module.finrank ℚ (⊥ : Submodule ℚ (Fin n₁ → ℚ)) = 0 := Submodule.finrank_bot
    rw [hbot, add_zero] at h_sup_inf
    exact h_sup_inf
...
theorem eckmann_hodge_nullity {n₀ n₁ n₂ : ℕ}
  ...
  have hn : Fintype.card (Fin n₁) = n₁ := Fintype.card_fin n₁
  rw [h_dom, hn] at h_rk
  have h_rank_Δ₁ : Module.finrank ℚ (LinearMap.range (Δ₁ : Matrix (Fin n₁) (Fin n₁) ℚ).mulVecLin) = del1.rank + del2.rank :=
    laplacian_rank_eq_sum del1 del2 h_boundary
  rw [h_rank_Δ₁] at h_rk
  -- h_rk: del1.rank + del2.rank + finrank(ker Δ₁) = n₁
  omega
