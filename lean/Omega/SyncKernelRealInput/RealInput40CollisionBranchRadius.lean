...
def branch_points_are_discriminant_roots : Prop :=
...
def taylor_radius_formula : Prop :=
...
theorem paper_real_input_40_collision_branch_radius
    : branch_points_are_discriminant_roots ∧
      theta_branch_lattice ∧
      taylor_radius_formula := by
  refine ⟨?_, ?_, ?_⟩
