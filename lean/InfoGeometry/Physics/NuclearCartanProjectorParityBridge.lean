...
theorem a_mul_numberOp_same (i : ι) :
  ...
  calc
    ...
    _ = car.a i - (car.adag i * car.a i) * car.a i := by
      rw [sub_mul, one_mul]
    _ = car.a i := by rw [numberOp_mul_a_same]; simp
  ...
  calc
    (car.numberOp i + car.numberOp i - 1) * car.adag i -
      ...
      comm (car.numberOp i) (car.adag i) +
        comm (car.numberOp i) (car.adag i) := by noncomm_ring
  ...
  calc
    (car.numberOp i + car.numberOp i - 1) * car.a i -
      ...
      comm (car.numberOp i) (car.a i) +
        comm (car.numberOp i) (car.a i) := by noncomm_ring
...
theorem fermionParity_anticomm_adag (i : ι) :
  ...
  unfold fermionParity
  rw [numberOp_mul_adag_same, adag_mul_numberOp_same]
  noncomm_ring
...
theorem isospinCartan_eq_pauli3 : isospinCartan = pauli3 := by
  ...
  fin_cases i <;> fin_cases j <;>
    simp [isospinCartan, isospin3, pauli3]
...
theorem isospinCartan_comm_plus :
  ...
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [isospinCartan, isospin3, isospinPlus, pauli1, pauli2, pauli3]
...
theorem isospinCartan_comm_minus :
  ...
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [isospinCartan, isospin3, isospinMinus, pauli1, pauli2, pauli3]
...
/-! ## Existing spin/isospin `ℤ₂ × ℤ₂` parity packet -/

/-- The tensor spin/isospin carrier has two commuting involutions. -/
theorem spin_isospin_involution_packet :
    spinChirality * spinChirality = (1 : SpinPair) ∧
      isospinChirality * isospinChirality = (1 : SpinPair) ∧
      spinChirality * isospinChirality =
        isospinChirality * spinChirality :=
  ⟨spinChirality_sq,
    isospinChirality_sq,
    spinChirality_isospinChirality_commute⟩

/-! ## Canonical five-graded Cartan relation shape -/

/-- The canonical `SL2SpinorLadder` owner carries the identical normalized
Cartan/raising/lowering relation shape.  This theorem records relation-shape
agreement only; it does not assert a Lie-algebra isomorphism with a nuclear CAR
subalgebra. -/
