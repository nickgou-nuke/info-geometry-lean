...
/-- Instantiation of SplitIdempotents to ensure it's not a vacuous shape. -/
example : SplitIdempotents (K := K) (2 : K) (0 : K) where
  mul_zero := by simp
  mul_zero_rev := by simp
  proj_E := by simp [smul_eq_mul, mul_two]
  proj_Ebar := by simp
...
/-- Instantiation of FiveGradedDecomposition to ensure it's not a vacuous shape. -/
def fiveGradedDecompositionExample : FiveGradedDecomposition K A :=
  { g_neg_two := 0
    g_neg_one := 0
    g_zero := 0
    g_pos_one := 0
...
/-- Instantiation of NullWeightSector to ensure it's not a vacuous shape. -/
example {K A : Type*} [CommRing K] [Ring A] [Algebra K A] : NullWeightSector (fiveGradedDecompositionExample (K := K) (A := A)) :=
  NullWeightSector.posOne rfl
