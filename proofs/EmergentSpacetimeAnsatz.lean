...
/-- Hermitian predicate on the concrete `2 × 2` carrier. -/
def is_hermitian (X : Matrix (Fin 2) (Fin 2) ℂ) : Prop :=
  X = X.conjTranspose

/-- Pauli coordinates of a Hermitian matrix. -/
...
/-- The determinant equals the Pauli/Minkowski quadratic form. -/
...
/-- Null-cone locus on the Hermitian carrier. -/
...
/-- The finite spacetime ansatz is now a genuine determinant/form identity. -/
def emergent_spacetime_ansatz : Prop :=
  ∀ (X : Matrix (Fin 2) (Fin 2) ℂ) (hx : is_hermitian X),
    determinant_is_minkowski_norm X hx ↔
      X.det = (PauliZornTrifactor.minkowskiNorm
        (hermitian_minkowski_iso X hx).1
        (hermitian_minkowski_iso X hx).2.1
        (hermitian_minkowski_iso X hx).2.2.1
