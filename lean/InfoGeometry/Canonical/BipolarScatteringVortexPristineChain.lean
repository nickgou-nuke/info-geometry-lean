...
2. geometric reflection acts by inverse conjugation and flips radial scale;
3. particle-hole phase closure is a separate anti-linear involution;
4. a critical-line point is merely a marked node unless an actual zero law is
   supplied;
5. a vortex at such a node requires a separate translated pole/residue datum;
6. a critical window can carry an ordinary unitary two-port scattering block;
7. a transfer block may instead satisfy determinant-one `J`-unitarity;
8. the planar gradient and Hodge-rotated channels are orthogonal and equal-norm.
...
/-- Particle-hole phase conjugation is a distinct involutive structure. -/
...
/-- Master packet for the recovered second stream. -/
