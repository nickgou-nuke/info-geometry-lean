...
theorem core_peel2_basis8_seven_x1_false
    {f : SplitOctF2Aut} (hf : f ∈ nativeFlagStabilizer) :
    ((peel2 f).1 (basis8 7)).x1 = false := by
  have h3x1 : (f.1 (basis8 3)).x1 = true :=
    nativeFlagStabilizer_basis8_three_x1 hf
  have h6x1 : (f.1 (basis8 6)).x1 = false := by
    exact congrArg SplitOctF2.x1 (nativeFlagStabilizer_basis8_six_coordinate_form hf)
...
theorem core_peel34_basis8_seven_x1_eq
    {f : SplitOctF2Aut} (hf : f ∈ nativeFlagStabilizer) :
  ...
  · have h4 := nativeFlagStabilizer_basis8_four hf
    have h4x1 : (f.1 (basis8 4)).x1 = false := by rw [h4]; rfl
    rw [automorphism_map_ePlus_eMinus_basis8_4_basis8_7]
    dsimp [add, add2, one]
    rw [h4x1]
    simp
  · have h4 := nativeFlagStabilizer_basis8_four hf
    ...
    have h6 := nativeFlagStabilizer_basis8_six_coordinate_form hf
    have h6x1 : (f.1 (basis8 6)).x1 = false := congrArg SplitOctF2.x1 h6
    rw [automorphism_map_ePlus_eMinus_basis8_4_basis8_6_basis8_7]
    dsimp [add, add2, one]
    rw [h4x1, h6x1]
    simp
  · rw [automorphism_map_add]
...
theorem core_peel5_basis8_seven_x1_false
    {f : SplitOctF2Aut} (hf : f ∈ nativeFlagStabilizer)
  ...
  split
  · rw [automorphism_mul_apply]
    have heq : G2TwoSylowPCAutomorphisms.pc6Aut.1 (basis8 7) =
        add (basis8 5) (basis8 7) := by
      ext <;> dsimp [basis8, up0, up1, up2, down0, down1, down2,
        ePlus, eMinus, G2TwoSylowPCAutomorphisms.pc6Aut, add, add2]
      all_goals decide
...
theorem nativeFlagStabilizer_fullPeel_basis8_seven_x1_false_core
    {g : SplitOctF2Aut} (hg : g ∈ nativeFlagStabilizer) :
  ...
  have h2_x1 : ((peel2 (peel1 (peel0 g))).1 (basis8 7)).x1 = false :=
    core_peel2_basis8_seven_x1_false h1
  have h34_x1 : ((peel34 (peel2 (peel1 (peel0 g)))).1 (basis8 7)).x1 = false := by
    rw [core_peel34_basis8_seven_x1_eq h2, h2_x1]
  exact core_peel5_basis8_seven_x1_false h34 h34_x1
