...
theorem particleHole_hamiltonian (energy : ℝ) (v : Spinor) :
    particleHole (hamiltonian energy *ᵥ v) =
      -(hamiltonian energy *ᵥ particleHole v) := by
  funext i
  fin_cases i <;>
...
theorem particleHole_eigenpair (energy eigenvalue : ℝ) (v : Spinor)
    (hv : v ≠ 0) (heig : hamiltonian energy *ᵥ v = (eigenvalue : ℂ) • v) :
    particleHole v ≠ 0 ∧
      hamiltonian energy *ᵥ particleHole v = (-eigenvalue : ℂ) • particleHole v := by
  constructor
  · intro h
    apply hv
