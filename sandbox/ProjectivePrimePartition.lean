...
/--
A concrete instantiation of the `ProjectivePrimeCalibration` structure,
demonstrating that the geometric state space is inhabited and non-vacuous.
-/
def trivialCalibration : ProjectivePrimeCalibration (Finset ℕ) where
  stateOfFinset := id
  modularFlowReadout A u := projectivePrimePartition A u
  flow_eq_projectivePrimePartition _A _u _hu := rfl
