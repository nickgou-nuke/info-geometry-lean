...
structure HorizonAttractorMicrostateLedger
  ...
  chargeReadout : State → Charge
  /-- Central-charge or attractor-potential readout. -/
  centralCharge : State → Scalar
  /-- Entropy/microstate readout. -/
  entropyReadout : State → Scalar
  /-- Hidden memory/microstate reservoir. -/
  hiddenMemory : State → Memory
  /-- Entropy as a function of the installed charge datum. -/
  entropyOfCharge : Charge → Scalar
  /-- Central-charge readout as a function of the installed charge datum. -/
  centralChargeOfCharge : Charge → Scalar
  /-- Attractor-style entropy law. -/
  entropy_attractor_True : ∀ s : State, entropyReadout s = entropyOfCharge (chargeReadout s)
  /-- Attractor-style central-charge law. -/
  centralCharge_attractor_True : ∀ s : State, centralCharge s = centralChargeOfCharge (chargeReadout s)

namespace HorizonAttractorMicrostateLedger

variable {State Charge Scalar Memory : Type*}
variable (L : HorizonAttractorMicrostateLedger State Charge Scalar Memory)

/-- The horizon entropy readout is determined by charge data. -/
theorem entropyReadout_eq_entropyOfCharge (s : State) :
...
theorem centralCharge_eq_centralChargeOfCharge (s : State) :
...
theorem entropyOfCharge_eq_of_chargeReadout_eq {s₁ s₂ : State}
    (hcharge : L.chargeReadout s₁ = L.chargeReadout s₂) :
    L.entropyOfCharge (L.chargeReadout s₁) = L.entropyOfCharge (L.chargeReadout s₂) := by
  rw [hcharge]

/-- Lemma 2: equal charge readouts give equal central-charge-of-charge values. -/
theorem centralChargeOfCharge_eq_of_chargeReadout_eq {s₁ s₂ : State}
    (hcharge : L.chargeReadout s₁ = L.chargeReadout s₂) :
    L.centralChargeOfCharge (L.chargeReadout s₁) = L.centralChargeOfCharge (L.chargeReadout s₂) := by
  rw [hcharge]

/-- Lemma 3: entropy readout is constant on charge fibers. -/
theorem entropyReadout_eq_of_chargeReadout_eq {s₁ s₂ : State}
    (hcharge : L.chargeReadout s₁ = L.chargeReadout s₂) :
...
theorem centralCharge_eq_of_chargeReadout_eq {s₁ s₂ : State}
    (hcharge : L.chargeReadout s₁ = L.chargeReadout s₂) :
...
theorem attractorReadouts_eq_of_chargeReadout_eq {s₁ s₂ : State}
    (hcharge : L.chargeReadout s₁ = L.chargeReadout s₂) :
    L.entropyReadout s₁ = L.entropyReadout s₂ ∧
      L.centralCharge s₁ = L.centralCharge s₂ :=
  ⟨L.entropyReadout_eq_of_chargeReadout_eq hcharge,
...
structure HorizonThermalLedger
  ...
  /-- Thermal/KMS readout. -/
  thermalReadout : State → ThermalReadout
  /-- Predicate saying a state is thermally/KMS calibrated. -/
  IsThermal : State → Prop

namespace HorizonThermalLedger

variable {State ThermalReadout : Type*}
variable (T : HorizonThermalLedger State ThermalReadout)
...
theorem horizonAttractorMicrostateOwnerTarget
    ...
    (s : State) :
    L.entropyReadout s = L.entropyOfCharge (L.chargeReadout s) :=
  L.entropyReadout_eq_entropyOfCharge s
...
/-- Concrete instantiation of the `HorizonAttractorMicrostateLedger` to verify it is not vacuous. -/
def trivialLedger : HorizonAttractorMicrostateLedger ℕ ℕ ℕ ℕ where
  chargeReadout := id
  centralCharge := id
  entropyReadout := id
  hiddenMemory := id
  entropyOfCharge := id
  centralChargeOfCharge := id
  entropy_attractor_True := fun _ => rfl
  centralCharge_attractor_True := fun _ => rfl

/-- Concrete instantiation of `HorizonMemoryRecoveryWitness` to verify it is not vacuous. -/
def trivialRecovery : HorizonMemoryRecoveryWitness trivialLedger where
  recoveredMemory := id
  recovery_True := fun _ => rfl

/-- Concrete instantiation of `HorizonThermalLedger` to verify it is not vacuous. -/
def trivialThermalLedger : HorizonThermalLedger ℕ ℕ where
  thermalReadout := id
  IsThermal := fun _ => True
