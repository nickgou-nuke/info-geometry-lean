...
theorem cosmicCrossoverInterpretation_valid :
...
def toCosmicCrossoverWitness : CosmicCrossoverWitness Op where
  ...
  reflection_law := D.cptClosure_theta_P_left
  crossover_interpretation_law := D.cosmicCrossoverInterpretationLaw
  crossover_interpretation_certificate := D.cosmicCrossoverInterpretation_valid
