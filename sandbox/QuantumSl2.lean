...
theorem K_F_Kinv_eq_qinv_sq_F (q : ℂ) (hq : q ≠ 0) : K q * F * Kinv q = ((q⁻¹ ^ 2) • F) := by
  ...
  · simp [K, F, Kinv, Matrix.mul_apply, Matrix.smul_apply]
  · -- (1,0): q⁻¹·q⁻¹ = (q²)⁻¹
    simp [K, F, Kinv, Matrix.mul_apply, Matrix.smul_apply, sq]
  · simp [K, F, Kinv, Matrix.mul_apply, Matrix.smul_apply]
...
/-! ### The U_q(sl(2)) Relations Abstract Structure -/

/-- Abstract structure for the defining relations of U_q(sl(2)). -/
structure Uqsl2Relations (q : ℂ) (K Kinv E F : Matrix (Fin 2) (Fin 2) ℂ) : Prop where
  K_Kinv_eq_one : K * Kinv = 1
  Kinv_K_eq_one : Kinv * K = 1
  K_E_Kinv_eq_qsq_E : K * E * Kinv = (q ^ 2) • E
  K_F_Kinv_eq_qinv_sq_F : K * F * Kinv = (q⁻¹ ^ 2) • F
  comm_EF : E * F - F * E = (q - q⁻¹)⁻¹ • (K - Kinv)

/-- The standard 2×2 representation provides a concrete instantiation of U_q(sl(2)) relations. -/
def standardRep (q : ℂ) (hq : q ≠ 0) (hq_sq_ne_one : q ^ 2 ≠ 1) : Uqsl2Relations q (K q) (Kinv q) E F :=
  { K_Kinv_eq_one := K_Kinv_eq_one q hq,
    Kinv_K_eq_one := Kinv_K_eq_one q hq,
    K_E_Kinv_eq_qsq_E := K_E_Kinv_eq_qsq_E q,
    K_F_Kinv_eq_qinv_sq_F := K_F_Kinv_eq_qinv_sq_F q hq,
    comm_EF := by rw [comm_EF, K_minus_Kinv_div_q_minus_qinv_eq_diag q hq hq_sq_ne_one] }
...
-/
theorem phi_sq_eq_phi_add_one : phi * phi = phi + 1 := by
  unfold phi InfoGeometryCore.phiC
