...
/-- Smoke reference for the Drazin/projective-count readout packet theorem. -/
abbrev drazinIsProjectiveCountReadoutRef
    (G T Target Coeff Algebra ModuliOperator : Type*) [Ring Algebra] :=
  @InfoGeometry.GromovWittenErlangen.ProjectiveCountDrazinFrobeniusBridge.drazin_is_projective_count_readout
    (G := G) (T := T) (Target := Target) (Coeff := Coeff)
    (Algebra := Algebra) (ModuliOperator := ModuliOperator)
