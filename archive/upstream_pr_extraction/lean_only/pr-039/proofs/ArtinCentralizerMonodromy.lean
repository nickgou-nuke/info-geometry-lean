...
theorem neg_one_mem_pin55 : (-1 : Clifford55.Cl55) ∈ Clifford55.Pin55 := by
  classical
  rw [pinGroup.mem_iff]
  constructor
  · have hQunit : IsUnit (Clifford55.Q55 (Clifford55.f_neg 0)) := by
      rw [Clifford55.Q55_f_neg]
      exact ⟨-1, rfl⟩
    let u : Clifford55.Cl55ˣ :=
      (CliffordAlgebra.isUnit_ι_of_isUnit Clifford55.Q55 hQunit).unit
    have huval : (u : Clifford55.Cl55) = Clifford55.ι55 (Clifford55.f_neg 0) :=
      (CliffordAlgebra.isUnit_ι_of_isUnit Clifford55.Q55 hQunit).unit_spec
    have hgen :
        u ∈ ((↑) ⁻¹' Set.range Clifford55.ι55 : Set Clifford55.Cl55ˣ) := by
      refine ⟨Clifford55.f_neg 0, ?_⟩
      simpa using huval
    have hulip : u ∈ Clifford55.LipschitzGroup55 :=
      Subgroup.subset_closure hgen
    have hu2lip : u * u ∈ Clifford55.LipschitzGroup55 :=
      mul_mem hulip hulip
    refine ⟨u * u, hu2lip, ?_⟩
    change (u : Clifford55.Cl55) * (u : Clifford55.Cl55) = (-1 : Clifford55.Cl55)
    rw [huval, Clifford55.f_neg_mul_self]
  · rw [Unitary.mem_iff]
    simp

/-- The nontrivial `{I,-I}` element as an actual `Pin(5,5)` element. -/
def negOnePin55 : Clifford55.Pin55 :=
  ⟨(-1 : Clifford55.Cl55), neg_one_mem_pin55⟩

/-- The centralizer elements map into the Pin(5,5) group over the identity. -/
def centralizerElement (c : CentralSign) : Clifford55.Pin55 :=
  match c with
  | I => 1
  | negI => negOnePin55
