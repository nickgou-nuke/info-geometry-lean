...
/-- Gauge conjugation is additive on observables. -/
theorem gaugeTransform_add (u : Aˣ) (X Y : A) :
    gaugeTransform u (X + Y) = gaugeTransform u X + gaugeTransform u Y := by
  dsimp [gaugeTransform]
  simp only [add_mul, mul_add]

/-- The gauge action bundled as an `R`-linear endomorphism. -/
def gaugeTransformLinear (u : Aˣ) : A →ₗ[R] A where
  toFun := gaugeTransform u
  map_add' X Y := gaugeTransform_add u X Y
  map_smul' r X := gaugeTransform_smul u r X
...
theorem gaugeTransform_pow (u : Aˣ) (X : A) (n : ℕ) :
  ...
  induction n with
  | zero => simp [gaugeTransform]
  | succ n ih =>
      rw [pow_succ, gaugeTransform_observable_mul, ih, pow_succ]
...
/-- Repeated gauge transformation by a unit power is the corresponding
iterate of the single-unit connection action. -/
theorem gaugeTransformConnection_pow (u : Aˣ) (A_conn : A) (n : ℕ) :
    gaugeTransformConnection D (u ^ n) A_conn =
      (gaugeTransformConnection D u)^[n] A_conn := by
  induction n generalizing A_conn with
  | zero => simp [gaugeTransformConnection]
  | succ n ih =>
      rw [pow_succ, gaugeTransformConnection_comp D u (u ^ n)]
