...
def edgeEndpoints : Edge → Concept × Concept
  ...
  | .furey_triality_to_twelvefold => (.fureyTrialityTwelvefold, .nativeTwelvefoldExactOrder)
...
def entry : Concept → Entry
  ...
  | .nativeTwelvefoldExactOrder =>
      ⟨.nativeTwelvefoldExactOrder, "native masterTwelve has exact order twelve", .theorem,
        some ``InfoGeometry.Physics.QCDTwelveFoldSchurTrialityBridge.native_twelvefold_order_packet,
        "The concrete repository operator has exact order 12 and explicit order-2/order-3 projections."⟩
  | .artinNativeTwelvefoldParallel =>
  ...
  | .fureyTrialityTwelvefold =>
      ⟨.fureyTrialityTwelvefold, "Furey thirds / finite triality / twelvefold packet", .structuralBridge,
        some ``InfoGeometry.Physics.QCDTwelveFoldSchurTrialityBridge.furey_triality_twelvefold_packet,
        "Denominator-three occupation values, an order-three triality cycle, and exact order twelve coexist on distinct carriers."⟩
  | .schurArtinTwelvefold =>
