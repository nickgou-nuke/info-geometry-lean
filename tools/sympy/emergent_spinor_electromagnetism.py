...
def dirac_adjoint(psi, gamma0):
    return (psi.conjugate().T * gamma0)


def bilinear(psi, matrix, phi, gamma0):
...
def main():
    ...
    assert sp.simplify(bracket(tau1, tau2) - 2 * sp.I * tau3) == sp.zeros(2)
    assert tau1 != sp.zeros(2)

    lambda1 = sp.Matrix([[0, 1, 0], [1, 0, 0], [0, 0, 0]])
    lambda2 = sp.Matrix([[0, -sp.I, 0], [sp.I, 0, 0], [0, 0, 0]])
    lambda3 = sp.Matrix([[1, 0, 0], [0, -1, 0], [0, 0, 0]])
    assert sp.simplify(bracket(lambda1, lambda2) - 2 * sp.I * lambda3) == sp.zeros(3)
    assert lambda1 != sp.zeros(3)

    print("Emergent spinor electromagnetism finite bilinear checks passed.")
