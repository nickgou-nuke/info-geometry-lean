...
theorem twistorNorm_apply (Z W : TwistorSpace) :
  ...
  simpa [twistorNorm] using
    InfoGeometry.Twistor.PenroseTwistor.twistorHermitian_apply Z W
...
theorem twistorNorm_smul_right (c : ℂ) (Z W : TwistorSpace) :
  ...
  simpa [twistorNorm] using
    InfoGeometry.Twistor.PenroseTwistor.twistorHermitian_smul_right c Z W

theorem twistorNorm_smul_self (c : ℂ) (Z : TwistorSpace) :
    twistorNorm (c • Z) (c • Z) = (Complex.normSq c : ℂ) * twistorNorm Z Z := by
  simpa [twistorNorm] using
    InfoGeometry.Twistor.PenroseTwistor.twistorHermitian_smul_self c Z
...
theorem twistor_classification_disjoint (Z : TwistorSpace) :
  ...
  simpa [IsPositiveTwistor, IsNegativeTwistor, IsNullTwistor] using
...
theorem projective_twistor_classification_disjoint (p : ProjectiveTwistorSpace) :
    ...
    ¬ (InfoGeometry.Twistor.PenroseTwistor.IsProjectivePositive p ∧
      InfoGeometry.Twistor.PenroseTwistor.IsProjectiveNull p) ∧
    ¬ (InfoGeometry.Twistor.PenroseTwistor.IsProjectiveNegative p ∧
      InfoGeometry.Twistor.PenroseTwistor.IsProjectiveNull p) := by
  simpa using
    InfoGeometry.Twistor.PenroseTwistor.projective_twistor_classification_disjoint p
...
theorem projective_twistor_sign_trichotomy (p : ProjectiveTwistorSpace) :
    InfoGeometry.Twistor.PenroseTwistor.IsProjectivePositive p ∨
      ...
      InfoGeometry.Twistor.PenroseTwistor.IsProjectiveNull p := by
  simpa using
    InfoGeometry.Twistor.PenroseTwistor.projective_twistor_sign_trichotomy p

def twistorMk (Z : TwistorSpace) (hZ : Z ≠ 0) (hnull : twistorNorm Z Z = 0) :
    NullTwistorSpace :=
  InfoGeometry.Twistor.PenroseTwistor.twistorMk Z hZ <| by
    simpa [twistorNorm, InfoGeometry.Twistor.PenroseTwistor.helicity] using
      congrArg Complex.re hnull
...
@[simp] theorem isProjectiveNull_mk_iff (Z : TwistorSpace) (hZ : Z ≠ 0) :
  ...
  simpa using
    InfoGeometry.Twistor.PenroseTwistor.isProjectiveNull_mk_iff Z hZ

@[simp] theorem twistorMk_rep (Z : TwistorSpace) (hZ : Z ≠ 0) (hnull : twistorNorm Z Z = 0) :
    (twistorMk Z hZ hnull).1 = Projectivization.mk ℂ Z hZ := by
  simpa [twistorMk, twistorNorm] using
    InfoGeometry.Twistor.PenroseTwistor.twistorMk_rep Z hZ hnull

@[simp] theorem realNullTwistorMk_rep (Z : TwistorSpace) (hZ : Z ≠ 0)
    (hnull : twistorRealQuadraticForm Z = 0) :
    (realNullTwistorMk Z hZ hnull).1 = Projectivization.mk ℝ Z hZ := by
