...
def entry : Concept → Entry
  ...
  | .realColorRepresentation =>
      ⟨.realColorRepresentation,
        ...
        .theorem,
        some ``InfoGeometry.Physics.QCDHestenesRealColorRepresentation.realColorAction_injective,
        "The transported gl3(C) action is faithful, preserves commutators, and commutes with J."⟩
  | .fockHestenesPhase =>
      ⟨.fockHestenesPhase,
        "real Cl(5,5) master Fock Hestenes phase",
        .theorem,
        some ``InfoGeometry.Physics.QCDHestenesKreinFockPhaseBridge.fockJ_sq,
  ...
  | .realZornFureyCarrier =>
      ⟨.realZornFureyCarrier,
        ...
        .structuralBridge,
        some ``InfoGeometry.Physics.QCDZornCl55FureyBridge.canonicalZornToCl55_injective,
