...
def operatorZornParity : InternalParity OZ where
  gamma := InfoGeometry.Physics.NCG.chiralGradingOperator
  gamma_sq := by
    rw [chiral_grading_sq_native]
...
theorem dirac_internal_odd (Delta : A) :
  ...
  calc
    ...
    _ = -InfoGeometry.Physics.NCG.diracOperator Delta := by
      rw [(operatorZornParity (A := A)).gamma_sq]
      simp
...
theorem dirac_square_internal_even (Delta : A) :
  ...
  let D : OZ := InfoGeometry.Physics.NCG.diracOperator Delta
  let G : OZ := InfoGeometry.Physics.NCG.chiralGradingOperator
...
even square. -/
def diracSuperHamiltonian (Delta : A) : Block2 OZ :=
  starBlockHamiltonian
    (InfoGeometry.Physics.NCG.diracOperator Delta *
      InfoGeometry.Physics.NCG.diracOperator Delta)
    (InfoGeometry.Physics.NCG.diracOperator Delta *
      InfoGeometry.Physics.NCG.diracOperator Delta)
