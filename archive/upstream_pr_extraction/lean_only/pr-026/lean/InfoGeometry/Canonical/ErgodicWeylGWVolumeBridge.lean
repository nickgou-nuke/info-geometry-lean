...
is kept as explicit model data.  The proved content is the readback chain:

```text
self-similar fixed operator
...
/--
Fusion of an ergodic self-similar fixed-point readout with the already-owned
...
structure Bridge where
  ...
  /-- Phase-volume is invariant under the calibrated modular/KMS flow. -/
  phaseVolume_flow_invariant :
    ∀ (t : ℝ) (A : EndE),
      phaseVolume (ergodicOmega.ergodic.kms.kmsFlow t A) = phaseVolume A
...
corresponding self-similar ergodic fixed operator.
-/
@[rep_depth projective]
...
/--
The word fixed-point phase-volume is invariant under modular/KMS time.
-/
@[rep_depth projective]
theorem fixedOperator_phaseVolume_flow_invariant
    (w : Word) (t : ℝ) :
    B.phaseVolume
        (B.ergodicOmega.ergodic.kms.kmsFlow t
