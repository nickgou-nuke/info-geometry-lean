...
abbrev BraidGroupDiagram := ℕ ⥤ GrpCat.{u}
...
abbrev BraidGroupColimit (B : BraidGroupDiagram.{u}) : GrpCat.{u} :=
  GrpCat.FilteredColimits.colimit B

/-- The finite stage with n adjacent braid generators. -/
abbrev finiteBraidStage (n : ℕ) : GrpCat :=
  GrpCat.of (Braid.braid_group (n + 1))
...
noncomputable def straightStrandBond
  ...
  GrpCat.ofHom (PresentedGroup.toGroup (f := straightStrandGenerator n)
...
theorem canonicalStraightStrand_adjacent
  ...
  have hrel' :
    ...
    simpa only [straightStrandGenerator, Braid.σ, Braid.σ', hindex] using hrel
...
theorem canonicalStraightStrand_far
  ...
  have hrel' :
    ...
    simpa only [straightStrandGenerator, Braid.σ, Braid.σ', hright] using hrel
...
theorem canonicalStraightStrandRelators (n : ℕ) :
  ...
  | zero =>
      intro r hr
      exact False.elim (by simpa [Braid.braid_rels] using hr)
  | succ n =>
      cases n with
      | zero =>
          intro r hr
...
abbrev canonicalBraidGroupColimit : GrpCat :=
  GrpCat.FilteredColimits.colimit canonicalBraidGroupTower.diagram
...
abbrev finitePresentedBraidColimit
    (T : FinitePresentedBraidTowerInput) : GrpCat :=
  GrpCat.FilteredColimits.colimit (finitePresentedBraidDiagram T)
...
theorem finitePresentedBraidTower_synthesis
    ...
    (finitePresentedBraidColimit T =
      GrpCat.FilteredColimits.colimit (finitePresentedBraidDiagram T)) := by
...
abbrev BraidColimit (B : BraidGroupDiagram.{u}) : GrpCat.{u} :=
  BraidGroupColimit B
...
abbrev stageInjection (B : BraidGroupDiagram.{u}) (n : ℕ) :
  ...
  GrpCat.FilteredColimits.colimitCocone B |>.ι.app n
...
natural endomorphism.  Naming this cocone is important: `IsColimit.map`
uses the original cocone, whereas descent needs these transported legs. -/
noncomputable def endomorphismCocone
    (B : BraidGroupDiagram.{u}) (η : B ⟶ B) : Cocone B :=
  Cocone.mk (BraidGroupColimit B)
    (NatTrans.mk (fun n => η.app n ≫ stageInjection B n) (by
      intro X Y f
      simp only [Functor.const_obj_obj, Functor.const_obj_map, Category.comp_id,
        Category.assoc]
      rw [← Category.assoc, η.naturality, Category.assoc,
        (GrpCat.FilteredColimits.colimitCocone B).w f]))
...
abbrev descendedEndomorphism
  ...
  (GrpCat.FilteredColimits.colimitCoconeIsColimit B).desc
    (endomorphismCocone B η)
...
theorem stageInjection_descended
  ...
  simpa [stageInjection, descendedEndomorphism, endomorphismCocone] using
    (GrpCat.FilteredColimits.colimitCoconeIsColimit B).fac
      (endomorphismCocone B η) n
...
theorem stageInjection_descended_comp
    (B : BraidGroupDiagram.{u}) (η : B ⟶ B) (n : ℕ)
    (f : BraidGroupColimit B ⟶ BraidGroupColimit B) :
    stageInjection B n ≫ descendedEndomorphism B η ≫ f =
      η.app n ≫ stageInjection B n ≫ f := by
  simpa only [Category.assoc] using congrArg (fun k => k ≫ f)
    (stageInjection_descended B η n)

theorem stageInjection_descended_comp₂
    (B : BraidGroupDiagram.{u}) (η ξ : B ⟶ B) (n : ℕ) :
    stageInjection B n ≫ descendedEndomorphism B η ≫
        descendedEndomorphism B ξ =
      η.app n ≫ ξ.app n ≫ stageInjection B n := by
  calc
    _ = η.app n ≫ stageInjection B n ≫ descendedEndomorphism B ξ :=
      stageInjection_descended_comp B η n (descendedEndomorphism B ξ)
    _ = η.app n ≫ ξ.app n ≫ stageInjection B n := by
      simpa only [Category.assoc] using congrArg
        (fun k => η.app n ≫ k) (stageInjection_descended B ξ n)

theorem stageInjection_descended_comp₃
    (B : BraidGroupDiagram.{u}) (η ξ ζ : B ⟶ B) (n : ℕ) :
    stageInjection B n ≫ descendedEndomorphism B η ≫
        descendedEndomorphism B ξ ≫ descendedEndomorphism B ζ =
      η.app n ≫ ξ.app n ≫ ζ.app n ≫ stageInjection B n := by
  calc
    _ = η.app n ≫ ξ.app n ≫ stageInjection B n ≫
          descendedEndomorphism B ζ := by
      simpa only [Category.assoc] using congrArg
        (fun k => k ≫ descendedEndomorphism B ζ)
        (stageInjection_descended_comp₂ B η ξ n)
    _ = η.app n ≫ ξ.app n ≫ ζ.app n ≫ stageInjection B n := by
      simpa only [Category.assoc] using congrArg
        (fun k => η.app n ≫ ξ.app n ≫ k)
        (stageInjection_descended B ζ n)
...
theorem colimit_artin_relation
  ...
  apply (GrpCat.FilteredColimits.colimitCoconeIsColimit B).hom_ext
  intro n
  calc
    stageInjection B n ≫
        descendedEndomorphism B D.braid1 ≫
      ...
      D.braid1.app n ≫ D.braid2.app n ≫ D.braid1.app n ≫
        stageInjection B n := by
          exact stageInjection_descended_comp₃ B D.braid1 D.braid2 D.braid1 n
    _ =
    ...
    _ =
      stageInjection B n ≫
        ...
        descendedEndomorphism B D.braid1 ≫
        descendedEndomorphism B D.braid2 := by
          exact (stageInjection_descended_comp₃ B D.braid2 D.braid1 D.braid2 n).symm
...
theorem colimit_relation_of_stagewise
  ...
  calc
    stageInjection B n ≫ descendedEndomorphism B η ≫
        descendedEndomorphism B ξ =
      η.app n ≫ ξ.app n ≫ stageInjection B n := by
        exact stageInjection_descended_comp₂ B η ξ n
    ...
    _ = stageInjection B n ≫ descendedEndomorphism B ξ ≫
      ...
      exact (stageInjection_descended_comp₂ B ξ η n).symm
...
abbrev RepresentationCocone
    (B : BraidGroupDiagram.{u}) (G : GrpCat.{u}) :=
  B ⟶ (Functor.const ℕ).obj G
...
theorem stageInjection_descendedRepresentation
    ...
    stageInjection B n ≫ descendedRepresentation B G ρ =
      ρ.app n := by
  exact (GrpCat.FilteredColimits.colimitCoconeIsColimit B).fac
    (representationCocone B G ρ) n
...
abbrev descendedRep
  ...
  (GrpCat.FilteredColimits.colimitCoconeIsColimit B).desc rep_family
...
theorem rep_stage_commutation
  ...
  exact (GrpCat.FilteredColimits.colimitCoconeIsColimit B).fac rep_family n
...
theorem descended_commutation
  ...
  let ι_n : B.obj n →* BraidColimit B := (stageInclusion B n).hom
