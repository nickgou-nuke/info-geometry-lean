...
theorem rankOneKernel_posSemidef (ψ : Mode ι) :
  ...
  have hC : C * Matrix.conjTranspose C = rankOneKernel ψ := by
    ext i j
...
theorem rankOneKernel_scalarAction (u : ℂ) (ψ : Mode ι) :
    rankOneKernel (scalarAction u ψ) =
      (u * starRingEnd ℂ u) • rankOneKernel ψ := by
  ext i j
  change
    (u * ψ i) * starRingEnd ℂ (u * ψ j) =
      (u * starRingEnd ℂ u) * (ψ i * starRingEnd ℂ (ψ j))
  simp only [map_mul]
  ring
...
theorem rankOneKernel_orderParameter
  ...
  have hscalar :
      (Real.sqrt N0 : ℂ) * starRingEnd ℂ (Real.sqrt N0 : ℂ) =
        (N0 : ℂ) := by
    calc
      (Real.sqrt N0 : ℂ) * starRingEnd ℂ (Real.sqrt N0 : ℂ)
