...
The concrete direct-limit carrier already owned by `Cl11TensorTowerLimit` is
packaged as `TensorInductiveLimit`.  The functional statements remain
conditional on an extension law and are proved from that law.
-/

noncomputable section

namespace InfoGeometry.Canonical.Cl11TensorInductiveLimit
