...
/-!
# Scalar gauge action on the quantum Grassmannian

Entry scaling descends through the FRT presentation to an algebra endomorphism of
`QuantumMatrixAlgebra`. Units act by algebra automorphisms. The action preserves the
...
/-! ## Descent through the FRT quotient -/
...
theorem gaugeAlgHom_entry (i : Fin 2) (j : Fin 4) :
  ...
  have h : gaugeAlgHom R q u (entry R q i j) = gaugeComp R q u (freeEntry R i j) := by
    exact RingQuot.liftAlgHom_mkAlgHom_apply R (gaugeComp R q u)
      (gaugeComp_rel R q u) (freeEntry R i j)
  rw [h]
...
theorem gaugeCoordinateRing_mul (v : R) :
    (gaugeCoordinateRing R q u).comp (gaugeCoordinateRing R q v) =
      gaugeCoordinateRing R q (u * v) := by
  ext x
  exact AlgHom.congr_fun (gaugeAlgHom_mul R q u v) x.1

theorem gaugeCoordinateRing_one :
    gaugeCoordinateRing R q 1 = AlgHom.id R (coordinateRing R q) := by
  ext x
  exact AlgHom.congr_fun (gaugeAlgHom_one R q) x.1

noncomputable def gaugeCoordinateRingEquiv (unit : Rˣ) :
    coordinateRing R q ≃ₐ[R] coordinateRing R q :=
  AlgEquiv.ofAlgHom (gaugeCoordinateRing R q unit) (gaugeCoordinateRing R q ↑unit⁻¹)
    (by simpa [gaugeCoordinateRing_one] using
      gaugeCoordinateRing_mul R q (u := (unit : R)) (↑unit⁻¹ : R))
    (by simpa [gaugeCoordinateRing_one] using
      gaugeCoordinateRing_mul R q (u := (↑unit⁻¹ : R)) (unit : R))
