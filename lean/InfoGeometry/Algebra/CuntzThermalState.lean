...
/-- The scaled truncated exponential polynomial: exp_N(-β x). -/
noncomputable def truncExpNegBeta (β : ℂ) (N : ℕ) : Polynomial ℂ :=
  ∑ k ∈ Finset.range (N+1), Polynomial.monomial k (((-β) ^ k) / (Nat.factorial k : ℂ))
...
theorem truncExp_neg_beta_spectral (n : ℕ) (ε : Fin n → ℂ) (β : ℂ) (N : ℕ) :
    Polynomial.aeval (hamiltonian n ε) (truncExpNegBeta β N) =
    ∑ i : Fin n,
      (Polynomial.aeval (ε i) (truncExpNegBeta β N)) • P n i :=
  polynomial_spectral n ε (truncExpNegBeta β N)
...
theorem truncExp_neg_beta_explicit (n : ℕ) (ε : Fin n → ℂ) (β : ℂ) (N : ℕ) :
  ...
  congr 1
  simp only [truncExpNegBeta, map_sum, Polynomial.aeval_monomial, Algebra.algebraMap_self]
...
theorem primon_thermal_state (primes : Fin 3 → ℕ) (_hprimes : ∀ i, Nat.Prime (primes i)) (β : ℂ) (N : ℕ) :
    Polynomial.aeval (hamiltonian 3 (λ i => (Real.log (primes i : ℝ) : ℂ)))
      (truncExpNegBeta β N) =
    ∑ i : Fin 3,
      (∑ k ∈ Finset.range (N+1),
        ((-β * (Real.log (primes i : ℝ) : ℂ)) ^ k / (Nat.factorial k : ℂ))) • P 3 i := by
  simpa using
