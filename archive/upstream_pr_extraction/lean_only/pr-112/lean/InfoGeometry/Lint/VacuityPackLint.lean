...
/-!
# Vacuity-Pack Lint — Detection Logic
...
private def endsWithSorryField (s : String) : Bool :=
  s.endsWith "_sorry"
...
-/
def detectVacuityPackPairs (env : Environment) (structName : Name) :
...
def hasVacuityPackPairs (env : Environment) (structName : Name) : Bool :=
  !(detectVacuityPackPairs env structName).isEmpty
...
def renderVacuityPackDiag (structName : Name) (pair : VacuityPair) : MessageData :=
  match pair.companionField? with
  | some wf =>
      if wf == pair.statementField then
        m!"[Pauli/Vacuity-Pack] `{structName}` has a generic bare `Prop` field \
        ...
        m!"[Pauli/Vacuity-Pack] `{structName}` has a generic `Prop` field \
...
def renderAllVacuityPackDiags (structName : Name) (pairs : Array VacuityPair) :
    Array MessageData :=
