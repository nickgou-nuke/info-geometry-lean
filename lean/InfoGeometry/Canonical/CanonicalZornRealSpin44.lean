...
This owner supplies the real-form bridge above the maintained typed triality and
five-grade attachment. The vector carrier is the standard diagonal real
quadratic space `ℝ⁴ ⊕ ℝ⁴`. Its explicit linear embedding into the typed
canonical Zorn vector copy preserves the `(4,4)` quadratic form.
...
of Mathlib's spin group. The target is the underlying real general linear group
of the complex Dirac carrier; no irreducibility assertion is made.
-/
...
theorem realSplit44ToVector8_eq_canonical (x : RealSplit44) :
    realSplit44ToVector8 x =
      CanonicalZornCompositionFiveGradeBridge.realVector8
        (pac44ToCoreZorn (realSplit44ToPAC44 x)) := by
  apply ZornCopy.ext
...
theorem realSplit44_vector_grade_plus (x : RealSplit44) :
    CanonicalZornCompositionFiveGradeBridge.vectorGradePlus
        (realSplit44ToVector8 x) ∈
      CanonicalZornFiveGradedClosure.conformalGrade TKKJordanPairData.TKKGrade.p1 :=
  CanonicalZornCompositionFiveGradeBridge.vectorGradePlus_mem _
