...
structure Intertwiner (P Q : Presentation) where
  ...
  map_act : ∀ (o : P.Observable) (s : P.State),
    mapState (P.act o s) = Q.act (mapObservable o) (mapState s)
  map_support : ∀ s, P.support s → Q.support (mapState s)
  map_generator : ∀ s, mapState (P.generator s) = Q.generator (mapState s)

@[rep_depth krein]
abbrev ReadoutPreservation {P Q : Presentation} (F : Intertwiner P Q) : Prop :=
...
def idIntertwiner (P : Presentation) : Intertwiner P P where
  ...
  mapScalar := fun x => x
  map_act := by intro o s; rfl
  map_support := by intro s hs; exact hs
  map_generator := by intro s; rfl

@[rep_depth krein]
def compIntertwiner {P Q R : Presentation} (F : Intertwiner P Q)
    (G : Intertwiner Q R) : Intertwiner P R where
    ...
    calc
      G.mapState (F.mapState (P.act o s)) =
          G.mapState (Q.act (F.mapObservable o) (F.mapState s)) := by
            rw [F.map_act o s]
      _ = R.act (G.mapObservable (F.mapObservable o))
  ...
  map_generator := by
    ...
    calc
      G.mapState (F.mapState (P.generator s)) =
          G.mapState (Q.generator (F.mapState s)) := by rw [F.map_generator s]
