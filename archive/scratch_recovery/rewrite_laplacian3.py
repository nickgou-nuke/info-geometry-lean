...
lemma innerProduct_transpose_mulVec_eq {m n : Type*} [Fintype m] [Fintype n] (A : Matrix m n ℚ) (x : m → ℚ) (y : n → ℚ) :
  ...
  dsimp [innerProduct, mulVec, Matrix.dotProduct]
  simp_rw [mul_assoc]
  rw [Finset.sum_comm]
...
lemma posSemidef_AAtranspose (A : Matrix m n ℚ) (x : m → ℚ) : 0 ≤ innerProduct x ((A * Aᵀ).mulVec x) := by
  rw [← Matrix.mulVec_mulVec, innerProduct_transpose_mulVec_eq A x (Aᵀ.mulVec x)]
...
lemma posSemidef_BtransposeB (B : Matrix p m ℚ) (x : m → ℚ) : 0 ≤ innerProduct x ((Bᵀ * B).mulVec x) := by
  rw [← Matrix.mulVec_mulVec, innerProduct_transpose_mulVec_eq Bᵀ x (B.mulVec x)]
...
lemma orthogonal_ranges (A : Matrix m n ℚ) (B : Matrix p m ℚ) (hBA : B * A = 0) (x : n → ℚ) (y : p → ℚ) :
  ...
  calc
    innerProduct (A.mulVec x) (Bᵀ.mulVec y) = innerProduct (B.mulVec (A.mulVec x)) y := by
      rw [innerProduct_transpose_mulVec_eq B (A.mulVec x) y]
...
lemma ker_laplacian_eq_inter_ker (A : Matrix m n ℚ) (B : Matrix p m ℚ) (hBA : B * A = 0) (x : m → ℚ) :
  ...
  · intro h
    ...
    rw [← Matrix.mulVec_mulVec, innerProduct_transpose_mulVec_eq A x (Aᵀ.mulVec x)] at hA0
    have hAᵀ : Aᵀ.mulVec x = 0 := (innerProduct_self_eq_zero_iff (Aᵀ.mulVec x)).mp hA0
    rw [← Matrix.mulVec_mulVec, innerProduct_transpose_mulVec_eq Bᵀ x (B.mulVec x)] at hB0
    have h_trans : (Bᵀᵀ : Matrix p m ℚ) = B := Matrix.transpose_transpose B
    rw [h_trans] at hB0
    have hB : B.mulVec x = 0 := (innerProduct_self_eq_zero_iff (B.mulVec x)).mp hB0
    exact ⟨hAᵀ, hB⟩
  · intro h_and
    rcases h_and with ⟨hA_tr, hB⟩
...
theorem laplacian_rank_eq_add_rank (A : Matrix m n ℚ) (B : Matrix p m ℚ) (hBA : B * A = 0) :
  ...
  have h_rank_XXᵀ {k : Type*} [Fintype k] [DecidableEq k] (X : Matrix m k ℚ) : Matrix.rank (X * Xᵀ) = Matrix.rank X := by
    have h_ker_eq : LinearMap.ker (X * Xᵀ).mulVecLin = LinearMap.ker (Xᵀ : Matrix k m ℚ).mulVecLin := by
      ext x; constructor
    ...
    have h1 := h_RK (X * Xᵀ)
    have h2 : Matrix.rank Xᵀ + Module.finrank ℚ (LinearMap.ker (Xᵀ : Matrix k m ℚ).mulVecLin) = Fintype.card m := by
      have h := LinearMap.finrank_range_add_finrank_ker (Xᵀ : Matrix k m ℚ).mulVecLin
  ...
  have h_inter : LinearMap.range C.mulVecLin ⊓ LinearMap.range D.mulVecLin = ⊥ := by
    ...
    have h_dy : innerProduct y (D.mulVec (D.mulVec y)) = 0 := by rw [h_Dv]; simp [innerProduct]
