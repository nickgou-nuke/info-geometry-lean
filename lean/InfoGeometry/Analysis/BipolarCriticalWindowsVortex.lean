...
theorem translatedPole_mul {ρ z : ℂ} (hz : z ≠ ρ) :
  ...
  rw [translatedPoleCoeff]
  exact mul_inv_cancel₀ (sub_ne_zero.mpr hz)
...
theorem critical_zero_to_node
  ...
  apply Complex.ext
  · simp [criticalNode, criticalLine, CenteredZeroReadout.OnCriticalLine,
      CenteredZeroReadout.normalCoordinate] at hz ⊢
    linarith
  · simp [criticalNode, criticalLine, CenteredZeroReadout.tangentCoordinate]
...
theorem critical_zero_law
    (z : CenteredZeroReadout) (hz : z.OnCriticalLine) :
    ZeroAt z.zeroFunction (criticalNode z.tangentCoordinate) := by
