...
theorem wittZornEquiv_norm (z : TwistorCarrier) :
...
theorem wittZornEquiv_unit : wittZornEquiv chosenUnit = 1 := by
  ...
  have hc : penroseWittCoordinates (twistorChiralRealDecomposition chosenUnit) =
    ...
    fin_cases i <;> norm_num [penroseWittCoordinates, twistorChiralRealDecomposition,
      chosenUnit, Pi.single_apply] <;> decide
...
theorem selectedMul_norm (x y : TwistorCarrier) :
  ...
  exact InfoGeometry.Algebra.Zorn.KingdonCanonicalBridge.realZornCompositionDatum.detZ_mul _ _
...
theorem selectedMul_ne_literal_candidate (k : ℝ) :
  ...
  norm_num [quadratic_signature, helicity, testX, testY,
    twistorHermitian_apply, Fin.sum_univ_four, Matrix.cons_val_zero,
    Matrix.cons_val_one, Matrix.cons_val_two, Matrix.cons_val_three] at hq
