...
lemma rank_mul_transpose_eq_rank {m n : ℕ} (A : Matrix (Fin m) (Fin n) ℚ) :
  ...
  have h_trans : (Aᵀ : Matrix (Fin n) (Fin m) ℚ).rank = A.rank := Matrix.rank_transpose
  rw [h_eq, h_trans]
...
lemma laplacian_rank_eq_sum {n₀ n₁ n₂ : ℕ}
  ...
  have hD_rank : D.rank = del2.rank := by
    ...
    have h2 : (del2ᵀ : Matrix (Fin n₁) (Fin n₂) ℚ).rank = del2.rank := Matrix.rank_transpose
    rw [h1, h2]
  -- Orthogonality
  have h_CD : C * D = 0 := by
  ...
  have h_inter : LinearMap.range C.mulVecLin ⊓ LinearMap.range D.mulVecLin = ⊥ := by
    ...
    simp only [Matrix.mulVecLin_apply] at hv
  ...
  have h_imC_sub : LinearMap.range C.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    have h_add : (C + D).mulVec (C.mulVec y) = C.mulVec (C.mulVec y) + D.mulVec (C.mulVec y) := by rw [Matrix.add_mulVec]
  ...
  have h_imD_sub : LinearMap.range D.mulVecLin ≤ LinearMap.range (C + D).mulVecLin := by
    ...
    have h_add : (C + D).mulVec (D.mulVec z) = C.mulVec (D.mulVec z) + D.mulVec (D.mulVec z) := by rw [Matrix.add_mulVec]
    ...
    simp only [Matrix.mulVecLin_apply] at hy hz ⊢
  ...
  have h_rank_add : (C + D).rank = C.rank + D.rank := by
    ...
    rw [Submodule.finrank_sup_add_finrank_inf_eq, h_inter]
    have hbot : Module.finrank ℚ (⊥ : Submodule ℚ (Fin n₁ → ℚ)) = 0 := Submodule.finrank_eq_zero.mpr rfl
    rw [hbot, add_zero]
...
theorem eckmann_hodge_nullity {n₀ n₁ n₂ : ℕ}
  ...
  rw [h_dom, hn] at h_rk
  have hR : Matrix.rank Δ₁ = Module.finrank ℚ (LinearMap.range (Δ₁ : Matrix (Fin n₁) (Fin n₁) ℚ).mulVecLin) := rfl
  rw [← hR] at h_rk
  -- h_rk: finrank(range(Δ₁.mulVecLin)) + finrank(ker(Δ₁.mulVecLin)) = n₁
  -- i.e., rank(Δ₁) + finrank(ker Δ₁) = n₁
  -- Step 2: rank(Δ₁) = rank(del1) + rank(del2)
  have h_rank_Δ₁ : Matrix.rank (Δ₁ : Matrix (Fin n₁) (Fin n₁) ℚ) = del1.rank + del2.rank :=
