...
/-- P₊ = (M²+M)/2 -/
def P_plus (M : Matrix (Fin 2) (Fin 2) ℂ) : Matrix (Fin 2) (Fin 2) ℂ := (1 / 2 : ℂ) • (M*M + M)
/-- P₋ = (M²-M)/2 -/
def P_minus (M : Matrix (Fin 2) (Fin 2) ℂ) : Matrix (Fin 2) (Fin 2) ℂ := (1 / 2 : ℂ) • (M*M - M)
/-- P₀ = I - M² -/
...
theorem projector_sum (M : Matrix (Fin 2) (Fin 2) ℂ) :
  ...
  fin_cases i <;> fin_cases j <;>
    simp [I2, Matrix.mul_apply, Fin.sum_univ_two, Matrix.smul_apply] <;> ring_nf
...
theorem op_eq_plus_minus_minus (M : Matrix (Fin 2) (Fin 2) ℂ) :
  ...
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [Matrix.mul_apply, Fin.sum_univ_two, Matrix.smul_apply] <;> ring_nf
...
theorem s3_decomposition : P_plus s3 = !![(1:ℂ),0; 0,0] ∧ P_minus s3 = !![0,0; 0,(1:ℂ)] ∧ P_zero s3 = 0 := by
  ...
  · ext i j
    fin_cases i <;> fin_cases j <;>
      simp [s3, I2, Matrix.mul_apply, Fin.sum_univ_two, Matrix.smul_apply] <;> norm_num
  · ext i j
    fin_cases i <;> fin_cases j <;>
      simp [s3, I2, Matrix.mul_apply, Fin.sum_univ_two, Matrix.smul_apply] <;> norm_num
