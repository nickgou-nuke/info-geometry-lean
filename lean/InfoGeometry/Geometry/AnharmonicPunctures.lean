...
theorem ratio_hasDerivAt (z : Punctured) :
  ...
  change HasDerivAt (fun w : ℂ => w / (w - 1)) _ _
  convert (hasDerivAt_id z.1).div ((hasDerivAt_id z.1).sub_const 1)
    (sub_ne_zero.mpr z.2.2) using 1 <;> ring
...
theorem inversion_log_form (z : Punctured) :
  ...
  dsimp [logCoefficient]
  field_simp [z.2.1, sub_ne_zero.mpr z.2.2, hi] <;> ring
