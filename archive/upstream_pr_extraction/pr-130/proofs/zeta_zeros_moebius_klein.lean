...
/-- The critical strip used for the Möbius strip model, defined strictly via sub-typing. -/
...
theorem criticalStrip_involution (s : ℂ) (hs : 0 < s.re ∧ s.re < 1) :
    0 < (zetaInvolution s).re ∧ (zetaInvolution s).re < 1 := by
  rcases hs with ⟨h0, h1⟩
  rw [zetaInvolution_re]
  exact ⟨by linarith, by linarith⟩

/-- Möbius transformation lifted to the CriticalStrip subtype. -/
def reflectCritical (s : CriticalStrip) : CriticalStrip :=
  ⟨zetaInvolution s.val, criticalStrip_involution s.val s.property⟩

theorem reflectCritical_involutive (s : CriticalStrip) :
    reflectCritical (reflectCritical s) = s := by
  have h : (reflectCritical (reflectCritical s)).val = s.val := zetaInvolution_involutive s.val
  exact Subtype.ext h
...
def spectralMobiusStrip : MobiusStrip where
  ...
  h_preserves_strip := by
    intro s hs
    exact criticalStrip_involution s hs
...
theorem goutev_zeta_synthesis (Z : ZetaFunctionalEquation) :
    ...
    (∀ s (_hs : 0 < s.re ∧ s.re < 1), 0 < (zetaInvolution s).re ∧ (zetaInvolution s).re < 1) ∧
