  ...
  | succ n =>
      ...
      | succ n =>
          ...
          · rcases hAdj with ⟨i, rfl⟩
            ...
            simpa [braid_rel, FreeGroup.lift_apply_of, mul_assoc] using
              (freeGroup_lift_braid_relator
                (f := fun i : Fin (n + 2) => σi i.1)
                i.castSucc i.succ hArtin)
          · rcases hFar with ⟨i, j, hij, rfl⟩
            ...
            simpa [comm_rel, FreeGroup.lift_apply_of, mul_assoc] using
              (freeGroup_lift_comm_relator
                (f := fun i : Fin (n + 2) => σi i.1)
                i.castSucc.castSucc j.succ.succ hComm)
...
theorem finiteRelations_hold_after_succ (n : ℕ) :
  ...
  simpa [straightStrandGenerator] using
    (canonicalStraightStrandRelators n)
...
theorem finiteToInfiniteGroupHom_succ_compatible (n : ℕ) :
  ...
  apply MonoidHom.ext
  intro g
  exact PresentedGroup.toGroup.unique
    (f := fun i : Fin n => σi i.1)
    (rels := braid_rels n)
    (finiteRelations_hold_in_infinite n)
    ((finiteToInfiniteGroupHom (n + 1)).comp (finiteSuccGroupHom n))
    (by
      intro i
      change finiteToInfiniteGroupHom (n + 1)
        (finiteSuccGroupHom n (σ' n i)) = σi i.1
      rw [finiteSuccGroupHom_sigma]
      exact finiteToInfiniteGroupHom_sigma (n + 1) i.castSucc)
...
theorem finiteToInfiniteGroupHom_succ_apply
  ...
  exact congrArg (fun f => f g) (finiteToInfiniteGroupHom_succ_compatible n)
...
def braidGroupBoundaryCocone : Cocone braidGroupDiagram where
  ...
  ι :=
      ...
      (naturality := by
        ...
        simpa [braidGroupDiagram] using finiteToInfiniteGroupHom_succ_compatible n)

/-- Mathlib's categorical colimit object of the finite braid-group tower. -/
def braidGroupColimit : GrpCat :=
  GrpCat.FilteredColimits.colimit braidGroupDiagram
...
def groupFromColimit : braidGroupColimit ⟶ GrpCat.of braid_group_inf :=
  (GrpCat.FilteredColimits.colimitCoconeIsColimit braidGroupDiagram).desc
    braidGroupBoundaryCocone

/-- The comparison map restricts on every finite stage to the canonical
`B_{n+1} → B_∞` embedding. -/
theorem groupFromColimit_ι (n : ℕ) :
    (GrpCat.FilteredColimits.colimitCocone braidGroupDiagram).ι.app n ≫ groupFromColimit =
      GrpCat.ofHom (finiteToInfiniteGroupHom n) := by
  exact (GrpCat.FilteredColimits.colimitCoconeIsColimit braidGroupDiagram).fac
    braidGroupBoundaryCocone n

/-- Elementwise stage readback of the group-colimit comparison map. -/
theorem groupFromColimit_ι_apply
    (n : ℕ) (g : braid_group (n + 1)) :
    groupFromColimit ((GrpCat.FilteredColimits.colimitCocone braidGroupDiagram).ι.app n g) =
      finiteToInfiniteGroupHom n g := by
  have h := congrArg
