...
namespace InfoGeometry.Canonical.G2Basis8NativeLineAlignment
...
theorem basis8Point5_mem_baseIntrinsicLine : basis8Point5 ∈ baseIntrinsicLine.1 := by
  apply (mem_baseIntrinsicLine_iff basis8Point5).2
  change baseLineVector ∈ nativeBaseLineWitness
  simp [nativeBaseLineWitness, lineSet, nativeLineSetAt]

theorem basis8Point45_mem_baseIntrinsicLine : basis8Point45 ∈ baseIntrinsicLine.1 := by
...
theorem fullPeel_basis8_four_add_five_readback
    ...
    (fullPeel g).1 (add (basis8 4) (basis8 5)) = add (basis8 4) (basis8 5) := by
...
theorem basis8Point4_action_readback (g : SplitOctF2Aut) :
    embed (octImPointPerm g basis8Point4).1 = g⁻¹.1 (basis8 4) := by
  rw [embed_octImPointPerm, embed_basis8Point4]

theorem basis8Point5_action_readback (g : SplitOctF2Aut) :
    embed (octImPointPerm g basis8Point5).1 = g⁻¹.1 (basis8 5) := by
