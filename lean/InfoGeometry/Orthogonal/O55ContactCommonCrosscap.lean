...
theorem fermionCrosscap_conjugates_action (A : O55Lie) :
    fermionCrosscap * diagonalFermionAction A * fermionCrosscap =
      diagonalFermionAction (crosscapConjugation A) := by
  apply LinearMap.ext
  rintro ⟨x, y⟩
  apply Prod.ext <;> rfl
...
theorem commonCrosscap_conjugates_action (A : O55Lie) :
    commonCrosscap * commonAction A * commonCrosscap =
      commonAction (crosscapConjugation A) := by
...
conjugation. -/
theorem commonCrosscap_reverses_represented_grade
  ...
  exact commonRepresentation_preserves_grade (crosscap_reverses_grade hA)
