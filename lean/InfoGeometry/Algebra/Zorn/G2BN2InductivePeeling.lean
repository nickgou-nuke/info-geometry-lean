...
namespace InfoGeometry.Algebra.Zorn.G2InductivePeeling
...
theorem conj_wordProd_cons (s : G) (hs : s * s = 1)
  ...
  calc
    s * (e k * wordProd e L) * s
      = s * e k * (wordProd e L * s) := by simp only [mul_assoc]
    _ = s * e k * (1 * (wordProd e L * s)) := by rw [one_mul]
...
theorem comp_word_conj_mem (B : Subgroup G) (s : G) (hs : s * s = 1)
  ...
  | nil =>
    dsimp [wordProd]
    have h1 : s * 1 * s = 1 := by rw [mul_one, hs]
    rw [h1]
    exact B.one_mem
  | cons k ks ih =>
...
end InfoGeometry.Algebra.Zorn.G2InductivePeeling
