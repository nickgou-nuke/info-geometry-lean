...
def degreePhase : ExteriorAlgebra ℂ V →ₐ[ℂ] ExteriorAlgebra ℂ V :=
  ExteriorAlgebra.map (LinearMap.lsmul ℂ V Complex.I)
...
theorem degreePhase_square :
  ...
  intro v
  dsimp
  rw [degreePhase_ι, map_smul, degreePhase_ι]
  have hsmul : Complex.I • (Complex.I • (ExteriorAlgebra.ι ℂ v)) =
      (Complex.I * Complex.I) • (ExteriorAlgebra.ι ℂ v) := by
    rw [smul_smul (M := ℂ)]
  rw [hsmul, Complex.I_mul_I, neg_one_smul]
  exact (gradeInvolution_ι v).symm
...
theorem degreePhase_ιMulti (n : ℕ) (v : Fin n → V) :
  ...
  | succ n ih =>
      ...
      simp only [smul_mul_assoc, mul_smul_comm, smul_smul (M := ℂ), pow_succ']
      rw [mul_comm (Complex.I^n) Complex.I]
...
theorem degreePhase_contract (φ : Module.Dual ℂ V) (x : ExteriorAlgebra ℂ V) :
  ...
  | algebraMap r => simp
  | add x y hx hy => simp only [map_add, hx, hy, smul_add (M := ℂ)]
  | ι_mul x v hx =>
      simp only [CliffordAlgebra.contractLeft_ι_mul, map_sub, map_smul,
        degreePhase_wedge, hx, smul_sub (M := ℂ), smul_smul (M := ℂ), mul_smul_comm]
      congr 1
...
local instance instAModule : Module ℂ A := Algebra.toModule
local instance instAMulAction : MulAction ℂ A := instAModule.toMulAction
