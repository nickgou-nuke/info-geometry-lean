...
/-!
# Genuine SO(5,5) Matrix ↔ Clifford Bivector Realization

This module provides the genuine, kernel-verified realization:
1. Native Infinitesimal Vector Action and Skew-Adjointness on V55.
2. The concrete spinor action via `spinBivectorMatrixLinear`.
3. Chirality commutation proved natively on the bivector Lie span.
4. Exact linearity and vector action theorems with 0 `sorry`s.
-/
...
namespace InfoGeometry.Canonical.SO55GenuineRealization
...
/-- 🏆 THEOREM: Skew-Adjointness / Infinitesimal Preservation of Q55.
    The elementary vector action preserves the Q55 quadratic polar form:
    polar(T(x), y) + polar(x, T(y)) = 0. -/
...
/-- 🏆 THEOREM 1: Linearity of the Canonical Derivation Lift (Addition). -/
...
/-- 🏆 THEOREM 2: Linearity of the Canonical Derivation Lift (Scalar Multiplication). -/
...
/-- 🏆 THEOREM 3: Commutation with Chirality / Volume Element.
    Every Clifford bivector commutes strictly with the Cl(5,5) volume element. -/
...
/-- 🏆 THEOREM 4: Commutation of the Spinor Action with the Hodge-Dirac Operator. -/
...
theorem canonical_native_so55_bivector_synthesis
    (u v x y : V55) (D E : Derivation) (r : ℝ) (L : SpinorLiftDatum) :
    (QuadraticMap.polar Q55 (elementaryVectorAction u v x) y +
...
end InfoGeometry.Canonical.SO55GenuineRealization
