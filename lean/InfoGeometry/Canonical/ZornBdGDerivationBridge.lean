...
/-- THEOREM: The BdG readout respects the star involution.
    ...
    `block12 (bdgReadout (zornStar Z)) = (block21 (bdgReadout Z))†`,
    so the upper-right pairing is the adjoint of the lower-left pairing. -/
@[simp]
theorem bdgReadout_star (Z : Zorn ℂ) :
    block12 (bdgReadout (zornStar Z)) =
      (block21 (bdgReadout Z))† := by
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [zornStar, bdgReadout, block12, block21, sigmaVec, adjoint_apply]

/-- THEOREM: Star-compatible derivations preserve the Hermitian BdG pairing relation.
    If `D` commutes with `zornStar`, then
    `block12 (bdgReadout (D Z)) = (block21 (bdgReadout Z))†`. -/
theorem bdgCompatible_preserved_of_derivation_commutes_star
    (D : Zorn ℂ → Zorn ℂ)
    (hD : IsZornDerivation D)
    (hcomm : ∀ Z : Zorn ℂ, D (zornStar Z) = zornStar (D Z)) :
    ∀ Z : Zorn ℂ,
      block12 (bdgReadout (D Z)) =
        (block21 (bdgReadout Z))† := by
  intro Z
  have h := bdgReadout_star (D Z)
  simp [hcomm Z] at h
  exact h.symm

/-- THEOREM: The first-order BdG orbit preserves the reduced Zorn norm.
    For any derivation `D` and scalar parameter `t`,
    `zornNorm (firstOrderFlow D Z₀ t) = zornNorm Z₀`. -/
theorem bdgOrbit_preserves_reducedNorm
    (D : Zorn ℂ → Zorn ℂ)
    (hD : IsZornDerivation D)
    (Z₀ : Zorn ℂ)
    (t : ℂ) :
    zornNorm (firstOrderFlow D Z₀ t) = zornNorm Z₀ := by
  dsimp [firstOrderFlow, zornNorm, dotR]
  ring

/-- THEOREM: A self-adjoint normal seed remains self-adjoint along the first-order orbit.
    If `Z₀ = zornStar Z₀`, then `firstOrderFlow D Z₀ t = zornStar (firstOrderFlow D Z₀ t)`. -/
theorem bdgOrbit_selfAdjoint
    ...
    (hZ₀ : zornStar Z₀ = Z₀)
    (t : ℂ) :
    zornStar (firstOrderFlow D Z₀ t) = firstOrderFlow D Z₀ t := by
  dsimp [firstOrderFlow, zornStar]
  have hDstar := hstar Z₀
  rw [hZ₀, hDstar] at hDstar
  simp [hDstar]
  ext <;> simp [zornStar, zornSMul, zornAdd] <;> ring
...
theorem bdgOrbit_particleHole
    ...
    (hZ₀ : zornStar Z₀ = Z₀)
    (t : ℂ) :
    zornStar (firstOrderFlow D Z₀ t) = firstOrderFlow D Z₀ t :=
