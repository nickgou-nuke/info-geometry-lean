...
/-- Exact order data already owned by the repository's concrete twelvefold
operator. -/
theorem native_twelvefold_order_packet :
    orderOf masterTwelve = 12 ∧
      orderOf sixParity = 2 ∧
      orderOf sixShift = 3 ∧
      orderOf sixTriality = 6 ∧
      sixTriality ^ 3 = sixParity :=
...
/-- Structural coexistence of the denominator-three Furey spectrum, the finite
order-three triality cycle, and the exact native twelvefold operator.

No identification of the occupation readout with topological spin, flavor, or
electric charge is made. -/
