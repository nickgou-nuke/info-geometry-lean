...
`SplitOctonionSupertwistorBridge` owns the actual carrier
`SupertwistorSpace K := SplitOctonionBasis →₀ K`.  This file records only the
...
theorem splitOctonionBasis_slot_count :
    Fintype.card SplitOctonionBasis = 8 := by
  decide
...
theorem supertwistor_clifford_coordinate_count :
    Fintype.card (Fin 2) + Fintype.card (Fin 3) * Fintype.card (Fin 2) =
      Fintype.card SplitOctonionBasis := by
