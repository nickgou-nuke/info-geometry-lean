...
def seed : O55Lie :=
  splitRankTwo (embedOuterMinus (outerBasis 0))
    (embedOuterPlus (outerBasis 0))
...
def generator : ContactGeneratorLabel → O55Lie
  | minusTwo => gradeMinusTwoProjection seed
  | minusOne i j =>
      splitRankTwo
        (embedOuterMinus (outerBasis i))
        (embedMiddle (middleBasis j))
  | zeroOuter i j =>
      splitRankTwo
        (embedOuterMinus (outerBasis i))
  ...
  | plusOne i j =>
      splitRankTwo
        (embedMiddle (middleBasis j))
        (embedOuterPlus (outerBasis i))
  | plusTwo => gradePlusTwoProjection seed

/-- Every coordinate generator lies in its labeled grade. -/
theorem generator_mem_grade (g : ContactGeneratorLabel) :
    g.generator ∈ realContactGradeSpace g.degree := by
  rcases embedded_weight_packet with ⟨hm, h0, hp⟩
  have hs := grade_projection_membership seed
  cases g with
  | minusTwo => exact hs.1
  | minusOne i j =>
  ...
  | plusOne i j =>
      simpa [generator, degree] using
        splitRankTwo_grade (h0 (middleBasis j)) (hp (outerBasis i))
  | plusTwo => exact hs.2.2.2.2
