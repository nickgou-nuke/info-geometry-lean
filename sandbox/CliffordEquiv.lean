...
/-- Concrete instantiation of AbstractComplexStructure for ℂ over ℝ -/
def complexLinearI : ℂ →ₗ[ℝ] ℂ where
  toFun z := Complex.I * z
  map_add' x y := mul_add _ _ _
  map_smul' r x := by
    change Complex.I * (r • x) = r • (Complex.I * x)
    simp [Algebra.smul_def]
    ring

def canonicalAbstractComplexStructure : AbstractComplexStructure ℂ where
  S := complexLinearI
  S_sq := by
    ext z
    dsimp [complexLinearI]
    change Complex.I * (Complex.I * z) = -z
    rw [← mul_assoc, Complex.I_mul_I]
    simp

lemma induced_one_smul {V : Type*} [AddCommGroup V] [Module ℝ V] (C : AbstractComplexStructure V) (v : V) :
  (1 : ℂ).re • v + (1 : ℂ).im • C.S v = v := by simp

lemma induced_mul_smul {V : Type*} [AddCommGroup V] [Module ℝ V] (C : AbstractComplexStructure V) (z w : ℂ) (v : V) :
  ((z * w).re : ℝ) • v + ((z * w).im : ℝ) • C.S v =
    (z.re : ℝ) • (w.re • v + w.im • C.S v) + (z.im : ℝ) • C.S (w.re • v + w.im • C.S v) := by
...
lemma induced_add_smul {V : Type*} [AddCommGroup V] [Module ℝ V] (C : AbstractComplexStructure V) (z w : ℂ) (v : V) :
  ((z + w).re : ℝ) • v + ((z + w).im : ℝ) • C.S v =
    ((z.re : ℝ) • v + z.im • C.S v) + ((w.re : ℝ) • v + w.im • C.S v) := by
  simp only [Complex.add_re, Complex.add_im, add_smul]
  simp only [add_assoc, add_left_comm]

lemma induced_zero_smul {V : Type*} [AddCommGroup V] [Module ℝ V] (C : AbstractComplexStructure V) (v : V) :
  (0 : ℂ).re • v + (0 : ℂ).im • C.S v = 0 := by simp

lemma induced_smul_add {V : Type*} [AddCommGroup V] [Module ℝ V] (C : AbstractComplexStructure V) (z : ℂ) (v w : V) :
  (z.re : ℝ) • (v + w) + (z.im : ℝ) • C.S (v + w) =
    ((z.re : ℝ) • v + z.im • C.S v) + ((z.re : ℝ) • w + z.im • C.S w) := by
  simp only [map_add, smul_add]
  simp only [add_assoc, add_left_comm]

lemma induced_smul_zero {V : Type*} [AddCommGroup V] [Module ℝ V] (C : AbstractComplexStructure V) (z : ℂ) :
  (z.re : ℝ) • (0 : V) + (z.im : ℝ) • C.S 0 = 0 := by simp
...
def inducedComplexModule {V : Type*} [AddCommGroup V] [Module ℝ V]
  ...
  one_smul := induced_one_smul C
  mul_smul := induced_mul_smul C
  add_smul := induced_add_smul C
  zero_smul := induced_zero_smul C
  smul_add := induced_smul_add C
  smul_zero := induced_smul_zero C
...
lemma cliffordBivector_sq_expand : cl20_e₁ * cl20_e₂ * (cl20_e₁ * cl20_e₂) =
...
theorem cliffordBivector_sq : cliffordBivector * cliffordBivector = -1 := by
  ...
  rw [cliffordBivector_sq_expand, cl20_e₁_sq, cl20_e₂_sq]
  simp
...
lemma mem_complexSubalgebra_iff_mp {A : Type*} [Ring A] [Algebra ℝ A]
    (S : A) (hS : S * S = -1) (x : A) :
...
lemma mem_complexSubalgebra_iff_mpr {A : Type*} [Ring A] [Algebra ℝ A]
    (S : A) (hS : S * S = -1) (x : A) :
    (∃ (r s : ℝ), x = algebraMap ℝ A r + s • S) → x ∈ complexSubalgebra S hS := by
  rintro ⟨r, s, rfl⟩
  use ⟨r, s⟩
...
theorem mem_complexSubalgebra_iff {A : Type*} [Ring A] [Algebra ℝ A]
    (S : A) (hS : S * S = -1) (x : A) :
    x ∈ complexSubalgebra S hS ↔ ∃ (r s : ℝ), x = algebraMap ℝ A r + s • S :=
  ⟨mem_complexSubalgebra_iff_mp S hS x, mem_complexSubalgebra_iff_mpr S hS x⟩
...
lemma hestenesCommutator_double_comm_expand
...
theorem hestenesCommutator_double_comm
  ...
  rw [hestenesCommutator_double_comm_expand]
  rw [h_sq]
  noncomm_ring
...
lemma complexLiftRange_injective {A : Type*} [Ring A] [Algebra ℝ A] [Nontrivial A]
    (S : A) (hS : S * S = -1) : Function.Injective (complexLiftRange S hS) := by
  intro z1 z2 h
  ...
  exact sub_eq_zero.mp h_z1_z2

lemma complexLiftRange_surjective {A : Type*} [Ring A] [Algebra ℝ A] [Nontrivial A]
    (S : A) (hS : S * S = -1) : Function.Surjective (complexLiftRange S hS) := by
  intro ⟨x, ⟨z, hz⟩⟩
  use z
  ext
...
noncomputable def complexLiftEquiv {A : Type*} [Ring A] [Algebra ℝ A] [Nontrivial A]
  ...
  LinearEquiv.ofBijective (complexLiftRange S hS) ⟨complexLiftRange_injective S hS, complexLiftRange_surjective S hS⟩
