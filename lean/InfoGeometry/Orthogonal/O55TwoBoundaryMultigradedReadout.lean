...
abbrev O55State := FiniteTwoBoundaryWeakFunctional.State Index
abbrev O55Operator := FiniteTwoBoundaryWeakFunctional.Operator Index
abbrev O55BoundaryPair := RegularBoundaryPair Index
...
theorem sheetExchange_cartan_conjugation (h : Axis → ℂ) :
  ...
  rw [cartanAction_apply, cartanAction_apply, indexWeight_flip]
  simp
...
theorem gradingOperator_sheetExchange (a : Axis) :
  ...
  calc
    ...
    _ = -(sheetExchange * gradingOperator a) := by simp
...
theorem weakValue_exchangeBoundary
  ...
  have hact :
    ...
    apply congrArg (fun T : O55Operator => T B.pre)
    rw [← mul_assoc, sheetExchange_sq]
    simp
