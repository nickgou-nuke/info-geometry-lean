...
def S3OnH3ZornPeirce (σ : S3Perm) (P : H3ZornPeirce) : H3ZornPeirce :=
  ...
  | S3Perm.id => P
  | S3Perm.s12 =>
    { diag₁ := P.diag₂, diag₂ := P.diag₁, diag₃ := P.diag₃
  ...
  | S3Perm.s12_s23 =>
    { diag₁ := P.diag₃, diag₂ := P.diag₁, diag₃ := P.diag₂
      off₁₂ := P.off₃₁, off₂₃ := P.off₁₂, off₃₁ := P.off₂₃ }
  | S3Perm.s23_s12 =>
    { diag₁ := P.diag₂, diag₂ := P.diag₃, diag₃ := P.diag₁
      off₁₂ := P.off₂₃, off₂₃ := P.off₃₁, off₃₁ := P.off₁₂ }

@[simp] theorem S3OnH3ZornPeirce_s12_involutive (P : H3ZornPeirce) :
    S3OnH3ZornPeirce S3Perm.s12 (S3OnH3ZornPeirce S3Perm.s12 P) = P := by
  cases P
  simp [S3OnH3ZornPeirce, ZornVectorMatrix.conj_conj]

@[simp] theorem S3OnH3ZornPeirce_s23_involutive (P : H3ZornPeirce) :
    S3OnH3ZornPeirce S3Perm.s23 (S3OnH3ZornPeirce S3Perm.s23 P) = P := by
  cases P
  simp [S3OnH3ZornPeirce, ZornVectorMatrix.conj_conj]

@[simp] theorem S3OnH3ZornPeirce_s31_involutive (P : H3ZornPeirce) :
    S3OnH3ZornPeirce S3Perm.s31 (S3OnH3ZornPeirce S3Perm.s31 P) = P := by
  cases P
  simp [S3OnH3ZornPeirce, ZornVectorMatrix.conj_conj]

theorem S3OnH3ZornPeirce_group_closure (σ τ : S3Perm) :
    ∃ (ρ : S3Perm),
      S3OnH3ZornPeirce ρ = S3OnH3ZornPeirce τ ∘ S3OnH3ZornPeirce σ := by
  refine ⟨S3Perm.comp σ τ, ?_⟩
  funext P
...
theorem S3OnH3Zorn_add (σ : S3Perm) (X Y : H3Zorn ℝ) :
  ...
  <;> cases X <;> cases Y
  <;> simp [S3Perm.inverse, HMul.hMul, Mul.mul, S3Perm.comp, S3OnH3Zorn,
    S3OnH3ZornPeirce, h3zornPeirce, h3zornFromPeirce,
...
@[simp] theorem S3OnH3Zorn_one (σ : S3Perm) : S3OnH3Zorn σ (1 : H3Zorn ℝ) = 1 := by
  change S3OnH3Zorn σ H3Zorn.one = H3Zorn.one
  rcases σ with (_ | _ | _ | _ | _ | _)
  <;> simp [S3Perm.inverse, HMul.hMul, Mul.mul, S3Perm.comp, S3OnH3Zorn,
    S3OnH3ZornPeirce, h3zornPeirce, h3zornFromPeirce, H3Zorn.one,
    ZornVectorMatrix.conj, ZornVectorMatrix.zero]
...
theorem S3OnH3Zorn_linearTrace (σ : S3Perm) (X : H3Zorn ℝ) :
  ...
  <;> cases X
  <;> simp [S3Perm.inverse, HMul.hMul, Mul.mul, S3Perm.comp, S3OnH3Zorn,
    S3OnH3ZornPeirce, h3zornPeirce, h3zornFromPeirce, H3Zorn.linearTrace]
  <;> ring

def S3OnH3ZornLinearMap (σ : S3Perm) : H3Zorn ℝ →ₗ[ℝ] H3Zorn ℝ where
  toFun := S3OnH3Zorn σ
...
theorem S3OnH3Zorn_group_closure (σ τ : S3Perm) :
    ∃ (ρ : S3Perm), S3OnH3Zorn ρ = S3OnH3Zorn τ ∘ S3OnH3Zorn σ := by
...
theorem S3OnH3Zorn_comp (σ τ : S3Perm) :
    S3OnH3Zorn (S3Perm.comp σ τ) = S3OnH3Zorn τ ∘ S3OnH3Zorn σ := by
  funext X
...
@[simp] theorem S3OnH3Zorn_inverse_left (σ : S3Perm) (X : H3Zorn ℝ) :
  ...
  rcases σ with (_ | _ | _ | _ | _ | _)
  <;> cases X
  <;> simp [Inv.inv, S3Perm.inverse, S3Perm.comp, S3OnH3Zorn,
    S3OnH3ZornPeirce, h3zornPeirce, h3zornFromPeirce,
    ZornVectorMatrix.conj_conj, ZornVectorMatrix.conj_add,
...
def S3OnH3ZornLinearEquiv (σ : S3Perm) : H3Zorn ℝ ≃ₗ[ℝ] H3Zorn ℝ where
  toFun := S3OnH3Zorn σ
...
theorem S3OnH3ZornLinearEquiv_comp_closure (σ τ : S3Perm) :
    ∃ (ρ : S3Perm), S3OnH3ZornLinearEquiv ρ =
...
theorem S3OnH3ZornLinearEquiv_comp (σ τ : S3Perm) :
  ...
  apply LinearEquiv.ext
  intro X
  change S3OnH3Zorn (S3Perm.comp σ τ) X = S3OnH3Zorn τ (S3OnH3Zorn σ X)
...
@[simp] theorem S3OnH3Zorn_s31_involutive (X : H3Zorn ℝ) :
  ...
  unfold S3OnH3Zorn
  simp [S3OnH3ZornPeirce, h3zornPeirce, h3zornFromPeirce,
    ZornVectorMatrix.conj_conj]
...
theorem F4_dimension_decomposition : 14 + 16 + 22 = 52 := by norm_num

/-- The three Peirce-2 spaces in H₃(𝕆_s), each isomorphic to 𝕆_s. -/
