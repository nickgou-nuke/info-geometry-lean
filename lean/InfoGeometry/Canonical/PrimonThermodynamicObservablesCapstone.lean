...
theorem grand_canonical_primon_thermodynamics_synthesis
    ...
    (HasDerivAt (fun b : ℝ => (p : ℝ) ^ (-b))
      (-Real.log (p : ℝ) * (p : ℝ) ^ (-beta)) beta) ∧
    (HasDerivAt (fun b : ℝ => primeSurprisalPotential p b)
      (-primeMeanEnergy p beta) beta) ∧
    (deriv (fun b : ℝ => primeSurprisalPotential p b) beta =
      -primeMeanEnergy p beta) ∧
