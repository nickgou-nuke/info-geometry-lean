...
abbrev PerfectPentagonTensor := Matrix.unitaryGroup (Fin 8) ℂ
...
theorem h_isometry :
    (tensor : Matrix (Fin 8) (Fin 8) ℂ) *
      (tensor : Matrix (Fin 8) (Fin 8) ℂ).conjTranspose = 1 := by
  change (tensor : Matrix (Fin 8) (Fin 8) ℂ) *
      (tensor : Matrix (Fin 8) (Fin 8) ℂ).conjTranspose = 1
  exact Matrix.mem_unitaryGroup_iff.mp tensor.2
...
theorem perfect_tensor_trace_conservation :
    trace ((tensor : Matrix (Fin 8) (Fin 8) ℂ) *
      (tensor : Matrix (Fin 8) (Fin 8) ℂ).conjTranspose) = 8 := by
  rw [h_isometry tensor, trace_one]
  rfl

/-- HaPPY Network Bulk-to-Boundary Map Representation V ∈ Mₙ(ℂ). -/
abbrev HaPPYNetworkIsometry (n : ℕ) [Fintype (Fin n)] [DecidableEq (Fin n)] :=
  Matrix.unitaryGroup (Fin n) ℂ

namespace HaPPYNetworkIsometry

def happy_map {n : ℕ} [Fintype (Fin n)] [DecidableEq (Fin n)]
    (net : HaPPYNetworkIsometry n) : Matrix.unitaryGroup (Fin n) ℂ := net

end HaPPYNetworkIsometry

variable {n : ℕ} [Fintype (Fin n)] [DecidableEq (Fin n)] (net : HaPPYNetworkIsometry n)

def happyMapVal : Matrix (Fin n) (Fin n) ℂ :=
  HaPPYNetworkIsometry.happy_map net

theorem h_happy_isometry :
    (happyMapVal net).conjTranspose * happyMapVal net = 1 := by
  change (HaPPYNetworkIsometry.happy_map net : Matrix (Fin n) (Fin n) ℂ).conjTranspose *
      (HaPPYNetworkIsometry.happy_map net : Matrix (Fin n) (Fin n) ℂ) = 1
  exact Matrix.mem_unitaryGroup_iff'.mp (HaPPYNetworkIsometry.happy_map net).2
