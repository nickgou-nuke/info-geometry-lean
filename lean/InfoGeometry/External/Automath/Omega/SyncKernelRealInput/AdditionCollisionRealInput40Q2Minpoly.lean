...
/-- Paper label: `thm:addition-collision-real-input40-q2-minpoly`. -/
theorem paper_addition_collision_real_input40_q2_minpoly :
    ∃ P : Polynomial Int, P.natDegree = 26 ∧ Irreducible P := by
  let P : Polynomial Int := X ^ 26 - C (2 : Int)
  let I : Ideal Int := Ideal.span ({(2 : Int)} : Set Int)
  have hmonic : P.Monic := by
    dsimp [P]
    simpa using
      (Polynomial.monic_X_pow_sub_C (R := Int) (a := (2 : Int)) (n := 26) (by decide : 26 ≠ 0))
  have hprime : I.IsPrime := by
    dsimp [I]
    rw [Ideal.span_singleton_prime]
    · exact Int.prime_two
    · norm_num
  have hnatDegree : P.natDegree = 26 := by
    dsimp [P]
    simpa using (Polynomial.natDegree_X_pow_sub_C (n := 26) (r := (2 : Int)))
      ...
      · have hn26 : n ≠ 26 := by
        ...
        have hcoeff : P.coeff n = 0 := by
          ...
          have hC : ((2 : Polynomial Int).coeff n) = 0 := by
            ...
            · exact (h0 rfl).elim
            · simp
          simp [Polynomial.coeff_X_pow, hn26, hC]
        rw [hcoeff]
