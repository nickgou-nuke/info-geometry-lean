...
@[simp] theorem pairing_smul_left
  ...
  calc
    ∑ i, star ((c • post) i) * pre i =
      ...
      intro i hi
      simp [Pi.smul_apply, star_smul, mul_comm, mul_left_comm, mul_assoc]
...
@[simp] theorem pairing_smul_right
  ...
  calc
    (∑ i, star (post i) * (c * pre i)) =
        ∑ i, c * (star (post i) * pre i) := by
          apply Finset.sum_congr rfl
          intro i hi
          ring
    _ = c * ∑ i, star (post i) * pre i := by
      rw [Finset.mul_sum]
...
@[simp] theorem numerator_sub (A B : Operator ι) :
    numerator p (A - B) = numerator p A - numerator p B := by
  simp only [numerator]
  change (∑ i, star (p.post i) * (A p.pre i - B p.pre i)) = _
  simp only [mul_sub, Finset.sum_sub_distrib, pairing]
