...
lemma almostComplex_map_add_smul (S : V →ₗ[ℝ] V) (c d : ℝ) (v : V) :
    S (c • v + d • S v) = c • S v + d • S (S v) := by
  rw [S.map_add, S.map_smul, S.map_smul]

lemma almostComplex_sq (S : V →ₗ[ℝ] V) (hS : S.comp S = - LinearMap.id) (v : V) :
    S (S v) = -v := by
...
theorem complex_mul_smul (S : V →ₗ[ℝ] V) (hS : S.comp S = - LinearMap.id) (z1 z2 : ℂ) (v : V) :
  ...
  dsimp [complexSMul]
  have hS_linear := almostComplex_map_add_smul S (z2.re : ℝ) (z2.im : ℝ) v
  have hS_sq_v := almostComplex_sq S hS v
  rw [hS_linear, hS_sq_v]
