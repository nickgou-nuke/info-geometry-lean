...
theorem grand_dag_category_wheeler_synthesis
    ...
    -- 1. DAG 2-complex boundary vanishing
    (tc.b2 * tc.b1 = 0) ∧
    -- 2. Graph Hodge Laplacian symmetry
    ((laplacian0 tc).transpose = laplacian0 tc) ∧
    -- 3. Quiver morphism endpoint preservation
    (f.domHead = A.name ∧ g.codHead = C.name) ∧
    -- 4. Wheeler homological exactness and zero debt
    (Ch.d1 (Ch.d2 x) = 0) ∧
    (∃ y : C2, Ch.d2 y = z) := by
  refine ⟨dag_matrix_boundary_squared_zero tc,
