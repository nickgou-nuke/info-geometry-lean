...
def pac44ToCanonicalZorn (x : PACSplit44) : Zorn :=
...
def canonicalTriality (X : Zorn) : Zorn where
  a := X.a
  u := ![X.u 1, X.u 2, X.u 0]
  v := ![X.v 1, X.v 2, X.v 0]
  b := X.b

theorem canonicalTriality_order_three (X : Zorn) :
...
theorem canonicalTriality_norm (X : Zorn) :
...
theorem canonicalTriality_mul (X Y : Zorn) :
    canonicalTriality (zornMul X Y) = zornMul (canonicalTriality X) (canonicalTriality Y) := by
  apply zorn_ext
...
def canonicalLaneElement : SplitOctonionLane → Zorn
  | .diagonalProjector =>
