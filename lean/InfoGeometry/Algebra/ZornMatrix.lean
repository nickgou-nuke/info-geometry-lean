...
/-- Coordinate carrier for the native algebraic Zorn matrix. -/
abbrev Coord (R : Type*) [CommRing R] :=
  R × (Fin 3 → R) × (Fin 3 → R) × R

/-- Native coordinate equivalence for the algebraic Zorn carrier. -/
def coordEquiv : ZornMatrix R ≃ Coord R where
  ...
  right_inv c := by rcases c with ⟨a, v, w, b⟩; rfl
...
@[simp] theorem E11_add_E22 : (E11 : ZornMatrix R) + E22 = I := by
  ext j <;>
    simp [E11, E22, I, Vec3.add]

@[simp] theorem E11_mul_U (i : Fin 3) : (E11 : ZornMatrix R) * U i = U i := by
  fin_cases i <;> apply ZornMatrix.ext <;>
    simp [E11, U, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add,
      Vec3.sub, Vec3.smul] <;>
    try { funext j; fin_cases j <;> simp [E11, U, Vec3.basis, mul,
      Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul] }

@[simp] theorem U_mul_E22 (i : Fin 3) : (U i : ZornMatrix R) * E22 = U i := by
  fin_cases i <;> apply ZornMatrix.ext <;>
    simp [E22, U, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add,
      Vec3.sub, Vec3.smul] <;>
    try { funext j; fin_cases j <;> simp [E22, U, Vec3.basis, mul,
      Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul] }

@[simp] theorem E22_mul_V (i : Fin 3) : (E22 : ZornMatrix R) * V i = V i := by
  fin_cases i <;> ext j <;>
    simp [E22, V, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul] <;>
    try { fin_cases j <;> simp [E22, V, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul] }

@[simp] theorem V_mul_E11 (i : Fin 3) : (V i : ZornMatrix R) * E11 = V i := by
  fin_cases i <;> ext j <;>
    simp [E11, V, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul] <;>
    try { fin_cases j <;> simp [E11, V, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul] }

@[simp] theorem E22_mul_U (i : Fin 3) : (E22 : ZornMatrix R) * U i = 0 := by
  fin_cases i <;> ext j <;>
    simp [E22, U, zero, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul]

@[simp] theorem U_mul_E11 (i : Fin 3) : (U i : ZornMatrix R) * E11 = 0 := by
  fin_cases i <;> ext j <;>
    simp [E11, U, zero, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul]

@[simp] theorem E11_mul_V (i : Fin 3) : (E11 : ZornMatrix R) * V i = 0 := by
  fin_cases i <;> ext j <;>
    simp [E11, V, zero, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul]

@[simp] theorem V_mul_E22 (i : Fin 3) : (V i : ZornMatrix R) * E22 = 0 := by
  fin_cases i <;> ext j <;>
    simp [E22, V, zero, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul]

theorem U_mul_V (i j : Fin 3) :
    (U i : ZornMatrix R) * V j = if i = j then E11 else 0 := by
  fin_cases i <;> fin_cases j <;> ext k <;>
    simp [U, V, E11, zero, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul]

theorem V_mul_U (i j : Fin 3) :
    (V i : ZornMatrix R) * U j = if i = j then E22 else 0 := by
  fin_cases i <;> fin_cases j <;> ext k <;>
    simp [U, V, E22, zero, Vec3.basis, mul, Vec3.dot, Vec3.cross, Vec3.add, Vec3.sub, Vec3.smul]
...
theorem U_V_anticommutator (i j : Fin 3) :
    (U i : ZornMatrix R) * V j + V j * U i = if i = j then I else 0 := by
  fin_cases i <;> fin_cases j <;>
    ext k <;>
      simp [U, V, I, zero, Vec3.basis, mul, Vec3.dot, Vec3.cross,
...
theorem U_anticommute (i j : Fin 3) :
  ...
  fin_cases i <;> fin_cases j <;>
    ext k <;>
      simp [U, zero, Vec3.basis, mul, Vec3.dot, Vec3.cross,
        Vec3.add, Vec3.sub, Vec3.smul]
...
theorem V_anticommute (i j : Fin 3) :
  ...
  fin_cases i <;> fin_cases j <;>
    ext k <;>
      simp [V, zero, Vec3.basis, mul, Vec3.dot, Vec3.cross,
        Vec3.add, Vec3.sub, Vec3.smul]
