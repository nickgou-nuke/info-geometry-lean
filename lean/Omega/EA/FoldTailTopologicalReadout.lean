...
theorem paper_fold_tail_topological_readout (m k : ℕ) :
    foldOddGeneratorDegreeCount m k = foldTailMultiplicity m (k + 1) := by
  dsimp [foldOddGeneratorDegreeCount, foldTailMultiplicity]
