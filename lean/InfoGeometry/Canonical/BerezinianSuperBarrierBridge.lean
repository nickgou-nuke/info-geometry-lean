...
theorem canonicalBerezinian_eq_owner (A D : Block ι) :
...
theorem canonicalBerezinian_pos {A D : Block ι}
...
def bosonicDetBarrier (A : Block ι) : ℝ :=
  -Real.log (Matrix.det A)

/-- Graded log-volume barrier `-log Ber`. -/
def superBerezinianBarrier (A D : Block ι) : ℝ :=
...
theorem canonicalBerezinian_eq_one_of_det_eq
    (A D : Block ι) (hdet : Matrix.det A = Matrix.det D)
    (hD : Matrix.det D ≠ 0) :
...
theorem berezinian_super_barrier_packet
    ...
    canonicalBerezinian oddReflectionEven oddReflectionOdd = (-1 : ℝ) := by
