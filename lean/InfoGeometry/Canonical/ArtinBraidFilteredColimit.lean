...
def artinStageMap {n m : ℕ} (h : n ≤ m) : ArtinBraid n →* ArtinBraid m :=
  PresentedGroup.map (FreeGroup.map (generatorCast h)) (artinRelations_mapsTo h)
...
noncomputable def artinBraidDiagram : ℕ ⥤ GrpCat where
  ...
  map_comp f g := by
    apply GrpCat.hom_ext
    exact artinStageMap_comp (leOfHom f) (leOfHom g)
...
abbrev BInfinity : Type :=
  (colimit artinBraidDiagram : GrpCat)

/-- Canonical finite-stage inclusion into `B∞`. -/
def toBInfinity (n : ℕ) : ArtinBraid n →* BInfinity :=
  (colimit.ι artinBraidDiagram n).hom

/-- `B∞` carries the native colimit certificate. -/
def bInfinity_isColimit : IsColimit (colimit.cocone artinBraidDiagram) :=
  colimit.isColimit artinBraidDiagram
...
theorem toBInfinity_transition {n m : ℕ} (h : n ≤ m) (g : ArtinBraid n) :
  ...
  have hw := (colimit.cocone artinBraidDiagram).w (homOfLE h)
