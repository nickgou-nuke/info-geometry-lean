...
Chapter 5 discusses Lorentz symmetry from `SL(2,ℂ)`, the `SU(2)`/Kähler
...
* scalar unit-norm matrices act trivially by congruence, modeling the finite
  `{±I}` kernel shadow;
* a finite unit-circle slice `a·Id + b·I` of the complex-structure rotation
  preserves the Euclidean coordinate metric;
...
/-- Coordinate addition on the finite real carrier. -/
def addCoord4 (u v : Coord4) : Coord4 :=
  ⟨u.t + v.t, u.x + v.x, u.y + v.y, u.z + v.z⟩

/-- Coordinate scalar multiplication on the finite real carrier. -/
def smulCoord4 (r : ℝ) (u : Coord4) : Coord4 :=
  ⟨r * u.t, r * u.x, r * u.y, r * u.z⟩

/-- Unit-circle slice of the complex-structure rotation generated by `I4`. -/
