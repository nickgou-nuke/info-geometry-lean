...
theorem hasDerivAt_uniformChargeField (rho eps c1 x : ℝ) :
  ...
  convert ((hasDerivAt_id x).const_mul (rho / eps)).sub_const c1 using 1 <;>
    simp [uniformChargeField, mul_comm]
...
theorem uniformCharge_poisson (rho eps c0 c1 x : ℝ) (heps : eps ≠ 0) :
  ...
  rw [hf]
  have hneg : HasDerivAt (fun y => -uniformChargeField rho eps c1 y)
      (-(rho / eps)) x :=
    (hasDerivAt_uniformChargeField rho eps c1 x).neg
  rw [hneg.deriv]
  ring
