...
/-- Concrete instantiation of ThreeGradeClosureDefect. -/
def trivialThreeGradeClosureDefect : ThreeGradeClosureDefect Unit ℝ where
  defect _ := 0
...
/-- Concrete instantiation of DefectAbsorbedInPlusTwo. -/
def trivialDefectAbsorbedInPlusTwo
    (G : FiveGrading L) :
    DefectAbsorbedInPlusTwo L Unit ℝ G trivialThreeGradeClosureDefect where
  defectToPlusTwo := 0
  defect_mem_plus_two _ := G.gPosTwo.zero_mem
...
/-- Concrete instantiation of TKKDefectAbsorbedInPlusTwo -/
def trivialTKKDefectAbsorbedInPlusTwo
    (State Geometry : Type*)
    [AddCommGroup State] [Module ℝ State]
    [AddCommGroup Geometry] [Module ℝ Geometry]
    (R : TKKRicciFluxDatum L State Geometry)
    (G : FiveGrading L)
    (h_defect_zero : ∀ X s, R.closureDefect.defect X s = 0) :
    TKKDefectAbsorbedInPlusTwo L State Geometry R G where
  defectToPlusTwo _ _ := 0
  defect_mem_plus_two _ _ := G.gPosTwo.zero_mem
  plusTwoReadout _ := 0
  closureDefect_eq_plusTwoReadout X s := h_defect_zero X s
...
/-- Concrete instantiation of BlackHoleFiveGradeLedger. -/
def trivialBlackHoleFiveGradeLedger
    (G : FiveGrading L) :
    BlackHoleFiveGradeLedger L Unit Unit ℝ where
  grading := G
  visibleState _ := 0
  visible_mem_zero _ := G.gZero.zero_mem
  hiddenState _ := 0
  memory _ := 0
  memoryToPlusTwo := 0
  memory_mem_plus_two _ := G.gPosTwo.zero_mem
...
lemma visible_add_gradeTwo_evolution (t : ℝ) (s : State) :
    A.visible (A.evolution t s) + A.gradeTwo (A.evolution t s) =
...
theorem visibleLoss_eq_gradeTwoGain
  ...
  exact (A.visible_add_gradeTwo_evolution t s).symm
...
lemma visible_add_gradeTwo_stateAt (step : ℝ) (s : State) (n : ℕ) :
...
theorem recursive_visibleLoss_eq_gradeTwoGain
  ...
  exact (A.visible_add_gradeTwo_stateAt step s n).symm

end GradeTwoInformationLedger

/-- Concrete instantiation of GradeTwoInformationLedger. -/
def trivialGradeTwoInformationLedger : GradeTwoInformationLedger Unit ℝ where
  total _ := 0
  visible _ := 0
  gradeTwo _ := 0
  total_eq_visible_plus_gradeTwo _ := by rw [add_zero]
  evolution _ _ := ()
  total_conserved _ _ := rfl
...
/-- Concrete instantiation of BPSBoundDatum. -/
def trivialBPSBoundDatum : BPSBoundDatum Unit where
  mass _ := 0
  centralNorm _ := 0
  centralNorm_nonneg _ := le_refl 0
  bps_bound _ := le_refl 0
...
/-- Installed BPS data satisfy the central-norm nonnegativity target. -/
theorem bpsBoundOwnerTarget_nonneg
    (State : Type*)
    (B : BPSBoundDatum State)
    (s : State) :
    0 ≤ B.centralNorm s :=
  B.centralNorm_nonnegative s

/-- Installed BPS data satisfy the central-norm lower-bound target. -/
theorem bpsBoundOwnerTarget_le
    (State : Type*)
    (B : BPSBoundDatum State)
    (s : State) :
    B.centralNorm s ≤ B.mass s :=
  B.centralNorm_le_mass s
