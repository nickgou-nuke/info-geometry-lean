...
/-- Coarse-graining is a left inverse to refinement on sectors. -/
@[simp] theorem coarseSector_refineSector {n : ℕ} (s : Sector n) :
  ...
  · ext w
    constructor
    · intro hw
      have hm := hw (leftChild w) (truncateWord_leftChild w)
      change leftChild w ∈ refineSpatial S at hm
      simpa [refineSpatial] using hm
    · intro hw v hv
      exact Set.mem_preimage.mpr (hv ▸ hw)
  · rfl
...
/-- The unit inequality of the refinement/coarse-graining adjunction. -/
theorem refineSector_le_coarseSector_refineSector {n : ℕ} (s : Sector n) :
    s ≤ coarseSector (refineSector s) := by
  exact (sector_galois_connection n).le_u_l s
