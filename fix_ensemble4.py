...
theorem expectedDirectionalCharge_covariance (w : W) (beta v : Fin 2 → ℝ) :
    expectedDirectionalCharge (State := State) (parameterLeftAction (State := State) w beta) (parameterLeftAction (State := State) w v) =
  ...
  apply Finset.sum_congr rfl
  intro m _
  change realGibbsWeight D.base (parameterLeftAction (State := State) w beta) (w • m) * (∑ i, parameterLeftAction (State := State) w v i * D.base.momentMap (w • m) i) = _
...
theorem centeredDirectionalCharge_covariance (w : W) (beta v : Fin 2 → ℝ) (m : State) :
    centeredDirectionalCharge (State := State) (parameterLeftAction (State := State) w beta) (parameterLeftAction (State := State) w v) (w • m) =
      centeredDirectionalCharge (State := State) beta v m := by
  dsimp [centeredDirectionalCharge]
  have h_pair : (∑ i, parameterLeftAction (State := State) w v i * D.base.momentMap (w • m) i) = ∑ i, v i * D.base.momentMap m i := by
    dsimp [parameterLeftAction, MulAction.smul]
    exact D.pairing_diagonal_invariant w v m
  rw [h_pair]
  have h_exp : expectedDirectionalCharge (State := State) (parameterLeftAction (State := State) w beta) (parameterLeftAction (State := State) w v) = expectedDirectionalCharge (State := State) beta v := by
...
theorem fisherSouriau_covariance (w : W) (beta u v : Fin 2 → ℝ) :
    fisherSouriauBilinear (State := State) (parameterLeftAction (State := State) w beta)
  ...
  apply Finset.sum_congr rfl
  intro m _
  change realGibbsWeight D.base (parameterLeftAction (State := State) w beta) (w • m) *
    centeredDirectionalCharge (State := State) (parameterLeftAction (State := State) w beta) (parameterLeftAction (State := State) w u) (w • m) *
    centeredDirectionalCharge (State := State) (parameterLeftAction (State := State) w beta) (parameterLeftAction (State := State) w v) (w • m) = _
  rw [probability_diagonal_invariant]
